...
define(['blob/BlobClient', 'blob/BlobMetadata', 'http', 'https',
  ...
  function (BlobClient, BlobMetadata, http, https, StringStreamWriter) {
    ...
    BlobServerClient.prototype.putMetadata = function (metadataDescriptor,
      callback) {
      var self = this;
      ...
      self._sendHttpRequestWithContent(options, JSON.stringify(metadata.serialize(),
        null, 4), function (err, data) {
        if (err) {
          callback(err);
          return;
        }
    ...
    BlobServerClient.prototype.putFile = function (name, data, callback) {
      ...
      this._sendHttpRequestWithContent(options, data, function (err,
        responseData) {
        if (err) {
          callback(err);
          return;
    ...
    BlobServerClient.prototype._ensureAuthenticated = function (options,
      callback) {
    ...
    BlobServerClient.prototype._sendHttpRequestWithContent = function (options,
      data, callback) {
      var self = this;
      self._ensureAuthenticated(options, function (err, updatedOptions) {
        if (err) {
    ...
    BlobServerClient.prototype.__sendHttpRequestWithContent = function (options,
      data, callback) {
      // TODO: use the http or https
      var req = http.request(options, function (res) {
