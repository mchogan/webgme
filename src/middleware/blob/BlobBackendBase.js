...
define(['blob/BlobMetadata',
  ...
  'util/StringStreamWriter'
], function (BlobMetadata, fs, jszip, mime, GUID, StringStreamReader,
  StringStreamWriter) {
  ...
  BlobBackendBase.prototype.putObject = function (readStream, bucket,
    callback) {
    throw new Error('Not implemented yet.');
  };

  BlobBackendBase.prototype.getObject = function (hash, writeStream, bucket,
    callback) {
    throw new Error('Not implemented yet.');
  };
  ...
  BlobBackendBase.prototype.putFile = function (name, readStream, callback) {
    ...
    self.putObject(readStream, self.contentBucket, function (err, hash,
      length) {
      if (err) {
  ...
  BlobBackendBase.prototype.getFile = function (metadataHash, subpath,
    ...
    self.getMetadata(metadataHash, function (err, metadataHash, metadata) {
      ...
      if (metadata.contentType === BlobMetadata.CONTENT_TYPES.OBJECT) {
        self.getObject(metadata.content, writeStream, self.contentBucket,
          function (err) {
            if (err) {
              callback(err);
              return;
        ...
        if (subpath) {
          if (metadata.content.hasOwnProperty(subpath)) {
            ...
            if (contentObj.contentType === BlobMetadata.CONTENT_TYPES.OBJECT) {
              ...
              self.getObject(contentObj.content, writeStream, self.contentBucket,
                function (err) {
                  if (err) {
                    callback(err);
                    return;
            ...
            } else if (contentObj.contentType === BlobMetadata.CONTENT_TYPES
              .SOFT_LINK) {
          ...
          for (var i = 0; i < keys.length; i += 1) {
            (function (subpartHash, subpartType, subpartName) {
              ...
              if (subpartType === BlobMetadata.CONTENT_TYPES.OBJECT) {
                self.getObject(subpartHash, writeStream2, self.contentBucket,
                  contentReadyCallback);

              } else if (subpartType === BlobMetadata.CONTENT_TYPES.SOFT_LINK) {
                self.getFile(subpartHash, '', writeStream2,
                  contentReadyCallback);

              } else {
                // complex part within complex part is not supported
                callback('Subpart content type is not supported: ' +
                  subpartType + ' ' + subpartName + ' ' + subpartHash);
              }

            })(metadata.content[keys[i]].content, metadata.content[keys[i]]
              .contentType, keys[i])
          }
        }
      } else {
  ...
  BlobBackendBase.prototype.putMetadata = function (metadata, callback) {
    ...
    self.putObject(stringStream, self.metadataBucket, function (err,
      metadataHash) {
      if (err) {
  ...
  BlobBackendBase.prototype.getMetadata = function (metadataHash, callback) {
    ...
    self.getObject(metadataHash, writeStream, self.metadataBucket, function (
      err, fileInfo) {
      if (err) {
        callback(err);
        return;
