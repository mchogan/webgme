...
define(['./BlobBackendBase',
  ...
  function (BlobBackendBase, fs, crypto, path, util, GUID, ensureDir) {
      ...
      ensureDir(path.dirname(tempName), function (err) {
        ...
        writeStream.on('close', function () {
          ...
          var hash = shasum.digest('hex'),
            objectFilename = path.join(self.blobDir, bucket, self._getObjectRelativeLocation(
              hash));
    ...
    BlobFSBackend.prototype.getObject = function (hash, writeStream, bucket,
      callback) {
      var filename = path.join(this.blobDir, bucket, this._getObjectRelativeLocation(
          hash)),
        readStream;
