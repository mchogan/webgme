...
blobBackend.putFile(filename, fs.createReadStream(filename), function (err,
  hash) {
  if (err) {
    console.log(err);
    return;
  ...
  blobBackend.getFile(hash, process.stdout, function (err, filename,
    contentType) {
    if (err) {
      console.log(err);
      return;
...
var addFilesFromTestDir = function (testdir, callback) {
  ...
  for (var i = 0; i < Math.min(sourceFiles.length, maxItems); i += 1) {
    ...
    (function (file) {
      ...
      blobBackend.putFile(file, fs.createReadStream(file), function (err,
        hash) {
        if (err) {
          console.log(err);
        }
