...
define(['blob/BlobMetadata'], function (BlobMetadata) {
  ...
  Artifact.prototype.addFilesAsSoftLinks = function (files, callback) {
    var self = this,
      ...
      counterCallback = function (err, hash) {
        ...
        if (nbrOfFiles === 0) {
          if (error) {
            return callback('Failed adding files as soft-links: ' + error,
              hashes);
          }
          callback(null, hashes);
        }
    ...
    for (i = 0; i < fileNames.length; i += 1) {
      self.addFileAsSoftLink(fileNames[i], files[fileNames[i]],
        counterCallback);
    }
  };
  ...
  Artifact.prototype.addObjectHashes = function (objectHashes, callback) {
    ...
    for (i = 0; i < fileNames.length; i += 1) {
      self.addObjectHash(fileNames[i], objectHashes[fileNames[i]],
        counterCallback);
    }
  };
  ...
  Artifact.prototype.addMetadataHashes = function (objectHashes, callback) {
    ...
    for (i = 0; i < fileNames.length; i += 1) {
      self.addMetadataHash(fileNames[i], objectHashes[fileNames[i]],
        counterCallback);
    }
  };
