...
define(['./Artifact', 'blob/BlobMetadata', 'superagent'], function (Artifact,
  BlobMetadata, superagent) {
  ...
  var BlobClient = function (parameters) {
    ...
    if (parameters) {
      ...
      this.httpsecure = (parameters.httpsecure !== undefined) ? parameters.httpsecure :
        this.httpsecure;
    }
    this.blobUrl = '';
    if (this.httpsecure !== undefined && this.server && this.serverPort) {
      this.blobUrl = (this.httpsecure ? 'https://' : 'http://') + this.server +
        ':' + this.serverPort;
    }
  ...
  BlobClient.prototype.getArtifact = function (metadataHash, callback) {
    ...
    this.getMetadata(metadataHash, function (err, info) {
      ...
      if (info.contentType === BlobMetadata.CONTENT_TYPES.COMPLEX) {
        ...
        callback('not supported contentType ' + JSON.stringify(info, null,
          4));
      }
