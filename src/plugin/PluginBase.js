...
define(['plugin/PluginConfig',
  ...
  function (PluginConfig, PluginResult, PluginMessage, PluginNodeDescription) {
    ...
    PluginBase.prototype.getName = function () {
      throw new Error(
        'implement this function in the derived class - getting type automatically is a bad idea,' +
    ...
    PluginBase.prototype.updateSuccess = function (value, message) {
      ...
      this.logger.debug('Success was updated from ' + prevSuccess + ' to ' +
        newSuccessValue + msg);
    };
    ...
    PluginBase.prototype.baseIsMeta = function (node) {
      ...
      return self.META.hasOwnProperty(baseName) && self.META[baseName] ===
        baseNode;
    };
    ...
    PluginBase.prototype.save = function (message, callback) {
      ...
      var commitMessage = '[Plugin] ' + this.getName() + ' (v' + this.getVersion() +
        ') updated the model.';
      if (message) {
        commitMessage += ' - ' + message;
      }

      this.currentHash = this.project.makeCommit([this.currentHash],
        newRootHash, commitMessage, function (err) {
      ...
      if (this.branchName) {
        ...
        this.project.getBranchNames(function (err, branchNames) {
          if (branchNames.hasOwnProperty(self.branchName)) {
            ...
            if (branchHash === self.branchHash) {
              ...
              self.project.setBranchHash(self.branchName, self.branchHash,
                self.currentHash, function (err) {
                  if (err) {
                    // fast forward failed
                    self.logger.error(err);
                    self.logger.info('"' + self.branchName +
                      '" was NOT updated');
                    self.logger.info('Project was saved to ' + self.currentHash +
                      ' commit.');
                  } else {
                    // successful fast forward of branch to the new commit
                    self.logger.info('"' + self.branchName +
                      '" was updated to the new commit.');
              ...
              self.logger.warn('Cannot fast forward "' + self.branchName +
                '" branch. Merge is required but not supported yet.');
              self.logger.info('Project was saved to ' + self.currentHash +
                ' commit.');
              callback(null);
            }
        ...
        this.logger.info('Project was saved to ' + this.currentHash +
          ' commit.');
        callback(null);
      }
    };
