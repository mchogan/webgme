define(['plugin/PluginBase', 'plugin/PluginConfig',
  'plugin/Duplicator/Duplicator/second'
], function (PluginBase, PluginConfig, second) {
  "use strict";
      ...
      config.core.loadChildren(config.rootNode, function (err, children) {
        if (!err) {
          for (var i = 0; i < children.length; i++) {
            ...
            config.core.setAttribute(newChild, 'name', config.core.getAttribute(
              newChild, 'name') + "_duplicate");
          }
          config.core.persist(config.rootNode, function (err) {});
          var newRootHash = config.core.getHash(config.rootNode);
          result.commitHash = config.project.makeCommit([result.commitHash],
            newRootHash,
            'Plugin \'RootChildDuplicator\' updated the model.', function (
              err) {});
          result.success = true;
