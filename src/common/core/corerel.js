...
define(["util/assert", "core/coretree", "core/tasync", "util/canon"], function (
  ASSERT, CoreTree, TASYNC, CANON) {
  "use strict";
  ...
  function isValidRelid(relid) {
    return typeof relid === "string" && parseInt(relid, 10).toString() ===
      relid;
  }
  ...
  function CoreRel(coretree) {
    ...
    function overlayInsert(overlays, source, name, target) {
      ASSERT(isValidNode(overlays) && coretree.getRelid(overlays) ===
        OVERLAYS);
      ASSERT(coretree.isValidPath(source) && coretree.isValidPath(target) &&
        isPointerName(name));
    ...
    function copyNode(node, parent) {
      ...
      if (parent) {
        ...
        while (base) {
          ...
          aboveAncestor = (base === ancestor ? 0 : tempAncestor === base ? 1 :
            -1);

          var relativePath = aboveAncestor < 0 ? coretree.getPath(base,
            ancestor) : coretree.getPath(ancestor, base);
          ...
          for (var i = 0; i < list.length; ++i) {
            ...
            if (entry.p) {
              ...
              if (aboveAncestor < 0) {
                ...
                source = coretree.joinPaths(data.first, entry.s.substr(
                  baseOldPath.length));
                target = data.second;
              } else {
                ...
                source = relativePath + ancestorNewPath + entry.s.substr(
                  baseOldPath.length);
                target = entry.t;
    ...
    function copyNodes(nodes, parent) {
      ...
      for (i = 0; i < internalRelationPaths.length; i++) {
        ...
        for (j = 0; j < names.length; j++) {
          setPointer(copiedNodes[i], names[j], copiedNodes[
            internalRelationPaths[i][names[j]]]);
        }
      }
    ...
    function getPointerNames(node) {
      ...
      do {
        var child = coretree.getProperty(coretree.getChild(node, OVERLAYS),
          source);
        if (child) {
    ...
    function deletePointer(node, name) {
      ...
      do {
        ...
        var target = coretree.getProperty(coretree.getChild(overlays, source),
          name);
        if (target !== undefined) {
    ...
    function setPointer(node, name, target) {
      ASSERT(isValidNode(node) && typeof name === "string" && (!target ||
        isValidNode(target)));
