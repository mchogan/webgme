define(["util/assert", "core/core", "core/tasync", "util/jjv"], function (
  ASSERT, Core, TASYNC, JsonValidator) {
  "use strict";
  ...
  var MetaCore = function (oldcore) {
    ...
    core.isValidChildOf = function (node, parentNode) {
      ...
      var validChildTypePaths = core.getMemberPaths(MetaChildrenNode(
        parentNode), "items") || [];
      while (node) {
    ...
    core.getValidSetNames = function (node) {
      ...
      for (i = 0; i < validNames.length; i++) {
        metaPointerNode = MetaPointerNode(node, validNames[i]);
        if (metaPointerNode.max === undefined || metaPointerNode.max === -1 ||
          metaPointerNode > 1) { //TODO specify what makes something a pointer and what a set??? - can you extend a pointer to a set????
    ...
    core.isValidTargetOf = function (node, source, name) {
      ...
      if (pointerMetaNode) {
        var validTargetTypePaths = core.getMemberPaths(pointerMetaNode,
          "items") || [];
        while (node) {
    ...
    core.getJsonMeta = function (node) {
      ...
      for (i = 0; i < meta.children.items.length; i++) {
        meta.children.minItems.push(core.getMemberAttribute(tempNode,
          "items", meta.children.items[i], "min") || -1);
        meta.children.maxItems.push(core.getMemberAttribute(tempNode,
      ...
      for (i = 0; i < names.length; i++) {
        meta.attributes[names[i]] = core.getAttribute(MetaNode(node), names[
          i]);
      }
      ...
      for (i = 0; i < names.length; i++) {
        ...
        for (j = 0; j < pointer.items.length; j++) {
          pointer.minItems.push(core.getMemberAttribute(tempNode, "items",
            pointer.items[j], "min") || -1);
          pointer.maxItems.push(core.getMemberAttribute(tempNode, "items",
            pointer.items[j], "max") || -1);
    ...
    var getObjectDiff = function (bigger, smaller) {
      var diff = {},
        names, i, temp;
      if (smaller === null || smaller === undefined || isEmptyObject(
        smaller)) {
      ...
      for (i = 0; i < names.length; i++) {
        if (smaller[names[i]] === undefined) {
          ...
          if (typeof smaller[names[i]] === 'object') {
            ...
            if (JSON.stringify(smaller[names[i]]) !== JSON.stringify(
              bigger[names[i]])) {
              diff[names[i]] = bigger[names[i]];
            }
          }
    ...
    core.setChildMeta = function (node, child, min, max) {
      ...
      min = min || -1;
      max = max || -1;
      core.setMemberAttribute(MetaChildrenNode(node), 'items', core.getPath(
        child), 'min', min);
      core.setMemberAttribute(MetaChildrenNode(node), 'items', core.getPath(
        child), 'max', max);
    };
    ...
    core.setPointerMetaTarget = function (node, name, target, min, max) {
      ...
      min = min || -1;
      core.setMemberAttribute(_MetaPointerNode(node, name), 'items', core.getPath(
        target), 'min', min);
      max = max || -1;
      core.setMemberAttribute(_MetaPointerNode(node, name), 'items', core.getPath(
        target), 'max', max);
