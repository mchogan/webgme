...
define(["util/assert", "util/guid", "core/tasync"], function (ASSERT, GUID,
  TASYNC) {
  "use strict";

  var GUID_REGEXP = new RegExp("[a-z0-9]{8}(-[a-z0-9]{4}){3}-[a-z0-9]{12}",
    'i');
  ...
  function guidCore(_innerCore) {
    ...
    function toExternalGuid(myGuid) {
      var out = myGuid.substr(0, 8) + '-' + myGuid.substr(8, 4) + '-' +
        myGuid.substr(12, 4) + '-' + myGuid.substr(16, 4) + '-' + myGuid.substr(
          20);
      return out;
    }
    ...
    _core.setGuid = function (node, guid) {
      ...
      return TASYNC.call(function (nodeArray) {
        ...
        if (parent) {
          _core.setAttribute(node, OWN_GUID, xorGuids(newGuid, xorGuids(
            _core.getMiddleGuid(parent), getRelidGuid(node))));
        } else {
          _core.setAttribute(node, OWN_GUID, xorGuids(newGuid,
            getRelidGuid(node)));
        }
        ...
        for (var i = 0; i < nodeArray.length; i++) {
          ...
          _core.setAttribute(nodeArray[i], OWN_GUID, xorGuids(oldGuid,
            xorGuids(oldOwn, newOwn)));
        }
    ...
    _core.createNode = function (parameters) {
      ...
      var relguid = "";
      if (parent) {
        relguid = xorGuids(toInternalGuid(_core.getMiddleGuid(_core.getParent(
          node))), xorGuids(guid, getRelidGuid(node)));
      } else {
    ...
    _core.moveNode = function (node, parent) {
      ...
      _core.setAttribute(newNode, OWN_GUID, xorGuids(_core.getMiddleGuid(
        parent), xorGuids(oldGuid, getRelidGuid(newNode))));
