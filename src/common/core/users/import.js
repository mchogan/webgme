...
define([
    ...
    if (jNode && jNode.children && jNode.children.length) {
      ...
      if (needed > 0) {
        ...
        for (var i = 0; i < jNode.children.length; i++) {
          importNode(jNode.children[i], node, pIntPath + '/children[' + i +
            ']', true, function (err) {
              error = error || err;
              if (--needed === 0) {
                callback(error);
  ...
  function importPointer(node, jNode, pName, callback) {
    if (jNode.pointers[pName].to && jNode.pointers[pName].from) {
      var needed = jNode.pointers[pName].to.length + jNode.pointers[pName].from
        .length,
        i,
        error = null;
      var ownPointer = true;
      ...
      if (needed === 0) {
        ...
        if (ownPointer) {
          for (i = 0; i < jNode.pointers[pName].to.length; i++) {
            getReferenceNode(jNode.pointers[pName].to[i], function (err,
              target) {
              error = error || err;
        ...
        for (i = 0; i < jNode.pointers[pName].from.length; i++) {
          if (!isInternalReference(jNode.pointers[pName].from[i])) {
            getReferenceNode(jNode.pointers[pName].from[i], function (err,
              source) {
              error = error || err;
              if (source) {
      ...
      var needed = 0,
        importSetRegAndAtr = function (sOwner, sMember, atrAndReg) {
          ...
          for (var i in atrAndReg.attributes) {
            _core.setMemberAttribute(sOwner, sName, mPath, i, atrAndReg.attributes[
              i]);
          }
          atrAndReg.registry = atrAndReg.registry || {};
          for (var i in atrAndReg.registry) {
            _core.setMemberRegistry(sOwner, sName, mPath, i, atrAndReg.registry[
              i]);
          }
        },
        importSetReference = function (isTo, index, cb) {
          var jObj = isTo === true ? jNode.pointers[sName].to[index] : jNode.pointers[
            sName].from[index];
    ...
    if (jNode.pointers && jNode.pointers.base && jNode.pointers.base.to) {
      getReferenceNode(jNode.pointers.base.to[0], function (err, base) {
        if (err) {
          ...
          getEmptyNode(jNode, parentNode, base, noClear, function (err,
            node) {
            if (err) {
              callback(err);
            } else {
