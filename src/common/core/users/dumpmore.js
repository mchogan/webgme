        ...
        if (err) {
          ...
          if (children === null || children === undefined || !children.length >
            ...
            var setChildJson = function (child, cb) {
              ToJson(_core, child, urlPrefix, _refType, function (err,
                jChild) {
                if (err) {
                  cb(err);
                } else {
                  if (jChild) {
                    ...
                    for (var j = 0; j < dumpObject.children.length; j++) {
                      if (childPath === getRefObjectPath(dumpObject.children[
                        j])) {
                    ...
                    dumpChildren(child, dumpObject.children[j],
                      urlPrefix, childRelPath, cb);
                  }
                }
              })
            };
  ...
  var dumpJsonNode = function (core, node, urlPrefix, refType, callback) {
    ...
    ToJson(core, node, urlPrefix, _refType, function (err, jDump) {
      if (err) {
        ...
        dumpChildren(node, jDump, urlPrefix, _cache[_rootPath], function (
          err) {
          if (err) {
            callback(err);
          } else {
              ...
              if (err) {
                ...
                for (var i = 0; i < children.length; i++) {
                  dumpNode(children[i], relPath + '/children[' + i +
                    ']', containerDump[index].children, i, function (
                      err) {
                      error = error || err;
                      if (--needed === 0) {
                        callback(error);
