define(['util/assert'], function (ASSERT) {
  ...
  function fillContainmentTree(node, myTreeObject) {
    ...
    for (i = 0; i < childrenGuids.length; i++) {
      ...
      fillContainmentTree(_nodes[childrenGuids[i]], myTreeObject[
        childrenGuids[i]]);
    }
  }
  ...
  function fillInheritanceTree(node, myTreeObject) {
    var i,
      descendantGuids = pathsToSortedGuidList(_core.getCollectionPaths(node,
        'base'));
    for (i = 0; i < descendantGuids.length; i++) {
      myTreeObject[descendantGuids[i]] = {};
      fillInheritanceTree(_nodes[descendantGuids[i]], myTreeObject[
        descendantGuids[i]]);
    }
  }
          ...
          for (j = 0; j < tArray.length; j++) {
            ...
            for (k = 0; k < jsonObject[keys[i]][tArray[j]].length; k++) {
              if (_pathToGuidMap[jsonObject[keys[i]][tArray[j]][k]]) {
                jsonObject[keys[i]][tArray[j]][k] = _pathToGuidMap[jsonObject[
                  keys[i]][tArray[j]][k]];
              } else if (baseGuid(jsonObject[keys[i]][tArray[j]][k])) {
                jsonObject[keys[i]][tArray[j]][k] = baseGuid(jsonObject[keys[
                  i]][tArray[j]][k]);
              } else {
                toDelete.push(j);
              }
    ...
    for (i = 0; i < names.length; i++) {
      ...
      for (j = 0; j < targetGuids.length; j++) {
        ...
        for (k = 0; k < attributeNames.length; k++) {
          memberInfo.attributes[attributeNames[k]] = _core.getMemberAttribute(
            node, names[i], path, attributeNames[k]);
        }

        //registry
        registryNames = _core.getMemberRegistryNames(node, names[i], path).sort();
        for (k = 0; k < registryNames.length; k++) {
          memberInfo.registry[registryNames[k]] = _core.getMemberRegistry(
            node, names[i], path, registryNames[k]);
        }
  ...
  function importLibrary(core, originLibraryRoot, updatedLibraryJson,
    callback) {
    _core = core;
    ...
    exportLibrary(core, originLibraryRoot, function (err) {
      ...
      loadImportBases(_import.bases, _core.getRoot(originLibraryRoot),
        function (err) {
          if (err) {
            return callback(err);
          }
          ...
          for (i = 0; i < newkeys.length; i++) {
            if (oldkeys.indexOf(newkeys[i]) === -1) {
              log("node " + logId(_import.nodes, newkeys[i]) +
                " will be added")
          ...
          for (i = 0; i < _removedNodeGuids.length; i++) {
            ...
            if (parent && _removedNodeGuids.indexOf(_core.getGuid(parent)) ===
              -1) {
  ...
  function updateNodes(guid, parent, containmentTreeObject) {
    ...
    for (i = 0; i < keys.length; i++) {
      if (_updatedNodeGuids.indexOf(keys[i]) === -1) {
        ...
        _nodes[keys[i]] = _core.createNode({
          parent: node,
          guid: keys[i],
          relid: relid
        });
  ...
  function updateNode(guid, parent) {
    ...
    if (parent && _core.getParent(node) && _core.getGuid(parent) !== _core.getGuid(
      ...
      for (j = 0; j < jsonNode.sets[keys[i]].length; j++) {
        ...
        if (_nodes[memberGuid]) {
          ...
          for (k in jsonNode.sets[keys[i]][j].attributes) {
            _core.setMemberAttribute(node, keys[i], _core.getPath(_nodes[
              memberGuid]), k, jsonNode.sets[keys[i]][j].attributes[k]);
          }
          for (k in jsonNode.sets[keys[i]][j].registry) {
            _core.setMemberRegistry(node, keys[i], _core.getPath(_nodes[
              memberGuid]), k, jsonNode.sets[keys[i]][j].registry[k]);
          }
        }
      }
    }
  ...
  function updateInheritance() {
    var i, guidList = Object.keys(_import.nodes),
      base;
    for (i = 0; i < guidList.length; i++) {
  ...
  function updateChildrenMeta(guid) {
    var jsonMeta = _import.nodes[guid].meta.children || {
        items: [],
        minItems: [],
        maxItems: []
      },
      i;
    ASSERT(jsonMeta.items.length === jsonMeta.minItems.length && jsonMeta.minItems
      .length === jsonMeta.maxItems.length);

    _core.setChildrenMetaLimits(_nodes[guid], jsonMeta.min, jsonMeta.max);
    for (i = 0; i < jsonMeta.items.length; i++) {
      _core.setChildMeta(_nodes[guid], _nodes[jsonMeta.items[i]], jsonMeta.minItems[
        i], jsonMeta.maxItems[i]);
    }
  }
  ...
  function updatePointerMeta(guid) {
    ...
    for (i = 0; i < keys.length; i++) {
      ...
      for (j = 0; j < jsonMeta[keys[i]].items.length; j++) {
        _core.setPointerMetaTarget(_nodes[guid], keys[i], _nodes[jsonMeta[
          keys[i]].items[j]], jsonMeta[keys[i]].minItems[j], jsonMeta[keys[
          i]].maxItems[j]);
      }
      _core.setPointerMetaLimits(_nodes[guid], keys[i], jsonMeta[keys[i]].min,
        jsonMeta[keys[i]].max);
    }
  }
  ...
  function updateAspectMeta(guid) {
    ...
    for (i = 0; i < keys.length; i++) {
      for (j = 0; j < jsonMeta[keys[i]].length; j++) {
        _core.setAspectMetaTarget(_nodes[guid], keys[i], _nodes[jsonMeta[keys[
          i]][j]]);
      }
    }
  }
