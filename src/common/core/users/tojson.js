define([
  ...
  var changeRefObjects = function (refType, urlPrefix, object, core, root,
    callback) {
      ...
      if (needed > 0) {
        for (var i = 0; i < neededNames.length; i++) {
          if (object[neededNames[i]]['$ref']) {
            //refrence object
            pathToRefObjAsync(refType, urlPrefix, object[neededNames[i]][
              '$ref'
            ] /*.substring(1)*/ , core, root, function (err, refObj) {
              error = error || err;
            ...
            changeRefObjects(refType, urlPrefix, object[neededNames[i]],
              core, root, function (err) {
                error = error || err;
                if (--needed === 0) {
                  callback(error);
                }
  ...
  var pathToRefObj = function (refType, urlPrefix, path) {
    ...
    case _refTypes.url:
      ...
      return URL.urlToRefObject(urlPrefix + '&path=' + URL.addSpecialChars(
        path));
      break;
    case _refTypes.path:
  ...
  var pathToRefObjAsync = function (refType, urlPrefix, path, core, root,
    callback) {
    switch (refType) {
    case _refTypes.url:
      if (path === null) {
      ...
      callback(null, URL.urlToRefObject(urlPrefix + '&path=' + URL.addSpecialChars(
        path)));
      break;
    case _refTypes.path:
  ...
  var getMetaOfNode = function (core, node, urlPrefix, refType, callback) {
    ...
    changeRefObjects(refType, urlPrefix, meta, core, core.getRoot(node),
      function (err) {
        callback(err, meta);
      });
  };
  ...
  var getSetAttributesAndRegistry = function (core, node, setName,
    setOwnerPath, callback) {
      ...
      if (err) {
        ...
        if (owner) {
          ...
          for (var i = 0; i < names.length; i++) {
            atrAndReg.attributes[names[i]] = core.getMemberAttribute(
              owner, setName, path, names[i]);
          }
          names = core.getMemberRegistryNames(owner, setName, path);
          for (var i = 0; i < names.length; i++) {
            atrAndReg.registry[names[i]] = core.getMemberRegistry(owner,
              setName, path, names[i]);
          }
  ...
  var getMemberAttributesAndRegistry = function (core, node, setName,
    memberPath) {
    var retObj = {
      attributes: {},
      registry: {}
    ...
    for (i = 0; i < names.length; i++) {
      retObj.attributes[names[i]] = core.getMemberAttribute(node, setName,
        memberPath, names[i]);
    }
    names = core.getMemberRegistryNames(node, setName, memberPath);
    for (i = 0; i < names.length; i++) {
      retObj.registry[names[i]] = core.getMemberRegistry(node, setName,
        memberPath, names[i]);
    }
    return retObj;
  };
    ...
    var createOneSetInfo = function (setName, callback) {
      var needed,
        ...
        collectSetInfo = function (nodePath, container, callback) {
          if (container === true) {
            pathToRefObjAsync(refType, urlPrefix, nodePath, core, core.getRoot(
              node), function (err, refObj) {
              if (!err && refObj !== undefined && refObj !== null) {
                getSetAttributesAndRegistry(core, node, setName,
                  nodePath, function (err, atrAndReg) {
                    if (atrAndReg) {
            ...
            pathToRefObjAsync(refType, urlPrefix, nodePath, core, core.getRoot(
              ...
              if (refObj !== undefined && refObj !== null) {
                var atrAndReg = getMemberAttributesAndRegistry(core,
                  node, setName, nodePath);
                for (var j in atrAndReg) {
                  refObj[j] = atrAndReg[j];
                }
                callback(err, refObj);
  ...
  var getPointersGUIDs = function (core, node, callback) {
    ...
    if (needed > 0) {
      ...
      for (var i = 0; i < collectionNames.length; i++) {
        core.loadCollection(node, collectionNames[i], function (err,
          collection) {
          error = error || err;
          if (collection) {
            for (var j = 0; j < collection.length; j++) {
              if (gHash[core.getPath(collection[j])] === undefined) {
                gHash[core.getPath(collection[j])] = core.getGuid(
                  collection[j]);
              }
            }
          }
  ...
  var getJsonNode = function (core, node, urlPrefix, refType, callback) {
    ...
    var needed = 4,
      error = null;
    getChildrenOfNode(core, node, urlPrefix, refType, function (err,
      children) {
      error = error || err;
    ...
    getPointersOfNode(core, node, urlPrefix, refType, function (err,
      pointers) {
      error = error || err;
