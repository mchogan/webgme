...
define([], function () {
  ...
  function metaStorage() {
    ...
    function getMeta(path) {
      ...
      if (node) {
        ...
        for (var i = 0; i < meta.children.items.length; i++) {
          meta.children.minItems.push(_core.getMemberAttribute(childrenNode,
            "items", meta.children.items[i], "min") || -1);
          meta.children.maxItems.push(_core.getMemberAttribute(childrenNode,
            "items", meta.children.items[i], "max") || -1);
        ...
        for (var i = 0; i < atrNames.length; i++) {
          meta.attributes[atrNames[i]] = JSON.parse(JSON.stringify(_core.getAttribute(
            metaNode, atrNames[i])));
        }
        ...
        for (var i = 0; i < pointerNames.length; i++) {
          ...
          for (var j = 0; j < pointer.items.length; j++) {
            pointer.minItems.push(_core.getMemberAttribute(pointerNode,
              "items", pointer.items[j], "min") || -1);
            pointer.maxItems.push(_core.getMemberAttribute(pointerNode,
              "items", pointer.items[j], "max") || -1);
        ...
        if (aspectNames.length > 0) {
          ...
          for (var i = 0; i < aspectNames.length; i++) {
            var aspectNode = _core.getChild(aspectsNode, "_a_" + aspectNames[
              i]);
            meta.aspects[aspectNames[i]] = {
              items: []
            };
      ...
      if (node) {
            ...
            for (var i = 0; i < meta.children.items.length; i++) {
              ...
              if (typeof targetPath === 'string' && _nodes[targetPath]) {
                ...
                if (meta.children.minItems[i] !== -1) {
                  _core.setMemberAttribute(childrenNode, "items", targetPath,
                    "min", meta.children.minItems[i]);
                }
                if (meta.children.maxItems[i] !== -1) {
                  _core.setMemberAttribute(childrenNode, "items", targetPath,
                    "max", meta.children.maxItems[i]);
                }
              }
            }
              ...
              for (var j = 0; j < meta.pointers[i].items.length; j++) {
                ...
                if (typeof targetPath === 'string' && _nodes[targetPath]) {
                  ...
                  if (meta.pointers[i].minItems[j] !== -1) {
                    _core.setMemberAttribute(pointerNode, "items", targetPath,
                      "min", meta.pointers[i].minItems[j]);
                  }
                  if (meta.pointers[i].maxItems[j] !== -1) {
                    _core.setMemberAttribute(pointerNode, "items", targetPath,
                      "max", meta.pointers[i].maxItems[j]);
                  }
                }
              }
        ...
        if (meta.aspects) {
          ...
          if (aspectNames.length > 0) {
            meta.aspects = {};
            for (var i = 0; i < aspectNames.length; i++) {
              var aspectNode = _core.getChild(aspectsNode, "_a_" +
                aspectNames[i]);
              meta.aspects[aspectNames[i]] = {
                items: []
              };
              var items = _core.getMemberPaths(aspectNode, "items");
              for (var j = 0; j < items.length; j++) {
                meta.aspects[aspectNames[i]].items.push(pathToRefObject(items[
                  j]));
              }
            }
          }
    ...
    function getValidChildrenTypes(path) {
      var node = _nodes[path];
      if (node) {
        return _core.getMemberPaths(_core.getChild(_core.getChild(node,
          "_meta"), "children"), "items");
      }
      return [];
    }
    ...
    function getOwnValidTargetTypes(path, name) {
      ...
      if (node) {
        ...
        var base = getValidTargetTypes(_core.getPath(_core.getBase(node)),
          name);
        for (var i = 0; i < own.length; i++) {
          if (base.indexOf(own[i]) === -1) {
            items.push(own[i]);
        ...
        if (childrenMeta.items !== null) {
          for (var i = 0; i < childrenMeta.items.length; i++) {
            ...
            if (rawMeta.children.minItems) {
              child.min = rawMeta.children.minItems[i] === -1 ? undefined :
                rawMeta.children.minItems[i];
            }
            if (rawMeta.children.maxItems) {
              child.max = rawMeta.children.maxItems[i] === -1 ? undefined :
                rawMeta.children.maxItems[i];
            }
    ...
    function updateValidChildrenItem(path, newTypeObj) {
      if (newTypeObj && newTypeObj.id) {
        var rawMeta = getMeta(path);
        if (rawMeta) {
          if (rawMeta.children.minItems === null || rawMeta.children.minItems ==
            undefined) {
          ...
          if (rawMeta.children.maxItems === null || rawMeta.children.maxItems ==
            undefined) {
          ...
          var index = indexOfPathInRefObjArray(rawMeta.children.items,
            newTypeObj.id);
          if (index === -1) {
          ...
          (newTypeObj.min === null || newTypeObj.min === undefined) ? rawMeta
            .children.minItems[index] = -1 : rawMeta.children.minItems[index] =
            newTypeObj.min;
          (newTypeObj.max === null || newTypeObj.max === undefined) ? rawMeta
            .children.maxItems[index] = -1 : rawMeta.children.maxItems[index] =
            newTypeObj.max;
          ...
          for (var i = 0; i < pointerMeta.items.length; i++) {
            ...
            if (rawMeta.pointers[name].minItems) {
              child.min = rawMeta.pointers[name].minItems[i] === -1 ?
                undefined : rawMeta.pointers[name].minItems[i];
            }
            if (rawMeta.pointers[name].maxItems) {
              child.max = rawMeta.pointers[name].maxItems[i] === -1 ?
                undefined : rawMeta.pointers[name].maxItems[i];
            }
    ...
    function updateValidTargetItem(path, name, targetObj) {
      ...
      if (rawMeta && targetObj && targetObj.id) {
        ...
        (targetObj.min === null || targetObj.min === undefined) ? pointer.minItems[
          index] = -1 : pointer.minItems[index] = targetObj.min;
        (targetObj.max === null || targetObj.max === undefined) ? pointer.maxItems[
          index] = -1 : pointer.maxItems[index] = targetObj.max;
    ...
    function getOwnMetaAspectNames(path) {
      var names = getMetaAspectNames(path),
        ownNames = [];
      if (_nodes[path]) {
        var baseNames = getMetaAspectNames(_core.getPath(_core.getBase(_nodes[
          path])));
