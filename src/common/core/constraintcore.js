  ...
  function constraintCore(_innerCore) {
    ...
    var getConstraintRelId = function (constraintsNode, name) {
      ...
      for (var i = 0; i < relIds.length; i++) {
        if (name === _innerCore.getAttribute(_innerCore.getChild(
          constraintsNode, relIds[i]), "name")) {
          relId = relIds[i];
          break;
        }
      }
    ...
    _core.setConstraint = function (node, name, constraintObj) {
      ...
      constraintObj.script = constraintObj.script ||
        "console.log(\"empty constraint\");";
      ...
      _innerCore.setRegistry(node, getRegConstName(name), (_innerCore.getRegistry(
        node, getRegConstName(name)) || 0) + 1);
    };
    ...
    _core.getConstraintNames = function (node) {
      ...
      var names = [];
      for (var i = 0; i < relIds.length; i++) {
        names.push(_innerCore.getAttribute(_innerCore.getChild(
          constraintsNode, relIds[i]), "name"));
      }
      return names;
    };
