...
define(["util/assert", "core/core", "core/tasync"], function (ASSERT, Core,
  TASYNC) {
  "use strict";
  ...
  var CoreType = function (oldcore) {
    ...
    var loadDescendantByPath = function (node, pathArray, index) {
      ...
      return TASYNC.call(loadDescendantByPath, child, pathArray, index +
        1);
    };
    ...
    function __loadBase(node) {
      ASSERT(node === null || typeof node.base === "undefined" || typeof node
        .base === "object");
      ...
      if (typeof node.base === "undefined") {
        if (core.isEmpty(node)) {
          ...
          return null;
        } else if (node.parent && node.parent.base && oldcore.getChildrenRelids(
          node.parent.base).indexOf(node.relid) !== -1) {
          ...
          return TASYNC.call(__loadBase2, node, oldcore.loadPointer(node,
            "base"));
        }
      } else {
    ...
    core.getChildrenRelids = function (node) {
      var inheritRelIds = node.base === null ? [] : core.getChildrenRelids(
        core.getBase(node));
    ...
    function _isInheritedChild(node) {
      ...
      if (baseParent && parentBase && core.getPath(baseParent) === core.getPath(
        parentBase)) {
        return true;
      }
      return false;
    ...
    core.setBase = function (node, base) {
      ASSERT(isValidNode(node) && (base === undefined || base === null ||
        isValidNode(base)));
      ASSERT(!base || core.getPath(core.getParent(node)) !== core.getPath(
        base));
    ...
    core.getChild = function (node, relid) {
      ASSERT(isValidNode(node) && (typeof node.base === 'undefined' ||
    ...
    core.copyNodes = function (nodes, parent) {
      ...
      for (i = 0; i < nodes.length; i++) {
        ...
        for (j = 0; j < names.length; j++) {
          core.setPointer(copiedNodes[i], names[j], copiedNodes[relations[i]
            [names[j]]]);
        }
      }
