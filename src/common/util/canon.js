...
define([], function () {
  ...
  CANON.stringify = (function () {
    ...
    canonicalize = function (value) {
      ...
      case '[object Date]':
        return ['Date'].concat(isFinite(+value) ? value.getUTCFullYear() +
          '-' + pad(value.getUTCMonth() + 1) + '-' + pad(value.getUTCDate()) +
          'T' + pad(value.getUTCHours()) + ':' + pad(value.getUTCMinutes()) +
          ':' + pad(value.getUTCSeconds()) + '.' + pad(value.getUTCMilliseconds(),
            3) + 'Z' : null);
      ...
      case '[object Object]':
        ...
        return (_ref = ['Object']).concat.apply(_ref, map(keys(value).sort(),
          pair));
      case '[object RegExp]':
        return ['RegExp', "" + value];
  ...
  CANON.parse = (function () {
    ...
    canonicalize = function (value) {
      ...
      case 'RegExp':
        ...
        })(RegExp, /^[/](.+)[/]([gimy]*)$/.exec(element).slice(1),
          function () {});
      case 'Undefined':
        return void 0;
      default:
