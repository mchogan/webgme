define([
  ...
  function Rest(_parameters) {
    ...
    _parameters.authorization = /*_parameters.authorization || */ function (
      token, projectname, callback) {
      callback(null, true);
    ...
    var _storage = new Storage({
        ...
        'log': logManager.create('REST-actor')
      }),
        ...
        commands: {
          'GET': {
            ...
            'commit': {
              ...
              'example': _baseUrl +
                '/commit?project=projectName&commit=commitHash'
            },
            'commits': {
            ...
            'node': {
              ...
              'example': _baseUrl +
                '/node?project=projectName&root=rootHash&path=pathOfNode'
            },
            'dump': {
              ...
              'example': _baseUrl +
                '/dump?project=projectName&root=rootHash&path=pathOfNode'
            },
            'etf': {
            ...
            if (err) {
              ...
              for (var i in names) {
                names[i] = URL.urlToRefObject(_baseUrl +
                  '/commit?project=' + projectName + '&commit=' + URL.addSpecialChars(
                    names[i]));
              }
              callback(_HTTPError.ok, names);
            }
        ...
        if (err) {
          ...
          project.loadObject(commitHash, function (err, commit) {
            if (err) {
              ...
              var myCommit = {};
              myCommit.self = URL.urlToRefObject(_baseUrl +
                '/commit?ptoject=' + projectName + '&commit=' + URL.addSpecialChars(
                  commitHash));
              myCommit.root = URL.urlToRefObject(_baseUrl +
                '/node?project=' + projectName + '&root=' + URL.addSpecialChars(
                  commit.root));
              myCommit.parents = [];
              for (var i = 0; i < commit.parents.length; i++) {
                myCommit.parents.push(URL.urlToRefObject(_baseUrl +
                  '/commit?project=' + projectName + '&commit=' + URL.addSpecialChars(
                    commit.parents[i])));
              }
    ...
    function listCommits(projectName, latestNCommit, callback) {
      _storage.openProject(projectName, function (err, project) {
        if (err) {
          ...
          project.getCommits((new Date()).getTime(), latestNCommit,
            function (err, commits) {
              if (err) {
                callback(_HTTPError.internalServerError, err);
              } else {
                for (var i = 0; i < commits.length; i++) {
                  commits[i] = URL.urlToRefObject(_baseUrl +
                    '/commit?project=' + projectName + '&commit=' + URL.addSpecialChars(
                      commits[i]['_id']));
                }
            ...
            if (err) {
              ...
              core.loadByPath(root, path, function (err, node) {
                if (err) {
                  ...
                  ToJson(core, node, _baseUrl + '/node?project=' +
                    projectName + '&root=' + URL.addSpecialChars(
                      rootHash), 'url', function (err, jNode) {
                      if (err) {
                        callback(_HTTPError.internalServerError, err);
                      } else {
            ...
            if (err) {
              ...
              core.loadByPath(root, path, function (err, node) {
                if (err) {
                  ...
                  Dump(core, node, _baseUrl + '/dump/' + projectName +
                    '/' + URL.addSpecialChars(rootHash), 'guid',
                    function (err, dump) {
                      if (err) {
                        callback(_HTTPError.internalServerError, err);
                      } else {
    ...
    function doGET(command, token, parameters, callback) {
      ...
      case _commands.branches:
        _parameters.authorization(token, parameters.project, function (err,
          canGo) {
          if (err) {
            callback(_HTTPError.internalServerError, err);
          } else {
      ...
      case _commands.commits:
        _parameters.authorization(token, parameters.project, function (err,
          ...
          if (err) {
            ...
            if (canGo === true) {
              listCommits(parameters.project, Number(parameters.number) ===
                'NaN' ? 1 : Number(parameters.number), callback);
            } else {
              callback(_HTTPError.forbidden);
            }
          }
      ...
      case _commands.node:
        _parameters.authorization(token, parameters.project, function (err,
          ...
          if (err) {
            ...
            if (canGo === true) {
              printNode(parameters.project, parameters.root, parameters.path ||
                "", callback);
            } else {
              callback(_HTTPError.forbidden);
            }
      ...
      case _commands.dump:
        _parameters.authorization(token, parameters.project, function (err,
          ...
          if (err) {
            ...
            if (canGo === true) {
              dumpNode(parameters.project, parameters.root, parameters.path ||
                "", callback);
            } else {
              callback(_HTTPError.forbidden);
            }
      ...
      case _commands.etf:
        _parameters.authorization(token, parameters.project, function (err,
          ...
          if (err) {
            ...
            if (canGo === true) {
              exportProject(parameters.project, parameters.root, parameters
                .branch, parameters.commit, callback);
            } else {
              callback(_HTTPError.forbidden);
            }
          }
    ...
    function doRESTCommand(requestType, command, token, parameters, callback) {
      if (_opened) {
        if (_parameters.baseUrl.indexOf('/rest/token') !== -1) {
          _baseUrl = _parameters.baseUrl.replace('/rest/token', '/rest/' +
            token);
        } else {
          _baseUrl = _parameters.baseUrl;
        }
        ...
        callback(_HTTPError.serviceUnavailable, {
          'msg': 'REST actor not yet connected to the database!'
        });
      }
    }
