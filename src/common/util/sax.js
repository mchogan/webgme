define(function (require, exports, module) {
  ...
  (function (sax) {
    ...
    ["text", "processinginstruction", "sgmldeclaration", "doctype",
      "comment", "attribute", "opentag", "closetag", "opencdata", "cdata",
      "closecdata", "error", "end", "ready", "script", "opennamespace",
      "closenamespace"
    ]
    ...
    function SAXParser(strict, opt) {
      ...
      parser.looseCase = parser.opt.lowercase ? "toLowerCase" :
        "toUpperCase"
      parser.tags = []
    ...
    SAXStream.prototype.on = function (ev, handler) {
      ...
      if (!me._parser["on" + ev] && streamWraps.indexOf(ev) !== -1) {
        me._parser["on" + ev] = function () {
          var args = arguments.length === 1 ? [arguments[0]] : Array.apply(
            null, arguments)
          args.splice(0, 0, ev)
    ...
    function attrib(parser) {
      ...
      if (parser.opt.xmlns) {
        ...
        if (prefix === "xmlns") {
          ...
          } else if (local === "xmlns" && parser.attribValue !==
            XMLNS_NAMESPACE) {
    ...
    function openTag(parser, selfClosing) {
      if (parser.opt.xmlns) {
        ...
        if (tag.prefix && !tag.uri) {
          strictFail(parser, "Unbound namespace prefix: " + JSON.stringify(
            parser.tagName))
        }
    ...
    function write(chunk) {
      ...
      //   parser.cdata = ""
      // }
      if (parser.position >= parser.bufferCheckPosition) checkBufferLength(
        parser)
      return parser
    }
