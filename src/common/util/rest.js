define([
  ...
  function Rest(_configuration) {
          ...
          if (err) {
            ...
            for (var i = 0; i < collection.length; i++) {
              ...
              if (path.indexOf('/_sets/') === -1) {
                ...
                path = path.split('/');
                var setOwner = "",
                  jMax = path.indexOf('_sets');
                for (var j = 0; j < jMax; j++) {
                  setOwner += "/" + path[j];
                }
                if (!outNode.member[_buffer.core.getSetName(path[jMax +
                  1])]) {
    ...
    var persistProject = function (callback, isdelete) {
      ...
      var newCommitHash = _buffer.project.makeCommit([_buffer.commit],
        newRootHash, "REST commit", function () {});
      if (isdelete) {
        ...
        loadingNode(_buffer.core, _buffer.coreNode, function (err, node) {
          if (err) {
            callback(null, JSON.parse(addingSpecialChars(JSON.stringify(
              retVal))));
          } else {
            retVal.node = node;
            callback(null, JSON.parse(addingSpecialChars(JSON.stringify(
    ...
    var updateNode = function (data, callback) {
      ...
      var updatePointer = function (pointerName, callback) {
        _buffer.core.loadByPath(specialCharHandling(data.pointer[
          pointerName]), function (err, node) {
          if (!err && node) {
      ...
      var updateMember = function (setName, memberPath, isDelete, callback) {
        _buffer.core.loadByPath(specialCharHandling(memberPath), function (
          err, node) {
          if (!err && node) {
            if (isDelete) {
      ...
      for (i in _buffer.formattedNode.sets) {
        if (!data.sets[i]) {
          ...
          for (var j = 0; j < _buffer.formattedNode.sets[i].length; j++) {
            memberToUpdate.push({
              set: i,
              path: _buffer.formattedNode.sets[i][j],
              del: true
            });
          }
        }
      ...
      for (i in data.sets) {
        if (!_buffer.formattedNode.sets[i]) {
          ...
          for (j = 0; j < _buffer.formattedNode.sets[i].length; j++) {
            if (data.sets[i].indexOf(_buffer.formattedNode.sets[i][j]) ===
              -1) {
              memberToUpdate.push({
                set: i,
                path: _buffer.formattedNode.sets[i][j],
          ...
          //then the inserts
          for (j = 0; j < data.sets[i].length; j++) {
            if (_buffer.formattedNode.sets[i].indexOf(data.sets[i][j]) ===
              -1) {
              memberToUpdate.push({
                set: i,
                path: data.sets[i][j],
      ...
      if (needToGo < 1) {
        ...
        for (j = 0; j < memberToUpdate.length; j++) {
          updateMember(memberToUpdate[j].set, memberToUpdate[j].path,
            memberToUpdate[j].del, function (err) {
              if (err) {
                error = err;
              }
              if (--needToGo === 0) {
                  ...
                  if (pars[0].indexOf('#') === 0) {
                    ...
                    getBranches(project, function (err, branches) {
                      if (err) {
                        ...
                        if (branches[pars[0]]) {
                          callback(null, addingSpecialChars(branches[
                            pars[0]]));
                        } else {
          ...
          if (typeof fNode === 'string') {
            ...
            if (data.oldhash && data.newhash) {
              if (data.oldhash === fNode) {
                _buffer.project.setBranchHash(_buffer.branch, data.oldhash,
                  data.newhash, callback);
              } else {
      ...
      processGET(uri, function (err, parent) {
        if (err) {
          if (err === 'no such branch found') {
            //branch creation
            _buffer.project.setBranchHash(_buffer.branch, "",
              specialCharHandling(data), callback);
          } else {
            callback(err);
          }
        } else {
          ...
          loadingNode(_buffer.core, _buffer.coreNode, function (err,
            formatted) {
            if (err) {
              callback(err, null);
            } else {
      ...
      processGET(uri, function (err, node) {
        if (err) {
          ...
          if (typeof node === "string") {
            //branch deletion
            _buffer.project.setBranchHash(_buffer.branch,
              specialCharHandling(node), "", callback);
          } else {
            //node deletion
