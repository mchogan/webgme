...
define(["util/assert", "storage/mongo", "storage/cache", "storage/commit",
  "core/tasync", "util/sax", "fs", "bin/getconfig", "storage/client",
  "core/core"
], function (ASSERT, Mongo, Cache, Commit, TASYNC, SAX, FS, CONFIG, Client,
  Core) {
  ...
  function openDatabase() {
    ...
    var isSIO = paramsMongo === null && paramsSIO !== null && paramsSIO.length >
      0;
    ...
    if (isSIO) {
      opt.type = "node";
      console.log("Opening database through socket.io connection on " + opt.host +
        (opt.port ? ":" + opt.port : ""));
      database = new Commit(new Cache(new Client(opt), {}));
    } else {
      console.log("Opening mongo database " + opt.database + " on " + opt.host +
        (opt.port ? ":" + opt.port : ""));
  ...
  var saxParse = TASYNC.wrap(function (xmlfile, handler, callback) {
    ...
    stream.on("error", function (err) {
      setProgress(null);
      callback(new Error(err.code === "ENOENT" ? "File not found: " +
        xmlfile : "Unknown file error: " + JSON.stringify(err)));
    });
    ...
    setProgress(function () {
      ...
      console.log("  at line " + parser._parser.line + (typeof stat ===
        "string" ? " (" + stat + ")" : ""));
    });
  });
