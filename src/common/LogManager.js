...
define([], function () {
  ...
  isComponentAllowedToLog = function (componentName) {
    ...
    for (i = 0; i < excludedComponents.length; i += 1) {
      ...
      if (excludedComponentName.substr(-1) === "*") {
        excludedComponentName = excludedComponentName.substring(0,
          excludedComponentName.length - 1);

        if (componentName.substring(0, excludedComponentName.length) ===
  ...
  logMessage = function (level, componentName, msg) {
    var logTime = new Date(),
      logTimeStr = (logTime.getHours() < 10) ? "0" + logTime.getHours() :
      logTime.getHours(),
      levelStr = level,
    ...
    //logTimeString
    logTimeStr += ":";
    logTimeStr += (logTime.getMinutes() < 10) ? "0" + logTime.getMinutes() :
      logTime.getMinutes();
    logTimeStr += ":";
    logTimeStr += (logTime.getSeconds() < 10) ? "0" + logTime.getSeconds() :
      logTime.getSeconds();
    logTimeStr += ".";
    logTimeStr += (logTime.getMilliseconds() < 10) ? "00" + logTime.getMilliseconds() :
      ((logTime.getMilliseconds() < 100) ? "0" + logTime.getMilliseconds() :
    ...
    if (isComponentAllowedToLog(componentName) === true) {
      if (logFilePath) {
        msg = levelStr + " - " + logTimeStr + " [" + componentName + "] - " +
          msg + "\n";
        if (logFileBuffer.length === 0) {
        ...
        if (logLevels[level] <= currentLogLevel) {
          ...
          if (console && console.log) {
            ...
            concreteLogger.call(console, levelStr + " - " + logTimeStr + " [" +
              componentName + "] - " + msg);
          }
        }
      }
