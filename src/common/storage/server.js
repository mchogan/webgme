...
define([ "util/assert", "util/guid", "util/url", "socket.io", "worker/serverworkermanager" ], function (ASSERT, GUID, URL, IO, SWM) {
  ...
  var server = function (_database, options) {
    ...
    options.authInfo = options.authInfo || function (sessionID, projectName, callback) {
    ...
    function open() {
      ...
      _socket.on('connection', function (socket) {
        ...
        socket.on('getCommonAncestorCommit', function (projectName, commitA, commitB, callback) {
          checkProject(getSessionID(socket), projectName, function (err, project) {
            if (err) {
              callback(err);
            } else {
              project.getCommonAncestorCommit(commitA, commitB, callback);
            }
          });
        });
