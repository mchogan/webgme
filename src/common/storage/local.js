...
define(["util/assert"], function (ASSERT) {
  ...
  function Database(options) {
    ...
    function getDatabaseStatus(oldstatus, callback) {
      ASSERT(typeof callback === "function" && (typeof oldstatus === "string" ||
        oldstatus === null));
    ...
    function openProject(project, callback) {
      ...
      function loadObject(hash, callback) {
        ...
        var object = storage.getItem(database + SEPARATOR + project +
          SEPARATOR + hash);
        if (object) {
      ...
      function insertObject(object, callback) {
        ...
        try {
          storage.setItem(database + SEPARATOR + project + SEPARATOR + object
            ._id, JSON.stringify(object));
        } catch (e) {
          callback(e);
        }
        callback(null);
      ...
      function findHash(beginning, callback) {
        ASSERT(typeof beginning === "string" && typeof callback ===
          "function");
        ...
        if (!HASH_REGEXP.test(beginning)) {
          ...
          var found = 0,
            fullKey = "";
          for (var i = 0; i < storage.length; i++) {
            if (storage.key(i).indexOf(database + SEPARATOR + project +
              SEPARATOR + beginning) === 0) {
              found++;
      ...
      function getBranchHash(branch, oldhash, callback) {
        ...
        ASSERT(typeof oldhash === "string" && (oldhash === "" || HASH_REGEXP.test(
          oldhash)));
        ASSERT(typeof callback === "function");

        var hash = storage.getItem(database + SEPARATOR + project + SEPARATOR +
          branch);
        if (hash) {
        ...
        if (hash !== oldhash) {
          ...
          setTimeout(function () {
            hash = storage.getItem(database + SEPARATOR + project +
              SEPARATOR + branch);
            if (hash) {
              hash = JSON.parse(hash);
            }
      ...
      function setBranchHash(branch, oldhash, newhash, callback) {
        ...
        ASSERT(typeof oldhash === "string" && (oldhash === "" || HASH_REGEXP.test(
          oldhash)));
        ASSERT(typeof newhash === "string" && (newhash === "" || HASH_REGEXP.test(
          newhash)));
        ASSERT(typeof callback === "function");
        ...
        if (oldhash === newhash) {
          ...
          if (oldhash === hash) {
            if (newhash === "") {
              storage.removeItem(database + SEPARATOR + project + SEPARATOR +
                branch);
            } else {
              storage.setItem(database + SEPARATOR + project + SEPARATOR +
