...
define(["util/assert", "util/guid", "util/url", "socket.io",
  "worker/serverworkermanager"
], function (ASSERT, GUID, URL, IO, SWM) {
  ...
  var server = function (_database, options) {
    ...
    options.authorization = options.authorization || function (sessionID,
      projectName, type, callback) {
      callback(null, true);
    };
    options.sessioncheck = options.sessioncheck || function (sessionID,
      callback) {
      callback(null, true);
    };
    options.authInfo = options.authInfo || function (sessionID, projectName,
      callback) {
      callback(null, {
        'read': true,
        'write': true,
      ...
      if (_databaseOpened) {
        ...
        _database.openDatabase(function (err) {
          if (err) {
            ...
            console.log('Error in mongoDB connection initiation!!! - ',
              err);
    ...
    function checkProject(client, projectName, callback) {
      options.authorization(client, projectName, 'read', function (err,
        cando) {
        if (!err && cando === true) {
    ...
    function eventRequest(latestGuid, callback) {
      ...
      if (index === -1 || index === 0) {
        ...
        callback(null, _eventHistory[index - 1], _events[_eventHistory[
          index - 1]]);
      }
    }
    ...
    function open() {
      ...
      _socket.set('authorization', function (data, accept) {
        ...
        if (options.session === true) {
          ...
          if (data.webGMESessionId === undefined) {
            if (data.query.webGMESessionId && data.query.webGMESessionId !==
              'undefined') {
          ...
          if (sessionID === null || sessionID === undefined) {
            if (data.headers.cookie) {
              ...
              if (cookie[options.cookieID] !== undefined || cookie[
                options.cookieID] !== null) {
                sessionID = require('connect').utils.parseSignedCookie(
                  cookie[options.cookieID], options.secret);
      ...
      _socket.server.on('error', function (err) {
        console.log("Error have been raised on socket.io level!!! - ",
          err);
      ...
      _socket.on('connection', function (socket) {
              ...
              _database.getProjectNames(function (err, names) {
                if (!err) {
                  ...
                  var isProjectReadable = function (name, callback) {
                    options.authInfo(getSessionID(socket), name,
        ...
        socket.on('deleteProject', function (projectName, callback) {
          options.authorization(getSessionID(socket), projectName,
            'delete', function (err, cando) {
              if (err || !cando) {
                callback(err);
              } else {
                _database.deleteProject(projectName, function (err) {
                  if (err) {
                    ...
                    fireEvent({
                      type: SERVER_EVENT.PROJECT_DELETED,
                      project: projectName
                    });
                    callback(null);
                  }
            ...
            if (err) {
              ...
              _database.getProjectNames(function (err, names) {
                if (names.indexOf(projectName) === -1) {
                  //project creation
                  createProject(getSessionID(socket), projectName,
                    function (err, project) {
                      if (!err) {
                        fireEvent({
                          type: SERVER_EVENT.PROJECT_CREATED,
                          project: projectName
                        });
                      }
                  ...
                  checkProject(getSessionID(socket), projectName,
                    callback);
                }
              });
            }
        ...
        socket.on('closeProject', function (projectName, callback) {
          ...
          checkProject(getSessionID(socket), projectName, function (err,
            project) {
            if (err) {
              callback(err);
            } else {
        ...
        socket.on('getBranchHash', function (projectName, branch, oldhash,
          callback) {
          checkProject(getSessionID(socket), projectName, function (err,
            project) {
            if (err) {
        ...
        socket.on('setBranchHash', function (projectName, branch, oldhash,
          newhash, callback) {
          checkProject(getSessionID(socket), projectName, function (err,
            project) {
            if (err) {
              ...
              options.authorization(getSessionID(socket), projectName,
                'write', function (err, cando) {
                  if (!err && cando === true) {
                    project.setBranchHash(branch, oldhash, newhash,
                      function (err) {
                        if (!err) {
        ...
        socket.on('getCommits', function (projectName, before, number,
          callback) {
          checkProject(getSessionID(socket), projectName, function (err,
            project) {
            if (err) {
