  ...
  function Database(options) {
    ...
    function reSendGetBranches() {
      ...
      for (var i in getBranchHashCallbacks) {
        projects[getBranchHashCallbacks[i].project].getBranchHash(i,
          getBranchHashCallbacks[i].oldhash, getBranchHashCallbacks[i].cb);
      }
    }
          ...
          socket.on('connect', function () {
            ...
            if (firstConnection) {
              ...
              socket.emit('openDatabase', function (err) {
                if (!err) {
                  socket.emit('getDatabaseStatus', null, function (err,
                    newstatus) {
                    if (!err && newstatus) {
                      status = newstatus;
                    }
              ...
              socket.emit('getDatabaseStatus', status, function (err,
                newstatus) {
                if (!err && newstatus) {
                  status = newstatus;
                  clearDbCallbacks();
    ...
    function getDatabaseStatus(oldstatus, callback) {
      ...
      if (status !== oldstatus) {
        ...
        if (status !== STATUS_NETWORK_DISCONNECTED) {
          socket.emit('getDatabaseStatus', oldstatus, function (err,
            newstatus) {
            if (!err && newstatus) {
              status = newstatus;
            }
    ...
    function getNextServerEvent(latestGuid, callback) {
      if (socketConnected) {
        ...
        socket.emit('getNextServerEvent', latestGuid, function (err, newGuid,
          eventParams) {
          if (callbacks[guid]) {
            clearTimeout(callbacks[guid].to);
            delete callbacks[guid];
    ...
    function openProject(project, callback) {
      ...
      function getBranchHash(branch, oldhash, callback) {
        ...
        if (socketConnected) {
          socket.emit('getBranchHash', project, branch, oldhash, function (
            err, newhash, forkedhash) {
      ...
      function setBranchHash(branch, oldhash, newhash, callback) {
        ...
        if (socketConnected) {
          ...
          socket.emit('setBranchHash', project, branch, oldhash, newhash,
            function (err) {
              if (callbacks[guid]) {
                clearTimeout(callbacks[guid].to);
                delete callbacks[guid];
      ...
      function getCommits(before, number, callback) {
        ...
        if (socketConnected) {
          ...
          socket.emit('getCommits', project, before, number, function (err,
            commits) {
            if (callbacks[guid]) {
              clearTimeout(callbacks[guid].to);
              delete callbacks[guid];
      ...
      function makeCommit(parents, roothash, msg, callback) {
        ...
        if (socketConnected) {
          ...
          socket.emit('makeCommit', project, parents, roothash, msg, function (
            err) {
            if (callbacks[guid]) {
              clearTimeout(callbacks[guid].to);
              delete callbacks[guid];
