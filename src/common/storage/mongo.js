...
define(["mongodb", "util/assert", "util/canon"], function (MONGODB, ASSERT,
  CANON) {
  "use strict";
  ...
  function Database(options) {
    ...
    function openDatabase(callback) {
      ...
      MONGODB.MongoClient.connect("mongodb://" + options.host + ":" + options
        .port + "/" + options.database, {
          'w': 1,
          'native-parser': true,
          'auto_reconnect': true,
          'poolSize': 20,
    ...
    function getDatabaseStatus(oldstatus, callback) {
      ...
      if (mongo === null) {
        ...
        mongo.command({
          ...
          reportStatus(oldstatus, err ? STATUS_UNREACHABLE :
            STATUS_CONNECTED, callback);
        });
      }
    }
    ...
    function openProject(name, callback) {
        ...
        collection.insert(object, function (err) {
          ...
          if (err && err.code === 11000) {
            collection.findOne({
              ...
              if (!err2 && CANON.stringify(object) === CANON.stringify(
                data)) {
                callback(null);
              } else {
                callback(err);
      ...
      function getBranchHash(branch, oldhash, callback) {
        ...
        ASSERT(oldhash === null || (typeof oldhash === "string" && (oldhash ===
          "" || HASH_REGEXP.test(oldhash))));
        ASSERT(typeof callback === "function");
