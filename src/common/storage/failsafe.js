  ...
  function Database(_database, options) {
    ...
    function savePending() {
      ...
      for (var i in pendingStorage) {
        storage.setItem(fsId + SEPARATOR + dbId + SEPARATOR + i, JSON.stringify(
          pendingStorage[i]));
      }
    }
    ...
    function openProject(projectName, callback) {
      ...
      function synchronise(callback) {
        if (pendingStorage[projectName]) {
          ...
          for (i in pendingStorage[projectName][BRANCH_OBJ_ID]) {
            if (pendingStorage[projectName][BRANCH_OBJ_ID][i].state ===
              BRANCH_STATES.DISCONNECTED) {
              if (pendingStorage[projectName][BRANCH_OBJ_ID][i].local.length >
                0) {
                pendingStorage[projectName][BRANCH_OBJ_ID][i].state =
                  BRANCH_STATES.AHEAD;
      ...
      function synchroniseBranch(branchname, callback) {
        ...
        project.getBranchHash(branchname, branchObj.local[0], function (err,
          newhash, forked) {
          if (!err && newhash) {
            if (branchObj.local.indexOf(newhash) !== -1) {
              project.setBranchHash(branchname, newhash, branchObj.local[0],
                callback);
            } else {
              //we forked
        ...
        project.loadObject(hash, function (err, object) {
          if (!err && object) {
            ...
            if (exceptionErrors.indexOf(err) !== -1) {
              ...
              if (pendingStorage[projectName] && pendingStorage[projectName]
                [hash]) {
      ...
      function getBranchNames(callback) {
        project.getBranchNames(function (err, names) {
          ...
          for (var i in pendingStorage[projectName][BRANCH_OBJ_ID]) {
            if (pendingStorage[projectName][BRANCH_OBJ_ID][i].local.length >
              0) {
              locals[i] = pendingStorage[projectName][BRANCH_OBJ_ID][i].local[
                0];
            } else if (pendingStorage[projectName][BRANCH_OBJ_ID][i].fork ===
              null && pendingStorage[projectName][BRANCH_OBJ_ID][i].remote !==
              null) {
          ...
          if (err) {
            ...
            for (i in names) {
              if (!locals[i]) {
                locals[i] = names[i];
              } else if (locals[i] === pendingStorage[projectName][
                BRANCH_OBJ_ID
              ][i].remote) {
                locals[i] = names[i];
              }
      ...
      function getBranchHash(branch, oldhash, callback) {
        ...
        if (branchObj.state === BRANCH_STATES.SYNC || branchObj.state ===
          BRANCH_STATES.AHEAD) {
          project.getBranchHash(branch, oldhash, function (err, newhash,
            forkedhash) {
            if (!err && newhash) {
