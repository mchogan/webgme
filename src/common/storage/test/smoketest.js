...
requirejs([
  ...
  function (
        ...
        if (!err && oldstatus) {
          if (oldstatus.indexOf('discon') === -1) {
            ...
            object.getDatabaseStatus(oldstatus, function (err, newstatus) {
              console.log('checkServerClose->getDatabaseStatus', err,
                newstatus);
      ...
      object.getDatabaseStatus(null, function (err, oldstatus) {
        ...
        if (!err && oldstatus) {
          if (oldstatus.indexOf('discon') !== -1) {
            var canGoOn = false;
            object.getDatabaseStatus(oldstatus, function (err, newstatus) {
              console.log('checkServerReopen->getDatabaseStatus', err,
                newstatus);
              if (!err && newstatus && newstatus !== oldstatus) {
                canGoOn = true;
                  ...
                  checkFsync(clientTwo, function () {
                    checkFsync(projectOne, function () {
                      checkFsync(projectTwo, function () {
                        checkObjects(projectOne,
                          firstSetObjects, function () {
                            callback();
                          });
                      });
                    });
    ...
    function basicServerRestartTest(callback) {
      checkServerClose(clientOne, function () {
        writeObjects(projectTwo, secondSetObjects, function () {
          projectOne.loadObject(secondSetObjects[0]._id, function (err,
            object) {
            console.log('basicServerCloseTest->loadObject->1', err,
              object);
            if (err.indexOf('discon') !== -1) {
              checkServerReopen(projectTwo, function () {
                projectOne.loadObject(secondSetObjects[0]._id, function (
                  err, object) {
                  console.log('basicServerRestartTest->loadObject->2',
                    err, object);
                  if (!err && object) {
                    callback();
                  } else {
                    throw new Error(
                      'basicServerRestartTest->loadObject->2 failed');
                  }
                });
              });
            } else {
              throw new Error(
                'basicServerRestartTest->loadObject->1 failed');
            }
          });
        });
      });
    ...
    function basicBranchTesting(callback) {
      ...
      projectOne.setBranchHash(branch, "", firstSetObjects[0]._id, function (
        err) {
        console.log('basicBranchTesting->setBranchHash->1', err);
        if (!err) {
          projectOne.getBranchHash(branch, "", function (err, newhash) {
            console.log('basicBranchTesting->getBranchHash->1', err,
              newhash);
            if (!err && newhash) {
              if (newhash === firstSetObjects[0]._id) {
                projectOne.getBranchNames(function (err, names) {
                  console.log('basicBranchTesting->getBranchNames->1',
                    err, names);
                  if (!err && names) {
                    if (names.length === 1 && names[0] === branch) {
                      projectOne.getBranchHash(branch, firstSetObjects[0]
                        ._id, function (err, newhash) {
                          console.log(
                            'basicBranchTesting->getBranchHash->2', err,
                            newhash);
                          if (!err && newhash) {
                            if (newhash === firstSetObjects[1]._id) {
                              projectOne.setBranchHash(branch,
                                firstSetObjects[1]._id, "", function (err) {
                                  console.log(
                                    'basicBranchTesting->setBranchHash->3',
                                    err);
                                  if (!err) {
                                    projectTwo.getBranchNames(function (
                                      err, names) {
                                      console.log(
                                        'basicBranchTesting->getBranchNames->2',
                                        err, names);
                                      if (!err && names) {
                                        if (names.length === 0) {
                                          callback();
                                          ...
                                          throw new Error(
                                            'basicBranchTesting -11- failed'
                                          );
                                        }
                                      } else {
                                        throw new Error(
                                          'basicBranchTesting -10- failed'
                                        );
                                      }
                                    });
                                  } else {
                                    throw new Error(
                                      'basicBranchTesting -9- failed');
                                  }
                                });
                            } else {
                              throw new Error(
                                'basicBranchTesting -8- failed');
                            }
                          } else {
                            throw new Error(
                              'basicBranchTesting -7- failed');
                          }
                        });
                      projectTwo.setBranchHash(branch, firstSetObjects[0]
                        ._id, firstSetObjects[1]._id, function (err) {
                          console.log(
                            'basicBranchTesting->setBranchHash->2', err);
                          if (!err) {
                            //nothing to do we wait for return with the other client
                          } else {
                            throw new Error(
                              'basicBranchTesting -6- failed');
                          }
                        });
                    } else {
                  ...
                  clientTwo.deleteProject('smoketest', function (err) {
                    ...
                    if (!err) {
                      clientTwo.closeDatabase(function (err) {
                        console.log('testEnding->closeDatabase->2',
                          err);
                        if (!err) {
                          callback();
                        } else {
