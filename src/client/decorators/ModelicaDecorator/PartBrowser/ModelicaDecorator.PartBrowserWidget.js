...
define(['js/Constants',
  ...
  'css!./ModelicaDecorator.PartBrowserWidget.css'
], function (CONSTANTS,
  ...
  ModelicaDecoratorPartBrowserWidget.prototype._renderContent = function () {
    ...
    if (nodeObj) {
      this.skinParts.$name.text(nodeObj.getAttribute(nodePropertyNames.Attributes
        .name) || "");
    }
    ...
    if (svg) {
      ...
      this._modelicaDecoratorCore.resizeSVGToWidth(svg,
        PART_BROWSER_ICON_WIDTH);
      ...
      var modelicaModelObject = this._modelicaDecoratorCore.getFirstChildObjectByKeyValue(
        control, gmeID, 'kind', 'ModelicaModel');
      if (modelicaModelObject) {
        this._unregisterChildrenOtherThen(modelicaModelObject.getId());
      }
    } else {
      //do progressbar instead
      this.__loader = new LoaderProgressBar({
        "containerElement": this.$el.find('.svg-container')
      });
      this.__loader.start();
    }
  };
  ...
  ModelicaDecoratorPartBrowserWidget.prototype.notifyComponentEvent =
  ...
  ModelicaDecoratorPartBrowserWidget.prototype._unregisterChildrenOtherThen =
    function (id) {
      var client = this._control._client,
        nodeObj = client.getNode(this._metaInfo[CONSTANTS.GME_ID]),
        childrenIDs = nodeObj ? nodeObj.getChildrenIds() : [],
      ...
      while (len--) {
        if (childrenIDs[len] !== id) {
          this._control.unregisterComponentIDFromPartID(childrenIDs[len],
            partId);
        }
      }
  };
