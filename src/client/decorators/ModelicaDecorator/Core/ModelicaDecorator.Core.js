...
define(['js/Constants',
  ...
  ModelicaDecoratorCore.prototype.getSVGByGMEId = function (control, gmeID) {
    ...
    modelicaModelObject = this.getFirstChildObjectByKeyValue(control, gmeID,
      'kind', 'ModelicaModel');
    if (modelicaModelObject) {
      modelicaClassName = modelicaModelObject.getAttribute('URI') ||
        modelicaModelObject.getAttribute('Class');

      if (modelicaClassName === undefined || modelicaClassName === null ||
        modelicaClassName === "") {
        returnSVG = this._errorSVGBase.clone();
      } else {
        if (ModelicaDecoratorCore.prototype._svgCache[modelicaClassName]) {
          returnSVG = ModelicaDecoratorCore.prototype._svgCache[
            modelicaClassName].clone();
        } else {
          ...
          $.ajax(svg_resource_url, {
            ...
            .done(function (data) {
              ModelicaDecoratorCore.prototype._svgCache[modelicaClassName] =
                $($(data)[2]);
              returnSVG = ModelicaDecoratorCore.prototype._svgCache[
                modelicaClassName].clone();
            })
  ...
  ModelicaDecoratorCore.prototype.getFirstChildObjectByKeyValue = function (
    control, gmeID, key, value) {
  ...
  ModelicaDecoratorCore.prototype.renderTextsOnSVG = function (svg, control,
    gmeID) {
    var client = control._client,
      nodeObj = client.getNode(gmeID),
      len,
          ...
          for (var i = 0; i < parts.length; i++) {
            ...
            if (idx !== -1) {
              ...
              if (bindParam === "name") {
                ...
                var childObj = this.getFirstChildObjectByKeyValue(control,
                  gmeID, 'name', bindParam);
                if (childObj) {
  ...
  ModelicaDecoratorCore.prototype.getSVGWidthHeightRatioAndScale = function (
    svg) {
  ...
  ModelicaDecoratorCore.prototype.resizeSVGToWidth = function (svg, width) {
    ...
    strokesToFix.each(function () {
      ...
      el.attr(STROKE_WIDTH_ATTR, strokeWidthStr.replace(strokeWidth,
        strokeWidth / obj.scale));
    });
  };
