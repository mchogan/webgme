...
define(['js/Constants',
  ...
  'css!./ModelicaDecorator.DiagramDesignerWidget.css'
], function (CONSTANTS,
  nodePropertyNames,
  ...
  ModelicaDecoratorDiagramDesignerWidget.prototype._renderContent = function () {
    ...
    this.skinParts.$svg = this._modelicaDecoratorCore.getSVGByGMEId(control,
      gmeID);
    if (this.skinParts.$svg) {
      this._modelicaDecoratorCore.resizeSVGToWidth(this.skinParts.$svg,
        DIAGRAM_DESIGNER_ICON_WIDTH);

      this.$el.find('.svg-container').append(this.skinParts.$svg);

      //get the scale of the SVG based on its width/height and viewBox
      var obj = this._modelicaDecoratorCore.getSVGWidthHeightRatioAndScale(
        this.skinParts.$svg);
      this._svg_scale = obj.scale;

      //render the connectors
      this.skinParts.$connectorContainer = this.$el.find(
        '.connector-container');
  ...
  ModelicaDecoratorDiagramDesignerWidget.prototype.getConnectionAreas =
    function (id, isEnd, connectionMetaInfo) {
      var result = [],
        edge = 10,
        LEN = 20;
      ...
      if (id === undefined || id == this.hostDesignerItem.id) {
        ...
        result.push({
          "id": id,
          "x1": this._portCoordinates[id].x + this._portCoordinates[id].w /
            2,
          "y1": this._portCoordinates[id].y + this._portCoordinates[id].h /
            2,
          "x2": this._portCoordinates[id].x + this._portCoordinates[id].w /
            2,
          "y2": this._portCoordinates[id].y + this._portCoordinates[id].h /
            2,
          "angle1": 0,
          "angle2": 360,
          "len": LEN
  ...
  ModelicaDecoratorDiagramDesignerWidget.prototype._toolTipBase = $(
    '<div class="port_info"> \
  ...
  ModelicaDecoratorDiagramDesignerWidget.prototype._buildToolTip = function (
    portConnector, svgPort) {
    ...
    while (len--) {
      ...
      if (port) {
        ...
        if (svgPort.length > 0) {
          ...
          var portConnectorSvg = document.createElementNS(
          ...
          this._svgPortConnectors.push([svgPort, portConnector, {},
            portId
          ]);
          ...
          this.logger.debug('Cannot find svg child object for port: ' +
            portName);
        }
  ...
  ModelicaDecoratorDiagramDesignerWidget.prototype._renderTexts = function () {
    this.__svgTexts = this._modelicaDecoratorCore.renderTextsOnSVG(this.skinParts
      .$svg, this._control, this._metaInfo[CONSTANTS.GME_ID]);
  };

  ModelicaDecoratorDiagramDesignerWidget.prototype.onRenderGetLayoutInfo =
    function () {
      var len;
      ...
      if (this.__svgTexts) {
        ...
        while (len--) {
          ...
          var scale = Math.min(mexHeight / renderedHeight, mexWidth /
            renderedWidth);
          var fontSize = parseInt(this.__svgTexts[len][0].getAttribute(
            'font-size'));
  ...
  ModelicaDecoratorDiagramDesignerWidget.prototype.onRenderSetLayoutInfo =
    function () {
      var len;
      ...
      if (this._svgPortConnectors) {
        ...
        while (len--) {
          ...
          var x = bbox.x * this._svg_scale - (w - bbox.width * this._svg_scale) /
            2 - 1; // border width
          var y = bbox.y * this._svg_scale - (h - bbox.height * this._svg_scale) /
            2 - 1; // border width
          ...
          g.setAttribute('transform', 'translate(' + -bbox.x * this._svg_scale *
            CONNECTOR_RATIO + ' ' + -bbox.y * this._svg_scale *
            CONNECTOR_RATIO + ') scale(' + this._svg_scale * CONNECTOR_RATIO +
            ' ' + this._svg_scale * CONNECTOR_RATIO + ')');
  ...
  ModelicaDecoratorDiagramDesignerWidget.prototype.showSourceConnectors =
    function (params) {
  ...
  ModelicaDecoratorDiagramDesignerWidget.prototype.hideSourceConnectors =
    function () {
      this.$sourceConnectors.hide();
  };

  ModelicaDecoratorDiagramDesignerWidget.prototype.showEndConnectors =
    function (params) {
      var client = this._control._client,
        nodeObj,
        srcSubCompMetaInfo;
      ...
      srcSubCompMetaInfo = srcSubCompMetaInfo.slice(0, srcSubCompMetaInfo.length -
        2);
  ...
  //Hides the 'connectors' - detaches them from the DOM
  ModelicaDecoratorDiagramDesignerWidget.prototype.hideEndConnectors =
    function () {
      this.$endConnectors.hide();
  };

  /************ CUSTOM CONNACTIBILITY LOGIC **********************/
  ModelicaDecoratorDiagramDesignerWidget.prototype.getConnectorMetaInfo =
    function (id) {
      var metaInfo,
  ...
  ModelicaDecoratorDiagramDesignerWidget.prototype.doSearch = function (
    searchDesc) {
