...
define(['js/Constants',
  ...
  SVGPort.prototype._initialize = function () {
    ...
    if (this._decorator._displayConnectors === true) {
      if (this._decorator.hostDesignerItem) {
        this._decorator.hostDesignerItem.registerConnectors(this.$connector,
          this._id);
      }
      this.hideConnectors();
    } else {
  ...
  SVGPort.prototype._updateCoordinates = function () {
    ...
    this.positionX = parseInt(portNode.getRegistry(RegistryKeys.POSITION).x,
      10);
    this.positionY = parseInt(portNode.getRegistry(RegistryKeys.POSITION).y,
      10);
  };
