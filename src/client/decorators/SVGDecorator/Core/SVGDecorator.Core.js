...
define(['js/Constants',
  ...
  SVGDecoratorCore.prototype._discoverCustomConnectionAreas = function (
    ...
    if (len > 0) {
      ...
      while (len--) {
        ...
        if (!lineData.hasOwnProperty(DATA_ANGLE) &&
          !(lineData.hasOwnProperty(DATA_ANGLE1) && lineData.hasOwnProperty(
            DATA_ANGLE2))) {
  ...
  SVGDecoratorCore.prototype._updatePortPositions = function () {
    ...
    this.$leftPorts.find('.port > .title').css('width', this._portContainerWidth -
      TITLE_PADDING);
    this.$leftPorts.find('.port > .icon').css('left', -PORT_HEIGHT);
    this.$leftPorts.find('.port > .' + DiagramDesignerWidgetConstants.CONNECTOR_CLASS)
      .css('left', -PORT_HEIGHT + 1);
    ...
    this.$rightPorts.find('.port > .title').css('right', -this._portContainerWidth +
      TITLE_PADDING);
    this.$rightPorts.find('.port > .title').css('width', this._portContainerWidth -
      TITLE_PADDING);
    this.$rightPorts.find('.port > .icon').css('left', this._portContainerWidth);
    this.$rightPorts.find('.port > .' + DiagramDesignerWidgetConstants.CONNECTOR_CLASS)
      .css('left', this._portContainerWidth);
    ...
    if (this._defaultSVGUsed === true) {
      ...
      var height = Math.max(leftPorts.length * PORT_HEIGHT, rightPorts.length *
        PORT_HEIGHT, DEFAULT_SVG_DEFAULT_HEIGHT);
      svg.attr('height', height);
      svgRect.attr('height', height - 1);

      var connectorSouth = this.$el.find('.' + DiagramDesignerWidgetConstants
        .CONNECTOR_CLASS + '.cs');
    ...
    if (this._leftPorts) {
      var connectorWest = this.$el.find('.' + DiagramDesignerWidgetConstants.CONNECTOR_CLASS +
        '.cw');
    ...
    if (this._rightPorts) {
      var connectorEast = this.$el.find('.' + DiagramDesignerWidgetConstants.CONNECTOR_CLASS +
        '.ce');
