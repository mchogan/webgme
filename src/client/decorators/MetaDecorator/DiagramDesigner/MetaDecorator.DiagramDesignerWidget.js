...
define(['js/Constants',
  ...
  var MetaDecoratorDiagramDesignerWidget,
    ...
    TEXT_META_EDIT_BTN_BASE = $(
      '<i class="glyphicon glyphicon-cog text-meta"/>');
  ...
  _.extend(MetaDecoratorDiagramDesignerWidget.prototype,
    DefaultDecoratorDiagramDesignerWidget.prototype);
  MetaDecoratorDiagramDesignerWidget.prototype.DECORATORID = DECORATOR_ID;
  _.extend(MetaDecoratorDiagramDesignerWidget.prototype,
    MetaDecoratorDiagramDesignerWidgetConstraints.prototype);
  _.extend(MetaDecoratorDiagramDesignerWidget.prototype,
    MetaDecoratorDiagramDesignerWidgetAspects.prototype);

  /*********************** OVERRIDE DECORATORBASE MEMBERS **************************/

  MetaDecoratorDiagramDesignerWidget.prototype.$DOMBase = $(
    MetaDecoratorTemplate);
  ...
  MetaDecoratorDiagramDesignerWidget.prototype._renderContent = function () {
    ...
    this.$el.attr({
      "data-id": this._metaInfo[CONSTANTS.GME_ID]
    });
    ...
    this._skinParts.$addAttributeContainer = this.$el.find(
      ".add-new-attribute");
    ...
    this._skinParts.$attributesContainer.on('dblclick', 'li', function (e) {
      if (self.hostDesignerItem.canvas.getIsReadOnlyMode() !== true) {
        var attrName = $(this).find('.n').text().replace(":", ""),
          ...
          atrMeta = client.getAttributeSchema(self._metaInfo[CONSTANTS.GME_ID],
            attrName);
        var desc = _.extend({}, {
          name: attrName,
          type: atrMeta.type,
          defaultValue: atrMeta.default
        });
  ...
  MetaDecoratorDiagramDesignerWidget.prototype._getNodeColorsFromRegistry =
    function () {
      var objID = this._metaInfo[CONSTANTS.GME_ID];

      this.fillColor = this.preferencesHelper.getRegistry(objID,
        REGISTRY_KEYS.COLOR, false);
      this.borderColor = this.preferencesHelper.getRegistry(objID,
        REGISTRY_KEYS.BORDER_COLOR, false);
      this.textColor = this.preferencesHelper.getRegistry(objID,
        REGISTRY_KEYS.TEXT_COLOR, false);
  };
  ...
  MetaDecoratorDiagramDesignerWidget.prototype._updateAttributes = function () {
    var client = this._control._client,
      newAttributes = client.getOwnValidAttributeNames(this._metaInfo[
        CONSTANTS.GME_ID]),
      len,
    ...
    for (i = 0; i < len; i += 1) {
      this._skinParts.$attributesContainer.append(attrLIBase.clone().append(
        this._attributes[this._attributeNames[i]].$el));
    }

  };

  MetaDecoratorDiagramDesignerWidget.prototype._addAttribute = function (
    attrName) {
    var client = this._control._client,
      attrMetaDescriptor = client.getAttributeSchema(this._metaInfo[CONSTANTS
        .GME_ID], attrName) ? {
        name: attrName,
        type: client.getAttributeSchema(this._metaInfo[CONSTANTS.GME_ID],
          attrName).type || "null"
      } : null;
  ...
  MetaDecoratorDiagramDesignerWidget.prototype._removeAttribute = function (
    attrName) {
  ...
  MetaDecoratorDiagramDesignerWidget.prototype._onNodeTitleChanged = function (
    oldValue, newValue) {
    var client = this._control._client;

    client.setAttributes(this._metaInfo[CONSTANTS.GME_ID], nodePropertyNames.Attributes
      .name, newValue);
  };
  ...
  MetaDecoratorDiagramDesignerWidget.prototype._onNewAttributeClick =
    function () {
      var client = this._control._client,
      ...
      this._onNewClick(client.getValidAttributeNames(objId), this._skinParts.$attributesContainer,
        this._skinParts.$addAttributeContainer, this._skinParts.$attributesTitle,
        this._onNewAttributeCreate);
  };

  MetaDecoratorDiagramDesignerWidget.prototype._onNewClick = function (
    existingNames, itemContainer, addNewContainer, titleContainer, saveFn) {
    var inputCtrl,
  ...
  MetaDecoratorDiagramDesignerWidget.prototype._onNewAttributeCreate =
    function (attrName) {
      var desc,
        self = this,
  ...
  MetaDecoratorDiagramDesignerWidget.prototype._isValidName = function (
    attrName, collection) {
    var result = true;
  ...
  MetaDecoratorDiagramDesignerWidget.prototype.readOnlyMode = function (
    readOnlyMode) {
    DefaultDecoratorDiagramDesignerWidget.prototype.readOnlyMode.call(this,
      readOnlyMode);

    this._setReadOnlyMode(readOnlyMode);
  };

  MetaDecoratorDiagramDesignerWidget.prototype._setReadOnlyMode = function (
    readOnly) {
    if (readOnly === true) {
      this._skinParts.$addAttributeContainer.detach();
      this._skinParts.$addConstraintContainer.detach();
  ...
  MetaDecoratorDiagramDesignerWidget.prototype.saveAttributeDescriptor =
    function (attrName, attrDesc) {
      var client = this._control._client,
        objID = this._metaInfo[CONSTANTS.GME_ID],
        attrSchema;
  ...
  MetaDecoratorDiagramDesignerWidget.prototype.deleteAttributeDescriptor =
    function (attrName) {
      var client = this._control._client,
        objID = this._metaInfo[CONSTANTS.GME_ID];
  ...
  MetaDecoratorDiagramDesignerWidget.prototype.getConnectionAreas = function (
    id, isEnd, connectionMetaInfo) {
    var result = [],
      edge = 10,
      LEN = 20,
      connType = connectionMetaInfo && connectionMetaInfo[MetaRelations.CONNECTION_META_INFO
        .TYPE] ? connectionMetaInfo[MetaRelations.CONNECTION_META_INFO.TYPE] :
      null,
      cN,
      cE,
      cW,
  ...
  MetaDecoratorDiagramDesignerWidget.prototype._showMetaTextEditorDialog =
    function () {
      var client = this._control._client,
      ...
      dialog.show(JSON.stringify(metaObj, undefined, 2), function (text) {
        try {
          ...
          self.logger.error(
            'Saving META failed... Either not JSON object or something else went wrong...'
          );
        }
      });
  };
