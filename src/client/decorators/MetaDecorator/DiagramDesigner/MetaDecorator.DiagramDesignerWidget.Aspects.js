...
define(['js/Constants',
  ...
  MetaDecoratorDiagramDesignerWidgetAspects.prototype._renderContentAspects =
    function () {
  ...
  MetaDecoratorDiagramDesignerWidgetAspects.prototype._onNewAspectClick =
    function () {
      var client = this._control._client,
        objId = this._metaInfo[CONSTANTS.GME_ID],
        existingNames = [CONSTANTS.ASPECT_ALL].concat(client.getMetaAspectNames(
          objId));

      this._onNewClick(existingNames, this._skinParts.$aspectsContainer, this
        ._skinParts.$addAspectContainer, this._skinParts.$aspectsTitle, this._onNewAspectCreate
      );
  };

  MetaDecoratorDiagramDesignerWidgetAspects.prototype._updateAspects =
    function () {
      var client = this._control._client,
        objId = this._metaInfo[CONSTANTS.GME_ID],
      ...
      for (i = 0; i < len; i += 1) {
        this._skinParts.$aspectsContainer.append(cLIBase.clone().append(this._aspects[
          this._aspectNames[i]].$el));
      }
  };

  MetaDecoratorDiagramDesignerWidgetAspects.prototype._addAspect = function (
    cName) {
    var client = this._control._client,
      objId = this._metaInfo[CONSTANTS.GME_ID],
  ...
  MetaDecoratorDiagramDesignerWidgetAspects.prototype._updateAspect =
    function (cName) {
      var client = this._control._client,
        objId = this._metaInfo[CONSTANTS.GME_ID],
        aspect = client.getMetaAspect(objId, cName);
  ...
  MetaDecoratorDiagramDesignerWidgetAspects.prototype._removeAspect =
    function (cName) {
  ...
  MetaDecoratorDiagramDesignerWidgetAspects.prototype._onNewAspectCreate =
    function (cName) {
      var desc,
        self = this,
  ...
  MetaDecoratorDiagramDesignerWidgetAspects.prototype._getAspectDescriptorValidChildrenTypes =
    function () {
      ...
      validChildrenTypeIDs = GMEConcepts.getMETAAspectMergedValidChildrenTypes(
        objId);
  ...
  MetaDecoratorDiagramDesignerWidgetAspects.prototype.saveAspectDescriptor =
  ...
  MetaDecoratorDiagramDesignerWidgetAspects.prototype.deleteAspectDescriptor =
    function (cName) {
      var client = this._control._client,
        objID = this._metaInfo[CONSTANTS.GME_ID];
