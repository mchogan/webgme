...
define(['js/Constants',
  ...
  MetaDecoratorDiagramDesignerWidgetConstraints.prototype._renderContentConstraints =
    function () {
      ...
      this._skinParts.$addConstraintContainer = this.$el.find(
        ".add-new-constraint");
      ...
      this._skinParts.$constraintsContainer.on('dblclick', 'li', function (e) {
        if (self.hostDesignerItem.canvas.getIsReadOnlyMode() !== true) {
          ...
          constraintNames.splice(self._constraintNames.indexOf(
            constraintName), 1);
      ...
      this._skinParts.$addConstraintContainer.on("click", null, function (
        event) {
        if (self.hostDesignerItem.canvas.getIsReadOnlyMode() !== true) {
  ...
  MetaDecoratorDiagramDesignerWidgetConstraints.prototype._onNewConstraintClick =
    function () {
      this._onNewClick(this._constraintNames, this._skinParts.$constraintsContainer,
        this._skinParts.$addConstraintContainer, this._skinParts.$constraintsTitle,
        this._onNewConstraintCreate);
  };

  MetaDecoratorDiagramDesignerWidgetConstraints.prototype._updateConstraints =
    function () {
      var client = this._control._client,
        nodeObj = client.getNode(this._metaInfo[CONSTANTS.GME_ID]),
      ...
      for (i = 0; i < len; i += 1) {
        this._skinParts.$constraintsContainer.append(cLIBase.clone().append(
          this._constraints[this._constraintNames[i]].$el));
      }
  };

  MetaDecoratorDiagramDesignerWidgetConstraints.prototype._addConstraint =
    function (cName) {
      var client = this._control._client,
        nodeObj = client.getNode(this._metaInfo[CONSTANTS.GME_ID]),
  ...
  MetaDecoratorDiagramDesignerWidgetConstraints.prototype._updateConstraint =
    function (cName) {
      var client = this._control._client,
        nodeObj = client.getNode(this._metaInfo[CONSTANTS.GME_ID]),
        constraint = nodeObj.getConstraint(cName);
  ...
  MetaDecoratorDiagramDesignerWidgetConstraints.prototype._removeConstraint =
    function (cName) {
  ...
  MetaDecoratorDiagramDesignerWidgetConstraints.prototype._onNewConstraintCreate =
    function (cName) {
      var desc,
        self = this,
  ...
  MetaDecoratorDiagramDesignerWidgetConstraints.prototype.saveConstraintDescriptor =
  ...
  MetaDecoratorDiagramDesignerWidgetConstraints.prototype.deleteConstraintDescriptor =
    function (cName) {
      var client = this._control._client,
        objID = this._metaInfo[CONSTANTS.GME_ID];
