...
define(['logManager',
  ...
  Port.prototype._DOMPortBase = $(
    '<div  id="__ID__" class="port" data-id="__ID__"></div>');
  Port.prototype._DOMTitleWrapper = $(
    '<div class="title-wrapper"><span class="title">__NAME__</span></div>');
  ...
  Port.prototype._DOMBaseLeftTemplate = Port.prototype._DOMPortBase.clone().append(
    Port.prototype._DOMDot.clone()).append(Port.prototype._DOMConnector.clone())
    .append(Port.prototype._DOMSVGIcon.clone()).append(Port.prototype._DOMTitleWrapper
      .clone());

  Port.prototype._DOMBaseRightTemplate = Port.prototype._DOMPortBase.clone().append(
    Port.prototype._DOMTitleWrapper.clone()).append(Port.prototype._DOMSVGIcon
    .clone()).append(Port.prototype._DOMDot.clone()).append(Port.prototype._DOMConnector
    .clone());

  Port.prototype._initialize = function () {
    var concretePortTemplate = this.orientation === "W" ? this._DOMBaseLeftTemplate :
      this._DOMBaseRightTemplate;
    ...
    if (this.decorator._displayConnectors === true) {
      if (this.decorator.hostDesignerItem) {
        this.decorator.hostDesignerItem.registerConnectors(this.$connectors,
          this.id);
      }
      this.hideConnectors();
    } else {
  ...
  Port.prototype._updateIcon = function () {
    if (this.icon) {
      this.$portIcon.attr('src', CONSTANTS.ASSETS_DECORATOR_SVG_FOLDER + this
        .icon);
      this.$portIcon.show();
  ...
  Port.prototype.getConnectorArea = function () {
    ...
    this.connectionArea.x1 = this.orientation === "W" ? 0 : this.decorator.hostDesignerItem
      .getWidth();
    //fix the x coordinate for the dot/svg icon's width
    if (this.icon) {
      this.connectionArea.x1 += (this.orientation === "W" ? -1 : 1) * (
        PORT_ICON_WIDTH - 1);
    } else {
      this.connectionArea.x1 += (this.orientation === "W" ? -1 : 1) * (
        PORT_DOT_WIDTH - 1);
