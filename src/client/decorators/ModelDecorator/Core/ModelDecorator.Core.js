...
define(['js/Constants',
  ...
  var ModelDecoratorCore,
    ...
    CONN_TYPE_BASE = $('<div/>', {
      class: CONNECTION_TYPE_CLASS
    }),
    EMBEDDED_SVG_IMG_BASE = $('<img>', {
      'class': EMBEDDED_SVG_CLASS
    });
  ...
  ModelDecoratorCore.prototype.doSearch = function (searchDesc) {
    ...
    return (this.formattedName && this.formattedName.toLowerCase().indexOf(
      searchText) !== -1);
  };
  ...
  ModelDecoratorCore.prototype._renderContent = function () {
    ...
    this.skinParts.$portsContainerLeft = this.skinParts.$portsContainer.find(
      ".left");
    this.skinParts.$portsContainerRight = this.skinParts.$portsContainer.find(
      ".right");
    this.skinParts.$portsContainerCenter = this.skinParts.$portsContainer.find(
      ".center");
  ...
  ModelDecoratorCore.prototype._getNodeColorsFromRegistry = function () {
    ...
    this.fillColor = this.preferencesHelper.getRegistry(objID, REGISTRY_KEYS.COLOR,
      true);
    this.borderColor = this.preferencesHelper.getRegistry(objID,
      REGISTRY_KEYS.BORDER_COLOR, true);
    this.textColor = this.preferencesHelper.getRegistry(objID, REGISTRY_KEYS.TEXT_COLOR,
      true);
  };
  ...
  ModelDecoratorCore.prototype._updateConnectionType = function () {
    var isConnectionType = GMEConcepts.isConnectionType(this._metaInfo[
      CONSTANTS.GME_ID]);
  ...
  ModelDecoratorCore.prototype._addPortToContainer = function (portNode,
    ...
    var portId = portNode.getId(),
      ...
      portPosition = portNode.getRegistry(REGISTRY_KEYS.POSITION) || {
        "x": 0,
        "y": 0
      },
          ...
          if (this.ports[i].orientation === portInstance.orientation) {
            if ((portInstance.position.y < this.ports[i].position.y) ||
              ...
              if (portToAppendBefore === null) {
                ...
                if ((this.ports[i].position.y < this.ports[portToAppendBefore]
                    .position.y) ||
                  ((this.ports[i].position.y === this.ports[
                    portToAppendBefore].position.y) && (this.ports[i].title <
                    this.ports[portToAppendBefore].title))) {
                  portToAppendBefore = i;
                }
              }
            }
  ...
  ModelDecoratorCore.prototype._showPortProgressBar = function () {
    ...
    if (pgBar.length === 0) {
      pgBar = $('<div/>', {
        'class': ModelDecoratorConstants.PROGRESS_BAR_CLASS
      });
      this.$el.append(pgBar);

      this._loader = new LoaderProgressBar({
        "containerElement": pgBar
      });
      this._loader.start();
    }
  };
  ...
  ModelDecoratorCore.prototype._ptrUIDOMBase = $('<div class="' +
    ModelDecoratorConstants.POINTER_CLASS +
  ...
  ModelDecoratorCore.prototype._getPointerNames = function () {
    ...
    if (nodeObj) {
      ptrNames = _.difference(nodeObj.getPointerNames().slice(0),
        EXCLUDED_POINTERS);
    }
  ...
  ModelDecoratorCore.prototype._getValidPointersForTarget = function (
    targetId) {
    var client = this._control._client,
  ...
  ModelDecoratorCore.prototype._setPointerTarget = function (targetID,
    mousePos) {
  ...
  ModelDecoratorCore.prototype._selectPointerForTarget = function (ptrNames,
    targetID, mousePos) {
