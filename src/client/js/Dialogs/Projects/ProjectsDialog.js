...
define(['logManager',
  ...
  ProjectsDialog.prototype._initDialog = function () {
    ...
    this._loader = new LoaderCircles({
      "containerElement": this._btnRefresh
    });
    ...
    this._btnNewProjectCancel.on('click', function (event) {
      ...
      self._filter = self._dialog.find('.tabContainer li.active').data(
        'filter');
      self._updateProjectNameList();
  ...
  ProjectsDialog.prototype._updateProjectNameList = function () {
    var len = this._projectNames.length,
      ...
      count = 0,
      $emptyLi = $(
        '<li class="center"><i>No projects in this group...</i></li>');
    ...
    if (len > 0) {
      for (i = 0; i < len; i += 1) {
        ...
        if (this._filter !== undefined) {
          displayProject = (this._projectNames[i].toUpperCase()[0] >= this._filter[
              0] &&
  ...
  ProjectsDialog.prototype._createProjectFromFile = function (projectName,
    jsonContent) {
    var _client = this._client,
      _logger = this._logger,
    ...
    _client.createProjectFromFileAsync(projectName, jsonContent, function (
      err) {
      if (!err) {
