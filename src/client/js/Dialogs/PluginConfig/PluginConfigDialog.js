...
define([
  ...
  function (
    ...
    PluginConfigDialog = function () {
      ...
      this._propertyGridWidgetManager.registerWidgetForType('boolean',
        'iCheckBox');
    };
    ...
    var PLUGIN_CONFIG_SECTION_BASE = $(
      '<div><fieldset><legend></legend><form class="form-horizontal" role="form"></form><fieldset></div>'
    );
    PluginConfigDialog.prototype._initDialog = function (pluginConfigs) {
      ...
      for (var p in pluginConfigs) {
        if (pluginConfigs.hasOwnProperty(p)) {
          ...
          this._generatePluginSection(p, pluginConfigs[p], pluginSectionEl.find(
            '.form-horizontal'));
        }
      }
    ...
    var ENTRY_BASE = $(
      '<div class="form-group"><label class="col-sm-4 control-label">NAME</label><div class="col-sm-8 controls"></div></div>'
    );
    var DESCRIPTION_BASE = $('<div class="desc muted"></div>');
    PluginConfigDialog.prototype._generatePluginSection = function (pluginName,
      pluginConfig, containerEl) {
      ...
      for (i = 0; i < len; i += 1) {
        ...
        widget = this._propertyGridWidgetManager.getWidgetForProperty(
          pluginConfigEntry);
