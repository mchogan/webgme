...
define(['js/Constants',
  ...
  var DecoratorSVGExplorerDialog,
    IMG_BASE = $(
      '<div class="img"><img src=""/><div class="desc">description</div></div>'
    ),
    SVG_DIR = CONSTANTS.ASSETS_DECORATOR_SVG_FOLDER,
    DecoratorSVGIconList = [''].concat(decoratorSVG.DecoratorSVGIconList.slice(
      0)),
  ...
  DecoratorSVGExplorerDialog.prototype.show = function (fnCallback, oldValue) {
    ...
    if (oldValue) {
      ...
      $originalSelected = self._modalBody.find("[" + DATA_FILENAME + "='" +
        oldValue + "']");
  ...
  DecoratorSVGExplorerDialog.prototype._filter = function (fileName) {
    ...
    if (fileName) {
      ...
      this._modalBody.find('div.img[' + DATA_SVG + '*="' + fileName.toLowerCase() +
        '"]').show();
    } else {
