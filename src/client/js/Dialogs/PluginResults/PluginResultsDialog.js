...
define(['clientUtil',
  ...
  var PluginResultsDialog,
    ...
    ICON_SUCCESS = $(
      '<i class="glyphicon glyphicon-ok glyphicon glyphicon-ok"/>'),
    ICON_ERROR = $(
      '<i class="glyphicon glyphicon-warning-sign glyphicon glyphicon-warning-sign"/>'
    ),
    RESULT_NAME_BASE = $('<span/>', {
      'class': 'title'
    }),
    RESULT_TIME_BASE = $('<span/>', {
      'class': 'time'
    }),
    RESULT_DETAILS_BTN_BASE = $(
      '<span class="btn btn-micro btn-details pull-right">Details</span>'),
    RESULT_DETAILS_BASE = $('<div/>', {
      'class': 'messages collapse'
    }),
    MESSAGE_ENTRY_BASE = $(
      '<div class="msg"><div class="msg-title"></div><div class="msg-body"></div></div>'
    ),
    MESSAGE_ENTRY_NODE_BTN_BASE = $(
      '<span class="btn btn-micro btn-node pull-right">Show node</span>'),
    RESULT_ARTIFACTS_BASE = $(
      '<div class="artifacts collapse"><div class="artifacts-title">Generated artifacts</div><div class="artifacts-body"><ul></ul></div></div>'
    ),
  ...
  PluginResultsDialog.prototype._initDialog = function (pluginResults) {
    ...
    for (var i = 0; i < pluginResults.length; i += 1) {
      ...
      var pluginName = result.getPluginName ? result.getPluginName() :
        'PluginName N/A';
      spanResultTitle = RESULT_NAME_BASE.clone();
      spanResultTitle.text(pluginName);
      resultHeader.append(spanResultTitle);

      var pluginTime = result.getFinishTime ? clientUtil.formattedDate(new Date(
        result.getFinishTime()), 'elapsed') : 'Time: N/A';
      ...
      for (j = 0; j < messages.length; j += 1) {
        ...
        if (messages[j].activeNode.id) {
          ...
          messageEntry.find('.btn-node').attr('node-result-details', JSON.stringify(
            messages[j]));
        }
        ...
        for (j = 0; j < artifacts.length; j += 1) {
          (function (hash, ulE) {
            blobClient.getArtifact(hash, function (err, artifact) {
              ...
              artifactEntryA.attr('href', (new BlobClient()).getDownloadURL(
                hash));
