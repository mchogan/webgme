...
define(['logManager',
  ...
  GraphVizWidget.prototype._initialize = function () {
    ...
    this._svg = this.__svg.append("g").attr("transform", "translate(" +
      MARGIN + "," + MARGIN + ")");
  ...
  GraphVizWidget.prototype._update = function (source) {
    ...
    var nodeEnter = node.enter().append("g")
      ...
      .attr("transform", function (d) {
        return d.parent ? "translate(" + d.parent.y0 + "," + d.parent.x0 +
          ")" : "translate(" + d.y + "," + d.x + ")";
      })
      .on("click", function (d) {
    ...
    var nodeExit = node.exit().transition()
      .duration(DURATION)
      .attr("transform", function (d) {
        return source ? "translate(" + source.y + "," + source.x + ")" : d.parent ?
          "translate(" + d.parent.y + "," + d.parent.x + ")" : "translate(" +
          d.y + "," + d.x + ")";
      })
      .remove();
    ...
    if (this.__resizing !== true) {
      ...
      if (len > 0) {
        ...
        this._resizeD3Tree(nodesYByDepth.length * TREE_LEVEL_DISTANCE, sum *
          NODE_SIZE);
  ...
  GraphVizWidget.prototype.setData = function (data) {
    ...
    if (this._root && this._root.name) {
      ...
      if (this._root.children) {
        rootNameWidth = (this._root.name + " [" + this._root.children.length +
          "]").length * 6;
      }
      this._el.find('svg > g').attr("transform", "translate(" + (
        rootNameWidth + MARGIN) + "," + MARGIN + ")");
    }
  };
