...
define(['clientUtil',
  ...
  MetaEditorPointerNamesDialog.prototype.show = function (existingPointerNames, notAllowedPointerNames, isSet,
    callBack) {
    var self = this;
  ...
  MetaEditorPointerNamesDialog.prototype._initDialog = function (existingPointerNames, notAllowedPointerNames, isSet,
    callBack) {
    var self = this,
      i,
    ...
    isValidPointerName = function (name) {
      return !(name === '' || existingPointerNames.indexOf(name) !== -1 || notAllowedPointerNames.indexOf(name) !== -
        1);
    };
    ...
    if (len) {
      ...
      for (i = 0; i < len; i += 1) {
        this._btnGroup.append($('<button class="btn btn-default">' + util.toSafeString(existingPointerNames[i]) +
          '</button>'));
      }
