...
define(['logManager',
  ...
  PartBrowserWidget.prototype.$_DOMBase = $('<div/>').attr({
    "class": PART_CLASS
  });
  ...
  PartBrowserWidget.prototype.getDragItems = function (el) {
    this._logger.warning(
      "PartBrowserWidget.getDragItems is not overridden in the controller!!!"
    );
    return [];
  };
  ...
  PartBrowserWidget.prototype.getDragEffects = function (el) {
    this._logger.warning(
      "PartBrowserWidget.getDragEffects is not overridden in the controller!!!"
    );
    return [];
  };
  ...
  PartBrowserWidget.prototype.getDragParams = function (el) {
    this._logger.debug(
      "PartBrowserWidget.getDragParams is not overridden in the controller!!!"
    );
    return undefined;
  };
  ...
  PartBrowserWidget.prototype.updatePart = function (partId, partDesc) {
    var partDecoratorInstance = this._parts[partId] ? this._parts[partId].decoratorInstance :
      undefined,
      DecoratorClass = partDesc.decoratorClass,
      partContainerDiv = this._getPartDiv(partId);

    if (partDecoratorInstance) {
      if (partDesc.decoratorClass && partDecoratorInstance.DECORATORID !==
        ...
        this._logger.debug("DesignerItem's ['" + this.id +
  ...
  PartBrowserWidget.prototype.setEnabled = function (partId, enabled) {
    var partContainerDiv = this._partDraggableEl[partId] ? this._partDraggableEl[
      partId] : undefined;
