...
define(['logManager'], function (logManager) {
  ...
  RepositoryLogControl.prototype._loadMoreCommits = function (num) {
    ...
    commitsLoaded = function (err, data) {
      ...
      self._logger.debug("commitsLoaded, err: '" + err + "', data: " + data ===
        true ? data.length : "null");

      if (err) {
        if (_.isEmpty(err)) {
          self._logger.error(
            'the mysterious error returned by "getCommitsAsync"');
        } else {
          self._logger.error(err);
        }
      } else {
  ...
  RepositoryLogControl.prototype._refreshBranches = function () {
    ...
    this._client.getBranchesAsync(function (err, data) {
      ...
      self._logger.debug("branchesLoaded, err: '" + err + "', data: " +
