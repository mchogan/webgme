  ...
  function (
    ...
    ProjectRepositoryWidget.prototype.onLoadMoreCommits = function (num) {
      this._logger.warning(
        "onLoadMoreCommits is not overridden in Controller...num: '" + num +
        "'");
    };

    ProjectRepositoryWidget.prototype.onLoadCommit = function (params) {
      this._logger.warning(
        "onLoadCommit is not overridden in Controller...params: '" + JSON.stringify(
          params) + "'");
    };

    ProjectRepositoryWidget.prototype.onDeleteBranchClick = function (branch) {
      this._logger.warning(
        "onDeleteBranchClick is not overridden in Controller...branch: '" +
        branch + "'");
    };

    ProjectRepositoryWidget.prototype.onCreateBranchFromCommit = function (
      params) {
      this._logger.warning(
        "onCreateBranchFromCommit is not overridden in Controller...params: '" +
    ...
    ProjectRepositoryWidget.prototype._initializeUI = function () {
      ...
      /*table layout*/
      this._table = $('<table/>', {
        "class": "table table-hover user-select-on"
      });
      this._tHead = $('<thead/>');
      this._tHead.append($(
        '<tr><th>Graph</th><th>Actions</th><th>Commit</th><th>Message</th><th>User</th><th>Time</th></tr>'
      ));
      this._tBody = $('<tbody/>');
      ...
      this._loader = new LoaderCircles({
        "containerElement": this._showMoreContainer
      });
      ...
      this._el.on("click." + BTN_LOAD_COMMIT_CLASS, "." + BTN_LOAD_COMMIT_CLASS,
        function () {
          var btn = $(this),
      ...
      this._el.off("click.btnCreateBranchFromCommit",
        ".btnCreateBranchFromCommit");
      this._el.on("click.btnCreateBranchFromCommit",
        ".btnCreateBranchFromCommit", function () {
          self._onCreateBranchFromCommitButtonClick($(this));
        });
    };

    ProjectRepositoryWidget.prototype._calculatePositionForCommit = function (
      cIndex) {
      ...
      if (foundTrack === true) {
        ...
        if (this._branches && this._branches.length > 0) {
          if (this._branches[0].name === MASTER_BRANCH_NAME && this._branches[0]
            .commitId === cCommit.id) {
    ...
    ProjectRepositoryWidget.prototype._render = function () {
      ...
      for (i = 0; i < len; i += 1) {
        ...
        if (cCommit[COMMIT_DATA].parents && cCommit[COMMIT_DATA].parents.length >
          0) {
          hasVisibleParent = false;
          for (j = 0; j < cCommit[COMMIT_DATA].parents.length; j += 1) {
            pIdx = this._orderedCommitIds.indexOf(cCommit[COMMIT_DATA].parents[
              j]);
            if (pIdx >= idx) {
    ...
    ProjectRepositoryWidget.prototype._addBranchHeaderLabels = function () {
      ...
      if (this._branchListUpdated === true) {
        ...
        while (len--) {
          ...
          if (idx !== -1) {
            this._applyBranchHeaderLabel(this._commits[idx], this._branches[len]
              .name, this._branches[len].sync);
          }
        }
    ...
    ProjectRepositoryWidget.prototype._applyBranchHeaderLabel = function (
      commit, branchName, sync) {
      ...
      td = this._tBody.children()[this._orderedCommitIds.indexOf(commit.id)].cells[
        this._tableCellMessageIndex];
    ...
    ProjectRepositoryWidget.prototype._trDOMBase = $(
      '<tr><td></td><td></td><td></td><td><div class="' + MESSAGE_DIV_CLASS +
    ...
    ProjectRepositoryWidget.prototype._loadCommitBtnDOMBase = $('' +
      '<button class="btn btn-default btn-xs ' + BTN_LOAD_COMMIT_CLASS +
      '" href="#" title="Load this commit"><i class="glyphicon glyphicon-share"></i></button>'
    );
    ...
    ProjectRepositoryWidget.prototype._createItem = function (params) {
      ...
      $(tr[0].cells[this._tableCellMessageIndex]).find('div.' +
        MESSAGE_DIV_CLASS).text(params.message);
    ...
    ProjectRepositoryWidget.prototype._generateLoadCommitBtnForCommit =
      function (commitId) {
    ...
    ProjectRepositoryWidget.prototype._updateActualCommitUI = function (oldId,
      newId) {
      ...
      if (idx !== -1) {
        ...
        this._tBody.children()[idx].cells[this._tableCellActionsIndex].appendChild(
          btn[0]);
      }
      ...
      if (idx !== -1) {
        ...
        $(this._tBody.children()[idx].cells[this._tableCellActionsIndex]).find(
          '.' + BTN_LOAD_COMMIT_CLASS).remove();
      }
    };
    ...
    ProjectRepositoryWidget.prototype._drawLine = function (srcDesc, dstDesc) {
      var pathDef,
        ...
        x = nonVisibleSource ? dstDesc.x + ITEM_WIDTH / 2 : srcDesc.x +
        ITEM_WIDTH / 2,
      ...
      if (dX === 0) {
        ...
        if (x2 < x) {
          ...
          y = srcDesc.y - 1;
          y2 += 1;
          pathDef = ["M", x, y, "L", x, y2 + LINE_CORNER_SIZE, "L", x -
            LINE_CORNER_SIZE, y2, "L", x2, y2
          ];
        } else {
          ...
          y += 1;
          pathDef = ["M", x, y, "L", x2 - LINE_CORNER_SIZE, y, "L", x2, y -
            LINE_CORNER_SIZE, "L", x2, y2
          ];
        }
      }
      ...
      if (nonVisibleSource === true) {
        ...
        if (pathDefGradient) {
          ...
          path.node.setAttribute("stroke", "url(#" +
            NON_EXISTING_PARENT_LINE_GRADIENT_NAME + ")");
          this._nonExistingParentPaths.push(path);
        }
      } else {
        path = this._svgPaper.path(pathDef.join(","));
    ...
    ProjectRepositoryWidget.prototype._resizeDialog = function (contentWidth,
      contentHeight) {
    ...
    ProjectRepositoryWidget.prototype._generateSVGGradientDefinition = function () {
      if (!this._svgPaper.canvas.getElementById(
        NON_EXISTING_PARENT_LINE_GRADIENT_NAME)) {
        ...
        linearGradient.setAttribute("id",
          NON_EXISTING_PARENT_LINE_GRADIENT_NAME);

        var stop0 = document.createElementNS("http://www.w3.org/2000/svg",
          'stop');
        stop0.setAttribute("offset", "0%");
        stop0.setAttribute("style", "stop-color: " + LINE_FILL_COLOR);

        var stop1 = document.createElementNS("http://www.w3.org/2000/svg",
          'stop');
    ...
    ProjectRepositoryWidget.prototype._onCreateBranchFromCommitButtonClick =
      function (btn) {
        var td = btn.parent(),
          createBranchHTML = $('<div class="input-append control-group"></div>'),
          txtInput = $('<input class="span2 input-mini" type="text">'),
          btnSave = $(
            '<button class="btn btn-default btn-xs" type="button" title="Create branch"><i class="glyphicon glyphicon-ok"></i></button>'
          ),
          btnCancel = $(
            '<button class="btn btn-default btn-xs" type="button" title="Cancel"><i class="glyphicon glyphicon-remove"></i></button>'
          ),
          self = this;

        //find already displayed branch create control and 'cancel' it
        var currentBranchCreateCtrl = this._tBody.find('.' +
          CREATE_BRANCH_EDIT_CONTROL_CLASS + ' > .btn');
        ...
        btnSave.on('click', function (event) {
          ...
          if (bName !== "" && self._branchNames.indexOf(bName) === -1) {
            ...
            self.onCreateBranchFromCommit({
              "commitId": td.find('.btnCreateBranchFromCommit').data(
                COMMIT_IT),
              "name": bName
            });
          }
