...
define(['js/Widgets/DiagramDesigner/Connection',
  ...
  DiagramDesignerWidget.prototype._onStartConnectionCreate = function (params) {
    var srcItemID = params.srcId,
      ...
      availableEndPoints = [],
      srcItemMetaInfo = this.items[srcItemID]._decoratorInstance ? this.items[srcItemID]._decoratorInstance.getConnectorMetaInfo() :
      undefined,
      srcSubCompMetaInfo = srcSubCompID ? this.items[srcItemID]._decoratorInstance.getConnectorMetaInfo(srcSubCompID) :
      undefined,
      i,
      objID,
  ...
  DiagramDesignerWidget.prototype._onStartConnectionReconnect = function (params) {
    ...
    if (srcDragged === true) {
      ...
      if (this.connectionEndIDs[connID]) {
        ...
        dstItemMetaInfo = this.items[dstItemID]._decoratorInstance ? this.items[dstItemID]._decoratorInstance.getConnectorMetaInfo() :
          undefined;
        dstSubCompMetaInfo = dstSubCompID ? this.items[dstItemID]._decoratorInstance.getConnectorMetaInfo(
          dstSubCompID) : undefined;
      }
    } else {
      ...
      if (this.connectionEndIDs[connID]) {
        ...
        srcItemMetaInfo = this.items[srcItemID]._decoratorInstance ? this.items[srcItemID]._decoratorInstance.getConnectorMetaInfo() :
          undefined;
        srcSubCompMetaInfo = srcSubCompID ? this.items[srcItemID]._decoratorInstance.getConnectorMetaInfo(
    ...
    while (filteredResult.length > 0) {
      ...
      decoratorPackages.push([objID, srcDragged ? srcItemMetaInfo : dstItemMetaInfo, srcDragged ? srcSubCompMetaInfo :
