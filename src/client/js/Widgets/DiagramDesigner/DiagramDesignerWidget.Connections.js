...
define(['js/Widgets/DiagramDesigner/Connection',
  ...
  DiagramDesignerWidget.prototype.createConnection = function (objD) {
    ...
    this.logger.debug("Creating connection component with parameters: " +
      JSON.stringify(objDescriptor));
    ...
    this._saveConnectionIDbyEndID(connectionId, sourceId,
      sourceSubcomponentId);
    this._saveConnectionIDbyEndID(connectionId, targetId,
      targetSubcomponentId);
  ...
  DiagramDesignerWidget.prototype.updateConnection = function (id,
    objDescriptor) {
  ...
  DiagramDesignerWidget.prototype._saveConnectionIDbyEndID = function (connId,
    objId, subCompId) {
    subCompId = subCompId || DiagramDesignerWidgetConstants.SELF;
    this.connectionIDbyEndID[objId] = this.connectionIDbyEndID[objId] || {};
    this.connectionIDbyEndID[objId][subCompId] = this.connectionIDbyEndID[
      objId][subCompId] || [];
    this.connectionIDbyEndID[objId][subCompId].push(connId);
  };

  DiagramDesignerWidget.prototype._removeConnectionIDfromEnd = function (
    connId, isSource) {
    var objId = isSource ? this.connectionEndIDs[connId].srcObjId : this.connectionEndIDs[
        connId].dstObjId,
      subComponentId = isSource ? this.connectionEndIDs[connId].srcSubCompId :
      this.connectionEndIDs[connId].dstSubCompId,
      idx;
  ...
  DiagramDesignerWidget.prototype.onCreateNewConnection = function (params) {
    this.logger.warning("onCreateNewConnection with parameters: '" + JSON.stringify(
      params) + "'");
  };
  ...
  DiagramDesignerWidget.prototype._onModifyConnectionEnd = function (params) {
    ...
    this.logger.debug("Modifying connection with parameters: '" + JSON.stringify(
      params) + "'");
  ...
  DiagramDesignerWidget.prototype.onModifyConnectionEnd = function (params) {
    this.logger.warning("onModifyConnectionEnd with parameters: '" + JSON.stringify(
      params) + "'");
  };
  ...
  DiagramDesignerWidget.prototype._onStartConnectionCreate = function (params) {
    var srcItemID = params.srcId,
      ...
      availableEndPoints = [],
      srcItemMetaInfo = this.items[srcItemID]._decoratorInstance ? this.items[
        srcItemID]._decoratorInstance.getConnectorMetaInfo() : undefined,
      srcSubCompMetaInfo = srcSubCompID ? this.items[srcItemID]._decoratorInstance
      .getConnectorMetaInfo(srcSubCompID) : undefined,
      i,
      objID,
    ...
    this.logger.debug('_onStartConnectionCreate filteredDroppableEnds: ' +
      JSON.stringify(filteredDroppableEnds));
    ...
    while (filteredDroppableEnds.length > 0) {
      ...
      decoratorPackages.push([objID, srcItemMetaInfo, srcSubCompMetaInfo,
        decoratorUpdatePackage
      ]);
    ...
    this.logger.debug('_onStartConnectionCreate decorator update package: ' +
      JSON.stringify(decoratorPackages));
  ...
  DiagramDesignerWidget.prototype.onFilterNewConnectionDroppableEnds =
    function (params) {
      this.logger.warning(
        "DiagramDesignerWidget.prototype.onFilterNewConnectionDroppableEnds not overridden in controller. params: " +
  ...
  DiagramDesignerWidget.prototype._onStartConnectionReconnect = function (
    params) {
    ...
    if (srcDragged === true) {
      ...
      if (this.connectionEndIDs[connID]) {
        ...
        dstItemMetaInfo = this.items[dstItemID]._decoratorInstance ? this.items[
          dstItemID]._decoratorInstance.getConnectorMetaInfo() : undefined;
        dstSubCompMetaInfo = dstSubCompID ? this.items[dstItemID]._decoratorInstance
          .getConnectorMetaInfo(dstSubCompID) : undefined;
      }
    } else {
    ...
    this.logger.debug('_onStartConnectionReconnect filteredResult:' + JSON.stringify(
      filteredResult));
    ...
    while (filteredResult.length > 0) {
      ...
      decoratorPackages.push([objID, srcDragged ? srcItemMetaInfo :
        dstItemMetaInfo, srcDragged ? srcSubCompMetaInfo :
        dstSubCompMetaInfo, decoratorUpdatePackage
      ]);
      processedIndices.sort(function (a, b) {
        return a - b;
      });
    ...
    this.logger.debug(
      '_onStartConnectionReconnect decorator update package: ' + JSON.stringify(
        decoratorPackages));
    i = decoratorPackages.length;
    while (i--) {
      objID = decoratorPackages[i][0];
  ...
  DiagramDesignerWidget.prototype.onFilterReconnectionDroppableEnds =
    function (params) {
      var srcDragged = params.draggedEnd === DiagramDesignerWidgetConstants.CONNECTION_END_SRC;

      this.logger.warning(
        "DiagramDesignerWidget.prototype.onFilterReconnectionDroppableEnds not overridden in controller. params: " +
        JSON.stringify(params));
