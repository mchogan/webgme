...
define(['logManager',
  ...
  SelectionManager.prototype.initialize = function (el) {
    ...
    this._diagramDesigner.addEventListener(this._diagramDesigner.events.ON_COMPONENT_DELETE, function (
      __diagramDesigner, componentId) {
  ...
  SelectionManager.prototype._selectItemsByRubberBand = function (params) {
    ...
    this.logger.debug('Select children by rubber band: [' + rbBBox.x + ',' + rbBBox.y + '], [' + rbBBox.x2 + ',' +
      rbBBox.y2 + ']');
    ...
    selectionContainsBBox = function (itemBBox) {
      ...
      if (itemBBox) {
        if (clientUtil.overlap(rbBBox, itemBBox)) {
          ...
          interSectionRatio = (interSectionRect.x2 - interSectionRect.x) * (interSectionRect.y2 - interSectionRect.y) /
            ((itemBBox.x2 - itemBBox.x) * (itemBBox.y2 - itemBBox.y));
  ...
  SelectionManager.prototype._renderRotateHandlers = function () {
    ...
    if (rotateEnabled) {
      ...
      this._diagramDesigner.skinParts.$selectionOutline.on('mousedown.' + MOUSE_EVENT_POSTFIX, '.rotate', function (
        event) {
        var rotateBtn = $(this);
  ...
  SelectionManager.prototype._startSelectionRotate = function (rotateBtn, event) {
    ...
    if (leftButton) {
      this._rotateDesc = {
        'startX': mousePos.mX,
        'startY': mousePos.mY,
        'oX': parseInt(this._diagramDesigner.skinParts.$selectionOutline.css('left'), 10) + parseInt(this._diagramDesigner
          .skinParts.$selectionOutline.css('width'), 10) / 2,
        'oY': parseInt(this._diagramDesigner.skinParts.$selectionOutline.css('top'), 10) + parseInt(this._diagramDesigner
          .skinParts.$selectionOutline.css('height'), 10) / 2,
  ...
  SelectionManager.prototype.onSelectionRotated = function (deg, selectedIds) {
    this.logger.warning('SelectionManager.prototype.onSelectionRotated IS NOT OVERRIDDEN IN HOST COMPONENT. deg: \'' +
