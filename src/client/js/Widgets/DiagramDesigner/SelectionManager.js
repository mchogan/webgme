...
define(['logManager',
  ...
  SelectionManager.prototype._activateMouseListeners = function () {
    ...
    this._diagramDesigner.onItemMouseDown = function (itemId, eventDetails) {
      if (self._diagramDesigner.mode === self._diagramDesigner.OPERATING_MODES
        ...
        self._setSelection([itemId], self._isMultiSelectionModifierKeyPressed(
          eventDetails));
      }
    };

    this._diagramDesigner.onConnectionMouseDown = function (connId,
      eventDetails) {
      if (self._diagramDesigner.mode === self._diagramDesigner.OPERATING_MODES
        .READ_ONLY ||
        self._diagramDesigner.mode === self._diagramDesigner.OPERATING_MODES.DESIGN
      ) {
        self._setSelection([connId], self._isMultiSelectionModifierKeyPressed(
          eventDetails));
      }
    };
  ...
  SelectionManager.prototype.setSelection = function (idList,
    addToExistingSelection) {
    this._setSelection(idList, addToExistingSelection);
  };

  SelectionManager.prototype.onSelectionCommandClicked = function (command,
    selectedIds) {
    this.logger.warning(
      "SelectionManager.prototype.onSelectionCommandClicked IS NOT OVERRIDDEN IN HOST COMPONENT. command: '" +
      command + "', selectedIds: " + selectedIds);
  };

  SelectionManager.prototype.onSelectionChanged = function (selectedIDs) {
    this.logger.warning(
      "SelectionManager.prototype.onSelectionChanged IS NOT OVERRIDDEN IN HOST COMPONENT. selectedIDs: " +
      selectedIDs);
  };

  /********************** MULTIPLE SELECTION MODIFIER KEY CHECK ********************/
  SelectionManager.prototype._isMultiSelectionModifierKeyPressed = function (
    event) {
  ...
  SelectionManager.prototype._onBackgroundMouseDown = function (event) {
    ...
    this.logger.debug("SelectionManager._onBackgroundMouseDown at: " + JSON.stringify(
      mousePos));
  ...
  SelectionManager.prototype._onBackgroundMouseUp = function (event) {
    ...
    if (this._rubberbandSelection) {
      ...
      params = {
        ...
        "x2": Math.max(this._rubberbandSelection.x, this._rubberbandSelection
          .x2),
        "y": Math.min(this._rubberbandSelection.y, this._rubberbandSelection.y2),
        "y2": Math.max(this._rubberbandSelection.y, this._rubberbandSelection
          .y2)
      };
  ...
  SelectionManager.prototype._selectItemsByRubberBand = function (params) {
    ...
    this.logger.debug("Select children by rubber band: [" + rbBBox.x + "," +
      rbBBox.y + "], [" + rbBBox.x2 + "," + rbBBox.y2 + "]");
      ...
      if (itemBBox) {
        if (clientUtil.overlap(rbBBox, itemBBox)) {
          ...
          interSectionRatio = (interSectionRect.x2 - interSectionRect.x) * (
            interSectionRect.y2 - interSectionRect.y) / ((itemBBox.x2 -
            itemBBox.x) * (itemBBox.y2 - itemBBox.y));
  ...
  /*********************** SET SELECTION *********************************/
  SelectionManager.prototype._setSelection = function (idList,
    addToExistingSelection) {
    var i,
      len = idList.length,
      item,
    ...
    this.logger.debug("setSelection: " + idList +
      ", addToExistingSelection: " + addToExistingSelection);
    ...
    if (bBox && bBox.hasOwnProperty("x")) {
      ...
      if (this._diagramDesigner.skinParts.$selectionOutline) {
        ...
        this._diagramDesigner.skinParts.$selectionOutline =
          SELECTION_OUTLINE_BASE.clone();

        this._diagramDesigner.skinParts.$itemsContainer.prepend(this._diagramDesigner
          .skinParts.$selectionOutline);
      }
  ...
  SelectionManager.prototype._renderSelectionActions = function () {
    ...
    if (this._diagramDesigner.getIsReadOnlyMode() !== true) {
      ...
      this._diagramDesigner._makeDraggable({
        '$el': moveBtn
      });
    }
    ...
    this._diagramDesigner.skinParts.$selectionOutline.off("mousedown").off(
      "click", ".s-btn");
    this._diagramDesigner.skinParts.$selectionOutline.on("mousedown",
      function (event) {
        event.stopPropagation();
      }).on("click", ".s-btn", function (event) {
      var command = $(this).attr("command");
      self.logger.debug("Selection button clicked with command: '" +
        command + "'");
  ...
  SelectionManager.prototype._renderRotateHandlers = function () {
    ...
    if (rotateEnabled) {
      ...
      this._diagramDesigner.skinParts.$selectionOutline.append(
        rotateBtnBottom);
      this._diagramDesigner.skinParts.$selectionOutline.append(this._rotationDegree);

      this._diagramDesigner.skinParts.$selectionOutline.off("mousedown." +
        MOUSE_EVENT_POSTFIX, ".rotate");
      this._diagramDesigner.skinParts.$selectionOutline.on("mousedown." +
        MOUSE_EVENT_POSTFIX, ".rotate", function (event) {
          var rotateBtn = $(this);
  ...
  SelectionManager.prototype._startSelectionRotate = function (rotateBtn,
    event) {
    ...
    if (leftButton) {
      this._rotateDesc = {
        "startX": mousePos.mX,
        "startY": mousePos.mY,
        "oX": parseInt(this._diagramDesigner.skinParts.$selectionOutline.css(
          "left"), 10) + parseInt(this._diagramDesigner.skinParts.$selectionOutline
          .css("width"), 10) / 2,
        "oY": parseInt(this._diagramDesigner.skinParts.$selectionOutline.css(
          "top"), 10) + parseInt(this._diagramDesigner.skinParts.$selectionOutline
          .css("height"), 10) / 2,
      ...
      $(document).on("mousemove.rotate." + MOUSE_EVENT_POSTFIX, function (
        event) {
  ...
  SelectionManager.prototype._onSelectionRotate = function (event) {
    var mousePos = this._diagramDesigner.getAdjustedMousePos(event),
      ...
      roundTo = event.shiftKey ? 10 : (event.ctrlKey || event.metaKey ? 45 :
        0),
      deg = this._getRotationDegree(this._rotateDesc.horizontal ? dx : dy,
        roundTo);
  ...
  SelectionManager.prototype._rotateSelectionBy = function (deg) {
    ...
    while (i--) {
      if (this._diagramDesigner.itemIds.indexOf(this._selectedElements[i]) !==
        -1) {
  ...
  SelectionManager.prototype.onSelectionRotated = function (deg, selectedIds) {
    this.logger.warning(
      "SelectionManager.prototype.onSelectionRotated IS NOT OVERRIDDEN IN HOST COMPONENT. deg: '" +
