...
define(['./AutoRouter.Constants',
  ...
  var _getOptimalPorts = function (ports, tgt) {
    ...
    function createPriority(port, center) {
      var priority = 0,
        ...
        point = [port.getRect().getCenter().x - center.x, port.getRect().getCenter()
          .y - center.y
        ],
      ...
      if (point[major] > 0 === vector[major] > 0 && (point[major] === 0) ===
        (vector[major] === 0)) { //handling the === 0 error
        //If they have the same parity, assign the priority to maximize that is > 1
        priority = (Math.abs(vector[major]) / Math.abs(vector[major] -
          point[major])) * 25;
      }

      if (point[minor] > 0 === vector[minor] > 0 && (point[minor] === 0) ===
        (vector[minor] === 0)) { //handling the === 0 error
        //If they have the same parity, assign the priority to maximize that is < 1
        priority += vector[minor] !== point[minor] ? (Math.abs(vector[minor]) /
          Math.abs(vector[minor] - point[minor])) * 1 : 0;
      }
    ...
    assert(port.getOwner(),
      "ARGraph.getOptimalPorts: port have invalid owner");
  ...
  var _distanceSquareFromLine = function (start, end, pt) {
    ...
    //            |end-start|
    //
    var nom = Math.abs((end.x - start.x) * (start.y - pt.y) - (start.x - pt
        .x) * (end.y - start.y)),
      denom_square = ((end.x - start.x) * (end.x - start.x) + (end.y -
  ...
  var _isOnEdge = function (start, end, pt) {
    ...
    {
      if (end.x === pt.x && pt.y <= Math.max(end.y, start.y) && pt.y >=
        Math.min(end.y, start.y)) {
        return true;
      }
    } else if (start.y === end.y) // horizontal line, vertical move
    {
      if (start.y === pt.y && pt.x <= Math.max(end.x, start.x) && pt.x >=
  ...
  var _isLineMeetHLine = function (start, end, x1, x2, y) {
    ...
    if (end2.y === 0) {
      return y === 0 && ((x1 <= 0 && 0 <= x2) || (x1 <= end2.x && end2.x <=
        x2));
    }
  ...
  var _isLineMeetVLine = function (start, end, y1, y2, x) {
    ...
    if (end2.x === 0) {
      return x === 0 && ((y1 <= 0 && 0 <= y2) || (y1 <= end2.y && end2.y <=
        y2));
    }
  ...
  var _getLineClipRectIntersect = function (start, end, rect) {
    ...
    assert(_isRightAngle(dir),
      "ArHelper.getLineClipRectIntersect: _isRightAngle(dir) FAILED");
  ...
  var _intersect = function (a1, a2, b1, b2) {
    return Math.min(a1, a2) <= Math.max(b1, b2) && Math.min(b1, b2) <= Math
      .max(a1, a2);
  };
  ...
  var _stepOneInDir = function (point, dir) {
    assert(_isRightAngle(dir),
      "ArHelper.stepOnInDir: _isRightAngle(dir) FAILED");
  ...
  var _getRectCoord = function (rect, dir) {
    assert(_isRightAngle(dir),
      "ArHelper.getRectCoord: _isRightAngle(dir) FAILED");
  ...
  var _getChildRectOuterCoordFrom = function (bufferObject, inDir, point) { //Point travels inDir until hits child box
    ...
    assert(_isRightAngle(inDir),
      "getChildRectOuterCoordFrom: _isRightAngle(inDir) FAILED");
    ...
    assert(!_isPointInDirFrom(point, bufferObject.box, inDir),
      "getChildRectOuterCoordFrom: !isPointInDirFrom(point, bufferObject.box.getRect(), (inDir)) FAILED"
    );
    ...
    while (++i < children.length) {
      ...
      if (_isPointInDirFrom(point, children[i], _reverseDir(inDir)) &&
        ...
        _isCoordInDirFrom(res, _getRectOuterCoord(children[i], _reverseDir(
          inDir)), (inDir))) {
  ...
  var _getRectOuterCoord = function (rect, dir) {
    assert(_isRightAngle(dir),
      "ArHelper.getRectOuterCoord: _isRightAngle(dir) FAILED");
    var t = rect.ceil - 1,
      r = rect.right + 1,
      b = rect.floor + 1,
      l = rect.left - 1;
  ...
  var getDirTableIndex = function (offset) {
    return (offset.cx >= 0) * 4 + (offset.cy >= 0) * 2 + (Math.abs(offset.cx) >=
      Math.abs(offset.cy));
  };
  ...
  var _isPointInDirFromChildren = function (point, fromParent, dir) {
    ...
    assert(_isRightAngle(dir),
      "isPointInDirFromChildren: _isRightAngle(dir) FAILED");
  ...
  var _isPointInDirFrom = function (point, from, dir) {
    if (from instanceof ArRect) {
      var rect = from;
      assert(_isRightAngle(dir),
        "ArHelper.isPointInDirFrom: _isRightAngle(dir) FAILED");
  ...
  var _isCoordInDirFrom = function (coord, from, dir) {
    assert(_isRightAngle(dir),
      "ArHelper.isCoordInDirFrom: _isRightAngle(dir) FAILED");
