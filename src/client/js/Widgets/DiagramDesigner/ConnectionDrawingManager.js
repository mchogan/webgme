...
define(['logManager',
  ...
  ConnectionDrawingManager = function (options) {
    this.logger = logManager.create(((options && options.loggerName) ||
    ...
    if (this._diagramDesigner === undefined || this._diagramDesigner === null) {
      this.logger.error(
        "Trying to initialize a ConnectionDrawingManager without a diagramDesigner..."
      );
  ...
  ConnectionDrawingManager.prototype._activateMouseListeners = function () {
    ...
    this._el.on(MOUSEDOWN, '.' + DiagramDesignerWidgetConstants.CONNECTOR_CLASS,
      function (event) {
        var el = $(this),
        ...
        if (buttonLeft) {
          if (objId === undefined || objId === null) {
            self.logger.error(
              'MOUSEDOWN on "connector" element but attribute "' +
              DiagramDesignerWidgetConstants.DATA_ITEM_ID +
              '" is not specified');
          } else {
            if (self._diagramDesigner.mode === self._diagramDesigner.OPERATING_MODES
              .DESIGN) {
              self._startConnectionCreate(objId, sCompId, el, self._diagramDesigner
                .getAdjustedMousePos(event));
    ...
    this._el.on(MOUSEDOWN, '.' + DiagramDesignerWidgetConstants.CONNECTION_DRAGGABLE_END_CLASS,
      function (event) {
        var el = $(this),
        ...
        if (buttonLeft) {
          if (self._diagramDesigner.mode === self._diagramDesigner.OPERATING_MODES
            .DESIGN) {
            self._startConnectionReconnect(connId, draggedEnd, self._diagramDesigner
              .getAdjustedMousePos(event));
            self._attachMouseListeners();
          }
        }
    ...
    this._el.on(MOUSEUP, '.' + DiagramDesignerWidgetConstants.CONNECTOR_CLASS,
      function ( /*event*/ ) {
        var el = $(this),
          objId = el.attr(DiagramDesignerWidgetConstants.DATA_ITEM_ID),
          sCompId = el.attr(DiagramDesignerWidgetConstants.DATA_SUBCOMPONENT_ID);
        ...
        if (self._connectionInDraw === true) {
          if (objId === undefined || objId === null) {
            self.logger.error(
              'MOUSEUP on "connector" element but attribute "' +
              DiagramDesignerWidgetConstants.DATA_ITEM_ID +
              '" is not specified');
          } else {
            if (self._diagramDesigner.mode === self._diagramDesigner.OPERATING_MODES
   ...
   */
  ConnectionDrawingManager.prototype.setConnectionInDrawProperties = function (
    params) {
    this._connectionPathProps.strokeWidth = params[
      DiagramDesignerWidgetConstants.LINE_WIDTH] || this._connectionPathProps
      .strokeWidth;

    this._connectionPathProps.strokeColor = params[
      DiagramDesignerWidgetConstants.LINE_COLOR] || this._connectionPathProps
      .strokeColor;
    this._connectionPathProps.arrowStart = params[
      DiagramDesignerWidgetConstants.LINE_START_ARROW] || this._connectionPathProps
      .arrowStart;
    this._connectionPathProps.arrowEnd = params[
      DiagramDesignerWidgetConstants.LINE_END_ARROW] || this._connectionPathProps
      .arrowEnd;

    this._connectionPathProps.lineType = params[
      DiagramDesignerWidgetConstants.LINE_PATTERN] || this._connectionPathProps
      .lineType;
  };
  ...
  ConnectionDrawingManager.prototype._drawConnection = function () {
    ...
    if (this._connectionInDrawProps.type === DRAW_TYPE_RECONNECT) {
      var connProps = this._diagramDesigner.items[this._connectionInDrawProps
        .connId].getConnectionProps();
   ...
   */
  ConnectionDrawingManager.prototype._getClosestConnectionPointCoordinates =
    function (objId, sCompId, mX, mY) {
      ...
      if (item) {
        ...
        len = connPoints.length;
        result = {
          'x': connPoints[len - 1].x1 + (connPoints[len - 1].x2 - connPoints[
            len - 1].x1) / 2,
          'y': connPoints[len - 1].y1 + (connPoints[len - 1].y2 - connPoints[
            len - 1].y1) / 2
        };
        delta = Math.sqrt((result.x - mX) * (result.x - mX) + (result.y - mY) *
          (result.y - mY));

        len -= 1;

        while (len--) {
          x = connPoints[len].x1 + (connPoints[len].x2 - connPoints[len].x1) /
            2;
          y = connPoints[len].y1 + (connPoints[len].y2 - connPoints[len].y1) /
            2;
   ...
   */
  ConnectionDrawingManager.prototype._connectionEndDrop = function (
    endPointId, sCompId) {
    var desc;
    ...
    if (this._connectionInDrawProps.type === DRAW_TYPE_CREATE) {
      ...
      if (this._connectionPathProps.strokeWidth !== DEFAULT_WIDTH) {
        desc.visualStyle[DiagramDesignerWidgetConstants.LINE_WIDTH] = this._connectionPathProps
          .strokeWidth;
      }

      if (this._connectionPathProps.strokeColor !== DEFAULT_COLOR) {
        desc.visualStyle[DiagramDesignerWidgetConstants.LINE_COLOR] = this._connectionPathProps
          .strokeColor;
      }
      ...
      if (this._connectionPathProps.arrowEnd !== DEFAULT_END_ARROW) {
        desc.visualStyle[DiagramDesignerWidgetConstants.LINE_END_ARROW] =
          this._connectionPathProps.arrowEnd;
      }

      if (this._connectionPathProps.lineType !== DEFAULT_PATTERN) {
        desc.visualStyle[DiagramDesignerWidgetConstants.LINE_PATTERN] = this._connectionPathProps
          .lineType;
      }
   ...
   */
  ConnectionDrawingManager.prototype._startConnectionCreate = function (objId,
    sCompId, el, mousePos) {
    var srcCoord;

    this.logger.debug("Start connection drawing from DesignerItem: '" + objId +
      "', subcomponent: '" + sCompId + "'");
    ...
    this._setTempConnectionInDrawProperties(this._diagramDesigner.items[objId]
      .getDrawnConnectionVisualStyle(sCompId));
    ...
    srcCoord = this._getClosestConnectionPointCoordinates(objId, sCompId,
   ...
   */
  ConnectionDrawingManager.prototype.onStartConnectionCreate = function (
    params) {
    this.logger.warning("onStartConnectionCreate with params: '" + JSON.stringify(
      params));
  };
   ...
   * mousePos: mouse position event belongs to the 'mousedown' event on the connector
   */
  ConnectionDrawingManager.prototype._startConnectionReconnect = function (
    connectionId, draggedEnd, mousePos) {
    this.logger.debug('_startConnectionReconnect connectionId:' +
      connectionId + ', draggedEnd:' + draggedEnd);
   ...
   */
  ConnectionDrawingManager.prototype.onStartConnectionReconnect = function (
    params) {
    this.logger.warning("onStartConnectionReconnect with params: '" + JSON.stringify(
      params));
  };
   ...
   */
  ConnectionDrawingManager.prototype._setConnectionRedrawCoordinates =
    function (mousePos) {
      var connID = this._connectionInDrawProps.connId,
        srcDragged = this._connectionInDrawProps.draggedEnd ===
      ...
      if (connID) {
        if (srcDragged === true) {
          ...
          if (this._diagramDesigner.connectionEndIDs[connID]) {
            ...
            var dstSubCompId = this._diagramDesigner.connectionEndIDs[connID]
              .dstSubCompId;

            coord = this._getClosestConnectionPointCoordinates(dstObjId,
              dstSubCompId, mousePos.mX, mousePos.mY);
          ...
          if (this._diagramDesigner.connectionEndIDs[connID]) {
            ...
            var srcSubCompId = this._diagramDesigner.connectionEndIDs[connID]
              .srcSubCompId;

            coord = this._getClosestConnectionPointCoordinates(srcObjId,
              srcSubCompId, mousePos.mX, mousePos.mY);
   ...
   * mousePos: the current mouse position
   */
  ConnectionDrawingManager.prototype._setConnectionCreateCoordinates =
    function (mousePos) {
      var srcCoord = this._getClosestConnectionPointCoordinates(this._connectionInDrawProps
        .src, this._connectionInDrawProps.sCompId, mousePos.mX, mousePos.mY
      );
  ...
  ConnectionDrawingManager.prototype._onBackgroundMouseMove = function (event) {
    ...
    if (this._connectionInDraw === true) {
      if (this._minimalDistanceMet === true) {
        ...
        if (Math.abs(dx) >= MINIMAL_DISTANCE || Math.abs(dy) >=
          MINIMAL_DISTANCE) {
   ...
   */
  ConnectionDrawingManager.prototype._updateDrawnConnection = function (
    mousePos) {
    if (this._connectionInDrawProps.type === DRAW_TYPE_CREATE) {
   ...
   */
  ConnectionDrawingManager.prototype._designerItemPositionChanged = function (
    objId) {
    ...
    if (this._connectionInDrawProps.type === DRAW_TYPE_CREATE) {
      ...
      if (this._connectionInDrawProps.draggedEnd ===
        DiagramDesignerWidgetConstants.CONNECTION_END_SRC) {
        if (this._diagramDesigner.connectionEndIDs[this._connectionInDrawProps
          .connId].dstObjId === objId) {
          //'source' end of the connection is dragged
   ...
   */
  ConnectionDrawingManager.prototype._onComponentDelete = function (objId,
    sCompID) {
    ...
    if (this._connectionInDrawProps.type === DRAW_TYPE_CREATE) {
      ...
      if (objId === this._connectionInDrawProps.connId) {
        ...
        cancelDraw = true;
        this.logger.debug('Existing connection currently being redrawn "' +
          objId + '" is being deleted. Connection re-connect canceled.');
      } else {
        if (sCompID === undefined || sCompID === null) {
          //a designer-item is being deleted
          if (this._diagramDesigner.connectionEndIDs[this._connectionInDrawProps
              .connId].srcObjId === objId ||
            this._diagramDesigner.connectionEndIDs[this._connectionInDrawProps
              .connId].dstObjId === objId) {
            //connection is being drawn from/to this component
            cancelDraw = true;
          ...
          if ((this._diagramDesigner.connectionEndIDs[this._connectionInDrawProps
                .connId].srcObjId === objId &&
              this._diagramDesigner.connectionEndIDs[this._connectionInDrawProps
                .connId].srcSubCompId === sCompID) ||
            (this._diagramDesigner.connectionEndIDs[this._connectionInDrawProps
                .connId].dstObjId === objId &&
              this._diagramDesigner.connectionEndIDs[this._connectionInDrawProps
                .connId].dstSubCompId === sCompID)) {
  ...
  ConnectionDrawingManager.prototype._setTempConnectionInDrawProperties =
    function (params) {
      if (params) {
  ...
  ConnectionDrawingManager.prototype._resetTempConnectionInDrawProperties =
    function () {
  ...
  ConnectionDrawingManager.prototype._saveTempConnectionInDrawProperties =
    function () {
  ...
  ConnectionDrawingManager.prototype._revertSavedConnectionInDrawProperties =
    function () {
