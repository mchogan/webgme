...
define(['logManager',
  ...
  ConnectionDrawingManager.prototype.initialize = function (el) {
    ...
    this._diagramDesigner.addEventListener(this._diagramDesigner.events.ITEM_POSITION_CHANGED, function (_canvas,
      event) {
      self._designerItemPositionChanged(event.ID);
    });

    this._diagramDesigner.addEventListener(this._diagramDesigner.events.ITEM_SUBCOMPONENT_POSITION_CHANGED, function (
      _canvas, event) {
      self._designerItemPositionChanged(event.ItemID);
    });

    this._diagramDesigner.addEventListener(this._diagramDesigner.events.ON_COMPONENT_DELETE, function (_canvas,
      componentId) {
      self._onComponentDelete(componentId);
    });

    this._diagramDesigner.addEventListener(this._diagramDesigner.events.ON_UNREGISTER_SUBCOMPONENT, function (_canvas,
      eventArgs) {
  ...
  ConnectionDrawingManager.prototype._activateMouseListeners = function () {
    ...
    this._el.on(MOUSEDOWN, '.' + DiagramDesignerWidgetConstants.CONNECTOR_CLASS, function (event) {
      ...
      if (buttonLeft) {
        if (objId === undefined || objId === null) {
          self.logger.error('MOUSEDOWN on "connector" element but attribute "' + DiagramDesignerWidgetConstants.DATA_ITEM_ID +
            '" is not specified');
        } else {
    ...
    this._el.on(MOUSEUP, '.' + DiagramDesignerWidgetConstants.CONNECTOR_CLASS, function ( /*event*/ ) {
      ...
      if (self._connectionInDraw === true) {
        if (objId === undefined || objId === null) {
          self.logger.error('MOUSEUP on "connector" element but attribute "' + DiagramDesignerWidgetConstants.DATA_ITEM_ID +
            '" is not specified');
        } else {
          if (self._diagramDesigner.mode === self._diagramDesigner.OPERATING_MODES.DESIGN) {
  ...
  ConnectionDrawingManager.prototype.setConnectionInDrawProperties = function (params) {
    this._connectionPathProps.strokeWidth = params[DiagramDesignerWidgetConstants.LINE_WIDTH] || this._connectionPathProps
      .strokeWidth;

    this._connectionPathProps.strokeColor = params[DiagramDesignerWidgetConstants.LINE_COLOR] || this._connectionPathProps
      .strokeColor;
    this._connectionPathProps.arrowStart = params[DiagramDesignerWidgetConstants.LINE_START_ARROW] || this._connectionPathProps
      .arrowStart;
    this._connectionPathProps.arrowEnd = params[DiagramDesignerWidgetConstants.LINE_END_ARROW] || this._connectionPathProps
      .arrowEnd;

    this._connectionPathProps.lineType = params[DiagramDesignerWidgetConstants.LINE_PATTERN] || this._connectionPathProps
      .lineType;
  };
  ...
  ConnectionDrawingManager.prototype._setConnectionCreateCoordinates = function (mousePos) {
    var srcCoord = this._getClosestConnectionPointCoordinates(this._connectionInDrawProps.src, this._connectionInDrawProps
      .sCompId, mousePos.mX, mousePos.mY);
