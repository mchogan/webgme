...
define(['logManager',
  ...
  DiagramDesignerWidget.prototype._createLineStyleMenuItem = function (width, color, pattern, startArrow, endArrow,
    ...
    if (type === DiagramDesignerWidgetConstants.LINE_TYPES.BEZIER) {
      path = paper.path('M 5,' + (Math.round(vSize / 2) + 0.5) + ' C' + (5 + bezierControlOffset) + ',' + (Math.round(
        vSize / 2) + 0.5 - bezierControlOffset * 2) + ' ' + (hSize - bezierControlOffset) + ',' + (Math.round(vSize /
        2) + 0.5 + bezierControlOffset * 2) + ' ' + (hSize - 5) + ',' + (Math.round(vSize / 2) + 0.5));
    } else {
      path = paper.path('M 5,' + (Math.round(vSize / 2) + 0.5) + ' L' + (hSize - 5) + ',' + (Math.round(vSize / 2) +
        0.5));
    }
  ...
  DiagramDesignerWidget.prototype._refreshScreen = function () {
    ...
    connectionIDsToUpdate = this._insertedConnectionIDs.concat(this._updatedConnectionIDs, this._getAssociatedConnectionsForItems(
      affectedItems));
  ...
  DiagramDesignerWidget.prototype.onSelectionDelete = function (selectedIds) {
    this.logger.warning('DiagramDesignerWidget.onSelectionDelete IS NOT OVERRIDDEN IN A CONTROLLER. ID: \'' +
      selectedIds + '\'');
  };

  DiagramDesignerWidget.prototype.onSelectionContextMenu = function (selectedIds, mousePos) {
    this.logger.warning('DiagramDesignerWidget.onSelectionContextMenu IS NOT OVERRIDDEN IN A CONTROLLER. ID: \'' +
  ...
  DiagramDesignerWidget.prototype.onClipboardCopy = function (selectedIds) {
    this.logger.warning(
      'DiagramDesignerWidget.prototype.onClipboardCopy not overridden in controller!!! selectedIds: \'' + selectedIds +
      '\'');
  };
  ...
  DiagramDesignerWidget.prototype.onConnectionSegmentPointsChange = function (params) {
    this.logger.warning(
      'DiagramDesignerWidget.prototype.onConnectionSegmentPointsChange not overridden in controller. params: ' + JSON
      .stringify(params));
  };
  ...
  DiagramDesignerWidget.prototype.onDragStartDesignerItemDraggable = function (itemID) {
    this.logger.warning(
      'DiagramDesignerWidget.prototype.onDesignerItemDraggable not overridden in controller. itemID: ' + itemID);
  ...
  DiagramDesignerWidget.prototype.onUnhighlight = function (idList) {
    this.logger.warning('DiagramDesignerWidget.prototype.onUnhighlight not overridden in controller. idList: ' +
      idList);
  };
  /************************* HIGHLIGHTED / UNHIGHLIGHTED EVENT *****************************/

  DiagramDesignerWidget.prototype.onSelectionRotated = function (deg, selectedIds) {
    this.logger.warning('DiagramDesignerWidget.prototype.onSelectionRotated IS NOT OVERRIDDEN IN CONTROLLER. deg: \'' +
  ...
  DiagramDesignerWidget.prototype.onConnectionNameChanged = function (connId, oldValue, newValue) {
    this.logger.warning(
      'DiagramDesignerWidget.prototype.onConnectionNameChanged IS NOT OVERRIDDEN IN CONTROLLER. connId: \'' + connId +
  ...
  DiagramDesignerWidget.prototype.onSetConnectionProperty = function (params) {
    this.logger.warning(
      'DiagramDesignerWidget.prototype.onSetConnectionProperty IS NOT OVERRIDDEN IN CONTROLLER. params: \'' + JSON.stringify(
        params));
  };
