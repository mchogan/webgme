...
define(['logManager',
  ...
  var defaultParams = {
    'loggerName': 'DiagramDesignerWidget',
    'gridSize': 10,
    'droppable': true,
  ...
  DiagramDesignerWidget = function (container, par) {
    ...
    this.selectionManager = params.selectionManager || new SelectionManager({
      "diagramDesigner": this
    });
    this.selectionManager.initialize(this.skinParts.$itemsContainer);
    this.selectionManager.onSelectionCommandClicked = function (command,
      selectedIds, event) {
    ...
    this.connectionRouteManager = params.connectionRouteManager || new DEFAULT_CONNECTION_ROUTE_MANAGER({
      "diagramDesigner": this
    });
    this.connectionRouteManager.initialize();

    //initiate connection drawing component and hook up event callbacks
    this.connectionDrawingManager = new ConnectionDrawingManager({
      "diagramDesigner": this
    });
    ...
    this.connectionDrawingManager.onStartConnectionReconnect = function (
      params) {
    ...
    this.setOperatingMode(DiagramDesignerWidgetOperatingModes.prototype.OPERATING_MODES
      .DESIGN);
  ...
  DiagramDesignerWidget.prototype.onWidgetContainerResize = function (width,
    height) {
  ...
  DiagramDesignerWidget.prototype._initializeUI = function () {
    ...
    this.__loader = new LoaderCircles({
      "containerElement": this.$el.parent()
    });
  ...
  DiagramDesignerWidget.prototype._createLineStyleMenuItem = function (width,
    color, pattern, startArrow, endArrow, type) {
    var el = $('<div/>'),
      path,
      hSize = 50,
      vSize = 20,
    ...
    el.attr({
      "style": "height: " + vSize + "px; width: " + hSize + "px;"
    });

    if (type === DiagramDesignerWidgetConstants.LINE_TYPES.BEZIER) {
      path = paper.path("M 5," + (Math.round(vSize / 2) + 0.5) + " C" + (5 +
          bezierControlOffset) + "," + (Math.round(vSize / 2) + 0.5 -
          bezierControlOffset * 2) + " " + (hSize - bezierControlOffset) +
        "," + (Math.round(vSize / 2) + 0.5 + bezierControlOffset * 2) + " " +
        (hSize - 5) + "," + (Math.round(vSize / 2) + 0.5));
    } else {
      path = paper.path("M 5," + (Math.round(vSize / 2) + 0.5) + " L" + (
        hSize - 5) + "," + (Math.round(vSize / 2) + 0.5));
    }
  ...
  DiagramDesignerWidget.prototype._resizeItemContainer = function () {
    ...
    this.logger.debug('this._actualSize: ' + this._actualSize.w + ', ' + this
      ._actualSize.h);
    ...
    paddingTop = parseInt(this.skinParts.$diagramDesignerWidgetBody.css(
      'padding-top').replace("px", ""));
    paddingLeft = parseInt(this.skinParts.$diagramDesignerWidgetBody.css(
  ...
  DiagramDesignerWidget.prototype.getAdjustedOffset = function (offset) {
    var childrenContainerOffset = this._offset,
      left = (offset.left - childrenContainerOffset.left) / this._zoomRatio +
      childrenContainerOffset.left,
      top = (offset.top - childrenContainerOffset.top) / this._zoomRatio +
      childrenContainerOffset.top;
  ...
  DiagramDesignerWidget.prototype._tryRefreshScreen = function () {
    ...
    if (this._updating !== true) {
      ...
      insertedLen += this._insertedDesignerItemIDs ? this._insertedDesignerItemIDs
        .length : 0;
      insertedLen += this._insertedConnectionIDs ? this._insertedConnectionIDs
        .length : 0;

      updatedLen += this._updatedDesignerItemIDs ? this._updatedDesignerItemIDs
        .length : 0;
      updatedLen += this._updatedConnectionIDs ? this._updatedConnectionIDs.length :
        0;

      deletedLen += this._deletedDesignerItemIDs ? this._deletedDesignerItemIDs
        .length : 0;
      deletedLen += this._deletedConnectionIDs ? this._deletedConnectionIDs.length :
        0;
  ...
  DiagramDesignerWidget.prototype._refreshScreen = function () {
    ...
    this.logger.debug('Redrawn/Requested: ' + redrawnConnectionIDs.length +
      '/' + connectionIDsToUpdate.length);
  ...
  DiagramDesignerWidget.prototype._alignPositionToGrid = function (pX, pY) {
    ...
    if (this.gridSize > 1) {
      ...
      if ((posXDelta !== 0) || (posYDelta !== 0)) {
        pX += (posXDelta < Math.floor(this.gridSize / 2) + 1 ? -1 * posXDelta :
          this.gridSize - posXDelta);
        pY += (posYDelta < Math.floor(this.gridSize / 2) + 1 ? -1 * posYDelta :
          this.gridSize - posYDelta);
      }
    }
  ...
  DiagramDesignerWidget.prototype._checkPositionOverlap = function (itemId,
    objDescriptor) {
    var i,
      posChanged = true,
      itemID,
  ...
  /************************** DRAG ITEM ***************************/
  DiagramDesignerWidget.prototype.onDesignerItemDragStart = function (
    draggedItemId, allDraggedItemIDs) {
  ...
  DiagramDesignerWidget.prototype.onDesignerItemDrag = function (
    draggedItemId, allDraggedItemIDs) {
    ...
    connectionIDsToUpdate = this._getAssociatedConnectionsForItems(
      allDraggedItemIDs).sort();
  ...
  DiagramDesignerWidget.prototype.onDesignerItemDragStop = function (
    draggedItemId, allDraggedItemIDs) {
    this.selectionManager.showSelectionOutline();
  ...
  DiagramDesignerWidget.prototype._onSelectionCommandClicked = function (
    command, selectedIds, event) {
    switch (command) {
    case 'delete':
  ...
  DiagramDesignerWidget.prototype.onSelectionDelete = function (selectedIds) {
    this.logger.warning(
      "DiagramDesignerWidget.onSelectionDelete IS NOT OVERRIDDEN IN A CONTROLLER. ID: '" +
      selectedIds + "'");
  };

  DiagramDesignerWidget.prototype.onSelectionContextMenu = function (
    selectedIds, mousePos) {
    this.logger.warning(
      "DiagramDesignerWidget.onSelectionContextMenu IS NOT OVERRIDDEN IN A CONTROLLER. ID: '" +
  ...
  DiagramDesignerWidget.prototype._onSelectionChanged = function (selectedIds) {
    ...
    if (this.toolbarItems) {
      ...
      if (this.toolbarItems.ddbtnConnectionArrowStart) {
        this.toolbarItems.ddbtnConnectionArrowStart.enabled(
          connectionSelected);
      }
  ...
  DiagramDesignerWidget.prototype.onSelectionChanged = function (selectedIds) {
    this.logger.debug(
      "DiagramDesignerWidget.onSelectionChanged IS NOT OVERRIDDEN IN A CONTROLLER..."
    );
  };
  ...
  DiagramDesignerWidget.prototype.itemAutoLayout = function (mode) {
    ...
    case "diagonal":
      while (i--) {
        ...
        this.updateDesignerItem(this.itemIds[i], {
          "position": {
            "x": x,
            "y": y
          }
  ...
  DiagramDesignerWidget.prototype._onConnectionRouteManagerChanged = function (
    type) {
    ...
    switch (type) {
    case "basic":
      this.connectionRouteManager = new ConnectionRouteManagerBasic({
        "diagramDesigner": this
      });
      break;
    case "basic2":
      this.connectionRouteManager = new ConnectionRouteManager2({
        "diagramDesigner": this
      });
      break;
    case "basic3":
      this.connectionRouteManager = new ConnectionRouteManager3({
        "diagramDesigner": this
      });
      break;
    default:
  ...
  DiagramDesignerWidget.prototype._getAssociatedConnectionsForItems =
    function (itemIdList) {
      var connList = [],
  ...
  DiagramDesignerWidget.prototype._getItemsForConnection = function (
    connectionId) {
    var items = [];
  ...
  DiagramDesignerWidget.prototype.setBackgroundText = function (text, params) {
    ...
    if (!this._backGroundText) {
      if (!text) {
        this.logger.error(
          "Invalid parameter 'text' for method 'setBackgroundText'");
      } else {
        this._backGroundText = this.skinParts.SVGPaper.text(this._svgPaperSize
          .w / 2, this._svgPaperSize.h / 2, text);
      }
    } else {
      svgParams.text = text;
  ...
  DiagramDesignerWidget.prototype.selectAll = function () {
    this.selectionManager.clear();
    this.selectionManager.setSelection(this.itemIds.concat(this.connectionIds),
      false);
  };
  ...
  DiagramDesignerWidget.prototype.onClipboardCopy = function (selectedIds) {
    this.logger.warning(
      "DiagramDesignerWidget.prototype.onClipboardCopy not overridden in controller!!! selectedIds: '" +
      selectedIds + "'");
  };

  DiagramDesignerWidget.prototype.onClipboardPaste = function () {
    this.logger.warning(
      "DiagramDesignerWidget.prototype.onClipboardPaste not overridden in controller!!!"
    );
  };
  ...
  DiagramDesignerWidget.prototype.onConnectionSegmentPointsChange = function (
    params) {
    this.logger.warning(
      "DiagramDesignerWidget.prototype.onConnectionSegmentPointsChange not overridden in controller. params: " +
  ...
  DiagramDesignerWidget.prototype.onDragStartDesignerItemDraggable = function (
    itemID) {
    this.logger.warning(
      "DiagramDesignerWidget.prototype.onDesignerItemDraggable not overridden in controller. itemID: " +
      itemID);

    return true;
  };

  DiagramDesignerWidget.prototype.onDragStartDesignerItemCopyable = function (
    itemID) {
    this.logger.warning(
      "DiagramDesignerWidget.prototype.onDragStartDesignerItemCopyable not overridden in controller. itemID: " +
      itemID);

    return true;
  };

  DiagramDesignerWidget.prototype.onDragStartDesignerConnectionCopyable =
    function (connectionID) {
      this.logger.warning(
        "DiagramDesignerWidget.prototype.onDragStartDesignerConnectionCopyable not overridden in controller. connectionID: " +
        connectionID);
  ...
  DiagramDesignerWidget.prototype.onHighlight = function (idList) {
    this.logger.warning(
      "DiagramDesignerWidget.prototype.onHighlight not overridden in controller. idList: " +
      idList);
  };

  DiagramDesignerWidget.prototype.onUnhighlight = function (idList) {
    this.logger.warning(
      "DiagramDesignerWidget.prototype.onUnhighlight not overridden in controller. idList: " +
      idList);
  };
  /************************* HIGHLIGHTED / UNHIGHLIGHTED EVENT *****************************/

  DiagramDesignerWidget.prototype.onSelectionRotated = function (deg,
    selectedIds) {
    this.logger.warning(
      "DiagramDesignerWidget.prototype.onSelectionRotated IS NOT OVERRIDDEN IN CONTROLLER. deg: '" +
      deg + "', selectedIds: " + selectedIds);
  };

  /*********************** CONNECTION TEXT CHANGED HANDLERS **************************/

  DiagramDesignerWidget.prototype.onConnectionNameChanged = function (connId,
    oldValue, newValue) {
    this.logger.warning(
      "DiagramDesignerWidget.prototype.onConnectionNameChanged IS NOT OVERRIDDEN IN CONTROLLER. connId: '" +
      connId + "', oldValue: " + oldValue + ", newValue:" + newValue);
  };

  DiagramDesignerWidget.prototype.onConnectionSrcTextChanged = function (
    connId, oldValue, newValue) {
    this.logger.warning(
      "DiagramDesignerWidget.prototype.onConnectionSrcTextChanged IS NOT OVERRIDDEN IN CONTROLLER. connId: '" +
      connId + "', oldValue: " + oldValue + ", newValue:" + newValue);
  };

  DiagramDesignerWidget.prototype.onConnectionDstTextChanged = function (
    connId, oldValue, newValue) {
    this.logger.warning(
      "DiagramDesignerWidget.prototype.onConnectionDstTextChanged IS NOT OVERRIDDEN IN CONTROLLER. connId: '" +
      connId + "', oldValue: " + oldValue + ", newValue:" + newValue);
  };
  ...
  DiagramDesignerWidget.prototype.connectToConnectionEnabled = function (
    enabled) {
  ...
  DiagramDesignerWidget.prototype._setConnectionXingJumpMode = function (
    enabled) {
  ...
  DiagramDesignerWidget.prototype.onSetConnectionProperty = function (params) {
    this.logger.warning(
      "DiagramDesignerWidget.prototype.onSetConnectionProperty IS NOT OVERRIDDEN IN CONTROLLER. params: '" +
      JSON.stringify(params));
  };
  ...
  DiagramDesignerWidget.prototype.onUIActivity = function () {
    this.logger.warning(
      "DiagramDesignerWidget.prototype.onUIActivity IS NOT OVERRIDDEN...");
  };
  ...
  DiagramDesignerWidget.prototype._redrawConnections = function (connIDs) {
    ...
    try {
      ...
      res = [];
      this.logger.error(
        'connectionRouteManager.redrawConnections failed with error: ' + exp.stack
      );
    }
    return res;
  };
  ...
  DiagramDesignerWidget.prototype.onSelectionFillColorChanged = function (
    selectedElements, color) {
    this.logger.warning(
      "DiagramDesignerWidget.prototype.onSelectionFillColorChanged(selectedElements, color) IS NOT OVERRIDDEN IN CONTROLLER. color: " +
      color);
  };

  DiagramDesignerWidget.prototype.onSelectionBorderColorChanged = function (
    selectedElements, color) {
    this.logger.warning(
      "DiagramDesignerWidget.prototype.onSelectionBorderColorChanged(selectedElements, color) IS NOT OVERRIDDEN IN CONTROLLER. color: " +
      color);
  };

  DiagramDesignerWidget.prototype.onSelectionTextColorChanged = function (
    selectedElements, color) {
    this.logger.warning(
      "DiagramDesignerWidget.prototype.onSelectionTextColorChanged(selectedElements, color) IS NOT OVERRIDDEN IN CONTROLLER. color: " +
      color);
  };
  ...
  _.extend(DiagramDesignerWidget.prototype,
    DiagramDesignerWidgetOperatingModes.prototype);
  _.extend(DiagramDesignerWidget.prototype,
    DiagramDesignerWidgetDesignerItems.prototype);
  _.extend(DiagramDesignerWidget.prototype, DiagramDesignerWidgetConnections.prototype);
  _.extend(DiagramDesignerWidget.prototype,
    DiagramDesignerWidgetSubcomponents.prototype);
  _.extend(DiagramDesignerWidget.prototype,
    DiagramDesignerWidgetEventDispatcher.prototype);
