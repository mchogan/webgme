...
define(['js/Constants',
  ...
  DiagramDesignerWidgetDecoratorBaseConnectionArea.prototype._initializeConnectionAreaUserSelection =
    function () {
      var self = this;
  ...
  DiagramDesignerWidgetDecoratorBaseConnectionArea.prototype._editConnectionAreas =
    function () {
      ...
      this._connAreaEditBackground = $('<div/>', {
        'class': CONN_AREA_EDIT_BACKGROUND
      });
      ...
      this._connAreaEditBackground.on(
        'mouseup mouseenter mouseleave dblclick', function (event) {
          event.stopPropagation();
        });
      ...
      this._connAreaEditBackground.on('mousedown.' + EVENT_POSTFIX, 'path.' +
        CONN_AREA_EDIT_CLASS, function (event) {
          //$(this.node).toggleClass(DISABLED);
          var c = $(this).attr("class");
          if (c === CONN_AREA_EDIT_CLASS) {
            $(this).attr({
              "class": CONN_AREA_EDIT_CLASS + ' ' + DISABLED
            });
          } else {
            $(this).attr({
              "class": CONN_AREA_EDIT_CLASS
            });
          }
          event.preventDefault();
          event.stopPropagation();
      ...
      this._svg = Raphael(this._connAreaEditBackground[0], w + CONN_AREA_SIZE,
        h + CONN_AREA_SIZE);
      ...
      while (j--) {
        ...
        var path = this._svg.path('M ' + a.x1 + ',' + a.y1 + 'L' + a.x2 + ',' +
          a.y2);
        $(path.node).data(DATA_CONN_AREA_ID, a.id);
        path.attr({
          "stroke-width": CONN_AREA_SIZE
        });
  ...
  DiagramDesignerWidgetDecoratorBaseConnectionArea.prototype._endEditConnectionAreas =
    function () {
      var connAreas = this._connAreaEditBackground.find('path.' +
          CONN_AREA_EDIT_CLASS),
        logger = this.logger,
        disabledAreas = [];
  ...
  DiagramDesignerWidgetDecoratorBaseConnectionArea.prototype._getDisabledConnectionAreas =
    function () {
  ...
  DiagramDesignerWidgetDecoratorBaseConnectionArea.prototype._setDisabledConnectionAreas =
    function (disabledAreaIdList) {
      ...
      if (disabledAreaIdList.length === 0) {
        ...
        this.preferencesHelper.setRegistry(objID, regKey, disabledAreaIdList.slice(
          0));
      }
