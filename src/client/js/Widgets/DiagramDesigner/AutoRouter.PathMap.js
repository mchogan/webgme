...
define(['logManager',
  ...
  ArPathMap.prototype.setSrcPort = function (index, port) {
    assert(port instanceof AutoRouterPort,
      "ArPathMap.setSrcPort: port instanceof AutoRouterPort FAILED");
    this.srcPorts[index] = port;
  };

  ArPathMap.prototype.setDstPort = function (index, port) {
    assert(port instanceof AutoRouterPort,
      "ArPathMap.setDstPort: port instanceof AutoRouterPort FAILED");
  ...
  ArPathMap.prototype.updateSrcPorts = function () {
    ...
    for (var i in this.srcPorts) {
      if (this.srcPorts.hasOwnProperty(i)) {
        assert(this.srcPorts[i] instanceof AutoRouterPort,
          "ArPathMap.updateSrcPorts: this.srcPorts[i] instanceof AutoRouterPort FAILED"
        );
      }
  ...
  ArPathMap.prototype.updateDstPorts = function () {
    ...
    for (var i in this.dstPorts) {
      if (this.dstPorts.hasOwnProperty(i)) {
        assert(this.dstPorts[i] instanceof AutoRouterPort,
          "ArPathMap.updateDstPorts: this.dstPorts[i] instanceof AutoRouterPort FAILED"
        );
      }
