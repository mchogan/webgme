...
define(['js/Widgets/DiagramDesigner/DesignerItem'], function (DesignerItem) {
  ...
  DiagramDesignerWidgetDesignerItems.prototype.createDesignerItem = function (
    objD) {
    var componentId = this._getGuid("I_"),
      objDescriptor = _.extend({}, objD),
      alignedPosition = this._alignPositionToGrid(objDescriptor.position.x,
    ...
    newComponent.__setDecorator(objDescriptor.decorator, objDescriptor.decoratorClass,
      objDescriptor.control, objDescriptor.metaInfo, objDescriptor.preferencesHelper,
  ...
  DiagramDesignerWidgetDesignerItems.prototype.updateDesignerItem = function (
    componentId, objDescriptor) {
    var alignedPosition;

    if (this.itemIds.indexOf(componentId) !== -1) {
      this.logger.debug("Updating model component with parameters: " +
        objDescriptor);

      //adjust its position to this canvas
      if (objDescriptor.position && _.isNumber(objDescriptor.position.x) && _
        .isNumber(objDescriptor.position.y)) {
        alignedPosition = this._alignPositionToGrid(objDescriptor.position.x,
          objDescriptor.position.y);
  ...
  DiagramDesignerWidgetDesignerItems.prototype.updateDesignerItemSubComponent =
    function (componentId, subComponentId) {
      if (this.itemIds.indexOf(componentId) !== -1) {
        this.logger.debug("Updating model component's [" + componentId +
  ...
  DiagramDesignerWidgetDesignerItems.prototype.deleteDesignerItem = function (
    id) {
    var idx;
  ...
  DiagramDesignerWidgetDesignerItems.prototype.onDesignerItemDoubleClick =
    function (id, event) {
      this.logger.debug("DesignerItem '" + id +
        "' received double click at pos: [" + event.offsetX + ", " + event.offsetY +
        "]");
  };

  DiagramDesignerWidgetDesignerItems.prototype.notifyItemComponentEvents =
    function (itemId, eventList) {
