...
define(['logManager', './AutoRouter', './Profiler'], function (logManager,
  AutoRouter, Profiler) {
  ...
  ConnectionRouteManager3.prototype.initialize = function () {
    ...
    this._onComponentUpdate = function (_canvas, ID) { //Boxes and lines
      if (self.diagramDesigner.itemIds.indexOf(ID) !== -1) {
        if (self.diagramDesigner.items[ID].rotation !== self._autorouterBoxRotation[
          ID]) { //Item has been rotated
          try {
            self._resizeItem(ID);
          } catch (e) {
            self.logger.error(
              'ConnectionRouteManager3.resizeItem failed with error: ' + e);
          }
        }
      }
    };
    this.diagramDesigner.addEventListener(this.diagramDesigner.events.ON_COMPONENT_UPDATE,
      this._onComponentUpdate);

    this._onComponentCreate = function (_canvas, ID) { //Boxes and lines
      if (self.diagramDesigner.itemIds.indexOf(ID) !== -1 && self._autorouterBoxes[
        ID] === undefined) {
        ...
        try {
          ...
          self.logger.error(
            'ConnectionRouteManager3.insertBox failed with error: ' + e);
        }
        ...
        try {
          ...
          self.logger.error(
            'ConnectionRouteManager3.insertConnection failed with error: ' +
            e);
        }
      }
    };
    this.diagramDesigner.addEventListener(this.diagramDesigner.events.ON_COMPONENT_CREATE,
    ...
    this.diagramDesigner.addEventListener(this.diagramDesigner.events.ITEM_SIZE_CHANGED,
    ...
    this._onComponentDelete = function (_canvas, ID) { //Boxes and lines
      try {
        ...
        self.logger.error(
          'ConnectionRouteManager3.deleteItem failed with error: ' + e);
      }
    };
    this.diagramDesigner.addEventListener(this.diagramDesigner.events.ON_COMPONENT_DELETE,
    ...
    this._onItemPositionChanged = function (_canvas, eventArgs) {
      if (self._autorouterBoxes[eventArgs.ID]) {
        ...
        try {
          self.autorouter.move(self._autorouterBoxes[eventArgs.ID].box, {
            "x": x,
            "y": y
          });
        } catch (e) {
          self.logger.error(
            'ConnectionRouteManager3.move failed with error: ' + e);
        }
      }
    };
    ...
    this._onClear = function (_canvas, eventArgs) {
      try {
        ...
        self.logger.error(
          'ConnectionRouteManager3.clearGraph failed with error: ' + e);
      }
    };
    this.diagramDesigner.addEventListener(this.diagramDesigner.events.ON_CLEAR,
      this._onClear);
  ...
  ConnectionRouteManager3.prototype.destroy = function () {
    ...
    this.diagramDesigner.removeEventListener(this.diagramDesigner.events.ITEM_POSITION_CHANGED,
      this._onItemPositionChanged);
    this.diagramDesigner.removeEventListener(this.diagramDesigner.events.ON_CLEAR,
      this._onClear);
  ...
  ConnectionRouteManager3.prototype.redrawConnections = function (idList) {
    ...
    for (var i = 0; i < idList.length; i++) {
      if (this._autorouterPaths[idList[i]]) {
        pathPoints = this.autorouter.getPathPoints(this._autorouterPaths[
          idList[i]]);
      } else {
        pathPoints = [];
      }
      ...
      this.diagramDesigner.items[idList[i]].setConnectionRenderData(
        realPathPoints);
    }
  ...
  ConnectionRouteManager3.prototype.insertConnection = function (connId) {
    var canvas = this.diagramDesigner,
      ...
      sId = srcSubCompId ? srcObjId + DESIGNERITEM_SUBCOMPONENT_SEPARATOR +
      srcSubCompId : srcObjId,
      tId = dstSubCompId ? dstObjId + DESIGNERITEM_SUBCOMPONENT_SEPARATOR +
      dstSubCompId : dstObjId,
      connMetaInfo = canvas.items[connId].getMetaInfo(),
      srcConnAreas = canvas.items[srcObjId].getConnectionAreas(srcSubCompId,
        false, connMetaInfo),
      dstConnAreas = canvas.items[dstObjId].getConnectionAreas(dstSubCompId,
        true, connMetaInfo),
      srcPorts = {},
      dstPorts = {},
      j;
    ...
    while (j--) {
      srcPorts[srcConnAreas[j].id] = this._autorouterBoxes[sId].ports[
        srcConnAreas[j].id];
    ...
    while (j--) {
      dstPorts[dstConnAreas[j].id] = this._autorouterBoxes[tId].ports[
        dstConnAreas[j].id];
      //dstPorts.push(this._autorouterBoxes[tId].ports[dstConnAreas[j].id]);
    }

    //If it has both a src and dst
    if (this._autorouterBoxes[sId].ports.length !== 0 && this._autorouterBoxes[
      tId].ports.length !== 0) {
      this._autorouterPaths[connId] = this.autorouter.addPath({
        "src": srcPorts,
        "dst": dstPorts
      });
    }
  ...
  ConnectionRouteManager3.prototype.insertBox = function (objId) {
    ...
    while (j < areas.length) {
      ...
      boxdefinition.ConnectionInfo.push({
        'id': areas[j].id,
        'area': [
          [areas[j].x1, areas[j].y1],
          [areas[j].x2, areas[j].y2]
        ],
  ...
  ConnectionRouteManager3.prototype.deleteItem = function (objId) {
    ...
    if (itemIds[objId]) {
      ...
      var i = this._autorouterPorts[objId] ? this._autorouterPorts[objId].length :
        0;
      while (i--) {
        var id = objId + DESIGNERITEM_SUBCOMPONENT_SEPARATOR + this._autorouterPorts[
          objId][i]; //ID of child port
  ...
  ConnectionRouteManager3.prototype._resizeItem = function (objId) {
    var canvas = this.diagramDesigner,
      ...
      connAreas = designerItem.getConnectionAreas(objId, isEnd,
        connectionMetaInfo),
      i;
    ...
    while (i--) {
      ...
      newBox.ConnectionInfo.push({
        'id': connAreas[i].id,
        'area': [
          [connAreas[i].x1, connAreas[i].y1],
          [connAreas[i].x2, connAreas[i].y2]
        ],
  ...
  ConnectionRouteManager3.prototype._updatePort = function (objId, subCompId) {
    ...
    if (subCompId !== undefined) { //Updating a port
      ...
      this._autorouterBoxes[objId] = this.autorouter.setConnectionInfo(this._autorouterBoxes[
        objId], connInfo);
    }
  };
  ...
  ConnectionRouteManager3.prototype._createPort = function (objId, subCompId) {
    ...
    this.autorouter.setComponent(this._autorouterBoxes[objId], this._autorouterBoxes[
      longid]);
  ...
  ConnectionRouteManager3.prototype._createPortInfo = function (objId,
    subCompId) {
    ...
    var longid = objId + DESIGNERITEM_SUBCOMPONENT_SEPARATOR + subCompId,
      ...
      areas = canvas.items[objId].getConnectionAreas(subCompId, true,
        connectionMetaInfo) || [],
      j = areas.length,
      newBox = {
        'x1': null,
