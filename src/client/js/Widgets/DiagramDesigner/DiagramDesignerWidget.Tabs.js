...
define(['js/Toolbar/ToolbarButton',
  ...
  var DiagramDesignerWidgetTabs,
    ...
    TAB_LI_BASE = $(
      '<li class=""><a href="#" data-toggle="tab"><div class="tab-title"></div></a></li>'
    ),
  ...
  DiagramDesignerWidgetTabs.prototype._initializeTabs = function () {
    ...
    this.$divTabList = $('<div/>', {
      'class': TAB_LIST_CONTAINER_CLASS
    });
    this.$ulTabTab = $('<ul/>', {
      'class': 'nav nav-tabs'
    });
    ...
    this.$ulTabTab.on("dblclick.editOnDblClick", '.tab-title', function (
      event) {
      if (self.getIsReadOnlyMode() !== true) {
  ...
  DiagramDesignerWidgetTabs.prototype._addTabsButtonEnabled = function (
    enabled) {
    if (this.$btnAddTab) {
  ...
  DiagramDesignerWidgetTabs.prototype._tabsScrollLeft = function () {
    if (this._tabScrollValue < 0) {
      this._scrollTabListBy(Math.min(Math.abs(this._tabScrollValue),
        TAB_SCROLL));
    }
  };
  ...
  DiagramDesignerWidgetTabs.prototype.selectTab = function (tabID) {
    ...
    if (this._selectedTab !== tabID) {
      ...
      if (liToSelect) {
        ...
        this.setBackgroundText(this.$ulTabTab.find('li.active').first().find(
          '.tab-title').text().toUpperCase());
      }
  ...
  DiagramDesignerWidgetTabs.prototype._scrollSelectedTabIntoView = function () {
    ...
    var visibleWidth = this.$tabsContainer.width() - this.$divAddTab.outerWidth(
      true);
  ...
  DiagramDesignerWidgetTabs.prototype._refreshTabTabsScrollOnResize =
    function () {
  ...
  DiagramDesignerWidgetTabs.prototype.onTabTitleChanged = function (tabID,
    oldValue, newValue) {
    this.logger.warning('onTabTitleChanged not implemented: ID: ' + tabID +
      ' "' + oldValue + '" --> "' + newValue + '"');
  };
