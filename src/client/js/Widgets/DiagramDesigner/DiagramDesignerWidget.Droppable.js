...
define(['js/DragDrop/DropTarget',
  ...
  DiagramDesignerWidgetDroppable.prototype._initDroppable = function () {
    ...
    this.skinParts.$dropRegion = $('<div/>', {
      "class": DiagramDesignerWidgetConstants.DROP_REGION_CLASS
    });
  ...
  DiagramDesignerWidgetDroppable.prototype._onDroppableOver = function (event,
    dragInfo) {
    ...
    if (dragInfo) {
      this._doAcceptDroppable(this.onBackgroundDroppableAccept(event,
        dragInfo), true);
    } else {
  ...
  DiagramDesignerWidgetDroppable.prototype._onBackgroundDrop = function (
    event, dragInfo) {
  ...
  DiagramDesignerWidgetDroppable.prototype._doAcceptDroppable = function (
    accept, uiFeedback) {
  ...
  DiagramDesignerWidgetDroppable.prototype.onBackgroundDroppableAccept =
    function (event, dragInfo) {
      this.logger.warning(
        "DiagramDesignerWidget.prototype.onBackgroundDroppableAccept(event, dragInfo) not overridden in controller!!! dragInfo:" +
        JSON.stringify(dragInfo));
      return false;
  };

  DiagramDesignerWidgetDroppable.prototype.onBackgroundDrop = function (event,
    dragInfo, position) {
    this.logger.warning(
      "DiagramDesignerWidget.prototype.onBackgroundDrop(event, dragInfo) not overridden in controller!!! dragInfo:" +
      JSON.stringify(dragInfo) + " , position: '" + JSON.stringify(position) +
      "'");
  };

  DiagramDesignerWidgetDroppable.prototype._enableDroppable = function (
    enabled) {
