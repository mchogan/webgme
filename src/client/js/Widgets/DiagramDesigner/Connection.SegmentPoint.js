...
define(['js/Widgets/DiagramDesigner/DiagramDesignerWidget.Constants',
  ...
  ConnectionSegmentPoint.prototype._render = function () {
    ...
    if (this.isBezier) {
      this.cpLine = this.svgPaper.path("M" + (this.point[0] - this.point[2]) +
        "," + (this.point[1] - this.point[3]) + " L" + (this.point[0] + this.point[
          2]) + "," + (this.point[1] + this.point[3]));
    }
    ...
    if (this.isBezier) {
      this.cpBeforeCircle = this.svgPaper.circle(this.point[0] - this.point[2],
        this.point[1] - this.point[3], this.width -
        BEZIER_CONTROL_POINT_WIDTH_DIFF);
      this.cpAfterCircle = this.svgPaper.circle(this.point[0] + this.point[2],
        this.point[1] + this.point[3], this.width -
        BEZIER_CONTROL_POINT_WIDTH_DIFF);
  ...
  ConnectionSegmentPoint.prototype._startSegmentPointMove = function (
    startPos) {
  ...
  ConnectionSegmentPoint.prototype._onMouseMove = function (event) {
    ...
    if (this._moving === true) {
      ...
      if (this.isBezier) {
        this.cpLine.attr({
          'path': "M" + (this.point[0] - this.point[2]) + "," + (this.point[
            1] - this.point[3]) + " L" + (this.point[0] + this.point[2]) +
  ...
  ConnectionSegmentPoint.prototype._onMouseUp = function (event) {
    ...
    if (this._moving === true) {
      if (this._moveType === MOVE_TYPE_SEGMENT_POINT) {
        point = this._snapCoordinate([mousePos.mX, mousePos.mY]);
        this.connection.setSegmentPoint(this.id, point[0], point[1], this.point[
          2], this.point[3]);
      } else if (this._moveType === MOVE_TYPE_BEZIER_CONTROL_POINT) {
        this.connection.setSegmentPoint(this.id, this.point[0], this.point[1],
  ...
  ConnectionSegmentPoint.prototype._startBezierControlPointMove = function (
    startPos, isPointBefore) {
    this._startPos = startPos;
  ...
  ConnectionSegmentPoint.prototype._attachBezierControlPointMouseListeners =
    function () {
      var self = this;
  ...
  ConnectionSegmentPoint.prototype._onBezierControlPointMouseMove = function (
    event) {
    var mousePos = this._getMousePos(event),
      dx = mousePos.mX - this._startPos.mX,
      dy = mousePos.mY - this._startPos.mY;
    ...
    if (this._moving === true) {
      ...
      this.cpLine.attr({
        'path': "M" + (this.point[0] - this.point[2]) + "," + (this.point[1] -
          this.point[3]) + " L" + (this.point[0] + this.point[2]) + "," + (
          this.point[1] + this.point[3])
      });
  ...
  ConnectionSegmentPoint.prototype._redrawMovePath = function () {
    ...
    if (this.isBezier) {
      ...
      pathDef.push("C" + (this.pointBefore[0] + this.pointBefore[2]) + "," +
        (this.pointBefore[1] + this.pointBefore[3]) + " " + (this.point[0] -
          this.point[2]) + "," + (this.point[1] - this.point[3]) + " " + this
        .point[0] + "," + this.point[1]);
      pathDef.push("C" + (this.point[0] + this.point[2]) + "," + (this.point[
          1] + this.point[3]) + " " + (this.pointAfter[0] - this.pointAfter[2]) +
        "," + (this.pointAfter[1] - this.pointAfter[3]) + " " + this.pointAfter[
          0] + "," + this.pointAfter[1]);
    } else {
      pathDef.push("M" + this.pointBefore[0] + "," + this.pointBefore[1]);
    ...
    if (this._movePath) {
      ...
      this._movePath.node.setAttribute('class',
        DiagramDesignerWidgetConstants.SEGMENT_POINT_MOVE_PATH_CLASS);

      //insert behind the segment-points
      this._movePath.node.parentNode.insertBefore(this._movePath.node, $(this
          .svgPaper.canvas).find('circle.' + DiagramDesignerWidgetConstants.CONNECTION_SEGMENT_POINT_CLASS)
        .first()[0]);
    }
  };
