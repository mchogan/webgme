...
define(['logManager',
  ...
  var AutoRouterPort = function () {
    ...
    this.points = [
      ...
      [],
      []
    ]; //For this.points on CONSTANTS.Dir_Top, CONSTANTS.Dir_Left, CONSTANTS.Dir_Right, etc
    this.selfPoints = [];
  ...
  AutoRouterPort.prototype.shiftBy = function (offset) {
    assert(!this.rect.isRectEmpty(),
      "ARPort.shiftBy: !this.rect.isRectEmpty() FAILED!");
  ...
  AutoRouterPort.prototype.canHaveStartEndPointOn = function (dir, isStart) {
    assert(0 <= dir && dir <= 3,
      "ARPort.canHaveStartEndPointOn: 0 <= dir && dir <= 3 FAILED!");
  ...
  AutoRouterPort.prototype.canHaveStartEndPoint = function (isStart) {
    return ((this.attributes & (isStart ? CONSTANTS.ARPORT_StartOnAll :
      CONSTANTS.ARPORT_EndOnAll)) !== 0);
  };

  AutoRouterPort.prototype.canHaveStartEndPointHorizontal = function (
    isHorizontal) {
    return ((this.attributes & (isHorizontal ? CONSTANTS.ARPORT_StartEndHorizontal :
      CONSTANTS.ARPORT_StartEndVertical)) !== 0);
  };

  AutoRouterPort.prototype.getStartEndDirTo = function (point, isStart,
    notthis) {
    assert(!this.rect.isRectEmpty(),
      "ARPort.getStartEndDirTo: !this.rect.isRectEmpty() FAILED!");
  ...
  AutoRouterPort.prototype.canCreateStartEndPointAt = function (point,
    isStart, nearness) {
    return this.canHaveStartEndPoint(isStart) && UTILS.isPointIn(point, this.rect,
      nearness);
  };

  AutoRouterPort.prototype.createStartEndPointAt = function (pt, isStart) {
    assert(!this.rect.isRectEmpty(),
      "ARPort.createStartEndPointAt: !this.rect.isRectEmpty() FAILED!");
    ...
    if (this.canHaveStartEndPointOn(CONSTANTS.Dir_Top, isStart) && nearest.HLine(
      this.rect.left, this.rect.right, this.rect.ceil)) {
      dir = CONSTANTS.Dir_Top;
    }

    if (this.canHaveStartEndPointOn(CONSTANTS.Dir_Right, isStart) && nearest.VLine(
      this.rect.ceil, this.rect.floor, this.rect.right)) {
      dir = CONSTANTS.Dir_Right;
    }

    if (this.canHaveStartEndPointOn(CONSTANTS.Dir_Bottom, isStart) && nearest
      .HLine(this.rect.left, this.rect.right, this.rect.floor)) {
      dir = CONSTANTS.Dir_Bottom;
    }

    if (this.canHaveStartEndPointOn(CONSTANTS.Dir_Left, isStart) && nearest.VLine(
      this.rect.ceil, this.rect.floor, this.rect.left)) {
      dir = CONSTANTS.Dir_Left;
    }

    assert(UTILS.isRightAngle(dir),
      "ArPort.createStartEndPointAt: UTILS.isRightAngle (dir) FAILED!");
  ...
  AutoRouterPort.prototype.createStartEndPointOn = function (dir) {
    ...
    assert(!this.rect.isRectEmpty(),
      "ARPort.createStartEndPointOn: !this.rect.isRectEmpty() FAILED!");
    assert(UTILS.isRightAngle(dir),
      "ARPort.createStartEndPointOn: UTILS.isRightAngle (dir) FAILED!");

    switch (dir) {

    case CONSTANTS.Dir_Top:
      return new ArPoint(this.roundToHalfGrid(this.rect.left, this.rect.right),
        this.rect.ceil);
    ...
    case CONSTANTS.Dir_Left:
      return new ArPoint(this.rect.left, this.roundToHalfGrid(this.rect.ceil,
        this.rect.floor));
    }

    return new ArPoint(this.rect.right, this.roundToHalfGrid(this.rect.ceil,
      this.rect.floor));
  };
  ...
  AutoRouterPort.prototype.createStartEndPointTo = function (point, dir) {
    ...
    resultPoint = new ArPoint((largerPt.x + smallerPt.x) / 2, (largerPt.y +
      smallerPt.y) / 2);
    ...
    if (closestArea !== -1 && this.isAvailable()) { //resultPoint needs to be moved to the closest available area
      var dir2 = UTILS.getDir(this.availableArea[closestArea][0].minus(
        resultPoint));
  ...
  AutoRouterPort.prototype.removePoint = function (pt) {
    ...
    if (!removed) {
      _logger.warning("point (" + pt.x + ", " + pt.y +
        ") was not removed from port");
    }
  };
  ...
  AutoRouterPort.prototype.resetAvailableArea = function () {
    ...
    if (this.canHaveStartEndPointOn(CONSTANTS.Dir_Top)) {
      this.availableArea.push([this.rect.getTopLeft(), new ArPoint(this.rect.right,
        this.rect.ceil)]);
    }

    if (this.canHaveStartEndPointOn(CONSTANTS.Dir_Right)) {
      this.availableArea.push([new ArPoint(this.rect.right, this.rect.ceil),
        this.rect.getBottomRight()
      ]);
    }

    if (this.canHaveStartEndPointOn(CONSTANTS.Dir_Bottom)) {
      this.availableArea.push([new ArPoint(this.rect.left, this.rect.floor),
        this.rect.getBottomRight()
      ]);
    }

    if (this.canHaveStartEndPointOn(CONSTANTS.Dir_Left)) {
      this.availableArea.push([this.rect.getTopLeft(), new ArPoint(this.rect.left,
        this.rect.floor)]);
    }
  ...
  AutoRouterPort.prototype.adjustAvailableArea = function (r) {
    ...
    while (i--) {
      ...
      if (UTILS.isLineClipRect(this.availableArea[i][0], this.availableArea[i]
        [1], r)) {
