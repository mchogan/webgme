...
define(['logManager',
  ...
  AutoRouterPort.prototype.canHaveStartEndPointHorizontal = function (isHorizontal) {
    return ((this.attributes & (isHorizontal ? CONSTANTS.ARPORT_StartEndHorizontal : CONSTANTS.ARPORT_StartEndVertical)) !==
      0);
  };
  ...
  AutoRouterPort.prototype.createStartEndPointAt = function (pt, isStart) {
    ...
    if (this.canHaveStartEndPointOn(CONSTANTS.Dir_Top, isStart) && nearest.HLine(this.rect.left, this.rect.right,
      this.rect.ceil)) {
      dir = CONSTANTS.Dir_Top;
    }

    if (this.canHaveStartEndPointOn(CONSTANTS.Dir_Right, isStart) && nearest.VLine(this.rect.ceil, this.rect.floor,
      this.rect.right)) {
      dir = CONSTANTS.Dir_Right;
    }

    if (this.canHaveStartEndPointOn(CONSTANTS.Dir_Bottom, isStart) && nearest.HLine(this.rect.left, this.rect.right,
      this.rect.floor)) {
      dir = CONSTANTS.Dir_Bottom;
    }

    if (this.canHaveStartEndPointOn(CONSTANTS.Dir_Left, isStart) && nearest.VLine(this.rect.ceil, this.rect.floor,
      this.rect.left)) {
