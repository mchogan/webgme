...
define(['logManager',
  ...
  HighlightManager = function (options) {
    ...
    if (this._diagramDesigner === undefined || this._diagramDesigner === null) {
      this.logger.error(
        "Trying to initialize a HighlightManager without a diagramDesigner..."
      );
  ...
  HighlightManager.prototype._highLight = function (id, highlightAssociated) {
    ...
    this.logger.debug('_highLight, ID: "' + id + '", highlightAssociated: ' +
      highlightAssociated);
    ...
    if (idx === -1) {
      ...
      if (highlightAssociated) {
        ...
        if (this._diagramDesigner.itemIds.indexOf(id) !== -1) {
          ...
          while (i--) {
            elementsToHighlight = _.union(elementsToHighlight, this._diagramDesigner
              ._getItemsForConnection(associatedIDs[i]));
          }
