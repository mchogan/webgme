...
define(['logManager',
  ...
  function (logManager, assert, CONSTANTS, UTILS, ArPoint, ArRect,
    ArPointListPath) {
    ...
    AutoRouterPath.prototype.calculateStartEndPorts = function () {
      return {
        'src': this.calculateStartPorts(),
        'dst': this.calculateEndPorts()
      };
    };
    ...
    AutoRouterPath.prototype.calculateStartPorts = function () {
      ...
      assert(this.startports.length > 0,
        "ArPath.calculateStartEndPorts: this.startports cannot be empty!");
      ...
      //Get available ports
      while (i--) {
        assert(this.startports[i].getOwner(),
          "ARPath.calculateStartEndPorts: this.startport has invalid this.owner!"
        );
      ...
      if (this.customPathData.length) {
        tgt = new ArPoint(this.customPathData[0].getX(), this.customPathData[0]
          .getY());
      } else {
        var x = 0,
          y = 0;
    ...
    AutoRouterPath.prototype.calculateEndPorts = function () {
      ...
      assert(this.endports.length > 0,
        "ArPath.calculateStartEndPorts: this.endports cannot be empty!");
      ...
      //Get available ports
      while (i--) {
        assert(this.endports[i].getOwner(),
          "ARPath.calculateStartEndPorts: this.endport has invalid this.owner!"
        );
      ...
      if (this.customPathData.length) {
        ...
        tgt = new ArPoint(this.customPathData[i].getX(), this.customPathData[i]
          .getY());
      } else {
        var x = 0,
          y = 0;
      ...
      if (enddir === CONSTANTS.Dir_None || // like above
        ...
        enddir = this.endport.getStartEndDirTo(tgt, false, this.startport ===
          this.endport ? startdir : CONSTANTS.Dir_None);
      }
    ...
    AutoRouterPath.prototype.getStartPoint = function () {
      ...
      if (this.points.getLength()) {
        assert(this.startpoint === this.points.get(0)[0],
          "ARPath.getEndPoint: this.startpoint === this.points.get(0)[0] FAILED"
        );
      }
    ...
    AutoRouterPath.prototype.getEndPoint = function () {
      if (this.points.getLength()) {
        assert(this.endpoint === this.points.get(this.points.getLength() - 1)[0],
          "ARPath.getEndPoint: this.endpoint === this.points.get(this.points.getLength() - 1)[0] FAILED"
        );
      }
    ...
    AutoRouterPath.prototype.getOutOfBoxStartPoint = function (hintDir) {
      ...
      assert(hintDir !== CONSTANTS.Dir_Skew,
        "ARPath.getOutOfBoxStartPoint: hintDir !== CONSTANTS.Dir_Skew FAILED");
      assert(this.points.getLength() >= 2,
        "ARPath.getOutOfBoxStartPoint: this.points.getLength() >= 2 FAILED");
      ...
      assert(UTILS.isRightAngle(d),
        "ARPath.getOutOfBoxStartPoint: UTILS.isRightAngle (d) FAILED");
    ...
    AutoRouterPath.prototype.getOutOfBoxEndPoint = function (hintDir) {
      ...
      assert(hintDir !== CONSTANTS.Dir_Skew,
        "ARPath.getOutOfBoxEndPoint: hintDir !== CONSTANTS.Dir_Skew FAILED");
      assert(this.points.getLength() >= 2,
        "ARPath.getOutOfBoxEndPoint: this.points.getLength() >= 2 FAILED");
      ...
      assert(UTILS.isRightAngle(d),
        "ARPath.getOutOfBoxEndPoint: UTILS.isRightAngle (d) FAILED");
    ...
    AutoRouterPath.prototype.simplifyTrivially = function () {
      assert(!this.isConnected(),
        "ARPath.simplifyTrivially: !isConnected() FAILED");
      ...
      assert(pos1 !== this.points.getLength(),
        "ARPath.simplifyTrivially: pos1 !== this.points.getLength() FAILED");
      var p1 = this.points.get(pos++)[0],
        pos2 = pos;

      assert(pos2 !== this.points.getLength(),
        "ARPath.simplifyTrivially: pos2 !== this.points.getLength() FAILED");
      var p2 = this.points.get(pos++)[0],
        dir12 = UTILS.getDir(p2.minus(p1)),
        pos3 = pos;

      assert(pos3 !== this.points.getLength(),
        "ARPath.simplifyTrivially: pos3 !== this.points.getLength() FAILED");
    ...
    AutoRouterPath.prototype.getSurroundRect = function () {
      var rect = new ArRect(CONSTANTS.INT_MAX, CONSTANTS.INT_MAX, CONSTANTS.INT_MIN,
          CONSTANTS.INT_MIN),
        pos = 0,
        point;
    ...
    AutoRouterPath.prototype.setEndDir = function (arpath_end) {
      this.attributes = (this.attributes & ~CONSTANTS.ARPATH_EndMask) +
        arpath_end;
    };

    AutoRouterPath.prototype.setStartDir = function (arpath_start) {
      this.attributes = (this.attributes & ~CONSTANTS.ARPATH_StartMask) +
        arpath_start;
    };
    ...
    AutoRouterPath.prototype.applyCustomizationsBeforeAutoConnectPoints =
      function () {
        var plist = [];
    ...
    AutoRouterPath.prototype.applyCustomizationsAfterAutoConnectPointsAndStuff =
      function () {
                ...
                if ((this.customPathData[ii]).isHorizontalOrVertical() ===
                  ...
                  if (currEdgeIndex === 0 || currEdgeIndex === numEdges - 1) {
                    ...
                    minLimit = (currEdgeIndex === 0 ?
                      ((this.customPathData[ii]).IsHorizontalOrVertical() ?
                        startRect.ceil : startRect.left) :
                      ((this.customPathData[ii]).IsHorizontalOrVertical() ?
                        endRect.ceil : endRect.left));
                    maxLimit = (currEdgeIndex === 0 ?
                      ((this.customPathData[ii]).IsHorizontalOrVertical() ?
                        startRect.floor : startRect.right) :
                      ((this.customPathData[ii]).IsHorizontalOrVertical() ?
                        endRect.floor : endRect.right));
                    valueToSet = (this.customPathData[ii]).IsHorizontalOrVertical() ?
                      yToSet : xToSet;
    ...
    AutoRouterPath.prototype.getCustomizedEdgeIndexes = function (indexes) {
      ...
      while (ii < this.customPathData.length) {
        if (this.isAutoRouted() && (this.customPathData[ii]).getType() ===
          CONSTANTS.SimpleEdgeDisplacement ||
          !this.isAutoRouted() && (this.customPathData[ii]).getType() !==
          CONSTANTS.SimpleEdgeDisplacement) {
    ...
    AutoRouterPath.prototype.assertValid = function () {
      ...
      if (this.isConnected()) {
        assert(this.points.getLength() !== 0,
          "ARPath.assertValid: this.points.getLength() !== 0 FAILED");
        var i = 0,
          point,
          ppoint;
