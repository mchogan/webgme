...
define(['logManager',
  ...
  function (logManager, assert, CONSTANTS, UTILS, ArPoint, ArRect,
    AutoRouterGraph, AutoRouterBox, AutoRouterPort,
    AutoRouterPath,
    ArPathMap,
    CustomPathData) {
    ...
    AutoRouter.prototype.addBox = function (size) {
      ...
      if (!((size.x1 !== undefined && size.x2 !== undefined) || (size.width !==
          undefined && (size.x1 !== undefined || size.x2 !== undefined))) ||
        !((size.y1 !== undefined && size.y2 !== undefined) || (size.height !==
          undefined && (size.y1 !== undefined || size.y2 !== undefined)))) {
        throw "AutoRouter:addBox missing required size details to determine x1,x2,y1,y2 (" +
          x1 + "," + x2 + "," + y1 + "," + y2 + ")";
      }
    ...
    AutoRouter.prototype._processConnArea = function (connData, x1, y1, x2, y2,
      box, p) {
      var id = connData.id,
        ...
        j = 0,
        port = box.createPort(),
        connArea = connData.area instanceof Array ? [connData.area] : //Line
        [connData.any, connData.in || connData.incoming, connData.out ||
          connData.outgoing
        ];
      ...
      do {
        ...
        if (connArea[j] instanceof Array) {
          ...
          } else if (connArea[j].length === 2 && connArea[j][0][0] !== connArea[
            j][1][0] && connArea[j][0][1] !== connArea[j][1][1]) {
          ...
          assert(x1 < arx1 && y1 < ary1 && x2 > arx2 && y2 > ary2,
            "AutoRouter.addBox Cannot add port outside of the box");
        ...
        } else if (typeof connArea[j] === "string") //Using words to designate connection area
        {
          ...
          attr = (connArea[j].indexOf("top") !== -1 ?
            ...
            ((j % 2 === 0 ? CONSTANTS.ARPORT_StartOnTop : 0) + (j < 2 ?
              CONSTANTS.ARPORT_EndOnTop : 0)) : 0) +
          //Connection area is on bottom
          (connArea[j].indexOf("bottom") !== -1 ?
            ((j % 2 === 0 ? CONSTANTS.ARPORT_StartOnBottom : 0) + (j < 2 ?
              CONSTANTS.ARPORT_EndOnBottom : 0)) : 0) +
          //Connection area is on left
          (connArea[j].indexOf("left") !== -1 ?
            ((j % 2 === 0 ? CONSTANTS.ARPORT_StartOnLeft : 0) + (j < 2 ?
              CONSTANTS.ARPORT_EndOnLeft : 0)) : 0) +
          //Connection area is on right
          (connArea[j].indexOf("right") !== -1 ?
            ((j % 2 === 0 ? CONSTANTS.ARPORT_StartOnRight : 0) + (j < 2 ?
              CONSTANTS.ARPORT_EndOnRight : 0)) : 0) ||
            (connArea[j].indexOf("all") !== -1 ? CONSTANTS.ARPORT_ConnectOnAll :
            0);
    ...
    AutoRouter.prototype._createPath = function (a) {
      ...
      assert(src instanceof AutoRouterPort || src instanceof Array || src.ports[
          0] instanceof AutoRouterPort,
        "AutoRouter:_createPath: src is not recognized as an AutoRouterPort");
      assert(dst instanceof AutoRouterPort || dst instanceof Array || dst.ports[
          0] instanceof AutoRouterPort,
        "AutoRouter:_createPath: dst is not recognized as an AutoRouterPort");
      path = this.router.addPath(autoroute, src, dst);

      if (startDir || endDir) {
        var start = startDir !== undefined ? (startDir.indexOf("top") !== -1 ?
            CONSTANTS.ARPATH_StartOnTop : 0) +
          (startDir.indexOf("bottom") !== -1 ? CONSTANTS.ARPATH_StartOnBottom :
            0) +
          (startDir.indexOf("left") !== -1 ? CONSTANTS.ARPATH_StartOnLeft : 0) +
          (startDir.indexOf("right") !== -1 ? CONSTANTS.ARPATH_StartOnRight : 0) ||
          (startDir.indexOf("all") !== -1 ? CONSTANTS.ARPATH_Default : 0) :
          CONSTANTS.ARPATH_Default;
        var end = endDir !== undefined ? (endDir.indexOf("top") !== -1 ?
            CONSTANTS.ARPATH_EndOnTop : 0) +
          ...
          (endDir.indexOf("all") !== -1 ? CONSTANTS.ARPATH_Default : 0) :
          CONSTANTS.ARPATH_Default;
    ...
    AutoRouter.prototype.setBox = function (boxObject, size) {
      var box = boxObject.box,
        ...
        rect = new ArRect(x1, y1, x2, y2),
        paths = {
          "in": this.boxId2Path[box.getID()].in,
          "out": this.boxId2Path[box.getID()].out
        },
        i = paths.in.length,
        pathSrc,
        pathDst,
    ...
    AutoRouter.prototype.remove = function (item) {
      assert(item !== undefined,
        "AutoRouter:remove Cannot remove undefined object");
      item = item.box || item;
    ...
    AutoRouter.prototype.move = function (box, details) {
      ...
      var dx = details.dx !== undefined ? details.dx : Math.round(details.x -
          box.getRect().left),
        dy = details.dy !== undefined ? details.dy : Math.round(details.y - box
          .getRect().ceil);

      assert(box instanceof AutoRouterBox,
        "AutoRouter:move First argument must be an AutoRouterBox or ArBoxObject"
      );

      this.router.shiftBoxBy(box, {
        "cx": dx,
        "cy": dy
      });
    };
