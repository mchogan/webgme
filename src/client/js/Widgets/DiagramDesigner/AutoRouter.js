...
define(['logManager',
  ...
  function (logManager, assert, CONSTANTS, UTILS, ArPoint, ArRect, AutoRouterGraph, AutoRouterBox, AutoRouterPort,
    ...
    AutoRouter.prototype.addBox = function (size) {
      ...
      if (!((size.x1 !== undefined && size.x2 !== undefined) || (size.width !== undefined && (size.x1 !== undefined ||
          size.x2 !== undefined))) ||
        !((size.y1 !== undefined && size.y2 !== undefined) || (size.height !== undefined && (size.y1 !== undefined ||
          size.y2 !== undefined)))) {
        throw 'AutoRouter:addBox missing required size details to determine x1,x2,y1,y2 (' + x1 + ',' + x2 + ',' + y1 +
          ',' + y2 + ')';
      }
    ...
    AutoRouter.prototype._processConnArea = function (connData, x1, y1, x2, y2, box, p) {
      ...
      do {
        ...
        if (connArea[j] instanceof Array) {
          ...
          } else if (connArea[j].length === 2 && connArea[j][0][0] !== connArea[j][1][0] && connArea[j][0][1] !==
            connArea[j][1][1]) {
