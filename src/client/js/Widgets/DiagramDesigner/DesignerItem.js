...
define(['logManager',
  ...
  DesignerItem.prototype.__setDecorator = function (decoratorName,
    decoratorClass, control, metaInfo, preferencesHelper, aspect,
    decoratorParams) {
  ...
  DesignerItem.prototype.$_DOMBase = $('<div/>').attr({
    "class": DiagramDesignerWidgetConstants.DESIGNER_ITEM_CLASS
  });
        ...
        this.$el.on(i + '.' + EVENT_POSTFIX, null, null, function (event) {
          ...
          if (eventHandlerOpts) {
            ...
            if (enabled) {
              ...
              handled = self._callDecoratorMethod(eventHandlerOpts.fn,
                event);
  ...
  DesignerItem.prototype.addToDocFragment = function (docFragment) {
    ...
    this.logger.debug("DesignerItem with id:'" + this.id +
      "' added to canvas.");
  };
  ...
  DesignerItem.prototype._callDecoratorMethod = function (fnName, args) {
    ...
    if (this._decoratorInstance) {
      if (_.isFunction(this._decoratorInstance[fnName])) {
        result = this._decoratorInstance[fnName].apply(this._decoratorInstance,
          args);
      } else {
        this.logger.warning("DecoratorInstance '" + $.type(this._decoratorInstance) +
  ...
  DesignerItem.prototype.update = function (objDescriptor) {
    ...
    //update position
    if (objDescriptor.position && _.isNumber(objDescriptor.position.x) && _.isNumber(
      objDescriptor.position.y)) {
    ...
    if (objDescriptor.decoratorClass && this._decoratorID !== objDescriptor.decoratorClass
      ...
      this.__setDecorator(objDescriptor.decorator, objDescriptor.decoratorClass,
        oldControl, oldMetaInfo, objDescriptor.preferencesHelper,
        objDescriptor.aspect, objDescriptor.decoratorParams);

      //attach new one
      this.$el.html(this._decoratorInstance.$el);

      this.logger.debug("DesignerItem's ['" + this.id +
  ...
  DesignerItem.prototype.getConnectionAreas = function (id, isEnd,
    connectionMetaInfo) {
    var result = [],
      areas = this._decoratorInstance.getConnectionAreas(id, isEnd,
        connectionMetaInfo),
  ...
  DesignerItem.prototype.registerSubcomponent = function (subComponentId,
    metaInfo) {
    this.logger.debug("registerSubcomponent - ID: '" + this.id +
  ...
  DesignerItem.prototype.unregisterSubcomponent = function (subComponentId) {
    this.logger.debug("unregisterSubcomponent - ID: '" + this.id +
      "', SubComponentID: '" + subComponentId + "'");
  ...
  DesignerItem.prototype.registerConnectors = function (el, subComponentId) {
    ...
    if (subComponentId !== undefined && subComponentId !== null) {
      el.attr(DiagramDesignerWidgetConstants.DATA_SUBCOMPONENT_ID,
        subComponentId);
    }
  };
  ...
  DesignerItem.prototype._rotatePoint = function (x, y) {
    var fixedRotation = (this.rotation < 0 ? 360 + this.rotation : this.rotation) %
      360;
