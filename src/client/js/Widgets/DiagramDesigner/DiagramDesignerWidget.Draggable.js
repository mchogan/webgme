...
define(['js/DragDrop/DragSource',
  ...
  var DiagramDesignerWidgetDraggable,
    ...
    DRAG_HELPER_EL_BASE = $('<div/>', {
      'class': DRAG_HELPER_CLASS
    }),
  ...
  DiagramDesignerWidgetDraggable.prototype._makeDraggable = function (item) {
    ...
    dragSource.makeDraggable(item.$el, {
      ...
      'dragParams': function (el, event) {
        return self.getDragParams(self.selectionManager.getSelectedElements(),
          event);
      },
      'start': function (event) {
        ...
        if (self.mode === self.OPERATING_MODES.DESIGN) {
          ...
          if (event.originalEvent.target instanceof SVGElement) {
            ...
            if (classDef) {
              ret = classDef.split(' ').indexOf(
                DiagramDesignerWidgetConstants.CONNECTOR_CLASS) === -1;
            } else {
              ret = true;
            }
          } else {
            ret = !$(event.originalEvent.target).hasClass(
  ...
  DiagramDesignerWidgetDraggable.prototype._dragHelper = function (el, event,
    ...
    if (selectionBBox) {
      helperEl.css({
        ...
        'line-height': ((selectionBBox.y2 - selectionBBox.y) * this._zoomRatio) +
          "px",
        'text-align': 'center',
        'border': '2px dashed #666',
        'background-color': 'rgba(100, 100, 100, 0.1)',
        'margin-top': (selectionBBox.y - mousePos.mY + dragSource.DEFAULT_CURSOR_AT
          .top) * this._zoomRatio,
        'margin-left': (selectionBBox.x - mousePos.mX + dragSource.DEFAULT_CURSOR_AT
          .left) * this._zoomRatio
      });
  ...
  DiagramDesignerWidgetDraggable.prototype.getDragItems = function (
    selectedElements) {
    this.logger.warning(
      "DiagramDesignerWidgetDraggable.getDragItems is not overridden in the controller!!! selectedElements: " +
      selectedElements);
    return [];
  };

  DiagramDesignerWidgetDraggable.prototype.getDragEffects = function (
  ...
  DiagramDesignerWidgetDraggable.prototype.getDragParams = function (
    selectedElements, event) {
    var params = {
        'positions': {}
      },
