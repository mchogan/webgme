...
define(['logManager'], function (logManager) {
  ...
  ConnectionRouteManager2 = function (options) {
    this.logger = (options && options.logger) || logManager.create(((options && options.loggerName) ||
  ...
  ConnectionRouteManager2.prototype._getEndpointConnectionAreas = function (connId, objId, subCompId, isEnd,
    ...
    if (subCompId === undefined ||
      (subCompId !== undefined && this.diagramDesigner._itemSubcomponentsMap[objId] && this.diagramDesigner._itemSubcomponentsMap[
        objId].indexOf(subCompId) !== -1)) {
  ...
  ConnectionRouteManager2.prototype._calculateClosestEndpoints = function (connectionId) {
    ...
    if (sourceConnectionPoints.length > 0 && targetConnectionPoints.length > 0) {
      ...
      this.connectionEndPoints[connectionId] = {
        'source': [sId, sourceCoordinates.id, this.endpointConnectionAreaConnectionInfo[sEId][sourceCoordinates.id]],
  ...
  ConnectionRouteManager2.prototype._updateConnectionCoordinates = function (connectionId) {
    var canvas = this.diagramDesigner,
      ...
      tEId = this._getEndPointID(undefined, dstObjId, dstSubCompId),
      segmentPoints = canvas.items[connectionId].segmentPoints,
      sourceConnectionPoint = this.connectionEndPoints[connectionId] ? this.connectionEndPoints[connectionId].source :
      undefined,
      targetConnectionPoint = this.connectionEndPoints[connectionId] ? this.connectionEndPoints[connectionId].target :
      undefined,
      sourceCoordinates = null,
      targetCoordinates = null,
    ...
    if (sourceConnectionPoint && targetConnectionPoint) {
      ...
      if (sourceCoordinates.len !== 0) {
        connectorDelta = this._getConnectorDelta(sourceCoordinates, this.endpointConnectionAreaConnectionInfo[sEId][
          sourceCoordinates.id
        ] + 1, sourceConnectionPoint[2]);
        connectionPathPoints.push({
          'x': sourceCoordinates.x + slicex * sourceConnectionPoint[2] + connectorDelta.dx + connectorDelta.dx *
            connExtender * (sourceConnectionPoint[2] - 1),
          'y': sourceCoordinates.y + slicey * sourceConnectionPoint[2] + connectorDelta.dy + connectorDelta.dy *
            connExtender * (sourceConnectionPoint[2] - 1)
        });
      }
      ...
      if (targetCoordinates.len !== 0) {
        connectorDelta = this._getConnectorDelta(targetCoordinates, this.endpointConnectionAreaConnectionInfo[tEId][
          targetCoordinates.id
        ] + 1, targetConnectionPoint[2]);
        connectionPathPoints.push({
          'x': targetCoordinates.x + slicex * targetConnectionPoint[2] + connectorDelta.dx + connectorDelta.dx *
            connExtender * (targetConnectionPoint[2] - 1),
          'y': targetCoordinates.y + slicey * targetConnectionPoint[2] + connectorDelta.dy + connectorDelta.dy *
            connExtender * (targetConnectionPoint[2] - 1)
        });
      }
