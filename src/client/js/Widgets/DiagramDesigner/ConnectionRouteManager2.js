...
define(['logManager'], function (logManager) {
  ...
  ConnectionRouteManager2 = function (options) {
    this.logger = (options && options.logger) || logManager.create(((options &&
      options.loggerName) || "ConnectionRouteManagerBasic"));

    this.diagramDesigner = options ? options.diagramDesigner : null;

    if (this.diagramDesigner === undefined || this.diagramDesigner === null) {
      this.logger.error(
        "Trying to initialize a ConnectionRouteManagerBasic without a canvas..."
      );
  ...
  ConnectionRouteManager2.prototype._updateEndpointInfo = function (idList) {
    ...
    while (i--) {
      ...
      if (!this._getEndpointConnectionAreas(connId, srcObjId, srcSubCompId,
        false, connectionMetaInfo)) {
        if (canvas.connectionIds.indexOf(srcObjId) !== -1) {
          dependantNotReadyYet.push(connId);
        }

      }
      if (!this._getEndpointConnectionAreas(connId, dstObjId, dstSubCompId,
        true, connectionMetaInfo)) {
  ...
  ConnectionRouteManager2.prototype._getEndPointID = function (connId, objId,
    subCompId) {
    var res = "";
  ...
  ConnectionRouteManager2.prototype._getEndpointConnectionAreas = function (
    connId, objId, subCompId, isEnd, connectionMetaInfo) {
    ...
    this.endpointConnectionAreaInfo[longid] = this.endpointConnectionAreaInfo[
      longid] || [];
    this.endpointConnectionAreaConnectionInfo[eID] = this.endpointConnectionAreaConnectionInfo[
      eID] || {};

    if (subCompId === undefined ||
      (subCompId !== undefined && this.diagramDesigner._itemSubcomponentsMap[
        objId] && this.diagramDesigner._itemSubcomponentsMap[objId].indexOf(
        subCompId) !== -1)) {
      ...
      while (j--) {
        ...
        if (!this.endpointConnectionAreaConnectionInfo[eID].hasOwnProperty(
          res[j].id)) {
  ...
  ConnectionRouteManager2.prototype._calculateClosestEndpoints = function (
    connectionId) {
    ...
    if (sourceConnectionPoints.length > 0 && targetConnectionPoints.length >
      0) {
      if (srcObjId === dstObjId && srcSubCompId === dstSubCompId) {
        ...
        closestConnPoints = this._getClosestPoints(sourceConnectionPoints,
          targetConnectionPoints, segmentPoints);
      ...
      this.connectionEndPoints[connectionId] = {
        "source": [sId, sourceCoordinates.id, this.endpointConnectionAreaConnectionInfo[
          sEId][sourceCoordinates.id]],
        "target": [tId, targetCoordinates.id, this.endpointConnectionAreaConnectionInfo[
          tEId][targetCoordinates.id]]
      };
    }
  };

  ConnectionRouteManager2.prototype._updateConnectionCoordinates = function (
    connectionId) {
    var canvas = this.diagramDesigner,
      srcObjId = canvas.connectionEndIDs[connectionId].srcObjId,
      srcSubCompId = canvas.connectionEndIDs[connectionId].srcSubCompId,
      ...
      tEId = this._getEndPointID(undefined, dstObjId, dstSubCompId),
      segmentPoints = canvas.items[connectionId].segmentPoints,
      sourceConnectionPoint = this.connectionEndPoints[connectionId] ? this.connectionEndPoints[
        connectionId].source : undefined,
      targetConnectionPoint = this.connectionEndPoints[connectionId] ? this.connectionEndPoints[
        connectionId].target : undefined,
      sourceCoordinates = null,
      targetCoordinates = null,
    ...
    if (sourceConnectionPoint && targetConnectionPoint) {
      sourceCoordinates = this.endpointConnectionAreaInfo[
        sourceConnectionPoint[0]][sourceConnectionPoint[1]];
      targetCoordinates = this.endpointConnectionAreaInfo[
        targetConnectionPoint[0]][targetConnectionPoint[1]];

      /****************startpoint ***************/
      slicex = sourceCoordinates.w / (this.endpointConnectionAreaConnectionInfo[
        sEId][sourceCoordinates.id] + 1);
      slicey = sourceCoordinates.h / (this.endpointConnectionAreaConnectionInfo[
        sEId][sourceCoordinates.id] + 1);
      ...
      if (sourceCoordinates.len !== 0) {
        connectorDelta = this._getConnectorDelta(sourceCoordinates, this.endpointConnectionAreaConnectionInfo[
          sEId][sourceCoordinates.id] + 1, sourceConnectionPoint[2]);
        connectionPathPoints.push({
          "x": sourceCoordinates.x + slicex * sourceConnectionPoint[2] +
            connectorDelta.dx + connectorDelta.dx * connExtender * (
              sourceConnectionPoint[2] - 1),
          "y": sourceCoordinates.y + slicey * sourceConnectionPoint[2] +
            connectorDelta.dy + connectorDelta.dy * connExtender * (
              sourceConnectionPoint[2] - 1)
        });
      }
      ...
      /****************endpoint ***************/
      slicex = targetCoordinates.w / (this.endpointConnectionAreaConnectionInfo[
        tEId][targetCoordinates.id] + 1);
      slicey = targetCoordinates.h / (this.endpointConnectionAreaConnectionInfo[
        tEId][targetCoordinates.id] + 1);

      /***************endpoint's defined connector length*********************/
      if (targetCoordinates.len !== 0) {
        connectorDelta = this._getConnectorDelta(targetCoordinates, this.endpointConnectionAreaConnectionInfo[
          tEId][targetCoordinates.id] + 1, targetConnectionPoint[2]);
        connectionPathPoints.push({
          "x": targetCoordinates.x + slicex * targetConnectionPoint[2] +
            connectorDelta.dx + connectorDelta.dx * connExtender * (
              targetConnectionPoint[2] - 1),
      ...
      if (srcObjId === dstObjId && srcSubCompId === dstSubCompId) {
        ...
        if (connectionPathPoints[0].x === connectionPathPoints[lastIdx].x) {
          if (connectionPathPoints[0].y === connectionPathPoints[lastIdx].y) {
            ...
            if (sourceCoordinates.angle1 >= 0 && sourceCoordinates.angle2 <
              90) {
              //EAST
              connectionPathPoints.splice(2, 0, {
                "x": connectionPathPoints[1].x + 20,
                "y": connectionPathPoints[1].y - 20
              }, {
                "x": connectionPathPoints[1].x + 40,
            ...
            } else if (sourceCoordinates.angle1 >= 90 && sourceCoordinates.angle2 <
              180) {
              //SOUTH
              connectionPathPoints.splice(2, 0, {
                "x": connectionPathPoints[1].x + 20,
            ...
            } else if (sourceCoordinates.angle1 >= 180 && sourceCoordinates.angle2 <
              270) {
              //WEST
              connectionPathPoints.splice(2, 0, {
                "x": connectionPathPoints[1].x - 20,
            ...
            } else if (sourceCoordinates.angle1 >= 270 && sourceCoordinates.angle2 <
              360) {
              //NORTH
              connectionPathPoints.splice(2, 0, {
                "x": connectionPathPoints[1].x - 20,
  ...
  ConnectionRouteManager2.prototype._getConnectorDelta = function (coordDesc,
    ...
    var dx = 0,
      dy = 0,
      angle = (coordDesc.angle1 + ((coordDesc.angle2 - coordDesc.angle1) /
        slices * connOrderNum)) * (Math.PI / 180),
      result = {
        "dx": dx,
        "dy": dy
      };
  ...
  ConnectionRouteManager2.prototype._getClosestPoints = function (
    srcConnectionPoints, tgtConnectionPoints, segmentPoints) {
    var i,
      j,
      dx,
      dy,
      ...
      for (i = 0; i < srcConnectionPoints.length; i += 1) {
        for (j = 0; j < tgtConnectionPoints.length; j += 1) {
          dx = {
            ...
            "tgt": Math.abs(tgtConnectionPoints[j].x - segmentPoints[
              segmentPoints.length - 1][0])
          };

          dy = {
            "src": Math.abs(srcConnectionPoints[i].y - segmentPoints[0][1]),
            "tgt": Math.abs(tgtConnectionPoints[j].y - segmentPoints[
              segmentPoints.length - 1][1])
          };

          cLength = Math.sqrt(dx.src * dx.src + dy.src * dy.src) + Math.sqrt(
            dx.tgt * dx.tgt + dy.tgt * dy.tgt);
