...
define(['logManager',
  ...
  './AutoRouter.Edge',
  './AutoRouter.EdgeList'
], function (logManager, assert, CONSTANTS, UTILS, ArPoint, ArPointListPath, ArRect,
  AutoRouterPath,
  AutoRouterPort,
  AutoRouterBox,
  AutoRouterEdge,
  ...
  AutoRouterGraph.prototype._goToNextBufferBox = function (args) {
    ...
    if (dir2 === dir) {
      dir2 = UTILS.isRightAngle(UTILS.exGetMinorDir(end.minus(point))) ? UTILS.exGetMinorDir(end.minus(point)) : (dir +
        1) % 4;
    }
    ...
    while (++i < this.bufferBoxes.length) {
      ...
      if (!UTILS.isPointInDirFrom(point, boxRect, dir) && //Add support for entering the parent box
        ...
        UTILS.isCoordInDirFrom(stophere, UTILS.getChildRectOuterCoordFrom(this.bufferBoxes[i], dir, point).coord, dir)
  ...
  AutoRouterGraph.prototype._hugChildren = function (bufferObject, point, dir1, dir2, exitCondition) {
    ...
    var children = bufferObject.children,
      ...
      child = this._goToNextBox(point, dir1, (dir1 === 1 || dir1 === 2 ? CONSTANTS.ED_MAXCOORD : CONSTANTS.ED_MINCOORD),
        children),
      finalPoint,
      dir = dir2,
  ...
  AutoRouterGraph.prototype._connect = function (path) {
    ...
    if (path.isAutoRouted() && this.box2bufferBox[startId] === this.box2bufferBox[endId] && startdir === UTILS.reverseDir(
      enddir) && startport.getOwner() !== endport.getOwner()) {
  ...
  AutoRouterGraph.prototype._connectPoints = function (ret, start, end, hintstartdir, hintenddir, flipped) {
    ...
    var thestart = new ArPoint(start),
      ...
      getToDir1Side = function (pt, bo) {
        return UTILS.getPointCoord(pt, dir1) === UTILS.getRectOuterCoord(bo.box, dir1) || (UTILS.isPointInDirFrom(pt,
          end, dir1));
      };
    //RETURN
    ...
    while (!start.equals(end)) {
      ...
      assert(dir2 === CONSTANTS.Dir_None || dir2 === UTILS.getMinorDir(end.minus(start)),
      ...
      if (start.equals(old)) {
        ...
        assert(dir1 !== dir2 && dir1 !== CONSTANTS.Dir_None && dir2 !== CONSTANTS.Dir_None,
        ...
        if (bufferObject.box.ptInRect(end) && !bufferObject.box.ptInRect(start) && flipped) {
          ...
          if (UTILS.reverseDir(dir2) === hintenddir &&
            UTILS.getChildRectOuterCoordFrom(bufferObject, UTILS.reverseDir(dir2), start) === UTILS.getRectOuterCoord(
              rect, UTILS.reverseDir(dir2))) { //And if point can exit that way 
            rev = 1;
  ...
  AutoRouterGraph.prototype._candeleteTwoEdgesAt = function (path, points, pos) {
    ...
    assert(pppointpos < points.getLength() && ppointpos < points.getLength() && pointpos < points.getLength() &&
  ...
  AutoRouterGraph.prototype._deleteTwoEdgesAt = function (path, points, pos) {
    ...
    if (nnnpointpos < points.getLength()) {
      ...
      assert(nnnedge.getStartPointPrev().equals(npoint[0]) && nnnedge.getStartPointPtr()[0].equals(nnpoint),
  ...
  AutoRouterGraph.prototype._deleteSamePointsAt = function (path, points, pos) {
    ...
    assert(ppointpos < points.getLength() && pointpos < points.getLength() && npointpos < points.getLength() &&
    ...
    if (pppointpos < points.getLength()) {
      ...
      assert(ppedge !== null && ppedge.getEndPoint().equals(ppoint) && ppedge.getEndPointNext().equals(point),
        'ARGraph.deleteSamePointsAt: ppedge !== null && ppedge.getEndPoint().equals(ppoint) && ppedge.getEndPointNext().equals(point) FAILED'
      );
      ppedge.setEndPointNext(nnpoint);
    }

    assert(nnedge.getStartPoint().equals(npoint) && nnedge.getStartPointPrev().equals(point),
    ...
    if (nnnpointpos < points.getLength()) {
      ...
      assert(nnnedge !== null && nnnedge.getStartPointPrev().equals(npoint) && nnnedge.getStartPoint().equals(nnpoint),
  ...
  AutoRouterGraph.prototype._centerStairsInPathPoints = function (path, hintstartdir, hintenddir) {
    ...
    while (pos < pointList.getLength()) {
      ...
      if (p4p < pointList.getLength() && d12 === d34) {
        assert(p1p < pointList.getLength() && p2p < pointList.getLength() && p3p < pointList.getLength() && p4p <
          pointList.getLength(),
  ...
  AutoRouterGraph.prototype._simplifyPathPoints = function (path) {
    ...
    while (pos < pointList.getLength()) {
      ...
      if (p5p < pointList.getLength()) {
        assert(p1p < pointList.getLength() && p2p < pointList.getLength() && p3p < pointList.getLength() && p4p <
          pointList.getLength() && p5p < pointList.getLength(),
