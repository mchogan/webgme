...
define(['logManager',
  ...
  './AutoRouter.Edge',
  './AutoRouter.EdgeList'
], function (logManager, assert, CONSTANTS, UTILS, ArPoint, ArPointListPath,
  ArRect,
  AutoRouterPath,
  AutoRouterPort,
  AutoRouterBox,
  ...
  AutoRouterGraph.prototype._remove = function (box) {
    if (box instanceof AutoRouterBox) {
      ...
      assert(this.boxes[box.getID()] !== undefined,
        "ARGraph.remove: Box does not exist");
  ...
  AutoRouterGraph.prototype._getOutOfBox = function (details) {
    ...
    assert(UTILS.isRightAngle(dir),
      "ARGraph.getOutOfBox: UTILS.isRightAngle (dir) FAILED");
    ...
    assert(!boxRect.ptInRect(point),
      "ARGraph.getOutOfBox: !boxRect.ptInRect( point ) FAILED");
  };
  ...
  AutoRouterGraph.prototype._goToNextBufferBox = function (args) {
    ...
    assert(UTILS.isRightAngle(dir),
      "ArGraph.goToNextBufferBox: UTILS.isRightAngle (dir) FAILED");
    ...
    while (++i < this.bufferBoxes.length) {
      ...
      if (!UTILS.isPointInDirFrom(point, boxRect, dir) && //Add support for entering the parent box
        ...
        UTILS.isCoordInDirFrom(stophere, UTILS.getChildRectOuterCoordFrom(
          this.bufferBoxes[i], dir, point).coord, dir)) { //Return extreme (parent box) for this comparison
        stophere = UTILS.getChildRectOuterCoordFrom(this.bufferBoxes[i], dir,
          point).coord;
  ...
  AutoRouterGraph.prototype._hugChildren = function (bufferObject, point,
    dir1, dir2, exitCondition) {
    ...
    assert((dir1 + dir2) % 2 === 1,
      "ARGraph.hugChildren: One and only one direction must be horizontal");
    var children = bufferObject.children,
      ...
      finalPoint,
      dir = dir2,
      nextDir = UTILS.nextClockwiseDir(dir1) === dir2 ? UTILS.nextClockwiseDir :
      UTILS.prevClockwiseDir,
      points = [
        [new ArPoint(point)]
      ],
      hasExit = true,
    ...
    while (hasExit && !exitCondition(point, bufferObject)) {
      ...
      nextChild = this._goToNextBox(point, dir, UTILS.getRectOuterCoord(child,
        dir), children);
      ...
      } else if (UTILS.isCoordInDirFrom(UTILS.getRectOuterCoord(nextChild,
          UTILS.reverseDir(nextDir(dir))),
        UTILS.getPointCoord(point, UTILS.reverseDir(nextDir(dir))), UTILS.reverseDir(
          nextDir(dir)))) {
        dir = nextDir(dir);
        child = nextChild;
      }
  ...
  AutoRouterGraph.prototype._goToNextBox = function (point, dir, stop1,
    boxList) {
    var stophere = stop1;
    ...
    assert(UTILS.isRightAngle(dir),
      "ArGraph.goToNextBox: UTILS.isRightAngle (dir) FAILED");
    assert(UTILS.getPointCoord(point, dir) !== stophere,
    ...
    while (iter--) {
      ...
      if (UTILS.isPointInDirFrom(point, boxRect, UTILS.reverseDir(dir)) &&
        ...
        UTILS.isCoordInDirFrom(stophere, UTILS.getRectOuterCoord(boxRect,
          UTILS.reverseDir(dir)), dir)) {
  ...
  AutoRouterGraph.prototype._getLimitsOfEdge = function (startPt, endPt, min,
    max) {
    var t,
    ...
    if (start.y === end.y) {
      ...
      assert(start.x === end.x,
        "ARGraph.this.getLimitsOfEdge: start.x === end.x FAILED");
  ...
  AutoRouterGraph.prototype._connect = function (path) {
    ...
    assert(startport.hasPoint(startpoint),
      "ARGraph.connect: startport.hasPoint(startpoint) FAILED");
    assert(endport.hasPoint(endpoint),
      "ARGraph.connect: endport.hasPoint(endpoint) FAILED");
    ...
    if (path.isAutoRouted() && this.box2bufferBox[startId] === this.box2bufferBox[
        endId] && startdir === UTILS.reverseDir(enddir) && startport.getOwner() !==
      endport.getOwner()) {

      return this._connectPointsSharingParentBox(path, startpoint, endpoint,
        startdir);
    } else {
  ...
  AutoRouterGraph.prototype._connectPathWithPoints = function (path,
    startpoint, endpoint) {
    assert(startpoint instanceof ArPoint,
      "ARGraph.connect: startpoint instanceof ArPoint FAILED");
    assert(path !== null && path.getOwner() === this,
      "ARGraph.connect: path !== null && path.getOwner() === self FAILED");
    assert(!path.isConnected(), "ARGraph.connect: !path.isConnected() FAILED");
    assert(!startpoint.equals(endpoint),
      "ARGraph.connect: !startpoint.equals(endpoint) FAILED");
    ...
    assert(!start.equals(startpoint),
      "ARGraph.connect: !start.equals(startpoint) FAILED");
    ...
    assert(!end.equals(endpoint),
      "ARGraph.connect: !end.equals(endpoint) FAILED");
  ...
  AutoRouterGraph.prototype._connectPointsSharingParentBox = function (path,
    startpoint, endpoint, startdir) {
  ...
  AutoRouterGraph.prototype._connectPoints = function (ret, start, end,
    hintstartdir, hintenddir, flipped) {
    assert(ret.getLength() === 0,
      "ArGraph.connectPoints: ret.getLength() === 0 FAILED");
    ...
    var thestart = new ArPoint(start),
      ...
      findExitToEndpoint = function (pt, bo) {
        return (pt.x === end.x || pt.y === end.y) && !UTILS.isLineClipRects(
          pt, end, bo.children);
      ...
      getToDir1Side = function (pt, bo) {
        return UTILS.getPointCoord(pt, dir1) === UTILS.getRectOuterCoord(bo.box,
          dir1) || (UTILS.isPointInDirFrom(pt, end, dir1));
      };
    //RETURN
    ...
    while (!start.equals(end)) {
      ...
      assert(dir1 !== CONSTANTS.Dir_None,
        "ARGraph.connectPoints: dir1 !== CONSTANTS.Dir_None FAILED");

      assert(dir1 === UTILS.getMajorDir(end.minus(start)),
        "ARGraph.connectPoints: dir1 === UTILS.getMajorDir(end.minus(start)) FAILED"
      );
      assert(dir2 === CONSTANTS.Dir_None || dir2 === UTILS.getMinorDir(end.minus(
          start)),
        "ARGraph.connectPoints: dir2 === CONSTANTS.Dir_None || dir2 === UTILS.getMinorDir(end.minus(start)) FAILED"
      );

      if (retend === ret.getLength() && dir2 === hintstartdir && dir2 !==
        CONSTANTS.Dir_None) {
      ...
      if (start.equals(old)) {
        ...
        assert(dir1 !== dir2 && dir1 !== CONSTANTS.Dir_None && dir2 !==
          CONSTANTS.Dir_None,
          "ARGraph.connectPoints: dir1 !== dir2 && dir1 !== CONSTANTS.Dir_None && dir2 !== CONSTANTS.Dir_None FAILED"
        );
        if (bufferObject.box.ptInRect(end) && !bufferObject.box.ptInRect(
          start) && flipped) {
          ...
          if (!flipped) {
            ...
            assert(start.equals(end),
              "ArGraph.connectPoints: start.equals(end) FAILED");
            ...
            if (pts !== null) { //There is a path from start -> end
              ...
              assert(UTILS.isRightAngle(dir1),
                "ARGraph.connectPoints: UTILS.isRightAngle (dir1) FAILED");
          ...
          if (start.equals(old)) { //Then we are in a corner
            ...
            if (pts !== null) {
              ...
              assert(UTILS.isRightAngle(dir1),
                "ARGraph.getOutOfBox: UTILS.isRightAngle (dir1) FAILED");
          ...
          assert(!UTILS.isPointIn(end, rect),
            "ARGraph.connectPoints: !UTILS.isPointIn(end, rect) FAILED");

          rev = 0;

          if (UTILS.reverseDir(dir2) === hintenddir &&
            UTILS.getChildRectOuterCoordFrom(bufferObject, UTILS.reverseDir(
              dir2), start) === UTILS.getRectOuterCoord(rect, UTILS.reverseDir(
              dir2))) { //And if point can exit that way 
            rev = 1;
          ...
          if (bufferObject.children.length === 1) {
            ...
            assert(!start.equals(old),
              "ARGraph.connectPoints: !start.equals(old) FAILED");
            assert(retend !== ret.getLength(),
              "ARGraph.connectPoints: retend !== ret.getLength() FAILED");
            retend++;
            if (retend === ret.getLength()) {
              ret.push([new ArPoint(start)]);
              retend--;
            ...
            if (UTILS.getPointCoord(start, dir1) !== UTILS.getPointCoord(end,
              dir1)) {
    ...
    if (obj instanceof AutoRouterBox) {
      ...
      while (iter--) {
        ...
        assert(startport !== null,
          "ARGraph.disconnectPathsFrom: startport !== null FAILED");
        startbox = startport.getOwner();
        assert(startbox !== null,
          "ARGraph.disconnectPathsFrom: startbox !== null FAILED");

        endport = path.getEndPort();
        assert(endport !== null,
          "ARGraph.disconnectPathsFrom: endport !== null FAILED");
        endbox = endport.getOwner();
        assert(endbox !== null,
          "ARGraph.disconnectPathsFrom: endbox !== null FAILED");
  ...
  AutoRouterGraph.prototype._candeleteTwoEdgesAt = function (path, points,
    pos) {
    if (CONSTANTS.DEBUG) {
      assert(path.getOwner() === this,
        "ARGraph.candeleteTwoEdgesAt: path.getOwner() === this FAILED");
      path.assertValid();
      assert(path.isConnected(),
        "ARGraph.candeleteTwoEdgesAt: path.isConnected() FAILED");
    ...
    assert(pppointpos < points.getLength() && ppointpos < points.getLength() &&
      pointpos < points.getLength() && npointpos < points.getLength() &&
      nnpointpos < points.getLength(),
    ...
    assert(UTILS.isRightAngle(dir),
      "ARGraph.candeleteTwoEdgesAt: UTILS.isRightAngle (dir) FAILED");
  ...
  AutoRouterGraph.prototype._deleteTwoEdgesAt = function (path, points, pos) {
    if (CONSTANTS.DEBUG) {
      assert(path.getOwner() === this,
        "ARGraph.deleteTwoEdgesAt: path.getOwner() === this FAILED");
      path.assertValid();
      assert(path.isConnected(),
        "ARGraph.deleteTwoEdgesAt: path.isConnected() FAILED");
      points.AssertValidPos(pos);
    }
    ...
    assert(pppointpos < points.getLength() && ppointpos < points.getLength() &&
      pointpos < points.getLength() && npointpos < points.getLength() &&
      nnpointpos < points.getLength(),
      "ARGraph.deleteTwoEdgesAt: pppointpos < points.getLength() && ppointpos < points.getLength() && pointpos < points.getLength() && npointpos < points.getLength() && nnpointpos < points.getLength() FAILED"
    );
    assert(pppoint !== null && ppoint !== null && point !== null && npoint !==
      null && nnpoint !== null,
      "ARGraph.deleteTwoEdgesAt: pppoint !== null && ppoint !== null && point !== null && npoint !== null && nnpoint !== null FAILED"
    );

    var dir = UTILS.getDir(npoint[0].minus(point));

    assert(UTILS.isRightAngle(dir),
      "ARGraph.deleteTwoEdgesAt: UTILS.isRightAngle (dir) FAILED");
    var ishorizontal = UTILS.isHorizontal(dir);
    ...
    assert(!this._isLineClipBoxes(newpoint, npoint[0]),
      "ARGraph.deleteTwoEdgesAt: !isLineClipBoxes(newpoint, npoint[0]) FAILED"
    );
    ...
    assert(ppedge !== null && pedge !== null && nedge !== null && nnedge !==
      null,
    ...
    if (nnnpointpos < points.getLength()) {
      ...
      assert(nnnedge.getStartPointPrev().equals(npoint[0]) && nnnedge.getStartPointPtr()[
          0].equals(nnpoint),
        "ARGraph.deleteTwoEdgesAt: nnnedge.getStartPointPrev().equals(npoint[0]) && nnnedge.getStartPoint().equals(nnpoint) FAILED"
      );
  ...
  AutoRouterGraph.prototype._deleteSamePointsAt = function (path, points, pos) {
    if (CONSTANTS.DEBUG) {
      assert(path.getOwner() === this,
        "ARGraph.deleteSamePointsAt: path.getOwner() === this FAILED");
      path.assertValid();
      assert(path.isConnected(),
        "ARGraph.deleteSamePointsAt: path.isConnected() FAILED");
      points.AssertValidPos(pos);
    }
    ...
    assert(ppointpos < points.getLength() && pointpos < points.getLength() &&
      npointpos < points.getLength() && nnpointpos < points.getLength(),
      "ARGraph.deleteSamePointsAt: ppointpos < points.getLength() && pointpos < points.getLength() && npointpos < points.getLength() && nnpointpos < points.getLength() FAILED"
    );
    assert(ppoint !== null && point !== null && npoint !== null && nnpoint !==
      null,
    ...
    assert(UTILS.isRightAngle(dir),
      "ARGraph.deleteSamePointsAt: UTILS.isRightAngle (dir) FAILED");
    ...
    assert(pedge !== null && nedge !== null && nnedge !== null,
      "ARGraph.deleteSamePointsAt: pedge !== null && nedge !== null && nnedge !== null FAILED"
    );
    ...
    if (pppointpos < points.getLength()) {
      ...
      assert(ppedge !== null && ppedge.getEndPoint().equals(ppoint) && ppedge
        .getEndPointNext().equals(point),
        "ARGraph.deleteSamePointsAt: ppedge !== null && ppedge.getEndPoint().equals(ppoint) && ppedge.getEndPointNext().equals(point) FAILED"
      );
      ppedge.setEndPointNext(nnpoint);
    }

    assert(nnedge.getStartPoint().equals(npoint) && nnedge.getStartPointPrev()
      .equals(point),
      "ARGraph.deleteSamePointsAt: nnedge.getStartPoint().equals(npoint) && nnedge.getStartPointPrev().equals(point) FAILED"
    );
    ...
    if (nnnpointpos < points.getLength()) {
      ...
      assert(nnnedge !== null && nnnedge.getStartPointPrev().equals(npoint) &&
        nnnedge.getStartPoint().equals(nnpoint),
        "ARGraph.deleteSamePointsAt: nnnedge !== null && nnnedge.getStartPointPrev().equals(npoint) && nnnedge.getStartPoint().equals(nnpoint) FAILED"
      );
      nnnedge.setStartPointPrev(ppoint);
    }
  ...
  AutoRouterGraph.prototype._centerStairsInPathPoints = function (path,
    hintstartdir, hintenddir) {
    assert(path !== null,
      "ARGraph.centerStairsInPathPoints: path !== null FAILED");
    assert(!path.isConnected(),
      "ARGraph.centerStairsInPathPoints: !path.isConnected() FAILED");

    var pointList = path.getPointList();
    assert(pointList.getLength() >= 2,
      "ARGraph.centerStairsInPathPoints: pointList.getLength() >= 2 FAILED");
    ...
    var p1,
      ...
      pos = 0;
    assert(pos < pointList.getLength(),
      "ARGraph.centerStairsInPathPoints pos < pointList.getLength() FAILED");
    ...
    while (pos < pointList.getLength()) {
      ...
      if (p2p < pointList.getLength()) {
        ...
        if (CONSTANTS.DEBUG) {
          assert(UTILS.isRightAngle(d12),
            "ARGraph.centerStairsInPathPoints: UTILS.isRightAngle (d12) FAILED"
          );
          if (p3p !== pointList.end()) {
            assert(UTILS.areInRightAngle(d12, d23),
              "ARGraph.centerStairsInPathPoints: UTILS.areInRightAngle (d12, d23) FAILED"
            );
          }
        }
      }
      ...
      if (p4p < pointList.getLength() && d12 === d34) {
        assert(p1p < pointList.getLength() && p2p < pointList.getLength() &&
          p3p < pointList.getLength() && p4p < pointList.getLength(),
          "ARGraph.centerStairsInPathPoints: p1p < pointList.getLength() && p2p < pointList.getLength() && p3p < pointList.getLength() && p4p < pointList.getLength() FAILED"
        );
        ...
        if (p4x < p3x && p3x < p1x) {
          ...
          if (!this._isLineClipBoxes(np2, np3) &&
            !this._isLineClipBoxes(p1p === pointList.getLength() ?
              outOfBoxEndPoint : p1, np2) &&
    ...
    if (len === 4) {
      ...
      if (startDir === UTILS.reverseDir(endDir)) {
        ...
        if (UTILS.isHorizontal(startDir)) {
          ...
          tstStart = new ArPoint(UTILS.getRectOuterCoord(startPort.getOwner()
            .getRect(), startDir), newStart.y);
          tstEnd = new ArPoint(UTILS.getRectOuterCoord(endPort.getOwner().getRect(),
            endDir), newEnd.y);
          ...
          tstStart = new ArPoint(newStart.x, UTILS.getRectOuterCoord(
            startPort.getOwner().getRect(), startDir));
          tstEnd = new ArPoint(newEnd.x, UTILS.getRectOuterCoord(endPort.getOwner()
            .getRect(), endDir));
        }

        if (startRect.ptInRect(newStart) && endRect.ptInRect(newEnd) && !this
          ._isLineClipBoxes(tstStart, tstEnd)) {
          ...
          edge.setPositionY(UTILS.getPointCoord(newStart, UTILS.nextClockwiseDir(
            startDir)));
          hlist.insert(edge);
  ...
  AutoRouterGraph.prototype._simplifyPathCurves = function (path) {
    ...
    while (i < pointList.getLength() - 3) {
      ...
      while (j-- > 0) {
        ...
        if (UTILS.isRightAngle(UTILS.getDir(p1.minus(p2))) && !this._isLineClipBoxes(
          p1, p2)) {
  ...
  AutoRouterGraph.prototype._simplifyPathPoints = function (path) {
    ...
    assert(!path.isConnected(),
      "ARGraph.simplifyPathPoints: !path.isConnected() FAILED");

    var pointList = path.getPointList();
    assert(pointList.getLength() >= 2,
      "ARGraph.simplifyPathPoints: pointList.length >= 2 FAILED");
    ...
    assert(pos < pointList.getLength(),
      "ARGraph.simplifyPathPoints: pos < pointList.getLength() FAILED");
    ...
    while (pos < pointList.getLength()) {
      ...
      if (p5p < pointList.getLength()) {
        assert(p1p < pointList.getLength() && p2p < pointList.getLength() &&
          p3p < pointList.getLength() && p4p < pointList.getLength() && p5p <
          pointList.getLength(),
          "ARGraph.simplifyPathPoints: p1p < pointList.getLength() && p2p < pointList.getLength() && p3p < pointList.getLength() && p4p < pointList.getLength() && p5p < pointList.getLength() FAILED"
        );
        assert(!p1.equals(p2) && !p2.equals(p3) && !p3.equals(p4) && !p4.equals(
            p5),
          "ARGraph.simplifyPathPoints: !p1.equals(p2) && !p2.equals(p3) && !p3.equals(p4) && !p4.equals(p5) FAILED"
        );

        d = UTILS.getDir(p2.minus(p1));
        assert(UTILS.isRightAngle(d),
          "ARGraph.simplifyPathPoints: UTILS.isRightAngle (d) FAILED");
  ...
  AutoRouterGraph.prototype._addToBufferBoxes = function (inputBox) {
    var i = this.bufferBoxes.length,
      box = {
        'rect': new ArRect(inputBox.getRect()),
        'id': inputBox.getID()
      },
      overlapBoxesIndices = [],
      bufferBox,
    ...
    this.bufferBoxes.push({
      "box": parentBox,
      "children": children
    });
    i = ids.length;
    while (i--) {
      this.box2bufferBox[ids[i]] = this.bufferBoxes[this.bufferBoxes.length -
        1];
    }
  ...
  AutoRouterGraph.prototype._removeFromBufferBoxes = function (box) {
    ...
    assert(i !== -1,
      "ARGraph.removeFromBufferBoxes: Can't find the correct bufferbox.");
    ...
    while (i--) {
      ...
      j = ids.length;
      while (j--) {
        this.box2bufferBox[ids[j]] = this.bufferBoxes[this.bufferBoxes.length -
          1];
      }
    }
      ...
      if (path.isAutoRouted()) { // comment this if you want the check to run for fully customizable connections
        ...
        if (path.areTherePathCustomizations()) {
          ...
          for (var box in this.boxes) {
            if (this.boxes.hasOwnProperty(box)) {
              boxRect = this.boxes[box].getRect();
              isStartOrEndRect = (!startBoxRect.isRectEmpty() && UTILS.isRectIn(
                  startBoxRect, boxRect) ||
                !endBoxRect.isRectEmpty() && UTILS.isRectIn(endBoxRect,
                  boxRect));
  ...
  AutoRouterGraph.prototype.deletePath = function (path) {
    ...
    if (path.hasOwner()) {
      assert(path.getOwner() === this,
        "ARGraph.deletePath: path.getOwner() === this FAILED");
  ...
  AutoRouterGraph.prototype.addPath = function (isAutoRouted, startports,
    endports) {
  ...
  AutoRouterGraph.prototype.isEdgeFixed = function (path, startpoint,
    endpoint) {
  ...
  AutoRouterGraph.prototype.assertValidBox = function (box) {
    box.assertValid();
    assert(box.getOwner().equals(this),
      "ARGraph.assertValidBox: box.getOwner().equals(this) FAILED");

    assert(this.boxes[box.getID()] !== undefined,
      "ARGraph.assertValidBox: this.boxes[box.getID()] !== undefined FAILED");
  };

  AutoRouterGraph.prototype._assertValidPath = function (path) {
    path.assertValid();
    assert(path.getOwner().equals(this),
      "ARGraph.assertValidPath: path.getOwner().equals(this) FAILED");
    ...
    assert(startPort !== null,
      "ARGraph.assertValidPath: startPort !== null FAILED");
    startPort.assertValid();
    var ownerBox = startPort.getOwner(),
      boxOwnerGraph = ownerBox.getOwner();
    assert(boxOwnerGraph.equals(this),
      "ARGraph.assertValidPath: boxOwnerGraph.equals(this) FAILED");
    ...
    assert(endPort !== null,
      "ARGraph.assertValidPath: endPort !== null FAILED");
    endPort.assertValid();
    var ownerBox2 = endPort.getOwner();
    assert(ownerBox2.getOwner().equals(this),
      "ARGraph.assertValidPath: ownerBox2.getOwner().equals(this) FAILED");
    ownerBox2.assertValidPort(endPort);

    if (path.isConnected()) {
      var itr = pointList.length;
      endPort.assertValidStartEndPoint(pointList[--itr], CONSTANTS.Dir_None,
        0);
    } else {
      assert(path.hasNoPoint(),
        "ARGraph.assertValidPath: path.hasNoPoint() FAILED");
    }
    ...
    if (pointList.length !== 0) {
      assert(pointList.length >= 2,
        "ARGraph.assertValidPath: pointList.length >= 2 FAILED");
      var pos = 0;
      assert(pos !== pointList.length,
        "ARGraph.assertValidPath: pos !== pointList.length FAILED");

      assert(this._isPointInBox(pointList[pos++]),
        "ARGraph.assertValidPath: isPointInBox(pointList[pos++]) FAILED");

      var p;
      while (pos < pointList.length) {
        p = pointList[pos++];
        if (pos !== pointList.length) {
          assert(!this._isPointInBox(p),
            "ARGraph.assertValidPath: !isPointInBox(p) FAILED");
        } else {
          assert(this._isPointInBox(p),
            "ARGraph.assertValidPath: isPointInBox(p) FAILED");
        }
