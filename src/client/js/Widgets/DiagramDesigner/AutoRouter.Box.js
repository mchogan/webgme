...
define(['logManager',
  ...
  AutoRouterBox.prototype.setRect = function (r) {
    assert(r instanceof ArRect,
      "Invalthis.id arg in ARBox.setRect. Requires ArRect");

    assert(r.getWidth() >= 3 && r.getHeight() >= 3,
      "ARBox.setRect: r.getWidth() >= 3 && r.getHeight() >= 3 FAILED!");
    assert(r.getTopLeft().x >= CONSTANTS.ED_MINCOORD && r.getTopLeft().y >=
      CONSTANTS.ED_MINCOORD,
      "ARBox.setRect: r.getTopLeft().x >= CONSTANTS.ED_MINCOORD && r.getTopLeft().y >= CONSTANTS.ED_MAXCOORD FAILED!"
    );
    assert(r.getBottomRight().x <= CONSTANTS.ED_MAXCOORD && r.getBottomRight()
      .y <= CONSTANTS.ED_MAXCOORD,
    ...
    if (this.atomic) {
      assert(this.ports.length === 1,
        "ARBox.setRect: this.ports.length === 1 FAILED!");
  ...
  AutoRouterBox.prototype.addChild = function (box) {
    assert(this.childBoxes.indexOf(box) === -1,
      "ARBox.addChild: box already is child of " + this.getID());
