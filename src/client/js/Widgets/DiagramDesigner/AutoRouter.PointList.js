...
define(['logManager',
  ...
  ArPointListPath.prototype.push = function (element) {
    if (CONSTANTS.DEBUG && this.ArPointList.length > 0) {
      assert(element[0].x === this.ArPointList[this.ArPointList.length - 1][0]
        .x ||
        element[0].y === this.ArPointList[this.ArPointList.length - 1][0].y,
        "ArPointListPath.push: point does not create horizontal or vertical edge!"
      );
    }
  ...
  ArPointListPath.prototype.getHeadEdge = function (start, end) {
    ...
    assert(pos < this.ArPointList.length,
      "ArPointListPath.getHeadEdge: pos < this.ArPointList.length FAILED");
  ...
  ArPointListPath.prototype.getNextEdge = function (pos, start, end) {
    ...
    pos++;
    assert(pos < this.ArPointList.length,
      "ArPointListPath.getNextEdge: pos < this.ArPointList.length FAILED");
  ...
  ArPointListPath.prototype.getEdge = function (pos, start, end) {
    ...
    start = this.ArPointList[pos++];
    assert(pos < this.ArPointList.length,
      "ArPointListPath.getEdge: pos < this.ArPointList.length FAILED");
  ...
  ArPointListPath.prototype.getHeadEdgePtrs = function (start, end) {
    ...
    assert(pos < this.ArPointList.length,
      "ArPointListPath.getHeadEdgePtrs: pos < this.ArPointList.length FAILED"
    );
  ...
  ArPointListPath.prototype.getTailEdgePtrs = function () {
    ...
    assert(--pos < this.ArPointList.length,
      "ArPointListPath.getTailEdgePtrs: --pos < this.ArPointList.length FAILED"
    );
  ...
  ArPointListPath.prototype.getEdgePtrs = function (pos, start, end) {
    ...
    assert(pos < this.ArPointList.length,
      "ArPointListPath.getEdgePtrs: pos < this.ArPointList.length FAILED");
  ...
  ArPointListPath.prototype.getEdgePosForStartPoint = function (start) {
    ...
    while (pos < this.ArPointList.length) {
      if (this.ArPointList[pos++] === start) {
        assert(pos < this.ArPointList.length,
          "ArPointListPath.getEdgePosForStartPoint: pos < this.ArPointList.length FAILED"
        );
        pos--;
        break;
      }
  ...
  ArPointListPath.prototype.assertValid = function () {
    ...
    while (i--) {
      if (!UTILS.isRightAngle(UTILS.getDir(this.ArPointList[i + 1][0].minus(
        this.ArPointList[i][0])))) {
  ...
  ArPointListPath.prototype.assertValidPos = function (pos) {
    assert(pos < this.ArPointList.length,
      "ArPointListPath.assertValidPos: pos < this.ArPointList.length FAILED");
