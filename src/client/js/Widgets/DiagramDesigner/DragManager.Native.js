...
define(['logManager',
  ...
  DragManager.prototype._activateMouseListeners = function () {
    ...
    this.$el.on(MOUSE_DOWN, 'div.' + DiagramDesignerWidgetConstants.DESIGNER_ITEM_CLASS,
      function (event) {
  ...
  DragManager.prototype._onItemMouseDown = function (event) {
    ...
    if (dragEnabled && leftButton) {
      this.logger.debug("DragManager._onItemMouseDown at: " + JSON.stringify(
        mousePos));

      if (this._diagramDesigner.mode === this._diagramDesigner.OPERATING_MODES
        .DESIGN) {
  ...
  DragManager.prototype._initDrag = function (mX, mY) {
    ...
    if (dragEnabled && this._diagramDesigner.mode === this._diagramDesigner.OPERATING_MODES
      .DESIGN) {
  ...
  DragManager.prototype._onBackgroundMouseMove = function (event) {
    ...
    if (this._dragDesc.dragging === true) {
      ...
      if (Math.abs(dx) >= MIN_DELTA_TO_DRAG || Math.abs(dy) >=
        MIN_DELTA_TO_DRAG) {
        //minimum delta is met, start the dragging
        this.logger.debug(
          'Mouse delta reached the minimum to handle as a drag');
  ...
  DragManager.prototype._startDrag = function (event) {
    ...
    this._diagramDesigner.onDesignerItemDragStart(undefined, this._dragDesc.params
      .draggedItemIDs);
  ...
  DragManager.prototype._startDragModeMove = function () {
    ...
    while (i--) {
      ...
      if (itemIDs.indexOf(id) !== -1) {
        ...
        if (this._diagramDesigner.onDragStartDesignerItemDraggable(id) ===
          true) {
  ...
  DragManager.prototype._startDragModeCopy = function () {
    ...
    while (i--) {
      ...
      if (itemIDs.indexOf(id) !== -1) {
        ...
        if (this._diagramDesigner.onDragStartDesignerItemCopyable(id) ===
          true) {
          objDesc = {};
          srcItem = items[id];
          objDesc.position = {
    ...
    while (i--) {
      ...
      if (connectionIDs.indexOf(id) !== -1) {
        if (this._diagramDesigner.onDragStartDesignerConnectionCopyable(id) ===
          true) {
          ...
          var copiedConnection = this._diagramDesigner.createConnection(
            objDesc);
    ...
    this._diagramDesigner.selectionManager.setSelection(newSelectionIDs,
      false);
      ...
      for (i in copyData) {
        if (copyData.hasOwnProperty(i)) {
          ...
          if (desc.hasOwnProperty("copiedItemId")) {
            ...
            copyDesc.connections[desc.copiedConnectionId] = {
              "oConnectionId": i
            };
          }
        }
    ...
    if (this._dragDesc) {
      ...
      while (i--) {
        if (this._dragDesc.params.minStartCoordinates.x) {
          this._dragDesc.params.minStartCoordinates.x = Math.min(this._dragDesc
            .params.minStartCoordinates.x, this._dragDesc.params.originalPositions[
              i].x);
        } else {
          this._dragDesc.params.minStartCoordinates.x = this._dragDesc.params
            .originalPositions[i].x;
        }

        if (this._dragDesc.params.minStartCoordinates.y) {
          this._dragDesc.params.minStartCoordinates.y = Math.min(this._dragDesc
            .params.minStartCoordinates.y, this._dragDesc.params.originalPositions[
              i].y);
        } else {
          this._dragDesc.params.minStartCoordinates.y = this._dragDesc.params
    ...
    if (this._dragDesc) {
      ...
      case this.DRAGMODE_MOVE:
        ...
        if (idx !== -1) {
          ...
          this.logger.debug(
            'One of the currently moved items is being deleted: ' +
            componentId);
      ...
      case this.DRAGMODE_COPY:
        if (modeSpecificData.hasOwnProperty(componentId)) {
          //one of the dragged items has been deleted
          this.logger.debug(
            'One of the currently copied items is being deleted: ' +
            componentId);
          ...
          idx = this._dragDesc.params.draggedItemIDs.indexOf(
            copiedComponentId);
          if (idx !== -1) {
            this._dragDesc.params.draggedItemIDs.splice(idx, 1);
            this._dragDesc.params.originalPositions.splice(idx, 1);
          ...
          idx = this._dragDesc.params.draggedConnectionIDs.indexOf(
            copiedComponentId);
          if (idx !== -1) {
