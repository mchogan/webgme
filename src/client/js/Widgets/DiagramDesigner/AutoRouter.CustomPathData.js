...
define(['logManager',
...
], function (logManager, assert, UTILS,
  CONSTANTS) {
  ...
  AutoRouterCustomPathData.prototype.serialize = function () {
    var outChannel = (this.getVersion() + "," + this.getAspect() + "," + this
      .getEdgeIndex() + "," + this.getEdgeCount() + "," + this.getType());

    outChannel += ("," + this.isHorizontalOrVertical() ? 1 : 0 + "," + this.getX() +
      "," + this.getY() + "," + this.getLongDataCount());
  ...
  AutoRouterCustomPathData.prototype.deserialize = function (inChannel) {
    ...
    assert(this.getVersion() === CONSTANTS.CONNECTIONCUSTOMIZATIONDATAVERSION,
      "AutoRouterCustomPathData.deserialize: getVersion() === CONSTANTS.CONNECTIONCUSTOMIZATIONDATAVERSION FAILED"
    );
    ...
    console.log("\tAsp " + this.getAspect() + ", Ind " + this.getEdgeIndex() +
      ", Cnt " + this.getEdgeCount() + ", Typ " + this.getType());
    ...
    assert(numOfExtraLongData >= 0 && numOfExtraLongData <= 4,
      "AutoRouterCustomPathData.deserialize: numOfExtraLongData >= 0 && numOfExtraLongData <= 4 FAILED"
    );

    console.log(", Dir " + UTILS.isHorizontalOrVertical() + ", x " + this.getX() +
      ", y " + this.getY() + ", num " + numOfExtraLongData);
    ...
    assert(numOfExtraDoubleData >= 0 && numOfExtraDoubleData <= 8,
      "AutoRouterCustomPathData.deserialize: numOfExtraDoubleData >= 0 && numOfExtraDoubleData <= 8 FAILED"
    );
  ...
  AutoRouterCustomPathData.prototype.setHorizontalOrVertical = function (
    parity) {
