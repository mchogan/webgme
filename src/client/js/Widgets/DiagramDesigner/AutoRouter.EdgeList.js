...
define(['logManager',
  ...
  AutoRouterEdgeList.prototype.addEdges = function (path) {
    ...
    if (path instanceof AutoRouterPath) {
      assert(path.getOwner() === this.owner,
        "AREdgeList.addEdges: path.getOwner() === owner FAILED!");
      ...
      assert(port.getOwner().getOwner() === this.owner,
        "AREdgeList.addEdges: port.getOwner() === (owner) FAILED!");
      ...
      for (i = 0; i < 4; i++) {
        ...
        canHaveStartEndPointHorizontal = port.canHaveStartEndPointHorizontal(
          this.ishorizontal);
      ...
      assert(box.getOwner() === this.owner,
        "AREdgeList.addEdges: box.getOwner() === (owner) FAILED!");
  ...
  AutoRouterEdgeList.prototype.getEdgeAt = function (point, nearness) {
    ...
    while (edge) {
      ...
      if (UTILS.isPointNearLine(point, edge.getStartPoint(), edge.getEndPoint(),
        nearness)) {
        return edge;
      }
  ...
  AutoRouterEdgeList.prototype.dumpEdges = function (msg) {
    ...
    while (edge !== null) {
      console.log('\t' + edge.getStartPoint().x + ', ' + edge.getStartPoint()
        .y + '\t\t' + edge.getEndPoint().x + ', ' + edge.getEndPoint().y +
        '\t\t\t(' + (edge.getEdgeFixed() ? "FIXED" : "MOVEABLE") + ')\t\t' +
        (edge.getBracketClosing() ? "Bracket Closing" : (edge.getBracketOpening() ?
          "Bracket Opening" : "")));
      edge = edge.getOrderNext();
      total++;
    }
    ...
    if (y === undefined) {
      if (this.ishorizontal) {
        assert(edge.getStartPoint().y === edge.getEndPoint().y,
          "AREdgeList.position_GetRealY: edge.getStartPoint().y === edge.getEndPoint().y FAILED!"
        );
        return edge.getStartPoint().y;
      }

      assert(edge.getStartPoint().x === edge.getEndPoint().x,
        "AREdgeList.position_GetRealY: edge.getStartPoint().x === edge.getEndPoint().x FAILED!"
      );
  ...
  AutoRouterEdgeList.prototype._position_SetRealY = function (edge, y) {
    ...
    if (this.ishorizontal) {
      assert(edge.getStartPoint().y === edge.getEndPoint().y,
        "AREdgeList.position_SetRealY: edge.getStartPoint().y === edge.getEndPoint().y FAILED"
      );
      edge.setStartPointY(y);
      edge.setEndPointY(y);
    } else {
      assert(edge.getStartPoint().x === edge.getEndPoint().x,
        "AREdgeList.position_SetRealY: edge.getStartPoint().x === edge.getEndPoint().x FAILED"
      );
      edge.setStartPointX(y);
      edge.setEndPointX(y);
    }
  ...
  AutoRouterEdgeList.prototype._position_GetRealX = function (edge) {
    ...
    if (this.ishorizontal) {
      assert(edge.getStartPoint().y === edge.getEndPoint().y,
        "AREdgeList.position_GetRealX: edge.getStartPoint().y === edge.getEndPoint().y FAILED"
      );
      ...
      assert(edge.getStartPoint().x === edge.getEndPoint().x,
        "AREdgeList.position_GetRealX: edge.getStartPoint().x === edge.getEndPoint().x FAILED"
      );
  ...
  AutoRouterEdgeList.prototype._position_GetRealO = function (edge) {
    ...
    if (this.ishorizontal) {
      assert(edge.getStartPoint().y === edge.getEndPoint().y,
        "AREdgeList.position_GetRealO: edge.getStartPoint().y === edge.getEndPoint().y FAILED"
      );
      if (edge.getStartPoint().x < edge.getEndPoint().x) {

        o1 = edge.isStartPointPrevNull() ? 0 : edge.getStartPointPrev().y -
          edge.getStartPoint().y;
        o2 = edge.isEndPointNextNull() ? 0 : edge.getEndPointNext().y - edge.getEndPoint()
          .y;
      } else {

        o1 = edge.isEndPointNextNull() ? 0 : edge.getEndPointNext().y - edge.getEndPoint()
          .y;
        o2 = edge.isStartPointPrevNull() ? 0 : edge.getStartPointPrev().y -
          edge.getStartPoint().y;
      }
    } else {
      assert(edge.getStartPoint().x === edge.getEndPoint().x,
        "AREdgeList.position_GetRealO: edge.getStartPoint().x === edge.getEndPoint().x FAILED"
      );
      if (edge.getStartPoint().y < edge.getEndPoint().y) {

        o1 = edge.isStartPointPrevNull() ? 0 : edge.getStartPointPrev().x -
          edge.getStartPoint().x;
        o2 = edge.isEndPointNextNull() ? 0 : edge.getEndPointNext().x - edge.getEndPoint()
          .x;
      } else {

        o1 = edge.isEndPointNextNull() ? 0 : edge.getEndPointNext().x - edge.getEndPoint()
          .x;
        o2 = edge.isStartPointPrevNull() ? 0 : edge.getStartPointPrev().x -
          edge.getStartPoint().x;
      }
    }
  ...
  AutoRouterEdgeList.prototype._position_LoadY = function (edge) {
    assert(edge !== null && edge.getOrderNext() === null && edge.getOrderPrev() ===
      null,
      "AREdgeList.position_LoadY: edge !== null && edge.getOrderNext() === null && edge.getOrderPrev() === null FAILED"
    );
  ...
  AutoRouterEdgeList.prototype._position_LoadB = function (edge) {
    ...
    edge.setBracketOpening(!edge.getEdgeFixed() && this.bracket_IsOpening(
      edge));
    edge.setBracketClosing(!edge.getEdgeFixed() && this.bracket_IsClosing(
      edge));
  };
  ...
  AutoRouterEdgeList.prototype.insertBefore = function (edge, before) {
    ...
    assert(edge.getOrderNext() === null && edge.getOrderPrev() === null,
      "AREdgeList.insertBefore: edge.getOrderNext() === null && edge.getOrderPrev() === null FAILED"
    );
    ...
    if (before.getOrderPrev()) {
      assert(before.getOrderPrev().getOrderNext() === before,
        "AREdgeList.insertBefore: before.getOrderPrev().getOrderNext() === before FAILED\nbefore.getOrderPrev().getOrderNext() is " +
        before.getOrderPrev().getOrderNext() + " and before is " + before);
  ...
  AutoRouterEdgeList.prototype.insertAfter = function (edge, after) {
    ...
    assert(edge.getOrderNext() === null && edge.getOrderPrev() === null,
      "AREdgeList.insertAfter: edge.getOrderNext() === null && edge.getOrderPrev() === null FAILED "
    );
    ...
    if (after.getOrderNext()) {
      assert(after.getOrderNext().getOrderPrev().equals(after),
        "AREdgeList.insertAfter:  after.getOrderNext().getOrderPrev().equals(after) FAILED"
      );
      after.getOrderNext().setOrderPrev(edge);

      assert(!this.order_last.equals(after),
        "AREdgeList.insertAfter: !order_last.equals(after) FAILED");
    } else {
      assert(this.order_last.equals(after),
        "AREdgeList.insertAfter: this.order_last.equals(after) FAILED");
  ...
  AutoRouterEdgeList.prototype.insertLast = function (edge) {
    ...
    assert(edge.getOrderPrev() === null && edge.getOrderNext() === null,
      "AREdgeList.insertLast: edge.getOrderPrev() === null && edge.getOrderNext() === null FAILED"
    );

    edge.setOrderPrev(this.order_last);

    if (this.order_last) {
      assert(this.order_last.getOrderNext() === null,
        "AREdgeList.insertLast: this.order_last.getOrderNext() === null FAILED"
      );
  ...
  AutoRouterEdgeList.prototype.insert = function (edge) {
    ...
    assert(edge.getOrderPrev() === null && edge.getOrderNext() === null,
      "AREdgeList.insert: edge.getOrderPrev() === null && edge.getOrderNext() === null FAILED"
    );

    var y = edge.getPositionY();

    assert(CONSTANTS.ED_MINCOORD <= y && y <= CONSTANTS.ED_MAXCOORD,
      "AREdgeList.insert: CONSTANTS.ED_MINCOORD <= y && y <= CONSTANTS.ED_MAXCOORD FAILED (y is " +
      y + ")");
  ...
  AutoRouterEdgeList.prototype._slideButNotPassEdges = function (edge, y) {
    assert(edge !== null,
      "AREdgeList.slideButNotPassEdges: edge != null FAILED");
    assert(CONSTANTS.ED_MINCOORD < y && y < CONSTANTS.ED_MAXCOORD,
    ...
    if (oldy < y) {
      while (insert.getOrderNext()) {
        ...
        if (!insert.getEdgeCanpassed() && UTILS.intersect(x1, x2, insert.getPositionX1(),
          insert.getPositionX2())) {
          ret = insert;
  ...
  AutoRouterEdgeList.prototype.checkSection = function () {
    if (!(this.section_blocker === null && this.section_ptr2blocked === null)) {
      ...
      this._logger.warning(
        "section_blocker and this.section_ptr2blocked are not null. Assuming last run did not exit cleanly. Fixing..."
      );
      this.section_blocker = null;
      this.section_ptr2blocked = null;
    }
  ...
  AutoRouterEdgeList.prototype.section_IsImmediate = function () {
    assert(this.section_blocker !== null && this.section_ptr2blocked !== null &&
      this.section_ptr2blocked !== null,
    ...
    if (e !== null) {
      e = (e.getStartPoint().equals(CONSTANTS.EMPTY_POINT) || e.getSectionX1() ===
        undefined ? null : e);
    }
    ...
    if (a1 <= b1) {
      while (!(e === null || e.getStartPoint().equals(CONSTANTS.EMPTY_POINT)) &&
        e.getSectionX2() < b1) {
        e = e.getSectionNext();
      }

      if (b2 <= a2) {
        return (e === null || e.getStartPoint().equals(CONSTANTS.EMPTY_POINT)) ||
          b2 < e.getSectionX1(); // case 3
      }
    ...
    if (b2 <= a2) {
      return a1 === p1 && ((e === null || e.getStartPoint().equals(CONSTANTS.EMPTY_POINT)) ||
        b2 < e.getSectionX1()); // case 5
    }
  ...
  AutoRouterEdgeList.prototype.section_HasBlockedEdge = function () {
    ...
    if (this.section_ptr2blocked === null) {
      ...
      this.section_first = this.section_first === null ? [new AutoRouterEdge()] :
        this.section_first;
    ...
    } else //section_ptr2blocked contains a null placeholder
    {
      ...
      assert(!current_edge.getStartPoint().equals(CONSTANTS.EMPTY_POINT),
        "AREdgeList.section_HasBlockedEdge: !current_edge.getStartPoint().equals(CONSTANTS.EMPTY_POINT) FAILED"
      );
      ...
      assert(a1 <= a2, "AREdgeList.section_HasBlockedEdge: a1 <= a2 FAILED (" +
        a1 + " <= " + a2 + ")");
      ...
      } else if (b1 <= a1 && a2 <= b2) // case 4
      {
        if (e && !e.getStartPoint().equals(CONSTANTS.EMPTY_POINT)) {
          while (e.getSectionNext() && !e.getSectionNext().getStartPoint().equals(
      ...
      } else if (b1 <= a1 && b2 < a2) // case 5
      {
        ...
        while ((e && !e.getStartPoint().equals(CONSTANTS.EMPTY_POINT)) && e.getSectionX1() <=
          a1) {
          assert(e.getSectionX1() <= e.getSectionX2(),
            "AREdgeList.section_HasBlockedEdge: e.getSectionX1() <= e.getSectionX2() FAILED"
          );
        ...
        if (o) { //Insert current_edge to be section_next of the given edge in the list of section_down (basically, collapsing current_edge into the section_down list. The values in the list following current_edge will then be set to be section_down of the current_edge.)
      ...
      } else // case 2
      {
        assert(a1 < b1 && b1 <= a2 && a2 <= b2,
          "AREdgeList.section_HasBlockedEdge:  a1 < b1 && b1 <= a2 && a2 <= b2 FAILED"
        );
        ...
        while (e && !e.getStartPoint().equals(CONSTANTS.EMPTY_POINT)) {
          ...
          if (o.getSectionX2() + 1 < b1 && (e === null || e.getStartPoint().equals(
            CONSTANTS.EMPTY_POINT) || o.getSectionX2() + 1 < e.getSectionX1())) {
        ...
        if (!this.section_ptr2blocked[0].getStartPoint().equals(CONSTANTS.EMPTY_POINT)) {
          ...
          current_edge.setSectionX2(
            (this.section_ptr2blocked[0].getSectionX1() < b1 ? this.section_ptr2blocked[
              0].getSectionX1() : b1) - 1);
    ...
    while (this.section_ptr2blocked[0] !== null && !this.section_ptr2blocked[
      0].getStartPoint().equals(CONSTANTS.EMPTY_POINT)) {
      ...
      //If this.section_ptr2blocked starts before and ends after this.section_blocker
      {
        ...
        for (;;) {
          ...
          if (e === null || e.getStartPoint().equals(CONSTANTS.EMPTY_POINT) ||
            x < e.getSectionX1()) {
            return true;
    ...
    assert(this.section_blocker.getSectionNext() === null && (this.section_blocker
        .getSectionDown() === null || this.section_blocker.getSectionDown().getStartPoint()
        .equals(CONSTANTS.EMPTY_POINT)),
  ...
  AutoRouterEdgeList.prototype.section_GetBlockedEdge = function () {
    assert(this.section_blocker !== null && this.section_ptr2blocked !== null,
      "AREdgeList.sectionGetBlockedEdge: this.section_blocker !== null && this.section_ptr2blocked !== null FAILED"
    );
  ...
  AutoRouterEdgeList.prototype.bracket_IsClosing = function (edge) {
    assert(edge !== null,
      "AREdgeList.bracket_IsClosing: edge !== null FAILED");
    ...
    return this.ishorizontal ?
      (edge.getStartPointPrev().y < start.y && edge.getEndPointNext().y < end
        .y) :
      (edge.getStartPointPrev().x < start.x && edge.getEndPointNext().x < end
        .x);
  };

  AutoRouterEdgeList.prototype.bracket_IsOpening = function (edge) {
    assert(edge !== null,
      "AREdgeList.bracket_IsOpening: edge !== null FAILED");
    assert(!edge.isStartPointNull() && !edge.isEndPointNull(),
  ...
  AutoRouterEdgeList.prototype.bracket_IsSmallGap = function (blocked,
    blocker) {
    return this.bracket_IsOpening(blocked) || this.bracket_IsClosing(blocker);
  };

  AutoRouterEdgeList.prototype.bracket_ShouldBeSwitched = function (edge,
    next) {
    ...
    if ((nx1 < ex1 && ex1 < nx2 && eo1 > 0) || (ex1 < nx1 && nx1 < ex2 && no1 <
      0)) {
      c1 = +1;
    ...
    if ((nx1 < ex2 && ex2 < nx2 && eo2 > 0) || (ex1 < nx2 && nx2 < ex2 && no2 <
      0)) {
      c2 = +1;
  ...
  AutoRouterEdgeList.prototype.block_PushBackward = function (blocked,
    blocker) {
    var modified = false;
    ...
    assert(blocked.getPositionY() <= blocker.getPositionY(),
      "AREdgeList.block_PushBackward: blocked.getPositionY() <= blocker.getPositionY() FAILED"
    );
    ...
    if (b + s > 1) {
      ...
      do {
        assert(edge !== null && edge.getBlockTrace() !== null,
          "AREdgeList.block_PushBackward: edge !== null && edge.getBlockTrace() !== null FAILED"
        );
      ...
      if (CONSTANTS.DEBUG) {
        ...
        assert(this.flt_equ(y, blocker.getPositionY()),
          "AREdgeList.block_PushBackward: flt_equ(y, blocker.getPositionY()) FAILED"
        );
      }
    }
  ...
  AutoRouterEdgeList.prototype.block_PushForward = function (blocked, blocker) {
    ...
    assert(blocked.getPositionY() >= blocker.getPositionY(),
      "AREdgeList.block_PushForward: blocked.getPositionY() >= blocker.getPositionY() FAILED"
    );
  ...
  AutoRouterEdgeList.prototype.block_SwitchWrongs = function () {
    ...
    while (second !== null) {
      if (second.getClosestPrev() !== null && second.getClosestPrev().getClosestNext() !==
        (second) && //Check if it references itself
        second.getClosestNext() !== null && second.getClosestNext().getClosestPrev() ===
        (second))

      {
        ...
        while (next !== null && edge === next.getClosestPrev()) {
          ...
          if (ey + 1 <= ny && this.bracket_ShouldBeSwitched(edge, next)) {
            ...
            a = this._slideButNotPassEdges(edge, (ny + ey) / 2 + 0.001) !==
              null;
            a = this._slideButNotPassEdges(next, (ny + ey) / 2 - 0.001) !==
              null || a;
            ...
            if (edge.getClosestPrev() !== null && edge.getClosestPrev().getClosestNext() ===
              edge) {
              edge.getClosestPrev().setClosestNext(next);
            }

            if (next.getClosestNext() !== null && next.getClosestNext().getClosestPrev() ===
              next) {
            ...
            if (next.getClosestPrev() !== null && next.getClosestPrev().getClosestNext() ===
              next) {
