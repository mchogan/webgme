...
define(['js/Widgets/DiagramDesigner/DiagramDesignerWidget.Constants',
    ...
    for (i = 0; i < len; i += 1) {
      if (i === 0) {
        ...
        if (this.connection.isBezier) {
          pathDef.push("C" + (p[i - 1][0] + p[i - 1][2]) + "," + (p[i - 1][1] +
            p[i - 1][3]) + " " + (p[i][0] - p[i][2]) + "," + (p[i][1] - p[i]
            [3]) + " " + p[i][0] + "," + p[i][1]);
        } else {
  ...
  ConnectionEditSegment.prototype._getSelectedPathPoint = function (mouseX,
    mouseY) {
    var w = this.width + 2,
      ...
      verticalPath = ["M", mouseX, mouseY - w, "L", mouseX, mouseY + w].join(
        ","),
      intersectionsHorizontal = Raphael.pathIntersection(this.pathDef,
        horizontalPath),
      intersectionsVertical = Raphael.pathIntersection(this.pathDef,
        verticalPath),
      distanceH = 1000,
      distanceW = 1000,
      resultPos;
    ...
    if (intersectionsHorizontal.length > 0) {
      distanceH = Math.abs(mouseX - intersectionsHorizontal[0].x) + Math.abs(
        mouseY - intersectionsHorizontal[0].y);
    }
    if (intersectionsVertical.length > 0) {
      distanceW = Math.abs(mouseX - intersectionsVertical[0].x) + Math.abs(
        mouseY - intersectionsVertical[0].y);
    }
