...
define(['./DiagramDesignerWidget.Constants'], function (DiagramDesignerWidgetConstants) {
  ...
  DiagramDesignerWidgetMouse.prototype._activateMouseListeners = function () {
    ...
    this.$el.on('mousedown.' + EVENT_POSTFIX, 'div.' + DiagramDesignerWidgetConstants.DESIGNER_ITEM_CLASS, function (
      event) {
      ...
      if (self.onItemMouseDown) {
        ...
        logger.warning('onItemMouseDown(itemId, eventDetails) is undefined, ItemID: ' + itemId + ' eventDetails: ' +
          JSON.stringify(eventDetails));
      }
    });

    //handle click on designer-connections
    this.$el.on('mousedown.' + EVENT_POSTFIX, 'path[class~="' + DiagramDesignerWidgetConstants.DESIGNER_CONNECTION_CLASS +
      '"]', function (event) {
        var connId = $(this).attr('id').replace(DiagramDesignerWidgetConstants.PATH_SHADOW_ARROW_END_ID_PREFIX, '').replace(
            DiagramDesignerWidgetConstants.PATH_SHADOW_ID_PREFIX, ''),
          eventDetails = self._processMouseEvent(event, true, true, true, true);
        ...
        if (self.onConnectionMouseDown) {
          ...
          logger.warning('onConnectionMouseDown(connId, eventDetails) is undefined, connId: ' + connId +
    ...
    this.$el.on('mousedown.' + EVENT_POSTFIX, function (event) {
      ...
      if (self.onBackgroundMouseDown) {
        ...
        logger.warning('onBackgroundMouseDown(eventDetails) is undefined, eventDetails: ' + JSON.stringify(
          eventDetails));
      }
    });
    ...
    this.$el.on('dblclick.' + EVENT_POSTFIX, function (event) {
      ...
      if (self.onBackgroundDblClick) {
        ...
        logger.warning('onBackgroundDblClick(eventDetails) is undefined, eventDetails: ' + JSON.stringify(
          eventDetails));
      }
  ...
  DiagramDesignerWidgetMouse.prototype._processMouseEvent = function (event, triggerUIActivity, preventDefault,
