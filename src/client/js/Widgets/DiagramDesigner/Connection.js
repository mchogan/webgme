...
define(['logManager',
  ...
  var Connection,
    ...
    CONNECTION_DEFAULT_PATTERN = DiagramDesignerWidgetConstants.LINE_PATTERNS
    .SOLID,
  ...
  Connection.prototype._initializeConnectionProps = function (objDescriptor) {
    ...
    /*PathAttributes*/
    this.designerAttributes.arrowStart = objDescriptor[
      DiagramDesignerWidgetConstants.LINE_START_ARROW] ||
      CONNECTION_DEFAULT_END;
    this.designerAttributes.arrowEnd = objDescriptor[
      DiagramDesignerWidgetConstants.LINE_END_ARROW] ||
      CONNECTION_DEFAULT_END;
    this.designerAttributes.color = objDescriptor[
      DiagramDesignerWidgetConstants.LINE_COLOR] || CONNECTION_DEFAULT_COLOR;
    this.designerAttributes.width = parseInt(objDescriptor[
      DiagramDesignerWidgetConstants.LINE_WIDTH], 10) ||
      CONNECTION_DEFAULT_WIDTH;
    this.designerAttributes.pattern = objDescriptor[
      DiagramDesignerWidgetConstants.LINE_PATTERN] ||
      CONNECTION_DEFAULT_PATTERN;
    this.designerAttributes.shadowWidth = this.designerAttributes.width +
      CONNECTION_SHADOW_DEFAULT_WIDTH - CONNECTION_DEFAULT_WIDTH;
    this.designerAttributes.shadowOpacity = CONNECTION_SHADOW_DEFAULT_OPACITY;
    this.designerAttributes.shadowOpacityWhenSelected =
      CONNECTION_SHADOW_DEFAULT_OPACITY_WHEN_SELECTED;
    this.designerAttributes.shadowColor = CONNECTION_SHADOW_DEFAULT_COLOR;
    this.designerAttributes.lineType = objDescriptor[
      DiagramDesignerWidgetConstants.LINE_TYPE] ||
      CONNECTION_DEFAULT_LINE_TYPE;

    this.designerAttributes.shadowEndArrowWidth = this.designerAttributes.width +
      SHADOW_MARKER_SIZE_INCREMENT;
    ...
    if (objDescriptor[DiagramDesignerWidgetConstants.LINE_POINTS]) {
      ...
      for (var i = 0; i < len; i += 1) {
        fixedP = this._fixXY({
          'x': objDescriptor[DiagramDesignerWidgetConstants.LINE_POINTS][i]
          [0],
          'y': objDescriptor[DiagramDesignerWidgetConstants.LINE_POINTS][i]
          [1]
        });
        cx = objDescriptor[DiagramDesignerWidgetConstants.LINE_POINTS][i].length >
          2 ? objDescriptor[DiagramDesignerWidgetConstants.LINE_POINTS][i][2] :
          0;
        cy = objDescriptor[DiagramDesignerWidgetConstants.LINE_POINTS][i].length >
          2 ? objDescriptor[DiagramDesignerWidgetConstants.LINE_POINTS][i][3] :
          0;
  ...
  Connection.prototype._raphaelArrowAdjustForSizeToRefSize = function (
    arrowType, size, refSize, isEnd) {
    ...
    if (isEnd) {
      this.designerAttributes.endArrowMarkerSize = this.designerAttributes.width *
        raphaelMarkerW;
      this.designerAttributes.shadowEndArrowMarkerSize = this.designerAttributes
        .shadowEndArrowWidth * raphaelMarkerW;
    } else {
      this.designerAttributes.startArrowMarkerSize = this.designerAttributes.width *
        raphaelMarkerW;
      this.designerAttributes.shadowStartArrowMarkerSize = this.designerAttributes
        .shadowEndArrowWidth * raphaelMarkerW;
    }
  ...
  Connection.prototype._raphaelArrowSizeToRefSize = function (arrowType,
    refSize) {
    var raphaelMarkerW = 3, //original RaphaelJS source settings
      raphaelMarkerH = 3,
      values = arrowType.toLowerCase().split("-"),
  ...
  Connection.prototype.getBoundingBox = function () {
    ...
    if (this.designerAttributes.arrowStart !== CONNECTION_NO_END) {
      bPoints = this._getRaphaelArrowEndBoundingPoints(this.designerAttributes
        .arrowStart, strokeWidthAdjust, this._pathStartAngle, false);
      ...
      len = bPoints.length;
      while (len--) {
        endMarkerBBox.x = Math.min(endMarkerBBox.x, this.sourceCoordinates.x -
          dx - bPoints[len].x);
        endMarkerBBox.y = Math.min(endMarkerBBox.y, this.sourceCoordinates.y -
          dy - bPoints[len].y);

        endMarkerBBox.x2 = Math.max(endMarkerBBox.x2, this.sourceCoordinates.x -
          dx - bPoints[len].x);
        endMarkerBBox.y2 = Math.max(endMarkerBBox.y2, this.sourceCoordinates.y -
          dy - bPoints[len].y);
      }
    }
    ...
    if (this.designerAttributes.arrowEnd !== CONNECTION_NO_END) {
      bPoints = this._getRaphaelArrowEndBoundingPoints(this.designerAttributes
        .arrowEnd, strokeWidthAdjust, this._pathEndAngle, true);
      ...
      len = bPoints.length;
      while (len--) {
        endMarkerBBox.x = Math.min(endMarkerBBox.x, this.endCoordinates.x +
          dx + bPoints[len].x);
        endMarkerBBox.y = Math.min(endMarkerBBox.y, this.endCoordinates.y +
          dy + bPoints[len].y);

        endMarkerBBox.x2 = Math.max(endMarkerBBox.x2, this.endCoordinates.x +
          dx + bPoints[len].x);
        endMarkerBBox.y2 = Math.max(endMarkerBBox.y2, this.endCoordinates.y +
          dy + bPoints[len].y);
      }
    }
    ...
    if (bBox.height === 0 && bBox.width !== 0) {
      ...
      if (Math.abs(bBox.x - this.sourceCoordinates.x) < Math.abs(bBox.x -
        this.endCoordinates.x)) {
        //source is on the left
        bBox.x -= Math.abs(Math.cos(Math.PI / 2 - this._pathStartAngle) *
          strokeWidthAdjust / 2);
        //target is on the right
        bBox.x2 += Math.abs(Math.cos(Math.PI / 2 - this._pathEndAngle) *
          strokeWidthAdjust / 2);
      } else {
        //target is on the left
        bBox.x -= Math.abs(Math.cos(Math.PI / 2 - this._pathEndAngle) *
          strokeWidthAdjust / 2);
        //source is on the right
        bBox.x2 += Math.abs(Math.cos(Math.PI / 2 - this._pathStartAngle) *
          strokeWidthAdjust / 2);
      }

      if (Math.abs(bBox.y - this.sourceCoordinates.y) < Math.abs(bBox.y -
        this.endCoordinates.y)) {
        //source is on the top
        bBox.y -= Math.abs(Math.sin(Math.PI / 2 - this._pathStartAngle) *
          strokeWidthAdjust / 2);
        //target is on the bottom
        bBox.y2 += Math.abs(Math.sin(Math.PI / 2 - this._pathEndAngle) *
          strokeWidthAdjust / 2);
      } else {
        //target is on the top
        bBox.y -= Math.abs(Math.sin(Math.PI / 2 - this._pathEndAngle) *
          strokeWidthAdjust / 2);
        //source is on the bottom
        bBox.y2 += Math.abs(Math.sin(Math.PI / 2 - this._pathStartAngle) *
          strokeWidthAdjust / 2);
      }
  ...
  Connection.prototype._getRaphaelArrowEndBoundingPoints = function (
    arrowType, arrowSize, angle, isEnd) {
    var bPoints = [],
      arrowEndSize,
      w,
      gamma,
    ...
    arrowEndSize = this._raphaelArrowSizeToRefSize(arrowType, arrowSize,
      isEnd);
    w = Math.sqrt(arrowEndSize.w / 2 * arrowEndSize.w / 2 + arrowEndSize.h /
      2 * arrowEndSize.h / 2);
    ...
    case "inheritance":
      ...
      bPoints.push({
        "x": -ref.x + bottomLeft.x,
        "y": -ref.y + bottomLeft.y
      });
      break;
    case "diamond":
    case "diamond2":
    ...
    default:
      ...
      bPoints.push({
        "x": -ref.x + bottomRight.x,
        "y": -ref.y + bottomRight.y
      });
      bPoints.push({
        "x": -ref.x + bottomLeft.x,
        "y": -ref.y + bottomLeft.y
  ...
  Connection.prototype._createPathShadow = function (segPoints) {
    ...
    if (this.skinParts.pathShadow === undefined || this.skinParts.pathShadow ===
      null) {
      ...
      if (this.designerAttributes.arrowStart !== CONNECTION_NO_END) {
        ...
        $(this.skinParts.pathShadowArrowStart.node).attr({
          "id": DiagramDesignerWidgetConstants.PATH_SHADOW_ARROW_END_ID_PREFIX +
            this.id,
          "class": DiagramDesignerWidgetConstants.DESIGNER_CONNECTION_CLASS
        });
      } else {
      ...
      if (this.skinParts.pathShadowArrowStart) {
        shadowArrowStart = this.designerAttributes.arrowStart.replace(
          "inheritance", "block");
      ...
      if (this.skinParts.pathShadowArrowEnd) {
        shadowArrowEnd = this.designerAttributes.arrowEnd.replace(
          "inheritance", "block");
        this.skinParts.pathShadowArrowEnd.attr({
          "stroke": this.designerAttributes.shadowColor,
          "stroke-width": this.designerAttributes.shadowEndArrowWidth,
  ...
  Connection.prototype._updatePathShadow = function (segPoints) {
    ...
    if (this.designerAttributes.arrowStart !== CONNECTION_NO_END) {
      ...
      if (dx !== 0) {
        osX = points[0].x;
        points[0].x += this.designerAttributes.startArrowMarkerSize / 2 *
          Math.cos(this._pathStartAngle);
      }

      if (dy !== 0) {
        osY = points[0].y;
        points[0].y += this.designerAttributes.startArrowMarkerSize / 2 *
          Math.sin(this._pathStartAngle);
      }
      ...
      if (this.designerAttributes.arrowStart.indexOf("block") !== -1 ||
        ...
        if (dx !== 0) {
          pointsEndArrow[0].x -= SHADOW_MARKER_BLOCK_FIX_OFFSET * (dx / Math.abs(
            dx));
        }
        if (dy !== 0) {
          pointsEndArrow[0].y -= SHADOW_MARKER_BLOCK_FIX_OFFSET * (dy / Math.abs(
            dy));
        }
      }
    }
    ...
    if (this.designerAttributes.arrowEnd !== CONNECTION_NO_END) {
      ...
      if (dx !== 0) {
        oeX = points[len - 1].x;
        points[len - 1].x -= this.designerAttributes.endArrowMarkerSize / 2 *
          Math.cos(this._pathEndAngle);
      }

      if (dy !== 0) {
        oeY = points[len - 1].y;
        points[len - 1].y -= this.designerAttributes.endArrowMarkerSize / 2 *
          Math.sin(this._pathEndAngle);
      }
      ...
      if (this.designerAttributes.arrowEnd.indexOf("block") !== -1 ||
        ...
        if (dx !== 0) {
          pointsEndArrow[len - 1].x += SHADOW_MARKER_BLOCK_FIX_OFFSET * (dx /
            Math.abs(dx));
        }
        if (dy !== 0) {
          pointsEndArrow[len - 1].y += SHADOW_MARKER_BLOCK_FIX_OFFSET * (dy /
            Math.abs(dy));
        }
      }
    }
    ...
    if (this.skinParts.pathShadowArrowStart) {
      ...
      dx = this.designerAttributes.shadowStartArrowMarkerSize * Math.cos(this
        ._pathStartAngle);
      dy = this.designerAttributes.shadowStartArrowMarkerSize * Math.sin(this
        ._pathStartAngle);
  ...
  Connection.prototype.showEndReconnectors = function () {
    if (this.reconnectable) {
      ...
      this.skinParts.srcDragPoint.css('transform', "scale(" + scale + "," +
        scale + ")");
      this.skinParts.dstDragPoint.css('transform', "scale(" + scale + "," +
        scale + ")");
    } else {
      this.hideEndReconnectors();
    }
    ...
    if (this.editable === true) {
      ...
      for (rIt = 0; rIt < routingPointsLen; rIt += 1) {
        ...
        if (sIt < segmentPointsLen && this._isSamePoint(routingPoints[rIt], {
          ...
          pathSegmentPoints.push([this.segmentPoints[sIt][0], this.segmentPoints[
            sIt][1], this.segmentPoints[sIt][2], this.segmentPoints[sIt][
            3
          ]]);
          ...
          pathSegmentPoints = [];
          pathSegmentPoints.push([this.segmentPoints[sIt - 1][0], this.segmentPoints[
            sIt - 1][1], this.segmentPoints[sIt - 1][2], this.segmentPoints[
            sIt - 1][3]]);
        } else {
  ...
  Connection.prototype._createEditSegment = function (points, num) {
    ...
    this.logger.debug('_createEditSegment: #' + num + ', ' + JSON.stringify(
      points));
    ...
    while (i--) {
      marker = new ConnectionSegmentPoint({
        ...
        'pointAfter': i === len - 1 ? [this._pathPoints[pointsLastIdx].x,
          this._pathPoints[pointsLastIdx].y, 0, 0
        ] : this.segmentPoints[i + 1],
        'pointBefore': i === 0 ? [this._pathPoints[0].x, this._pathPoints[0]
          .y, 0, 0
        ] : this.segmentPoints[i - 1]
      });
  ...
  Connection.prototype.highlight = function () {
    ...
    if (classes.indexOf(DiagramDesignerWidgetConstants.ITEM_HIGHLIGHT_CLASS) ===
      -1) {
  ...
  Connection.prototype.unHighlight = function () {
    ...
    if (this.skinParts.textContainer) {
      this.skinParts.textContainer.removeClass(DiagramDesignerWidgetConstants
        .ITEM_HIGHLIGHT_CLASS);
    }
  };
  ...
  Connection.prototype.update = function (objDescriptor) {
    ...
    if (this.skinParts.pathShadowArrowEnd) {
      shadowArrowEnd = this.designerAttributes.arrowEnd.replace("inheritance",
        "block");
  ...
  Connection.prototype.showEndConnectors = function () {
    this._connectionConnector = this._connectionConnector || $('<div/>', {
      'class': 'connector connection-connector'
    });

    this._connectionConnector.attr(DiagramDesignerWidgetConstants.DATA_ITEM_ID,
      this.id);
  ...
  Connection.prototype._showConnectionAreaMarker = function () {
    ...
    if (this.diagramDesigner.connectionIDbyEndID.hasOwnProperty(this.id)) {
      for (var i in this.diagramDesigner.connectionIDbyEndID[this.id]) {
        if (this.diagramDesigner.connectionIDbyEndID[this.id].hasOwnProperty(
          i)) {
          for (var j in this.diagramDesigner.connectionIDbyEndID[this.id][i]) {
            if (this.diagramDesigner.connectionIDbyEndID[this.id][i].hasOwnProperty(
              j)) {
              if (this.diagramDesigner.connectionIDbyEndID[this.id][i][j].length >
                0) {
                hasConnections = true;
                break;
              }
    ...
    if (this.skinParts.path && hasConnections) {
      ...
      this._connectionAreaMarker = $('<div/>', {
        'class': 'c-area'
      });
  ...
  Connection.prototype._textNameBase = $(
    '<div class="c-text"><span class="c-name"></span></div>');
  Connection.prototype._textSrcBase = $(
    '<div class="c-text"><span class="c-src"></span></div>');
  Connection.prototype._textDstBase = $(
    '<div class="c-text"><span class="c-dst"></span></div>');
  ...
  Connection.prototype._renderTexts = function () {
    ...
    if (this.name && this.name !== "") {
      ...
      this.skinParts.name.find('span').on("dblclick.editOnDblClick", null,
        function (event) {
          if (self.nameEdit === true && self.diagramDesigner.getIsReadOnlyMode() !==
            true) {
            $(this).editInPlace({
              "class": "",
    ...
    if (this.srcText && this.srcText !== "") {
      ...
      // set title editable on double-click
      this.skinParts.srcText.find('span').on("dblclick.editOnDblClick", null,
        function (event) {
          if (self.srcTextEdit === true && self.diagramDesigner.getIsReadOnlyMode() !==
            true) {
            $(this).editInPlace({
              "class": "",
              "onChange": function (oldValue, newValue) {
    ...
    if (this.dstText && this.dstText !== "") {
      ...
      // set title editable on double-click
      this.skinParts.dstText.find('span').on("dblclick.editOnDblClick", null,
        function (event) {
          if (self.dstTextEdit === true && self.diagramDesigner.getIsReadOnlyMode() !==
            true) {
            $(this).editInPlace({
              "class": "",
              "onChange": function (oldValue, newValue) {
    ...
    if (hasText) {
      $(this.diagramDesigner.skinParts.$itemsContainer.children()[0]).after(
        this.skinParts.textContainer);
    }
  };
  ...
  Connection.prototype._onSrcTextChanged = function (oldValue, newValue) {
    this.diagramDesigner.onConnectionSrcTextChanged(this.id, oldValue,
      newValue);
  };

  Connection.prototype._onDstTextChanged = function (oldValue, newValue) {
    this.diagramDesigner.onConnectionDstTextChanged(this.id, oldValue,
      newValue);
  };
  ...
  Connection.prototype._jumpOnCrossings = function (pathDefArray) {
    ...
    for (len = 0; len < intersectionSegments.length; len += 1) {
      ...
      for (i = 0; i < segmentXings.length; i += 1) {
        resultIntersectionPathDefs[segNum] = resultIntersectionPathDefs[
          segNum] || {
          't': [],
          'paths': {},
        ...
        xRadius = Math.max(this.designerAttributes.width, segmentXings[i].otherWidth) +
          JUMP_XING_RADIUS;
        ...
        pointBefore = this._getPointAtLength(segmentXings[i].path[0],
          segmentXings[i].path[1], segmentXings[i].path[2], segmentXings[i].path[
            3], atLength * segmentLength);

        atLength = segmentXings[i].t + pixDiffPercentage;
        if (atLength > 1) {
          atLength = 1;
        }
        pointAfter = this._getPointAtLength(segmentXings[i].path[0],
          segmentXings[i].path[1], segmentXings[i].path[2], segmentXings[i].path[
            3], atLength * segmentLength);
        ...
        xingCurve = "L" + pointBefore.x + "," + pointBefore.y + "A" + xRadius +
          "," + xRadius + " 0 0," + sweepFlag + " " + pointAfter.x + "," +
          pointAfter.y;

        resultIntersectionPathDefs[segNum].t.push(segmentXings[i].t);
        resultIntersectionPathDefs[segNum].paths[segmentXings[i].t] =
          xingCurve;
      ...
      for (j = 0; j < resultIntersectionPathDefs[segNum].t.length - 1; j += 1) {
        ...
        if (this._checkIntersect(xing.pointBefore.x, xing.pointBefore.y,
          ...
          xingCurve = "L" + xing.pointBefore.x + "," + xing.pointBefore.y +
            "A" + xRadius + "," + xRadius + " 0 0," + sweepFlag + " " + xing1
            .pointAfter.x + "," + xing1.pointAfter.y;
    ...
    for (i = 1; i < len; i += 1) {
      ...
      if (resultIntersectionPathDefs.hasOwnProperty(segNum)) {
        ...
        for (j = 0; j < resultIntersectionPathDefs[segNum].t.length; j += 1) {
          resultPathDefArray.push(resultIntersectionPathDefs[segNum].paths[
            resultIntersectionPathDefs[segNum].t[j]]);
        }
      }
  ...
  Connection.prototype._pathIntersect = function (otherConn) {
    ...
    if (clientUtil.overlap(this._pathPointsBBox, otherConn._pathPointsBBox)) {
      for (i = 0; i < p1len - 1; i += 1) {
        ...
        s1Length = Math.sqrt((s1.x2 - s1.x1) * (s1.x2 - s1.x1) + (s1.y2 - s1.y1) *
          (s1.y2 - s1.y1));
        ...
        for (j = 0; j < p2len - 1; j += 1) {
          ...
          s2Length = Math.sqrt((s2.x2 - s2.x1) * (s2.x2 - s2.x1) + (s2.y2 -
            s2.y1) * (s2.y2 - s2.y1));

          intr = this._getIntersect(s1.x1, s1.y1, s1.x2, s1.y2, s2.x1, s2.y1,
            s2.x2, s2.y2);
          if (intr) {
            intr.segment1 = i + 1;
            intr.segment2 = j + 1;
            ...
            tLength = Math.sqrt((intr.x - s1.x1) * (intr.x - s1.x1) + (intr.y -
              s1.y1) * (intr.y - s1.y1));
            intr.t1 = tLength / s1Length;

            tLength = Math.sqrt((intr.x - s2.x1) * (intr.x - s2.x1) + (intr.y -
              s2.y1) * (intr.y - s2.y1));
  ...
  Connection.prototype._getIntersect = function (x1, y1, x2, y2, x3, y3, x4,
    y4) {
    var mmax = Math.max,
      mmin = Math.min;
  ...
  Connection.prototype._checkIntersect = function (x1, y1, x2, y2, x3, y3, x4,
    y4) {
    var mmax = Math.max,
      mmin = Math.min;
  ...
  Connection.prototype._simplifyTrivially = function (pathPoints) {
    ...
    while (pos < pathPoints.length - 1) {
      ...
      dx1 = p2.x - p1.x;
      dy1 = p2.y - p1.y;
      a12 = dx1 === 0 ? (dy1 > 0 ? Math.PI / 2 : Math.PI / 2 * 3) : Math.atan(
        dy1 / dx1);

      //calculate p2 - p3 alpha
      dx2 = p3.x - p2.x;
      dy2 = p3.y - p2.y;
      a23 = dx2 === 0 ? (dy2 > 0 ? Math.PI / 2 : Math.PI / 2 * 3) : Math.atan(
        dy2 / dx2);
    ...
    for (i = 1; i < len; i += 1) {
      ...
      if (this.isBezier === false) {
        ...
        if (segmentPoints.length > 0 &&
          ...
          this._isSamePoint(p, {
            'x': segmentPoints[sIdx][0],
            'y': segmentPoints[sIdx][1]
          })) {
        ...
        pathDef.push("C" + (pp.x + pcX) + "," + (pp.y + pcY) + " " + (p.x -
          cX) + "," + (p.y - cY) + " " + p.x + "," + p.y);
