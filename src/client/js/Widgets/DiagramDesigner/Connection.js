...
define(['logManager',
  ...
  Connection.prototype._initializeConnectionProps = function (objDescriptor) {
    ...
    this.isBezier = (objDescriptor[DiagramDesignerWidgetConstants.LINE_TYPE] || DiagramDesignerWidgetConstants.LINE_TYPES
      .NONE).toLowerCase() === DiagramDesignerWidgetConstants.LINE_TYPES.BEZIER;
    ...
    this.designerAttributes.shadowArrowStartAdjust = this._raphaelArrowAdjustForSizeToRefSize(this.designerAttributes
      .arrowStart, this.designerAttributes.shadowEndArrowWidth, this.designerAttributes.width, false);
    this.designerAttributes.shadowArrowEndAdjust = this._raphaelArrowAdjustForSizeToRefSize(this.designerAttributes.arrowEnd,
      this.designerAttributes.shadowEndArrowWidth, this.designerAttributes.width, true);
    ...
    if (objDescriptor[DiagramDesignerWidgetConstants.LINE_POINTS]) {
      ...
      for (var i = 0; i < len; i += 1) {
        ...
        cx = objDescriptor[DiagramDesignerWidgetConstants.LINE_POINTS][i].length > 2 ? objDescriptor[
          DiagramDesignerWidgetConstants.LINE_POINTS][i][2] : 0;
        cy = objDescriptor[DiagramDesignerWidgetConstants.LINE_POINTS][i].length > 2 ? objDescriptor[
          DiagramDesignerWidgetConstants.LINE_POINTS][i][3] : 0;
  ...
  Connection.prototype.getBoundingBox = function () {
    ...
    if (this.designerAttributes.arrowStart !== CONNECTION_NO_END) {
      bPoints = this._getRaphaelArrowEndBoundingPoints(this.designerAttributes.arrowStart, strokeWidthAdjust, this._pathStartAngle,
        false);
    ...
    if (this.designerAttributes.arrowEnd !== CONNECTION_NO_END) {
      bPoints = this._getRaphaelArrowEndBoundingPoints(this.designerAttributes.arrowEnd, strokeWidthAdjust, this._pathEndAngle,
        true);
    ...
    if (this.editable === true) {
      ...
      for (rIt = 0; rIt < routingPointsLen; rIt += 1) {
        ...
        if (sIt < segmentPointsLen && this._isSamePoint(routingPoints[rIt], {
          ...
          pathSegmentPoints.push([this.segmentPoints[sIt][0], this.segmentPoints[sIt][1], this.segmentPoints[sIt][2],
            this.segmentPoints[sIt][3]
          ]);
          ...
          pathSegmentPoints = [];
          pathSegmentPoints.push([this.segmentPoints[sIt - 1][0], this.segmentPoints[sIt - 1][1], this.segmentPoints[
            sIt - 1][2], this.segmentPoints[sIt - 1][3]]);
        } else {
  ...
  Connection.prototype._showSegmentPoints = function () {
    ...
    while (i--) {
      marker = new ConnectionSegmentPoint({
        ...
        'pointAfter': i === len - 1 ? [this._pathPoints[pointsLastIdx].x, this._pathPoints[pointsLastIdx].y, 0, 0] : this
          .segmentPoints[i + 1],
  ...
  Connection.prototype._jumpOnCrossings = function (pathDefArray) {
    ...
    for (len = 0; len < intersectionSegments.length; len += 1) {
      ...
      for (i = 0; i < segmentXings.length; i += 1) {
        ...
        pointBefore = this._getPointAtLength(segmentXings[i].path[0], segmentXings[i].path[1], segmentXings[i].path[2],
          segmentXings[i].path[3], atLength * segmentLength);

        atLength = segmentXings[i].t + pixDiffPercentage;
        if (atLength > 1) {
          atLength = 1;
        }
        pointAfter = this._getPointAtLength(segmentXings[i].path[0], segmentXings[i].path[1], segmentXings[i].path[2],
          segmentXings[i].path[3], atLength * segmentLength);
        ...
        xingCurve = 'L' + pointBefore.x + ',' + pointBefore.y + 'A' + xRadius + ',' + xRadius + ' 0 0,' + sweepFlag +
          ' ' + pointAfter.x + ',' + pointAfter.y;
      ...
      for (j = 0; j < resultIntersectionPathDefs[segNum].t.length - 1; j += 1) {
        ...
        if (this._checkIntersect(xing.pointBefore.x, xing.pointBefore.y,
          ...
          xingCurve = 'L' + xing.pointBefore.x + ',' + xing.pointBefore.y + 'A' + xRadius + ',' + xRadius + ' 0 0,' +
  ...
  Connection.prototype._getPathDefFromPoints = function (points) {
    ...
    for (i = 1; i < len; i += 1) {
      ...
      if (this.isBezier === false) {
        ...
        pathDef.push('C' + (pp.x + pcX) + ',' + (pp.y + pcY) + ' ' + (p.x - cX) + ',' + (p.y - cY) + ' ' + p.x + ',' +
          p.y);
