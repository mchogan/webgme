...
define(['js/DragDrop/DropTarget'], function (dropTarget) {
  ...
  DataGridWidgetDroppable.prototype._attachDroppable = function (container,
    table) {
    var self = this;
  ...
  DataGridWidgetDroppable.prototype._activateDroppable = function (event,
    dragInfo) {
    var self = this;
    ...
    if (this._readOnly !== true && this._droppable === true) {
      ...
      this._droppableTable.on('mousemove.' + MOUSE_EVENT_POSTFIX, 'td',
        function ( /*event*/ ) {
          self._onCellMouseMove($(this));
        });
      this._droppableTable.on('mousemove.' + MOUSE_EVENT_POSTFIX, 'th',
        function ( /*event*/ ) {
          self._onCellMouseMove($(this));
        });
    }
    ...
    if (!this._oTD || (this._oTD[0] !== td[0])) {
      ...
      if (td[0].tagName === "TD" ||
        ...
        if (cellPos > 0 ||
          ...
          this._oTDAcceptDrop = this.onGridDroppableAccept(this._getCellDataDesc(
            this._oTD), this._draggedData);
        } else {
  ...
  DataGridWidgetDroppable.prototype._addDroppableStyle = function (el) {
    if (el) {
      el.addClass(this._oTDAcceptDrop === true ? ACCEPT_DROP_CLASS :
        NOACCEPT_DROP_CLASS);
    }
  };
  ...
  DataGridWidgetDroppable.prototype.onGridDroppableAccept = function (
    gridCellDesc, draggedData) {
    this.logger.warning(
      'default onGridDroppableAccept (gridCellDesc, draggedData) called... returning true'
    );
    return true;
  };

  DataGridWidgetDroppable.prototype.onGridDrop = function (gridCellDesc,
    draggedData) {
    this.logger.warning('onGridDrop --->\ngridCellDesc: ' + JSON.stringify(
