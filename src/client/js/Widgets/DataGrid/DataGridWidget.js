...
define(['logManager',
  ...
  DataGridWidget.prototype._initializeTable = function (columns) {
    ...
    if (this._rowEdit === true) {
      actionBtnColContent =
        '<i class="glyphicon glyphicon-edit pointer rowCommandBtn" data-action="' +
    ...
    if (this._rowDelete === true) {
      ...
      actionBtnColContent +=
        '<i class="glyphicon glyphicon-trash pointer rowCommandBtn" data-action="' +
    ...
    this.createColumnShowHideControl(_columns, this._groupColumns, this._actionButtonsInFirstColumn ===
      true);
  ...
  DataGridWidget.prototype._buildGroupedHeader = function (columns) {
    ...
    generateHeader = function () {
      ...
      for (i = 0; i < len; i += 1) {
        ...
        cellHtml = '<th rowspan="' + rowSpan + '" colspan="' + colSpan + '">' +
          row[i].sName + '</th>';
        rowHtml += cellHtml;
    ...
    for (i = 0; i < len; i++) {
      ...
      if (columns[i].sTitle.indexOf('.') !== 1) {
        columns[i].sTitle = columns[i].sTitle.substring(columns[i].sTitle.lastIndexOf(
          '.') + 1);
      }
    }
    processLayout(0);
        ...
        while (len--) {
          ...
          if (this._dataMap.hasOwnProperty(key)) {
            if (1 === this._oTable.fnUpdate(objects[len], this._dataMap[key])) {
              this.logger.warning("Updating object with dataMemberID '" + key +
                "' was unsuccessful");
            }
          } else {
            this.logger.warning("Can not update object with dataMemberID '" +
              key + "'. Object with this ID is not present in grid...");
          }
        }
      } else {
        this.logger.warning(
          "Cannot update grid since dataMemberID is not set. Can not match elements..."
        );
      }
    }
  };
      ...
      if (this.dataMemberID) {
        while (len--) {
          ...
          if (this._dataMap.hasOwnProperty(key)) {
            ...
            this.logger.warning("Can not delete object with dataMemberID '" +
              key +
        ...
        this.logger.warning(
          "Cannot delete objects from grid since dataMemberID is not set. Can not match elements..."
        );
      }
    }
  };
  ...
  DataGridWidget.prototype._extendColumnDefs = function () {
    ...
    while (len--) {
      ...
      if (this._noWrapColumns && this._noWrapColumns.indexOf(this._columns[
        len].mData) !== -1) {
  ...
  DataGridWidget.prototype._fnRowCallback = function (nRow, aData,
  ...
  DataGridWidget.prototype._onCellEdit = function (id, prop, oldValue,
    newValue) {
    this.onCellEdit({
      "id": id,
      "prop": prop,
  ...
  DataGridWidget.prototype.$_editSaveCancel = $(
    '<i class="glyphicon glyphicon-ok glyphicon glyphicon-ok editSave"></i> <i class="glyphicon glyphicon-remove editCancel"></i>'
  );
  ...
  DataGridWidget.prototype._createColumnShowHideControlInToolBar = function (
    columns, isColumnsGrouped, isActionButtonsInFirstColumn) {
    var i,
      len = columns.length,
      self = this;
  ...
  DataGridWidget.prototype.displayCommonColumnsOnly = function (
    commonColumnOnly) {
  ...
  DataGridWidget.prototype._fnDrawCallback = function (oSettings) {
    ...
    if (this._isInitializing === true || this._isClearing === true || this._isApplyingCommonColumnFilter ===
      true) {
      return;
    }
  ...
  DataGridWidget.prototype._enableColumnFilterCheckBox = function (index,
    enabled) {
  ...
  DataGridWidget.prototype._setCheckedColumnFilterCheckBox = function (index,
    enabled) {
    if (this._actionButtonsInFirstColumn) {
      index -= 1;
    }
  ...
  DataGridWidget.prototype._applyCommonColumnFilter = function () {
    ...
    if (this._oTable) {
      ...
      if (this._displayCommonColumnsOnly === true) {
        displayedData = this._oTable._('tr', {
          "filter": "applied"
        });
        ...
        while (len--) {
          ...
          for (prop in flattenedObj) {
            if (flattenedObj.hasOwnProperty(prop) && flattenedObj[prop] !==
              DEFAULT_NON_EXISTING_VALUE) {
  ...
  DataGridWidget.prototype.onCellEdit = function (params) {
    this.logger.warning("onCellEdit is not overridden... " + JSON.stringify(
      params));
  };

  DataGridWidget.prototype.onColumnsAutoDetected = function (columnDefs) {
    this.logger.warning("onColumnsAutoDetected is not overridden... " + JSON.stringify(
      columnDefs));
  };
  ...
  DataGridWidget.prototype.createColumnShowHideControl = function (columns,
    isColumnsGrouped, isActionButtonsInFirstColumn) {
    this._createColumnShowHideControlInToolBar(columns, isColumnsGrouped,
  ...
  DataGridWidget.prototype._initializeToolbar = function () {
    ...
    this.toolbarItems.ddColumnVisibility = toolBar.addDropDownButton({
      "text": "Columns"
    });
