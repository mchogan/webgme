...
define(['logManager',
  ...
  LogLevelManagerWidget.prototype._initializeUI = function () {
    ...
    this._dropUpMenu = new DropDownMenu({
      "dropUp": true,
      "pullRight": true,
      "size": "micro"
    });
    ...
    for (i in this._logLevelsById) {
      if (this._logLevelsById.hasOwnProperty(i)) {
        this._dropUpMenu.addItem({
          "text": this._logLevelsById[i],
  ...
  LogLevelManagerWidget.prototype._refreshButtonText = function () {
    ...
    this._dropUpMenu.setTitle(LOG_PREFIX + this._logLevelsById[
      currentLogLevel]);
    this._dropUpMenu.setColor(this._dropUpMenu.COLORS[this._statusColors[this
      ._logLevelsById[currentLogLevel]]]);
  };
