...
define(['logManager',
...
], function (logManager, CONSTANTS, TreeBrowserWidgetKeyboard, dragSource) {
        ...
        return {
          callback: function (key, options) {
            var node = $.ui.dynatree.getNode(options.$trigger),
              m = "clicked: '" + key + "' on '" + node.data.title + " (" +
              node.data.key + ")'";
            alert(m);
          },
      ...
      onDblClick: function (node, event) {
        ...
        if (editNoteTitle === true) {
          ...
          if ($.isFunction(self.onNodeDoubleClicked)) {
            self._logger.debug("default double-click handler: " + node.data
              .key);
   ...
   * @param objDescriptor
   */
  TreeBrowserWidget.prototype.createNode = function (parentNode,
    objDescriptor) {
    ...
    if (existingChildren) {
      ...
      for (i = existingChildren.length - 1; i >= 0; i -= 1) {
        if (objDescriptor.name.toLowerCase() < existingChildren[i].data.title
          .toLowerCase()) {
  ...
  TreeBrowserWidget.prototype.updateNode = function (node, objDescriptor) {
    ...
    if (objDescriptor.hasChildren === true || objDescriptor.hasChildren ===
      false) {
  ...
  TreeBrowserWidget.prototype.onNodeOpen = function (nodeId) {
    this._logger.warning("Default onNodeOpen for node " + nodeId +
      " called, doing nothing. Please override onNodeOpen(nodeId)");
  };
  ...
  TreeBrowserWidget.prototype.onNodeClose = function (nodeId) {
    this._logger.warning("Default onNodeClose for node " + nodeId +
      " called, doing nothing. Please override onNodeClose(nodeId)");
  };
   ...
   */
  TreeBrowserWidget.prototype.onNodeTitleChanged = function (nodeId, oldText,
    newText) {
    this._logger.warning("Default onNodeTitleChanged for node " + nodeId +
      " called, doing nothing. Please override onNodeTitleChanged(nodeId, oldText, newText)"
    );
    return true;
  };
  ...
  TreeBrowserWidget.prototype._onNodeMouseDown = function (node, event) {
    ...
    if (modifierKey) {
      ...
      this._logger.debug("_onNodeMouseDown: " + node.data.title +
        " --> toggleSelect");
      node.toggleSelect();
    } else {
  ...
  TreeBrowserWidget.prototype._dragHelper = function (el, event) {
    var helperEl = el.clone(),
      wrapper = $('<div class="' + TREE_BROWSER_CLASS +
        '"><ul class="dynatree-container"><li></li></ul></div>'),
      selectedIds,
      selNodes,
      i,
    ...
    if (selectedIds.length > 1) {
      ...
      helperEl.find('.dynatree-title').text(t + ' (+' + (selectedIds.length -
        1) + ')');
    }
  ...
  TreeBrowserWidget.prototype.getDragItems = function (el) {
    this._logger.warning(
      "TreeBrowserWidget.getDragItems is not overridden in the controller!!!"
    );
    return [];
  };
  ...
  TreeBrowserWidget.prototype.getDragEffects = function (el) {
    this._logger.warning(
      "TreeBrowserWidget.getDragEffects is not overridden in the controller!!!"
    );
    return [];
  };
  ...
  TreeBrowserWidget.prototype.getDragParams = function (el) {
    this._logger.debug(
      "TreeBrowserWidget.getDragParams is not overridden in the controller!!!"
    );
    return undefined;
  };
  ...
  TreeBrowserWidget.prototype._nodeEdit = function (nodeToEdit) {
    ...
    $(nodeToEdit.span).find('a').editInPlace({
      ...
      "onChange": function (oldValue, newValue) {
        self.onNodeTitleChanged.call(self, nodeToEdit.data.key, oldValue,
          newValue);
      }
    });
  };
  ...
  TreeBrowserWidget.prototype.onExtendMenuItems = function (nodeId, menuItems) {
    this._logger.debug(
      'onExtendMenuItems is not overridden for node with ID: "' + nodeId +
      '".');
  };

  TreeBrowserWidget.prototype.onCreatingContextMenu = function (nodeId,
    contextMenuOptions) {
    this._logger.debug(
      'onCreatingContextMenu is not overridden for node with ID: "' + nodeId +
      '", contextMenuOptions: ' + JSON.stringify(contextMenuOptions));
  };
