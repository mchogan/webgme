...
define(['lib/jquery/' + (DEBUG ? 'jquery.layout' : 'jquery.layout.min'),
  ...
  var DefaultLayout,
    ...
    SPACING_OPEN = WebGMEGlobal.SUPPORTS_TOUCH ? SPACING_OPEN_TOUCH :
    SPACING_OPEN_DESKTOP,
    SPACING_CLOSED = WebGMEGlobal.SUPPORTS_TOUCH ? SPACING_CLOSED_TOUCH :
    SPACING_CLOSED_DESKTOP,
    CONFIG = JSON.parse(DefaultLayoutConfigJSON),
    SIDE_PANEL_WIDTH = 202;

  DefaultLayout = function (params) {
    this._logger = (params && params.logger) || logManager.create(
      'DefaultLayout');
