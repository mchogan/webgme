...
define(['logManager'], function (logManager) {
  ...
  DecoratorManager.prototype.download = function (decorators, widget,
    fnCallBack) {
    ...
    processDecorators = function () {
      ...
      if (len > 0) {
        self._logger.debug("Remaining decorators for widget '" + widget +
          "': " + len + ", '" + queue + "'");
        ...
        self._downloadOne(decorator, function () {
          ...
          if (self._decorators[decorator]) {
            result = self._decorators[decorator].getDecoratorForWidget(
              widget);
            if (result) {
              if (_.isArray(result)) {
                i = result.len;
        ...
        self._logger.debug("No more decorators to download for widget '" +
          widget + "'");
        fnCallBack.call(self);
      }
    };

    this._logger.debug("Downloading decorators '" + decorators +
      "' for widget '" + widget + "'");
    processDecorators();
  };

  DecoratorManager.prototype.getDecoratorForWidget = function (decorator,
    widget) {
    var result;

    if (this._widgetDecoratorCache[widget] && this._widgetDecoratorCache[
      widget][decorator]) {
  ...
  DecoratorManager.prototype._lookupDecoratorForWidget = function (decorator,
    widget) {
    var result,
      widgetDecorator,
    ...
    this._logger.debug('_lookupDecoratorForWidget decorator: "' + decorator +
      '" for widget "' + widget + '"');
    ...
    processDecoratorChain = function (decoratorList) {
      ...
      while (i--) {
        ...
        if (self._decorators[fbDec]) {
          ...
          if (_.isArray(ret)) {
            self._logger.debug('recursive processDecoratorChain at: ' + fbDec +
              ' for: ' + ret);
    ...
    if (this._decorators[decorator]) {
      widgetDecorator = this._decorators[decorator].getDecoratorForWidget(
        widget);
      if (_.isArray(widgetDecorator)) {
        this._logger.debug('fallback decorator list for decorator "' +
          decorator + '" for widget "' + widget + '" is: ' + widgetDecorator);
        result = processDecoratorChain(widgetDecorator);
      } else {
        this._logger.debug('found decorator: "' + decorator +
          '" for widget "' + widget + '"');
    ...
    if (!result) {
      this._logger.debug("There is no decorator with name '" + decorator +
        "' that supports widget '" + widget + "'");
  ...
  DecoratorManager.prototype._downloadOne = function (decorator, fnCallBack) {
    ...
    if (this._decorators[decorator]) {
      this._logger.debug("Decorator '" + decorator +
        "' has already been downloaded...");
      fnCallBack();
    } else {
      require([decoratorPath],
        function (DecoratorClass) {
          self._logger.debug("Decorator '" + decorator +
          ...
          try {
            ...
            self._logger.error(
              "Error while trying to instantiate decorator '" + decorator +
              "'...");
          }
        ...
        function (err) {
          ...
          self._logger.error("Failed to download decorator '" + decorator +
            "' because of '" + err.requireType + "' with module '" + err.requireModules[
              0] + "'...");
          fnCallBack.call(self);
        });
    }
