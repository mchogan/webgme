...
define(['logManager',
  ...
  var _webGMEStart = function (afterPanelsLoaded) {
    ...
    layoutManager.loadLayout(initialThingsToDo.layoutToLoad, function () {
      ...
      client.addEventListener(client.events.BRANCH_CHANGED, function (
        __project, branchName) {
        layoutManager.setPanelReadOnly(client.isCommitReadOnly() ||
          client.isProjectReadOnly());
        WebGMEGlobal.State.registerActiveBranchName(branchName);
      });
      client.addEventListener(client.events.PROJECT_OPENED, function (
        __project, projectName) {
      ...
      client.addEventListener(client.events.PROJECT_CLOSED, function (
        __project, projectName) {
              ...
              if (err) {
                ...
                client.getAvailableProjectsAsync(function (err,
                  ...
                  if (err) {
                    ...
                    if (projectArray.indexOf(initialThingsToDo.projectToLoad) !==
                      -1) {
                      //we fallback to loading
                      client.selectProjectAsync(initialThingsToDo.projectToLoad,
                        function (err) {
                          if (err) {
                            logger.error(err);
                            openProjectLoadDialog();
                      ...
                      client.createProjectAsync(initialThingsToDo.projectToLoad,
                        function (err) {
                          if (err) {
                            logger.error(err);
                            openProjectLoadDialog();
                          } else {
                            client.selectProjectAsync(
              ...
              client.connectToDatabaseAsync({
                ...
                if (err) {
                  ...
                  if (initialThingsToDo.branchToLoad) {
                    ...
                    client.selectCommitAsync(initialThingsToDo.commitToLoad,
                      function (err) {
                        if (err) {
                          logger.error(err);
                          openProjectLoadDialog();
