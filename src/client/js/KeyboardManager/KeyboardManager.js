...
define(['logManager'], function (logManager) {
  ...
  var specialKeys = {
    ...
    'Your browser seems to be out of date :(. Please update your browser to the latest and greatest version!',
    _txtArea,
    _enabled = false,
  ...
  var _setEnabled = function (enabled) {
    ...
    if (_enabled !== enabled) {
      ...
      if (_enabled === true) {
        ...
        _txtArea.on("keydown.KeyboardManager keyup.KeyboardManager",
          _keyHandler);
  ...
  var _captureFocusOnBody = function (event) {
    ...
    var activeElementName = document.activeElement && document.activeElement
      .nodeName;
  ...
  var _keyHandler = function (event) {
    ...
    if ((/textarea|select/i.test(event.target.nodeName) || event.target.type ===
      "text")) {
    ...
    if (_listener) {
      ...
      eventArgs.character = specialKeys[event.which] || String.fromCharCode(
        event.which).toLowerCase();
