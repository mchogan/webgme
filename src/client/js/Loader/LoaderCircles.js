...
define(['css!/js/Loader/styles/LoaderCircles.css'], function () {
  ...
  LoaderCircles = function (params) {
    ...
    if (this._el.length === 0) {
      throw "LoaderCircles's container control with id:'" + params.containerElement +
        "' could not be found";
    }
  ...
  LoaderCircles.prototype._createElements = function () {
    if (!this._loaderContainer) {
      this._loaderContainer = $('<div/>', {
        "class": "loader-container"
      }).css(this._loaderContentCssProps);

      this._loaderBackground = $('<div/>', {
        "class": "loader-bg"
      }).css(this._loaderBackgroundCssProps);

      this._circleOuter = $('<div/>', {
        "class": "circle-o"
      });

      this._circleInner = $('<div/>', {
        "class": "circle-i"
      });
      ...
      if (this._size) {
        ...
        this._circleInner.css({
          ...
          "top": 0 - CIRCLE_O_SHADOW_SIZE / 3 - this._size / 2 - (this._size -
            CIRCLE_I_SIZE_DIFF) / 2 - 2 * CIRCLE_BORDER_SIZE
        });
