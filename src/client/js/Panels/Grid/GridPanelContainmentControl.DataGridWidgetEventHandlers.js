...
define(['js/NodePropertyNames',
  ...
  GridPanelContainmentControlEventHandlers.prototype.attachDataGridWidgetEventHandlers =
    function () {
      var self = this;
      ...
      this._dataGridWidget.onGridDroppableAccept = function (gridCellDesc,
        draggedData) {
  ...
  GridPanelContainmentControlEventHandlers.prototype._onCellEdit = function (
    params) {
    var gmeID = params.id,
      prop = params.prop,
  ...
  GridPanelContainmentControlEventHandlers.prototype._onColumnsAutoDetected =
    function (columnDefs) {
  ...
  GridPanelContainmentControlEventHandlers.prototype._onRowDelete = function (
    id, aData) {
    this._client.delMoreNodes([id]);
  };

  GridPanelContainmentControlEventHandlers.prototype._onRowEdit = function (
    id, oData, nData) {
  ...
  GridPanelContainmentControlEventHandlers.prototype._fetchData = function (
    object, data) {
    var a = data.split('.'),
      k = a[0];
  ...
  GridPanelContainmentControlEventHandlers.prototype._onGridDroppableAccept =
    function (gridCellDesc, draggedData) {
      if (DEBUG) {
        return true;
      }
  };
  ...
  GridPanelContainmentControlEventHandlers.prototype._onGridDrop = function (
    gridCellDesc, draggedData) {
    if (DEBUG) {
