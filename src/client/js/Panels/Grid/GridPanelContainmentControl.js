...
define(['logManager',
  ...
  GridPanelContainmentControl.prototype.selectedObjectChanged = function (
    nodeId) {
    var self = this;
    ...
    if (this._currentNodeId || this._currentNodeId === CONSTANTS.PROJECT_ROOT_ID) {
      ...
      var title = (desc.Attributes && desc.Attributes.name ? desc.Attributes.name +
        " " : "N/A ") + "(" + desc.ID + ")";
  ...
  GridPanelContainmentControl.prototype._discoverNode = function (gmeID) {
    ...
    _getPointerInfo = function (node) {
      ...
      while (len--) {
        result[availablePointers[len]] = node.getPointer(availablePointers[
          len]);
      }
    ...
    if (cNode) {
      ...
      nodeDescriptor.Attributes = _getNodePropertyValues(cNode,
        "getAttributeNames", "getAttribute");
      nodeDescriptor.Registry = _getNodePropertyValues(cNode,
        "getRegistryNames", "getRegistry");
  ...
  GridPanelContainmentControl.prototype._stateActiveObjectChanged = function (
  ...
  _.extend(GridPanelContainmentControl.prototype,
