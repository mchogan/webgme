...
define(['logManager',
  ...
  GridPanelSetsControl.prototype._processSetContainer = function () {
    ...
    if (setContainer) {
      title = (setContainer.getAttribute(nodePropertyNames.Attributes.name) ||
        'N/A') + title;
      //get set names
      //get set members
      //get set registries
      ...
      while (i--) {
        ...
        while (j--) {
          //get set registry
          memberRegistryNames = setContainer.getMemberRegistryNames(set,
            setMembers[j]);
          ...
          while (k--) {
            ...
            var keyValue = setMembers[j] + ': ' + JSON.stringify(setContainer
              .getMemberRegistry(set, setMembers[j], memberRegistryNames[k])
            );
  ...
  GridPanelSetsControl.prototype._stateActiveObjectChanged = function (model,
    activeObjectId) {
