...
define(['logManager',
  ...
  DiagramDesignerWidgetMultiTabMemberListControllerBase.prototype._stateActiveObjectChanged = function (model,
    activeObjectId) {
      ...
      if (orderedMemberListInfo.length > 0) {
        ...
        for (i = 0; i < orderedMemberListInfo.length; i += 1) {
          ...
          while (j--) {
            ...
            this._memberListMemberCoordinates[memberListID][gmeID] = memberListContainerObj.getMemberRegistry(
              memberListID, gmeID, MEMBER_POSITION_REGISTRY_KEY);
          }
  ...
  DiagramDesignerWidgetMultiTabMemberListControllerBase.prototype.getOrderedMemberListInfo = function (
    ...
    if (params &&
      ...
      for (i in params.positions) {
        if (params.positions.hasOwnProperty(i)) {
          ...
          _client.setMemberRegistry(memberListContainerID, i, memberListToAddTo, MEMBER_POSITION_REGISTRY_KEY, {
            'x': posX,
            'y': posY
          });
      ...
      if (gmeIDList.length > 0) {
        for (i = 0; i < gmeIDList.length; i += 1) {
          ...
          if (this._memberListMembers[memberListToAddTo].indexOf(componentID) === -1) {
            ...
            _client.setMemberRegistry(memberListContainerID, componentID, memberListToAddTo,
  ...
  DiagramDesignerWidgetMultiTabMemberListControllerBase.prototype._initializeSelectedMemberList = function () {
    ...
    if (this._selectedMemberListID && this._memberListMembers[this._selectedMemberListID]) {
      ...
      while (len--) {
        ...
        this._selectedMemberListMembersTerritoryPatterns[this._memberListMembers[this._selectedMemberListID][len]] = {
          'children': 0
        };
      }
    }
  ...
  DiagramDesignerWidgetMultiTabMemberListControllerBase.prototype._updateDecoratorTerritoryQuery = function (
    decorator, doDelete) {
    var query,
      entry,
      territoryPatterns = this._selectedMemberListMembersTerritoryPatterns,
  ...
  DiagramDesignerWidgetMultiTabMemberListControllerBase.prototype._onLoad = function (gmeID, desc) {
    ...
    if (this._selectedMemberListMembers.indexOf(gmeID) !== -1) {
      ...
      if (desc.isConnection === false) {
        ...
        territoryChanged = territoryChanged || this._updateDecoratorTerritoryQuery(uiComponent._decoratorInstance,
          false);
      } else {
    ...
    if (this._GMEID2ComponentID.hasOwnProperty(gmeID)) {
      ...
      while (len--) {
        ...
        if (this._widget.itemIds.indexOf(componentID) !== -1) {
          territoryChanged = territoryChanged || this._updateDecoratorTerritoryQuery(this._widget.items[componentID]._decoratorInstance,
            true);
        }
  ...
  DiagramDesignerWidgetMultiTabMemberListControllerBase.prototype.registerComponentIDForPartID = function (
    componentID, partId) {
  ...
  DiagramDesignerWidgetMultiTabMemberListControllerBase.prototype.unregisterComponentIDFromPartID = function (
    componentID, partId) {
    var idx;
