...
define(['logManager',
  ...
  _.extend(CrosscutController.prototype,
  ...
  CrosscutController.prototype._updateSelectedMemberListMembersTerritoryPatterns =
    function () {
  ...
  CrosscutController.prototype._memberListTerritoryCallback = function (
    events) {
      ...
      if ((events[len].etype === CONSTANTS.TERRITORY_EVENT_LOAD) || (events[
        ...
        if (obj) {
          ...
          if (events[len].desc.isConnection === true) {
            ...
            events[len].desc.decoratorParams = {
              'displayName': false
            };
          } else {
  ...
  CrosscutController.prototype.getOrderedMemberListInfo = function (
  ...
  CrosscutController.prototype.displayNoTabMessage = function () {
    this._widget.setBackgroundText(
      'No crosscuts defined yet. Press the + button in the top-left corner to create one...'
    );
  };
  ...
  /**********************************************************/
  CrosscutController.prototype._onBackgroundDroppableAccept = function (event,
    dragInfo) {
    var gmeIDList = DragHelper.getDragItems(dragInfo),
      accept = DiagramDesignerWidgetMultiTabMemberListControllerBase.prototype
    ...
    if (accept === true) {
      ...
      accept = GMEConcepts.isValidChildrenTypeInCrossCut(this._memberListContainerID,
        gmeIDList);
    }
  ...
  CrosscutController.prototype._onLoad = function (gmeID, desc) {
    ...
    territoryChanged = DiagramDesignerWidgetMultiTabMemberListControllerBase.prototype
      ._onLoad.call(this, gmeID, desc);
  ...
  CrosscutController.prototype._onUnload = function (gmeID) {
    ...
    while (len--) {
      ...
      this._saveConnectionToWaitingList(this._connectionListByID[connectionID]
        .GMESrcId, this._connectionListByID[connectionID].GMEDstId, this._connectionListByID[
          connectionID].type, this._connectionListByID[connectionID].connTexts
      );
  ...
  CrosscutController.prototype._processNodePointers = function (gmeID, isSet) {
    ...
    if (!isSet) {
      ...
      if (typeof pointerTo === 'string') {
        combinedName = gmeID + "_" + CONSTANTS.RELATION_CONTAINMENT + "_" +
          pointerTo;
  ...
  CrosscutController.prototype._processConnectionWaitingList = function (
    gmeID) {
    var len,
      gmeSrcID,
      gmeDstID,
  ...
  /****************************************************************************/
  CrosscutController.prototype._createConnection = function (gmeSrcId,
    ...
    if (this._GMEID2ComponentID[gmeSrcId] && this._GMEID2ComponentID[gmeSrcId]
      .length > 0 &&
      this._GMEID2ComponentID[gmeDstId] && this._GMEID2ComponentID[gmeDstId].length >
      0) {
  ...
  CrosscutController.prototype._saveConnectionToWaitingList = function (
    gmeSrcId, gmeDstId, connType, connTexts) {
    var scrDisplayed = this._GMEID2ComponentID[gmeSrcId] && this._GMEID2ComponentID[
        gmeSrcId].length > 0,
      dstDisplayed = this._GMEID2ComponentID[gmeDstId] && this._GMEID2ComponentID[
        gmeDstId].length > 0;
  ...
  CrosscutController.prototype._saveConnection = function (gmeSrcId, gmeDstId,
  ...
  /****************************************************************************/
  CrosscutController.prototype._removeConnection = function (gmeSrcId,
  ...
  CrosscutController.prototype._onSelectionDelete = function (idList) {
    ...
    while (len--) {
      ...
      if (gmeID) {
        ...
        logger.debug('removeMember memberListContainerID: ' +
          memberListContainerID + ', gmeID: ' + gmeID +
        ...
        if (GMEConcepts.isConnection(gmeID) && gmeObj.getParentId() ===
          memberListContainerID) {
          if (GMEConcepts.canDeleteNode(gmeID)) {
            logger.debug(
              'deleting connection from crosscut hierarchy too gmeID: ' +
              gmeID);
        ...
        if (lineDesc) {
          ...
          if (canDeletePointer) {
            logger.debug('deleting pointer from: ' + lineDesc.GMESrcId +
              ', type: ' + lineDesc.name);
            //it's a pointer that's allowed to be deleted
            _client.delPointer(lineDesc.GMESrcId, lineDesc.name);
          } else {
            logger.warning('can not delete pointer from: ' + lineDesc.GMESrcId +
              ', type: ' + lineDesc.name);
          }
        }
      }
  ...
  CrosscutController.prototype._onFilterNewConnectionDroppableEnds = function (
    params) {
    ...
    //check #1:
    validPointerTargetTypes = GMEConcepts.getValidPointerTargetTypesFromSource(
      sourceId, false);

    //check #2:
    validSetTargetTypes = GMEConcepts.getValidPointerTargetTypesFromSource(
      sourceId, true);

    //check #3:
    validConnectionTypes = GMEConcepts.getValidConnectionTypesFromSourceInAspect(
      sourceId, parentId, CONSTANTS.ASPECT_ALL);
    validConnectionEndTypes = [];
    ...
    while (i--) {
      if (GMEConcepts.canCreateChild(parentId, validConnectionTypes[i])) {
        pointerMetaDescriptor = this._client.getValidTargetItems(
          validConnectionTypes[i], CONSTANTS.POINTER_TARGET);
        if (pointerMetaDescriptor && pointerMetaDescriptor.length > 0) {
          j = pointerMetaDescriptor.length;
          while (j--) {
            if (validConnectionEndTypes.indexOf(pointerMetaDescriptor[j].id) ===
              -1) {
    ...
    validEndTypes = _.union(validPointerTargetTypes, validSetTargetTypes,
      validConnectionEndTypes);
  ...
  CrosscutController.prototype._onCreateNewConnection = function (params) {
    ...
    createPointer = function (srcId, dstId, ptrName) {
      logger.debug('createPointer srcId: ' + srcId + ', dstId: ' + dstId +
        ', ptrName: ' + ptrName);
      _client.makePointer(srcId, ptrName, dstId);
    };

    addToSet = function (containerId, objId, setName) {
      logger.debug('addToSet containerId: ' + containerId + ', objId: ' +
        objId + ', ptrName: ' + setName);
      _client.addMember(containerId, objId, setName);
    };

    createConnection = function (srcId, dstId, connType) {
      logger.debug('createConnection srcId: ' + srcId + ', dstId: ' + dstId +
        ', connType: ' + connType);
      _client.startTransaction();

      //create new object
      var newConnID = _client.createChild({
        'parentId': parentId,
        'baseId': connType
      });
      ...
      _client.setMemberRegistry(parentId, newConnID, aspect, REGISTRY_KEYS.POSITION, {
        'x': dx,
        'y': dy
      });
    ...
    //#1: pointers
    var validPointerTypes = GMEConcepts.getValidPointerTypesFromSourceToTarget(
      sourceId, targetId);

    //#2: sets
    var validSetTypes = GMEConcepts.getValidSetTypesFromContainerToMember(
      sourceId, targetId);

    //#3: WebGME-connections
    var validConnectionTypes = GMEConcepts.getValidConnectionTypesInAspect(
      sourceId, targetId, parentId, CONSTANTS.ASPECT_ALL);

    //if there is any option at all
    if (validPointerTypes.length + validSetTypes.length +
      validConnectionTypes.length > 0) {
      //if only 1 option, figure out automatically
      if (validPointerTypes.length + validSetTypes.length +
        validConnectionTypes.length === 1) {
        ...
        var sourceObjName = sourceObj.getAttribute(nodePropertyNames.Attributes
          .name);
        var targetObjName = targetObj.getAttribute(nodePropertyNames.Attributes
          .name);

        //'Create pointer'
        for (i = 0; i < validPointerTypes.length; i += 1) {
          menuItems[ptrAction + validPointerTypes[i]] = {
            "name": 'Create pointer \'' + validPointerTypes[i] + '\' from \'' +
              sourceObjName + '\' to \'' + targetObjName + '\'',
        ...
        for (i = 0; i < validSetTypes.length; i += 1) {
          menuItems[setAction + validSetTypes[i]] = {
            "name": 'Add \'' + targetObjName + '\' to set \'' + validSetTypes[
              i] + '\' of \'' + sourceObjName + '\'',
            "icon": false,
            "action": setAction,
        ...
        for (i = 0; i < validConnectionTypes.length; i += 1) {
          ...
          menuItems[connectionAction + validConnectionTypes[i]] = {
            "name": 'Create connection \'' + connObjName + '\' from \'' +
              sourceObjName + '\' to \'' + targetObjName + '\'',
  ...
  CrosscutController.prototype._attachClientEventListeners = function () {
    DiagramDesignerWidgetMultiTabMemberListControllerBase.prototype._attachClientEventListeners
      .call(this);
    WebGMEGlobal.State.on('change:' + CONSTANTS.STATE_ACTIVE_CROSSCUT, this._stateActiveCrosscutChanged,
      this);
  };
  ...
  CrosscutController.prototype._stateActiveCrosscutChanged = function (model,
    activeCrosscutId) {
    ...
    WebGMEGlobal.State.unset(CONSTANTS.STATE_ACTIVE_CROSSCUT, {
      'silent': true
    });
    if (tabToSelect) {
