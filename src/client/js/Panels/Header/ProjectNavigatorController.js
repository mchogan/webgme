...
define([
  ...
  ProjectNavigatorController.prototype.initialize = function () {
    ...
    if (self.gmeClient) {
      ...
      newProject = function (data) {
        self.dummyProjectsGenerator('New Project ' + Math.floor(Math.random() *
          10000), 4);
      };
    }
  ...
  ProjectNavigatorController.prototype.initWithClient = function () {
    ...
    self.gmeClient.addEventListener(self.gmeClient.events.NETWORKSTATUS_CHANGED,
      function (client) {
        if (self.gmeClient.getActualNetworkStatus() === self.gmeClient.networkStates
          .CONNECTED) {
          // get project list
          self.updateProjectList();
        } else {
          // get project list
          console.warn(self.gmeClient.getActualNetworkStatus() +
            " network status is not handled yet.");
        }

      });

    self.gmeClient.addEventListener(self.gmeClient.events.PROJECT_OPENED,
    ...
    self.gmeClient.addEventListener(self.gmeClient.events.PROJECT_CLOSED,
      function (client, projectId) {
        //            console.log(self.gmeClient.events.PROJECT_CLOSED, projectId);
        self.selectProject({});
      });

    self.gmeClient.addEventListener(self.gmeClient.events.SERVER_PROJECT_CREATED,
      function (client, projectId) {
        //            console.log(self.gmeClient.events.SERVER_PROJECT_CREATED, projectId);
        self.addProject(projectId);
      });

    self.gmeClient.addEventListener(self.gmeClient.events.SERVER_PROJECT_DELETED,
      function (client, projectId) {
        //            console.log(self.gmeClient.events.SERVER_PROJECT_DELETED, projectId);
        self.removeProject(projectId);
      });

    self.gmeClient.addEventListener(self.gmeClient.events.BRANCH_CHANGED,
      function (client, branchId) {
        //            console.log(self.gmeClient.events.BRANCH_CHANGED, branchId);
        self.selectBranch({
          projectId: self.gmeClient.getActiveProjectName(),
          branchId: branchId
        });
      });

    self.gmeClient.addEventListener(self.gmeClient.events.SERVER_BRANCH_CREATED,
    ...
    self.gmeClient.addEventListener(self.gmeClient.events.SERVER_BRANCH_UPDATED,
      function (client, parameters) {
        ...
        if (currentProject === parameters.project && currentBranch ===
          parameters.branch) {
    ...
    self.gmeClient.addEventListener(self.gmeClient.events.SERVER_BRANCH_DELETED,
      function (client, parameters) {
        ...
        if (projectList.hasOwnProperty(projectId)) {
          ...
          for (branchId in projectList[projectId].branches) {
            if (projectList[projectId].branches.hasOwnProperty(branchId)) {
              self.addBranch(projectId, branchId, projectList[projectId].branches[
                branchId]);
            }
          }
        }
  ...
  ProjectNavigatorController.prototype.addProject = function (projectId,
    ...
    if (self.gmeClient) {
      ...
      showAllBranches = function (data) {
        console.error(
          'showAllBranches: gmeClient version is not implemented yet.', data);
      };
    } else {
      // test version
      showHistory = function (data) {
    ...
    for (i = 0; i < self.root.menu.length; i += 1) {
      ...
      if (self.root.menu[i].id === 'projects') {
        ...
        self.root.menu[i].items = self.mapToArray(self.projects, ['name',
          'id'
        ]);
        break;
      }
  ...
  ProjectNavigatorController.prototype.addBranch = function (projectId,
    branchId, branchInfo) {
    var self = this,
      i,
      selectBranch,
    ...
    if (self.gmeClient) {
      ...
      createBranch = function (data) {
        ...
        self.gmeClient.createGenericBranchAsync(data.projectId, newBranchName,
          data.branchInfo, function (err) {
            if (err) {
              console.error(err);
              return;
            }
      ...
      deleteBranch = function (data) {
        self.gmeClient.deleteGenericBranchAsync(data.projectId, data.branchId,
          data.branchInfo, function (err) {
            if (err) {
              console.error(err);
              return;
    ...
    for (i = 0; i < self.projects[projectId].menu.length; i += 1) {
      ...
      if (self.projects[projectId].menu[i].id === 'branches') {
        ...
        self.projects[projectId].menu[i].items = self.mapToArray(self.projects[
          projectId].branches, ['name', 'id']);
        break;
      }
    }
  ...
  ProjectNavigatorController.prototype.removeProject = function (projectId,
    callback) {
    var self = this,
      i;
  ...
  ProjectNavigatorController.prototype.removeBranch = function (projectId,
    branchId) {
    var self = this,
      i;

    if (self.projects.hasOwnProperty(projectId) && self.projects[projectId].branches
      .hasOwnProperty(branchId)) {
  ...
  ProjectNavigatorController.prototype.selectBranch = function (data,
    ...
    if (projectId || projectId === '') {
      ...
      self.$scope.navigator.items[self.navIdProject] = self.projects[
        projectId];
      ...
      if (branchId || branchId === '') {
        ...
        if (self.projects[projectId].branches.hasOwnProperty(branchId) ===
          false) {
          console.error(projectId + ' - ' + branchId +
            ' branch does not exist in the navigation bar');
          self.requestedSelection = data;
          return;
        }

        // set selected branch
        self.$scope.navigator.items[self.navIdBranch] = self.projects[
  ...
  ProjectNavigatorController.prototype.dummyProjectsGenerator = function (
    name, maxCount) {
    var self = this,
      i,
      id,
  ...
  ProjectNavigatorController.prototype.dummyBranchGenerator = function (name,
    maxCount, projectId) {
    var self = this,
      i,
      id,
