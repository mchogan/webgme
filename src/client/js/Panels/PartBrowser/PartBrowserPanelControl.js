...
define(['logManager',
  ...
  PartBrowserControl = function (myClient, myPartBrowserView) {
    ...
    METAAspectHelper.addEventListener(METAAspectHelper.events.META_ASPECT_CHANGED,
      function () {
    ...
    WebGMEGlobal.State.on('change:' + CONSTANTS.STATE_ACTIVE_ASPECT, function (
      model, activeAspect) {
    ...
    if (this._containerNodeId || this._containerNodeId === CONSTANTS.PROJECT_ROOT_ID) {
      ...
      if (this._aspect !== CONSTANTS.ASPECT_ALL) {
        ...
        if (aspectNames.indexOf(this._aspect) === -1) {
          this._logger.warning('The currently selected aspect "' + this._aspect +
            '" does not exist in the object "' + nodeId +
            '", falling back to "All"');
          this._aspect = CONSTANTS.ASPECT_ALL;
        }
      }
      ...
      this._logger.debug('UPDATING TERRITORY: selectedObjectChanged' + JSON.stringify(
        this._selfPatterns));
  ...
  PartBrowserControl.prototype._eventCallback = function (events) {
    ...
    this._logger.debug("_eventCallback '" + i + "' items, events: " + JSON.stringify(
      events));
  ...
  PartBrowserControl.prototype._processContainerNode = function (gmeID) {
    ...
    if (node) {
      ...
      validChildrenTypes = GMEConcepts.getMETAAspectMergedValidChildrenTypes(
        gmeID);
  ...
  PartBrowserControl.prototype._doUpdateTerritory = function (async) {
    ...
    if (async === true) {
      setTimeout(function () {
        logger.debug('Updating territory with rules: ' + JSON.stringify(
          patterns));
  ...
  PartBrowserControl.prototype.registerComponentIDForPartID = function (
    componentID, partId) {
  ...
  PartBrowserControl.prototype.unregisterComponentIDFromPartID = function (
    componentID, partId) {
    var idx;
  ...
  PartBrowserControl.prototype._updateValidChildrenTypeDecorators = function () {
    ...
    if (decorators.length > 0) {
      this._client.decoratorManager.download(decorators, WIDGET_NAME,
        function () {
  ...
  PartBrowserControl.prototype._refreshPartList = function () {
    ...
    this._logger.debug('_refreshPartList this._validChildrenTypeIDs: ' + this
      ._validChildrenTypeIDs);
    ...
    //set aspect types
    if (this._aspect !== CONSTANTS.ASPECT_ALL) {
      var metaAspectDesc = this._client.getMetaAspect(this._containerNodeId,
        this._aspect);
      if (metaAspectDesc) {
    ...
    this._logger.debug('_refreshPartList childrenTypeToDisplay: ' +
      childrenTypeToDisplay);
    ...
    while (i--) {
      ...
      while (j--) {
        ...
        decoratorInstance = this._partBrowserView.addPart(id, this._getPartDescriptor(
          id));
