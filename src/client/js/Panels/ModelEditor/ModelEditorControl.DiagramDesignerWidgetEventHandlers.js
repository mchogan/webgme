...
define(['logManager',
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onDesignerItemsMove = function (repositionDesc) {
    ...
    for (id in repositionDesc) {
      if (repositionDesc.hasOwnProperty(id)) {
        this._client.setRegistry(this._ComponentID2GmeID[id], REGISTRY_KEYS.POSITION, {
    ...
    if (validConnectionTypes.length === 1) {
      ...
      for (i = 0; i < validConnectionTypes.length; i += 1) {
        ...
        menuItems[validConnectionTypes[i]] = {
          'name': 'Create type \'' + (connTypeObj ? connTypeObj.getAttribute(nodePropertyNames.Attributes.name) :
            validConnectionTypes[i]) + '\'',
          'icon': false
        };
      }
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._saveReposition = function (items, dragPositions,
    ...
    while (i--) {
      ...
      if (selectedAspect === CONSTANTS.ASPECT_ALL) {
        ...
        client.setMemberRegistry(modelID, gmeID, selectedAspect, REGISTRY_KEYS.POSITION, {
          'x': dropPosition.x + oldPos.x,
          'y': dropPosition.y + oldPos.y
        });
      }
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onFilterNewConnectionDroppableEnds = function (
    params) {
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onFilterReconnectionDroppableEnds = function (
    params) {
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onDragStartDesignerConnectionCopyable = function (
    connectionID) {
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSelectionFillColorChanged = function (
    selectedElements, color) {
    this._onSelectionSetColor(selectedElements, color, REGISTRY_KEYS.COLOR);
  };

  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSelectionBorderColorChanged = function (
    selectedElements, color) {
    this._onSelectionSetColor(selectedElements, color, REGISTRY_KEYS.BORDER_COLOR);
  };

  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSelectionTextColorChanged = function (
    selectedElements, color) {
