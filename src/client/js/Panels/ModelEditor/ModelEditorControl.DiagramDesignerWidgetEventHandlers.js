...
define(['logManager',
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype.attachDiagramDesignerWidgetEventHandlers =
    function () {
      var self = this;
      ...
      this.designerCanvas.onRegisterSubcomponent = function (objID, sCompID,
        metaInfo) {
      ...
      this.designerCanvas.onBackgroundDroppableAccept = function (event,
        dragInfo) {
        return self._onBackgroundDroppableAccept(event, dragInfo);
      };

      this.designerCanvas.onBackgroundDrop = function (event, dragInfo,
        position) {
      ...
      this.designerCanvas.onFilterNewConnectionDroppableEnds = function (
        params) {
        return self._onFilterNewConnectionDroppableEnds(params);
      };

      this.designerCanvas.onFilterReconnectionDroppableEnds = function (
        params) {
      ...
      this.designerCanvas.onDragStartDesignerConnectionCopyable = function (
        connectionID) {
      ...
      this.designerCanvas.onSelectionContextMenu = function (selectedIds,
        mousePos) {
        self._onSelectionContextMenu(selectedIds, mousePos);
      };

      this.designerCanvas.onSelectionFillColorChanged = function (
        selectedElements, color) {
        self._onSelectionFillColorChanged(selectedElements, color);
      };

      this.designerCanvas.onSelectionBorderColorChanged = function (
        selectedElements, color) {
        self._onSelectionBorderColorChanged(selectedElements, color);
      };

      this.designerCanvas.onSelectionTextColorChanged = function (
        selectedElements, color) {
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onDesignerItemsMove =
    function (repositionDesc) {
      ...
      for (id in repositionDesc) {
        if (repositionDesc.hasOwnProperty(id)) {
          this._client.setRegistry(this._ComponentID2GmeID[id], REGISTRY_KEYS
            .POSITION, {
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onDesignerItemsCopy =
    function (copyDesc) {
      var copyOpts = {
      ...
      for (id in copyDesc.items) {
        if (copyDesc.items.hasOwnProperty(id)) {
          ...
          copyOpts[gmeID][REGISTRY_STRING][REGISTRY_KEYS.POSITION] = {
            "x": desc.posX,
            "y": desc.posY
          };
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onCreateNewConnection =
    function (params) {
      var sourceId,
        targetId,
        parentId = this.currentNodeInfo.id,
      ...
      var validConnectionTypes = GMEConcepts.getValidConnectionTypesInAspect(
        sourceId, targetId, parentId, aspect);
      ...
      if (validConnectionTypes.length === 1) {
        ...
        for (i = 0; i < validConnectionTypes.length; i += 1) {
          ...
          menuItems[validConnectionTypes[i]] = {
            "name": 'Create type \'' + (connTypeObj ? connTypeObj.getAttribute(
              nodePropertyNames.Attributes.name) : validConnectionTypes[i]) +
              '\'',
            "icon": false
          };
        }
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSelectionDelete =
    function (idList) {
      var objIdList = [],
        i = idList.length,
        objID;
      ...
      while (i--) {
        ...
        if (GMEConcepts.canDeleteNode(objID)) {
          ...
          this.logger.warning('Can not delete item with ID: ' + objID +
            '. Possibly it is the ROOT or FCO');
        }
      }
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onDesignerItemDoubleClick =
    function (id, event) {
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onModifyConnectionEnd =
    function (params) {
      ...
      if (gmeID) {
        ...
        if (oldDesc.srcObjId !== newDesc.srcObjId ||
          ...
          if (newDesc.srcSubCompId !== undefined) {
            newEndPointGMEID = this._Subcomponent2GMEID[newDesc.srcObjId][
              newDesc.srcSubCompId
            ];
          } else {
        ...
        if (oldDesc.dstObjId !== newDesc.dstObjId ||
          ...
          if (newDesc.dstSubCompId !== undefined) {
            newEndPointGMEID = this._Subcomponent2GMEID[newDesc.dstObjId][
              newDesc.dstSubCompId
            ];
          } else {
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onRegisterSubcomponent =
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onUnregisterSubcomponent =
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._getPossibleDropActions =
    function (dragInfo) {
      ...
      if (items.length > 0) {
        ...
        while (i--) {
          ...
          case DragHelper.DRAG_EFFECTS.DRAG_CREATE_POINTER:
            if (items.length === 1) {
              validPointerTypes = GMEConcepts.getValidPointerTypes(parentID,
                items[0]);
      ...
      this.logger.debug('possibleDropActions: ' + JSON.stringify(
        possibleDropActions));

      return possibleDropActions;
  };

  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onBackgroundDroppableAccept =
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onBackgroundDrop =
    function (event, dragInfo, position) {
      ...
      if (len === 1) {
        ...
        for (i = 0; i < possibleDropActions.length; i += 1) {
          ...
          case DragHelper.DRAG_EFFECTS.DRAG_CREATE_POINTER:
            menuItems[i] = {
              "name": "Create pointer '" + possibleDropActions[i].pointer +
                "' of type '" + possibleDropActions[i].name + "'",
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._handleDropAction =
    function (dropAction, dragInfo, position) {
      ...
      case DragHelper.DRAG_EFFECTS.DRAG_COPY:
        ...
        while (i--) {
          ...
          oldPos = dragParams && dragParams.positions[gmeID] || {
            'x': 0,
            'y': 0
          };
      ...
      case DragHelper.DRAG_EFFECTS.DRAG_CREATE_INSTANCE:
        ...
        i = items.length;
        while (i--) {
          oldPos = dragParams && dragParams.positions[items[i]] || {
            'x': 0,
            'y': 0
          };
          params[items[i]] = {
      ...
      case DragHelper.DRAG_EFFECTS.DRAG_CREATE_POINTER:
        ...
        if (gmeID) {
          ...
          oldPos = dragParams && dragParams.positions[items[0]] || {
            'x': 0,
            'y': 0
          };
          //store new position
          ...
          if (origNode) {
            var ptrName = origNode.getAttribute(nodePropertyNames.Attributes.name) +
              "-" + dropAction.pointer;
            this._client.setAttributes(gmeID, nodePropertyNames.Attributes.name,
              ptrName);
          }
        }
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._repositionItems =
    function (items, dragPositions, dropPosition) {
      var i = items.length,
        oldPos,
        componentID,
        gmeID,
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._saveReposition =
    function (items, dragPositions, dropPosition) {
      var gmeID,
        oldPos,
        i,
      ...
      while (i--) {
        ...
        if (selectedAspect === CONSTANTS.ASPECT_ALL) {
          client.setRegistry(gmeID, REGISTRY_KEYS.POSITION, {
            "x": dropPosition.x + oldPos.x,
            "y": dropPosition.y + oldPos.y
          });
        } else {
          client.addMember(modelID, gmeID, selectedAspect);
          client.setMemberRegistry(modelID, gmeID, selectedAspect,
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSelectionChanged =
    function (selectedIds) {
      ...
      while (len--) {
        ...
        if (id) {
          ...
          onlyConnectionSelected = onlyConnectionSelected && GMEConcepts.isConnectionType(
            id);
        }
      }

      this.designerCanvas.toolbarItems.ddbtnConnectionArrowStart.enabled(
        onlyConnectionSelected);
      this.designerCanvas.toolbarItems.ddbtnConnectionPattern.enabled(
        onlyConnectionSelected);
      this.designerCanvas.toolbarItems.ddbtnConnectionArrowEnd.enabled(
        onlyConnectionSelected);
      this.designerCanvas.toolbarItems.ddbtnConnectionLineType.enabled(
        onlyConnectionSelected);
      this.designerCanvas.toolbarItems.ddbtnConnectionLineWidth.enabled(
        onlyConnectionSelected);
      ...
      if (gmeIDs.length === 0 && (this.currentNodeInfo.id || this.currentNodeInfo
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onClipboardCopy =
    function (selectedIds) {
      var gmeIDs = [],
        len = selectedIds.length,
        id;
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onClipboardPaste =
    function () {
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onConnectionSegmentPointsChange =
    function (params) {
      ...
      if (gmeID) {
        ...
        if (nodeObj) {
          this._client.setRegistry(gmeID, REGISTRY_KEYS.LINE_CUSTOM_POINTS,
            points);
        }
      }
  };

  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onFilterNewConnectionDroppableEnds =
    function (params) {
      ...
      while (i--) {
        ...
        while (j--) {
          if (client.isValidTarget(validConnectionTypes[j], CONSTANTS.POINTER_TARGET,
            targetId)) {
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onFilterReconnectionDroppableEnds =
    function (params) {
      ...
      if (srcDragged === true) {
        ...
        while (i--) {
          ...
          if (srcSubCompID !== undefined) {
            newEndPointGMEID = this._Subcomponent2GMEID[srcItemID][
              srcSubCompID
            ];
          } else {
          ...
          if (GMEConcepts.isValidConnection(newEndPointGMEID,
            oldEndPointGMEID, connectionGMEID) === true) {
        ...
        while (i--) {
          ...
          if (dstSubCompID !== undefined) {
            newEndPointGMEID = this._Subcomponent2GMEID[dstItemID][
              dstSubCompID
            ];
          } else {
            newEndPointGMEID = this._ComponentID2GmeID[dstItemID];
          }
          if (GMEConcepts.isValidConnection(oldEndPointGMEID,
            newEndPointGMEID, connectionGMEID) === true) {
            result.push(availableConnectionEnds[i]);
          }
        }
      }
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onDragStartDesignerItemDraggable =
    function (itemID) {
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onDragStartDesignerItemCopyable =
    function (itemID) {
      var nodeObj = this._client.getNode(this._ComponentID2GmeID[itemID]),
        result = true;
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onDragStartDesignerConnectionCopyable =
    function (connectionID) {
      return this._onDragStartDesignerItemCopyable(connectionID);
  };

  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSelectionRotated =
    function (degree, selectedIds) {
      ...
      while (i--) {
        ...
        regDegree = this._client.getNode(gmeID).getEditableRegistry(
          REGISTRY_KEYS.ROTATION);
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSetConnectionProperty =
    function (params) {
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onCopy =
    function () {
      var res = [],
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onPaste =
    function (data) {
      var len,
        objDesc,
        parentID = this.currentNodeInfo.id,
      ...
      if (parentID) {
        ...
        if (data && data.project && data.items) {
          if (projectName !== data.project) {
            alert('Trying to copy from project \'' + data.project +
            ...
            if (_.isArray(data.items)) {
              ...
              if (GMEConcepts.canCreateChildrenInAspect(parentID, childrenIDs,
                aspect)) {
                this._client.startTransaction();
                this._client.copyMoreNodes(params);
                this._client.completeTransaction();
                this.logger.warning('Pasted ' + childrenIDs.length +
                  ' items successfully into node (' + parentID + ')');
              } else {
                this.logger.warning(
                  'Can not paste items because not all the items on the clipboard can be created as a child of the currently opened node (' +
                  parentID + ')');
              }
            }
          }
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._getDragItems =
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._getDragParams =
    function (selectedElements, event) {
      var oParams = this._oGetDragParams.call(this.designerCanvas,
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSelectionContextMenu =
    function (selectedIds, mousePos) {
      var menuItems = {},
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._exIntConf =
    function (selectedIds) {
      var i = selectedIds.length,
        gmeIDs = [];
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._expLib =
    function (selectedIds) {
      var i = selectedIds.length,
        gmeIDs = [],
        id;
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._updLib =
    function (selectedIds) {
      var i = selectedIds.length,
        gmeIDs = [],
        id;
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSelectionFillColorChanged =
    function (selectedElements, color) {
      this._onSelectionSetColor(selectedElements, color, REGISTRY_KEYS.COLOR);
  };

  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSelectionBorderColorChanged =
    function (selectedElements, color) {
      this._onSelectionSetColor(selectedElements, color, REGISTRY_KEYS.BORDER_COLOR);
  };

  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSelectionTextColorChanged =
    function (selectedElements, color) {
      this._onSelectionSetColor(selectedElements, color, REGISTRY_KEYS.TEXT_COLOR);
  };

  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSelectionSetColor =
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSelectedTabChanged =
    function (tabID) {
      if (this._aspects[tabID] && this._selectedAspect !== this._aspects[
        tabID]) {
