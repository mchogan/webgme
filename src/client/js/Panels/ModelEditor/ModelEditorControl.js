...
define(['logManager',
  ...
  ModelEditorControl = function (options) {
    ...
    if (this.designerCanvas === undefined) {
      this.logger.error(
        "ModelEditorControl's DesignerCanvas is not specified...");
      throw ("ModelEditorControl can not be created");
    }
  ...
  ModelEditorControl.prototype.selectedObjectChanged = function (nodeId) {
    ...
    if (nodeId || nodeId === CONSTANTS.PROJECT_ROOT_ID) {
      ...
      if (this._selectedAspect === CONSTANTS.ASPECT_ALL) {
        ...
        this._selfPatterns[nodeId] = this._client.getAspectTerritoryPattern(
          nodeId, this._selectedAspect);
    ...
    if (nodeObj) {
      ...
      if (nodeId !== this.currentNodeInfo.id) {
        ...
        if (GMEConcepts.isConnection(nodeId)) {
          ...
          _.extend(objDescriptor, GMEVisualConcepts.getConnectionVisualProperties(
            nodeId));
          ...
          if (customPoints && _.isArray(customPoints)) {
            objDescriptor[CONSTANTS.LINE_STYLE.CUSTOM_POINTS] = $.extend(true, [],
              customPoints); //JSON.parse(JSON.stringify(customPoints));
          }
        } else {
          ...
          if (this._selectedAspect === CONSTANTS.ASPECT_ALL) {
            ...
            memberListContainerObj = this._client.getNode(this.currentNodeInfo
              .id);
            pos = memberListContainerObj.getMemberRegistry(this._selectedAspect,
              nodeId, REGISTRY_KEYS.POSITION) || nodeObj.getRegistry(
          ...
          if (objDescriptor.position.hasOwnProperty("x")) {
            objDescriptor.position.x = this._getDefaultValueForNumber(
          ...
          if (objDescriptor.position.hasOwnProperty("y")) {
            objDescriptor.position.y = this._getDefaultValueForNumber(
          ...
          objDescriptor.decorator = nodeObj.getRegistry(REGISTRY_KEYS.DECORATOR) ||
            "";
          objDescriptor.rotation = parseInt(nodeObj.getRegistry(REGISTRY_KEYS
            .ROTATION), 10) || 0;
        }
      }
    }
  ...
  ModelEditorControl.prototype._getDefaultValueForNumber = function (cValue,
    defaultValue) {
    ...
    if (len > 0) {
      ...
      while (len--) {
        if ((nextBatchInQueue[len].etype === CONSTANTS.TERRITORY_EVENT_LOAD) ||
          ...
          nextBatchInQueue[len].desc = nextBatchInQueue[len].debugEvent ? _.extend({},
            this._getObjectDescriptorDEBUG(nextBatchInQueue[len].eid)) : this
            ._getObjectDescriptor(nextBatchInQueue[len].eid);
      ...
      this._client.decoratorManager.download(decoratorsToDownload,
  ...
  ModelEditorControl.prototype._dispatchEvents = function (events) {
    ...
    if (territoryChanged) {
      ...
      if (this.___SLOW_CONN === true) {
        setTimeout(function () {
          self.logger.debug(
            'Updating territory with ruleset from decorators: ' + JSON.stringify(
              self._selfPatterns));
      ...
      if (objD) {
        if (objD.parentId == this.currentNodeInfo.id) {
          ...
          if (objDesc.kind === "CONNECTION") {
            ...
            var srcDst = this._getAllSourceDestinationPairsForConnection(
              objDesc.source, objDesc.target);
            ...
            while (k--) {
              while (l--) {
                ...
                if (len >= 0) {
                  ...
                  _.extend(objDesc, this.getConnectionDescriptor(gmeID));
                  var uiComponent = this.designerCanvas.createConnection(
                    objDesc);
                  this.logger.debug('Connection: ' + uiComponent.id +
                    ' for GME object: ' + objDesc.id);
                  this._GmeID2ComponentID[gmeID].push(uiComponent.id);
  ...
  ModelEditorControl.prototype._onUnload = function (gmeID) {
    ...
    if (gmeID === this.currentNodeInfo.id) {
      ...
      this.logger.debug(
        'The previously opened model does not exist... --- GMEID: "' + this.currentNodeInfo
        .id + '"');
      this.designerCanvas.setBackgroundText(
        'The previously opened model does not exist...', {
          'font-size': BACKGROUND_TEXT_SIZE,
          'color': BACKGROUND_TEXT_COLOR
        });
    } else {
      if (this._GmeID2ComponentID.hasOwnProperty(gmeID)) {
        ...
        while (len--) {
          ...
          if (this.designerCanvas.itemIds.indexOf(componentID) !== -1) {
            getDecoratorTerritoryQueries(this.designerCanvas.items[
              componentID]._decoratorInstance);
          }
  ...
  ModelEditorControl.prototype._getAllSourceDestinationPairsForConnection =
  ...
  ModelEditorControl.prototype.registerComponentIDForPartID = function (
    componentID, partId) {
  ...
  ModelEditorControl.prototype.unregisterComponentIDFromPartID = function (
    componentID, partId) {
    var idx;
  ...
  ModelEditorControl.prototype._checkComponentDependency = function (gmeID,
    eventType) {
    var len;
      ...
      if (this._notifyPackage.hasOwnProperty(gmeID)) {
        ...
        if (this._GmeID2ComponentID.hasOwnProperty(gmeID)) {
          ...
          while (i--) {
            ...
            this.designerCanvas.notifyItemComponentEvents(itemID, this._notifyPackage[
              gmeID]);
          }
        }
      }
  ...
  ModelEditorControl.prototype._stateActiveObjectChanged = function (model,
    activeObjectId) {
    this.selectedObjectChanged(activeObjectId);
  };

  ModelEditorControl.prototype._stateActiveSelectionChanged = function (model,
    activeSelection) {
  ...
  ModelEditorControl.prototype._attachClientEventListeners = function () {
    ...
    WebGMEGlobal.State.on('change:' + CONSTANTS.STATE_ACTIVE_OBJECT, this._stateActiveObjectChanged,
      this);
    WebGMEGlobal.State.on('change:' + CONSTANTS.STATE_ACTIVE_SELECTION, this._stateActiveSelectionChanged,
      this);
  };

  ModelEditorControl.prototype._detachClientEventListeners = function () {
    WebGMEGlobal.State.off('change:' + CONSTANTS.STATE_ACTIVE_OBJECT, this._stateActiveObjectChanged);
    WebGMEGlobal.State.off('change:' + CONSTANTS.STATE_ACTIVE_SELECTION, this
      ._stateActiveSelectionChanged);
  };
  ...
  ModelEditorControl.prototype.activeSelectionChanged = function (
    activeSelection) {
    var selectedIDs = [],
    ...
    while (len--) {
      if (this._GmeID2ComponentID.hasOwnProperty(activeSelection[len])) {
        selectedIDs = selectedIDs.concat(this._GmeID2ComponentID[
          activeSelection[len]]);
      }
    }
  ...
  ModelEditorControl.prototype._updateAspects = function () {
    ...
    if (this._selectedAspect !== CONSTANTS.ASPECT_ALL) {
      ...
      var newAspectRules = this._client.getAspectTerritoryPattern(nodeId,
        this._selectedAspect);
      var aspectRulesChanged = false;

      if (this._selfPatterns[nodeId].items && newAspectRules.items) {
        aspectRulesChanged = (_.difference(this._selfPatterns[nodeId].items,
          newAspectRules.items)).length > 0;
        if (aspectRulesChanged === false) {
          aspectRulesChanged = (_.difference(newAspectRules.items, this._selfPatterns[
            nodeId].items)).length > 0;
        }
      } else {
        if (!this._selfPatterns[nodeId].items && !newAspectRules.items) {
  ...
  _.extend(ModelEditorControl.prototype,
    ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype);
