...
define(['logManager',
  ...
  CrosscutBrowserControl.prototype._initialize = function () {
    ...
    this._selfPatterns[PROJECT_ROOT_ID] = {
      "children": 1
    };
  ...
  CrosscutBrowserControl.prototype._needsToBeInTree = function (nodeId) {
    ...
    if (nodeId === PROJECT_ROOT_ID) {
      ...
      result = nodeObj.getChildrenIds().length > 0 || GMEConcepts.getCrosscuts(
        nodeId).length > 0;
    }
  ...
  CrosscutBrowserControl.prototype._createXCutTreeNode = function (nodeId,
    crosscut) {
    var parentTreeNode,
    ...
    if (this._treeNodes[nodeId] &&
      ...
      this._crosscutTreeNodes[fqCrosscutID] = this._treeBrowser.createNode(
        parentTreeNode, {
          "id": fqCrosscutID,
          "name": crosscut.title,
          "hasChildren": false,
  ...
  CrosscutBrowserControl.prototype._getFullCrosscutID = function (nodeId,
    crosscutID) {
    return nodeId + '_' + crosscutID;
  };

  CrosscutBrowserControl.prototype._getCrosscutID = function (fqCrosscutID,
    nodeId) {
    ...
    deleteNodeAndChildrenFromLocalHash = function (childNodeId, deleteSelf) {
      ...
      if (self._nodes[childNodeId]) {
        for (xx = 0; xx < self._nodes[childNodeId].children.length; xx += 1) {
          deleteNodeAndChildrenFromLocalHash(self._nodes[childNodeId].children[
            xx], true);
        }

        //delete the node's crosscuts
        for (xx = 0; xx < self._nodes[childNodeId].crosscuts.length; xx += 1) {
          fqCrosscutID = self._getFullCrosscutID(childNodeId, self._nodes[
            childNodeId].crosscuts[xx]);
  ...
  CrosscutBrowserControl.prototype._onNodeDoubleClicked = function (nodeId) {
    ...
    if (this._crosscuts.hasOwnProperty(nodeId)) {
      settings[CONSTANTS.STATE_ACTIVE_CROSSCUT] = this._getCrosscutID(nodeId,
        this._crosscuts[nodeId]);
      WebGMEGlobal.State.set(settings);
    }
  };
  ...
  CrosscutBrowserControl.prototype._removeXCutTreeNode = function (nodeId,
    crossCutID) {
  ...
  CrosscutBrowserControl.prototype._updateXCutTreeNode = function (nodeId,
    crosscut) {
