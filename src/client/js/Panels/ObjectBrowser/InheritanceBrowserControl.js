...
define(['logManager',
  ...
  InheritanceBrowserControl.prototype._initialize = function () {
    ...
    this._treeBrowser.onCreatingContextMenu = function (nodeId,
      contextMenuOptions) {
      ...
      for (i = 0; i < inheritedIDs.length; i += 1) {
        ...
        if (childNode) {
          ...
          childTreeNode = this._treeBrowser.createNode(parentNode, {
            ...
            "hasChildren": (childNode.getCollectionPaths(CONSTANTS.POINTER_BASE))
              .length > 0,
          ...
          this._selfPatterns[currentChildId] = {
            'children': 0
          };
        }
      }
  ...
  InheritanceBrowserControl.prototype._onNodeClose = function (nodeId) {
    ...
    deleteNodeAndChildrenFromLocalHash = function (childNodeId, deleteSelf) {
      ...
      if (self._nodes[childNodeId]) {
        for (xx = 0; xx < self._nodes[childNodeId].inheritance.length; xx +=
          1) {
          deleteNodeAndChildrenFromLocalHash(self._nodes[childNodeId].inheritance[
            xx], true);
        }
  ...
  InheritanceBrowserControl.prototype._refresh = function (eventType,
    objectId) {
    ...
    this._logger.debug("Refresh event '" + eventType + "', with objectId: '" +
      objectId + "'");
        ...
        if (updatedObject) {
          ...
          if (this._nodes[objectId].state === STATE_LOADING) {
            ...
            nodeDescriptor = {
              ...
              "hasChildren": (updatedObject.getCollectionPaths(CONSTANTS.POINTER_BASE))
                .length > 0,
              "class": objType
            };

            //update the node's representation in the tree
            this._treeBrowser.updateNode(this._nodes[objectId].treeNode,
              nodeDescriptor);

            //update the object's children list in the local hashmap
            this._nodes[objectId].inheritance = updatedObject.getCollectionPaths(
            ...
            if (this._treeBrowser.isExpanded(this._nodes[objectId].treeNode)) {
              ...
              inheritanceDeleted = _.difference(oldInheritance,
                currentInheritance);
              ...
              deleteNodeAndChildrenFromLocalHash = function (childNodeId) {
                ...
                if (self._nodes[childNodeId]) {
                  for (xx = 0; xx < self._nodes[childNodeId].inheritance.length; xx +=
                    1) {
                    deleteNodeAndChildrenFromLocalHash(self._nodes[
                      childNodeId].inheritance[xx]);
                  }
            ...
            if (this._treeBrowser.isExpanded(this._nodes[objectId].treeNode)) {
              ...
              inheritanceAdded = _.difference(currentInheritance,
                oldInheritance);
              ...
              for (j = 0; j < inheritanceAdded.length; j += 1) {
                ...
                if (childNode) {
                  //the node was present on the client side, render ist full data
                  childTreeNode = this._treeBrowser.createNode(this._nodes[
                    objectId].treeNode, {
                    "id": currentChildId,
                    "name": childNode.getAttribute("name"),
                    "hasChildren": (childNode.getCollectionPaths(CONSTANTS.POINTER_BASE))
