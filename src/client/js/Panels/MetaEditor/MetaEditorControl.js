...
define(['logManager',
  ...
  MetaEditorControl = function (options) {
    ...
    this.logger = options.logger || logManager.create(options.loggerName ||
      "MetaEditorControl");
    ...
    if (this.diagramDesigner === undefined) {
      this.logger.error(
        "ModelEditorControl's DiagramDesigner is not specified...");
      throw ("ModelEditorControl can not be created");
    }
    ...
    this._client.decoratorManager.download([META_DECORATOR], WIDGET_NAME,
      function () {
  ...
  MetaEditorControl.prototype._loadMetaAspectContainerNode = function () {
    ...
    this._selfPatterns = {};
    this._selfPatterns[this.metaAspectContainerNodeID] = {
      "children": 0
    };
  ...
  MetaEditorControl.prototype._onUnload = function (gmeID) {
    ...
    if (gmeID === this.metaAspectContainerNodeID) {
      ...
      this.logger.debug(
        'The currently opened aspect has been deleted --- GMEID: "' + this.metaAspectContainerNodeID +
        '"');
      setTimeout(function () {
        self._loadMetaAspectContainerNode();
      }, 10);
  ...
  MetaEditorControl.prototype._printNodeData = function () {
    ...
    while (len--) {
      ...
      if (node) {
        ...
        this.logger.warning(nodeName + ' (' + gmeID +
          ')\'s containmentMetaDescriptor: ' + JSON.stringify(
            containmentMetaDescriptor));

        var pointerNames = node.getPointerNames();
        i = pointerNames.length;
        this.logger.warning(nodeName + ' (' + gmeID +
          ')\'s pointerMetaDescriptors num: ' + i);
        while (i--) {
          var pointerMetaDescriptor = node.getPointerDescriptor(pointerNames[
            i]);
          this.logger.warning(nodeName + ' (' + gmeID +
            ')\'s pointerMetaDescriptor "' + pointerNames[i] + '": ' + JSON.stringify(
              pointerMetaDescriptor));
        }

        this.logger.warning(nodeName + ' (' + gmeID +
          ')\'s metaInheritance: ' + node.getBaseId());

        var attributeNames = node.getAttributeNames();
        i = attributeNames.length;
        this.logger.warning(nodeName + ' (' + gmeID +
          ')\'s attributeMetaDescriptors num: ' + i);

        while (i--) {
          var attrMetaDescriptor = node.getAttributeDescriptor(attributeNames[
            i]);
          this.logger.warning(nodeName + ' (' + gmeID +
            ')\'s attributeMetaDescriptor "' + attributeNames[i] + '": ' +
            JSON.stringify(attrMetaDescriptor));
        }
      }
    }
  };
  ...
  MetaEditorControl.prototype._processMetaAspectContainerNode = function () {
    ...
    while (len--) {
      ...
      this._metaAspectMembersCoordinatesGlobal[gmeID] = aspectNode.getMemberRegistry(
        MetaEditorConstants.META_ASPECT_SET_NAME, gmeID, REGISTRY_KEYS.POSITION
      );
    }
    ...
    //check deleted nodes
    diff = _.difference(this._selectedMetaAspectSheetMembers,
      selectedSheetMembers);
    len = diff.length;
    while (len--) {
    ...
    len = diff.length;
    while (len--) {
      this._metaAspectMemberPatterns[diff[len]] = {
        "children": 0
      };
      territoryChanged = true;
    }
    ...
    diff = _.intersection(this._selectedMetaAspectSheetMembers,
      selectedSheetMembers);
    len = diff.length;
    while (len--) {
      gmeID = diff[len];
      ...
      if (this._metaAspectMembersCoordinatesPerSheet[this._selectedMetaAspectSet]
        [gmeID]) {
        objDesc.position.x = this._metaAspectMembersCoordinatesPerSheet[this._selectedMetaAspectSet]
        [gmeID].x;
        objDesc.position.y = this._metaAspectMembersCoordinatesPerSheet[this._selectedMetaAspectSet]
        [gmeID].y;
      }
  ...
  MetaEditorControl.prototype._processNodeLoad = function (gmeID) {
    ...
    if (this._GMENodes.indexOf(gmeID) === -1) {
      ...
      decClass = this._client.decoratorManager.getDecoratorForWidget(
        META_DECORATOR, WIDGET_NAME);
  ...
  MetaEditorControl.prototype._processConnectionWaitingList = function (gmeID) {
    ...
    gmeDstID = gmeID;
    if (this._connectionWaitingListByDstGMEID && this._connectionWaitingListByDstGMEID
      .hasOwnProperty(gmeDstID)) {
      for (gmeSrcID in this._connectionWaitingListByDstGMEID[gmeDstID]) {
        if (this._connectionWaitingListByDstGMEID[gmeDstID].hasOwnProperty(
          gmeSrcID)) {
          len = this._connectionWaitingListByDstGMEID[gmeDstID][gmeSrcID].length;
          while (len--) {
            connType = this._connectionWaitingListByDstGMEID[gmeDstID][
              gmeSrcID
            ][len][0];
            connTexts = this._connectionWaitingListByDstGMEID[gmeDstID][
              gmeSrcID
            ][len][1];
            c.push({
              'gmeSrcID': gmeSrcID,
    ...
    gmeSrcID = gmeID;
    if (this._connectionWaitingListBySrcGMEID && this._connectionWaitingListBySrcGMEID
      .hasOwnProperty(gmeSrcID)) {
      for (gmeDstID in this._connectionWaitingListBySrcGMEID[gmeSrcID]) {
        if (this._connectionWaitingListBySrcGMEID[gmeSrcID].hasOwnProperty(
          gmeDstID)) {
          len = this._connectionWaitingListBySrcGMEID[gmeSrcID][gmeDstID].length;
          while (len--) {
            connType = this._connectionWaitingListBySrcGMEID[gmeSrcID][
              gmeDstID
            ][len][0];
            connTexts = this._connectionWaitingListBySrcGMEID[gmeSrcID][
              gmeDstID
            ][len][1];
            c.push({
              'gmeSrcID': gmeSrcID,
  ...
  MetaEditorControl.prototype._processNodeUnload = function (gmeID) {
    ...
    if (this._GMEID2ComponentID.hasOwnProperty(gmeID)) {
      ...
      while (len--) {
        ...
        this._removeConnection(gmeID, otherEnd, MetaRelations.META_RELATIONS.POINTER,
          pointerName);
      }

      //INHERITANCE
      if (this._nodeMetaInheritance[gmeID] && !_.isEmpty(this._nodeMetaInheritance[
        gmeID])) {
        this._removeConnection(this._nodeMetaInheritance[gmeID], gmeID,
      ...
      while (len--) {
        ...
        this._removeConnection(gmeID, otherEnd, MetaRelations.META_RELATIONS.SET,
          pointerName);
      }
      ...
      while (len--) {
        ...
        this._saveConnectionToWaitingList(this._connectionListByID[
            connectionID].GMESrcId, this._connectionListByID[connectionID].GMEDstId,
          this._connectionListByID[connectionID].type, this._connectionListByID[
            connectionID].connTexts);
        this._removeConnection(this._connectionListByID[connectionID].GMESrcId,
          this._connectionListByID[connectionID].GMEDstId, this._connectionListByID[
            connectionID].type);
      }
  ...
  /****************************************************************************/
  MetaEditorControl.prototype._createConnection = function (gmeSrcId,
    gmeDstId, connType, connTexts) {
    var connDesc,
      connComponent,
      metaInfo;
    ...
    if (this._GMENodes.indexOf(gmeSrcId) !== -1 && this._GMENodes.indexOf(
      gmeDstId) !== -1) {
      ...
      if (this._filteredOutConnTypes.indexOf(connType) === -1) {
        ...
        this._filteredOutConnectionDescriptors[connType].push([gmeSrcId,
          gmeDstId, connTexts
        ]);
      }
    } else {
  ...
  MetaEditorControl.prototype._saveConnectionToWaitingList = function (
    gmeSrcId, gmeDstId, connType, connTexts) {
    if (this._GMENodes.indexOf(gmeSrcId) !== -1 && this._GMENodes.indexOf(
      gmeDstId) === -1) {
      //#1 - the destination object is missing from the screen
      this._connectionWaitingListByDstGMEID[gmeDstId] = this._connectionWaitingListByDstGMEID[
        gmeDstId] || {};
      this._connectionWaitingListByDstGMEID[gmeDstId][gmeSrcId] = this._connectionWaitingListByDstGMEID[
        gmeDstId][gmeSrcId] || [];
      this._connectionWaitingListByDstGMEID[gmeDstId][gmeSrcId].push([
        connType, connTexts
      ]);
    } else if (this._GMENodes.indexOf(gmeSrcId) === -1 && this._GMENodes.indexOf(
      gmeDstId) !== -1) {
      //#2 -  the source object is missing from the screen
      this._connectionWaitingListBySrcGMEID[gmeSrcId] = this._connectionWaitingListBySrcGMEID[
        gmeSrcId] || {};
      this._connectionWaitingListBySrcGMEID[gmeSrcId][gmeDstId] = this._connectionWaitingListBySrcGMEID[
        gmeSrcId][gmeDstId] || [];
      this._connectionWaitingListBySrcGMEID[gmeSrcId][gmeDstId].push([
        connType, connTexts
      ]);
    } else {
      ...
      this.logger.error(
        '_saveConnectionToWaitingList both gmeSrcId and gmeDstId is undefined...'
      );
    }
  };

  MetaEditorControl.prototype._saveConnection = function (gmeSrcId, gmeDstId,
    connType, connComponentId, connTexts) {
    //save by SRC
    this._connectionListBySrcGMEID[gmeSrcId] = this._connectionListBySrcGMEID[
      gmeSrcId] || {};
    this._connectionListBySrcGMEID[gmeSrcId][gmeDstId] = this._connectionListBySrcGMEID[
      gmeSrcId][gmeDstId] || {};
    this._connectionListBySrcGMEID[gmeSrcId][gmeDstId][connType] = this._connectionListBySrcGMEID[
      gmeSrcId][gmeDstId][connType] || [];
    this._connectionListBySrcGMEID[gmeSrcId][gmeDstId][connType].push(
      connComponentId);

    //save by DST
    this._connectionListByDstGMEID[gmeDstId] = this._connectionListByDstGMEID[
      gmeDstId] || {};
    this._connectionListByDstGMEID[gmeDstId][gmeSrcId] = this._connectionListByDstGMEID[
      gmeDstId][gmeSrcId] || {};
    this._connectionListByDstGMEID[gmeDstId][gmeSrcId][connType] = this._connectionListByDstGMEID[
      gmeDstId][gmeSrcId][connType] || [];
    this._connectionListByDstGMEID[gmeDstId][gmeSrcId][connType].push(
      connComponentId);

    //save by type
    this._connectionListByType[connType] = this._connectionListByType[
      connType] || [];
  ...
  /****************************************************************************/
  MetaEditorControl.prototype._removeConnection = function (gmeSrcId,
    gmeDstId, connType, pointerName) {
    var connectionID,
      idx,
      len,
    ...
    while (len--) {
      connectionID = this._connectionListBySrcGMEID[gmeSrcId][gmeDstId][
        connType
      ][len];
      ...
      if (connectionID) {
        ...
        idx = this._connectionListBySrcGMEID[gmeSrcId][gmeDstId][connType].indexOf(
          connectionID);
        this._connectionListBySrcGMEID[gmeSrcId][gmeDstId][connType].splice(
          idx, 1);

        idx = this._connectionListByDstGMEID[gmeDstId][gmeSrcId][connType].indexOf(
          connectionID);
        this._connectionListByDstGMEID[gmeDstId][gmeSrcId][connType].splice(
          idx, 1);
      }
    }
  };
  ...
  /*****************************************************************************/
  MetaEditorControl.prototype._updateConnectionText = function (gmeSrcId,
    gmeDstId, connType, connTexts) {
    var connectionID,
      idx,
      len = this._connectionListBySrcGMEID[gmeSrcId][gmeDstId][connType].length,
    ...
    if (!found) {
      ...
      if (this._GMENodes.indexOf(gmeSrcId) !== -1 && this._GMENodes.indexOf(
        ...
        for (idx = 0; idx < len; idx += 1) {
          connDesc = this._connectionWaitingListByDstGMEID[gmeDstId][gmeSrcId]
          [idx];
        ...
        len = this._connectionWaitingListBySrcGMEID[gmeSrcId][gmeDstId].length;
        for (idx = 0; idx < len; idx += 1) {
          connDesc = this._connectionWaitingListBySrcGMEID[gmeSrcId][gmeDstId]
          [idx];
          if (connDesc[0] === connType) {
            if (connType !== MetaRelations.META_RELATIONS.POINTER ||
              (connType === MetaRelations.META_RELATIONS.POINTER &&
  ...
  MetaEditorControl.prototype._processNodeMetaContainment = function (gmeID) {
    ...
    this._nodeMetaContainment[gmeID] = this._nodeMetaContainment[gmeID] || {
      'targets': []
    };
    ...
    while (len--) {
      if (containmentOwnTypes.indexOf(containmentMetaDescriptor[len].id) !==
        -1) {
    ...
    while (len--) {
      ...
      if (oldMetaContainment[containmentTarget].multiplicity !==
        ...
        //update accounting
        oldMetaContainment[containmentTarget].multiplicity =
          newMetaContainment[containmentTarget].multiplicity;

        //update connection text
        this._updateConnectionText(gmeID, containmentTarget, MetaRelations.META_RELATIONS
          .CONTAINMENT, {
    ...
    while (len--) {
      containmentTarget = diff[len];
      this._removeConnection(gmeID, containmentTarget, MetaRelations.META_RELATIONS
        .CONTAINMENT);
    ...
    while (len--) {
      ...
      oldMetaContainment[containmentTarget] = {
        'multiplicity': newMetaContainment[containmentTarget].multiplicity
      };

      this._createConnection(gmeID, containmentTarget, MetaRelations.META_RELATIONS
        .CONTAINMENT, {
          'dstText': newMetaContainment[containmentTarget].multiplicity,
          'dstTextEdit': true
        });
  ...
  MetaEditorControl.prototype._processNodeMetaPointers = function (gmeID,
    ...
    var node = this._client.getNode(gmeID),
      ...
      lenTargets,
      combinedName,
      ptrType = isSet === true ? MetaRelations.META_RELATIONS.SET :
      MetaRelations.META_RELATIONS.POINTER;
    ...
    if (isSet !== true) {
      ...
      this._nodeMetaSets[gmeID] = this._nodeMetaSets[gmeID] || {
        'names': [],
        'combinedNames': []
      };
    ...
    while (len--) {
      pointerMetaDescriptor = this._client.getValidTargetItems(gmeID,
        pointerNames[len]);
      pointerOwnMetaTypes = this._client.getOwnValidTargetTypes(gmeID,
        pointerNames[len]);

      if (pointerMetaDescriptor) {
        lenTargets = pointerMetaDescriptor.length;
        while (lenTargets--) {
          if (pointerOwnMetaTypes.indexOf(pointerMetaDescriptor[lenTargets].id) !==
            -1) {
            combinedName = pointerNames[len] + "_" + pointerMetaDescriptor[
              lenTargets].id;
    ...
    while (len--) {
      ...
      if (oldMetaPointers[combinedName].multiplicity !== newMetaPointers[
        combinedName].multiplicity) {
        pointerName = oldMetaPointers[combinedName].name;
        pointerTarget = oldMetaPointers[combinedName].target;

        oldMetaPointers[combinedName].multiplicity = newMetaPointers[
          combinedName].multiplicity;
  ...
  MetaEditorControl.prototype._processNodeMetaInheritance = function (gmeID) {
    ...
    if (oldMetaInheritance && (oldMetaInheritance !== newMetaInheritance)) {
      this._removeConnection(oldMetaInheritance, gmeID, MetaRelations.META_RELATIONS
        .INHERITANCE);
    ...
    if (newMetaInheritance && (oldMetaInheritance !== newMetaInheritance)) {
      ...
      this._createConnection(newMetaInheritance, gmeID, MetaRelations.META_RELATIONS
  ...
  MetaEditorControl.prototype._setNewConnectionType = function (connType) {
    ...
    if (this._connType !== connType) {
      ...
      this.diagramDesigner.connectionDrawingManager.setConnectionInDrawProperties(
        connProps);
  ...
  MetaEditorControl.prototype._createContainmentRelationship = function (
    ...
    if (containerNode && objectNode) {
      this._client.updateValidChildrenItem(containerID, {
        id: objectID
      });
    }
  };

  MetaEditorControl.prototype._deleteContainmentRelationship = function (
    containerID, objectID) {
    var containerNode = this._client.getNode(containerID),
      objectNode = this._client.getNode(objectID);
  ...
  MetaEditorControl.prototype._createPointerRelationship = function (sourceID,
    targetID, isSet) {
    ...
    if (sourceNode && targetNode) {
      ...
      existingPointerNames = _.difference(existingPointerNames,
        MetaEditorConstants.RESERVED_POINTER_NAMES);
      notAllowedPointerNames = notAllowedPointerNames.concat(
        MetaEditorConstants.RESERVED_POINTER_NAMES);

      //query pointer name from user
      this.diagramDesigner.selectNewPointerName(existingPointerNames,
        notAllowedPointerNames, isSet, function (userSelectedPointerName) {
          self._client.startTransaction();
          pointerMetaDescriptor = self._client.getValidTargetItems(sourceID,
          ...
          if (!pointerMetaDescriptor) {
            if (isSet !== true) {
              ...
              self._client.makePointer(sourceID, userSelectedPointerName,
                null);
            } else {
              //pointer list
              self._client.setPointerMeta(sourceID, userSelectedPointerName, {
  ...
  MetaEditorControl.prototype._deletePointerRelationship = function (sourceID,
    ...
    if (sourceNode && targetNode) {
      ...
      pointerMetaDescriptor = this._client.getValidTargetItems(sourceID,
        pointerName);
  ...
  MetaEditorControl.prototype._createInheritanceRelationship = function (
    parentID, objectID) {
  ...
  MetaEditorControl.prototype._deleteInheritanceRelationship = function (
    parentID, objectID) {
    ...
    if (objectNode) {
      ...
      if (objectBaseId) {
        ...
        if (baseNode) {
          objectBaseId = baseNode.getAttribute(nodePropertyNames.Attributes.name) +
            ' (' + objectBaseId + ')';
        }
        ...
        this.logger.warning('Deleting InheritanceRelationship from "' +
          objectNode.getAttribute(nodePropertyNames.Attributes.name) + '" (' +
          objectID + ') to parent "' + objectBaseId + '" is not allowed...');
      }
    }
  };
  /****************************************************************************/
  ...
  MetaEditorControl.prototype._initFilterPanel = function () {
    ...
    filterIcon = MetaRelations.createButtonIcon(16, MetaRelations.META_RELATIONS
      .CONTAINMENT);
    this.diagramDesigner.addFilterItem('Containment', MetaRelations.META_RELATIONS
      .CONTAINMENT, filterIcon);

    filterIcon = MetaRelations.createButtonIcon(16, MetaRelations.META_RELATIONS
      .POINTER);
    this.diagramDesigner.addFilterItem('Pointer', MetaRelations.META_RELATIONS
      .POINTER, filterIcon);

    filterIcon = MetaRelations.createButtonIcon(16, MetaRelations.META_RELATIONS
      .INHERITANCE);
    this.diagramDesigner.addFilterItem('Inheritance', MetaRelations.META_RELATIONS
      .INHERITANCE, filterIcon);

    filterIcon = MetaRelations.createButtonIcon(16, MetaRelations.META_RELATIONS
      .SET);
    this.diagramDesigner.addFilterItem('Set', MetaRelations.META_RELATIONS.SET,
      filterIcon);
  };

  MetaEditorControl.prototype._onConnectionTypeFilterCheckChanged = function (
    value, isChecked) {
    var idx;
  ...
  MetaEditorControl.prototype._filterConnType = function (connType) {
    var len = this._connectionListByType && this._connectionListByType.hasOwnProperty(
        connType) ? this._connectionListByType[connType].length : 0,
      connComponentId,
      gmeSrcId,
      gmeDstId,
      connTexts,
  ...
  MetaEditorControl.prototype._unfilterConnType = function (connType) {
    var len = this._filteredOutConnectionDescriptors && this._filteredOutConnectionDescriptors
      .hasOwnProperty(connType) ? this._filteredOutConnectionDescriptors[
        connType].length : 0,
      gmeSrcId,
      gmeDstId,
      connTexts;
  ...
  MetaEditorControl.prototype._onConnectionDstTextChanged = function (
    ...
    if (connDesc.type === MetaRelations.META_RELATIONS.CONTAINMENT) {
      this._containmentRelationshipMultiplicityUpdate(connDesc.GMESrcId,
        connDesc.GMEDstId, oldValue, newValue);
    } else if (connDesc.type === MetaRelations.META_RELATIONS.POINTER) {
      this._pointerRelationshipMultiplicityUpdate(connDesc.GMESrcId, connDesc
        .GMEDstId, connDesc.name, oldValue, newValue);
    } else if (connDesc.type === MetaRelations.META_RELATIONS.INHERITANCE) {
      //never can happen
    } else if (connDesc.type === MetaRelations.META_RELATIONS.SET) {
      this._setRelationshipMultiplicityUpdate(connDesc.GMESrcId, connDesc.GMEDstId,
        connDesc.name, oldValue, newValue);
    }
  };

  MetaEditorControl.prototype._containmentRelationshipMultiplicityUpdate =
    function (containerID, objectID, oldValue, newValue) {
      var containerNode = this._client.getNode(containerID),
        objectNode = this._client.getNode(objectID),
        multiplicity,
        multiplicityValid;
      ...
      if (containerNode && objectNode) {
        ...
        if (multiplicity) {
          ...
          this._updateConnectionText(containerID, objectID, MetaRelations.META_RELATIONS
            .CONTAINMENT, {
              'dstText': oldValue,
              'dstTextEdit': true
            });
  ...
  MetaEditorControl.prototype._pointerRelationshipMultiplicityUpdate =
    function (sourceID, targetID, pointerName, oldValue, newValue) {
      var sourceNode = this._client.getNode(sourceID),
        targetNode = this._client.getNode(targetID),
        multiplicityValid,
        multiplicity;
      ...
      if (sourceNode && targetNode) {
        ...
        if (multiplicity) {
          ...
          this._client.updateValidTargetItem(sourceID, pointerName,
            multiplicity);
        } else {
          this._updateConnectionText(sourceID, targetID, MetaRelations.META_RELATIONS
            .POINTER, {
              'name': pointerName,
              'dstText': oldValue,
              'dstTextEdit': true
  ...
  MetaEditorControl.prototype._setRelationshipMultiplicityUpdate = function (
    sourceID, targetID, pointerName, oldValue, newValue) {
    var sourceNode = this._client.getNode(sourceID),
      targetNode = this._client.getNode(targetID),
      multiplicityValid,
      multiplicity;
  ...
  MetaEditorControl.prototype._initializeToolbar = function () {
    ...
    this._radioButtonGroupMetaRelationType = toolBar.addRadioButtonGroup(
      function (data) {
    ...
    this._radioButtonGroupMetaRelationType.addButton({
      ...
      "selected": true,
      "data": {
        "connType": MetaRelations.META_RELATIONS.CONTAINMENT
      },
      "icon": MetaRelations.createButtonIcon(16, MetaRelations.META_RELATIONS
        .CONTAINMENT)
    });
    ...
    /************** PRINT NODE DATA *****************/
    this._btnPrintNodeMetaData = toolBar.addButton({
      "title": "Print node META data",
  ...
  MetaEditorControl.prototype._processMetaAspectSheetsRegistry = function () {
    var aspectNode = this._client.getNode(this.metaAspectContainerNodeID),
      metaAspectSheetsRegistry = aspectNode.getEditableRegistry(REGISTRY_KEYS
        .META_SHEETS) || [],
      i,
      len,
      sheetID,
    ...
    for (i = 0; i < len; i += 1) {
      ...
      sheetID = this.diagramDesigner.addTab(metaAspectSheetsRegistry[i].title,
        true, true);

      this._sheets[sheetID] = setName;

      //get the most up-to-date member list for each set
      this._metaAspectMembersPerSheet[setName] = aspectNode.getMemberIds(
        setName);
      ...
      if (this._metaAspectMembersPerSheet[setName].indexOf(CONSTANTS.PROJECT_ROOT_ID) >
        -1) {
      ...
      while (j--) {
        ...
        this._metaAspectMembersCoordinatesPerSheet[setName][gmeID] =
          aspectNode.getMemberRegistry(setName, gmeID, REGISTRY_KEYS.POSITION);
        this._metaAspectSheetsPerMember[gmeID] = this._metaAspectSheetsPerMember[
          gmeID] || [];
    ...
    if (this._selectedMetaAspectSet) {
      ...
      if (oldPositions && newPositions) {
        for (var oldItemId in oldPositions) {
          if (oldPositions.hasOwnProperty(oldItemId) && newPositions.hasOwnProperty(
            oldItemId)) {
  ...
  MetaEditorControl.prototype._initializeSelectedSheet = function () {
    ...
    if (this._selectedMetaAspectSet && this._metaAspectMembersPerSheet[this._selectedMetaAspectSet]) {
      ...
      while (len--) {
        this._selectedMetaAspectSheetMembers.push(this._metaAspectMembersPerSheet[
          this._selectedMetaAspectSet][len]);
        this._metaAspectMemberPatterns[this._metaAspectMembersPerSheet[this._selectedMetaAspectSet]
          [len]] = {
          "children": 0
        };
      }
    ...
    this._metaAspectMembersTerritoryId = this._client.addUI(this, function (
      events) {
      self._eventCallback(events);
    });
  ...
  _.extend(MetaEditorControl.prototype,
    MetaEditorControlDiagramDesignerWidgetEventHandlers.prototype);
