...
define(['logManager',
  ...
  MetaEditorControl.prototype._printNodeData = function () {
    ...
    while (len--) {
      ...
      if (node) {
        ...
        this.logger.warning(nodeName + ' (' + gmeID + ')\'s containmentMetaDescriptor: ' + JSON.stringify(
        ...
        while (i--) {
          ...
          this.logger.warning(nodeName + ' (' + gmeID + ')\'s pointerMetaDescriptor "' + pointerNames[i] + '": ' +
            JSON.stringify(pointerMetaDescriptor));
        }
        ...
        while (i--) {
          ...
          this.logger.warning(nodeName + ' (' + gmeID + ')\'s attributeMetaDescriptor "' + attributeNames[i] + '": ' +
  ...
  MetaEditorControl.prototype._processMetaAspectContainerNode = function () {
    ...
    while (len--) {
      ...
      this._metaAspectMembersCoordinatesGlobal[gmeID] = aspectNode.getMemberRegistry(MetaEditorConstants.META_ASPECT_SET_NAME,
        gmeID, REGISTRY_KEYS.POSITION);
    }
  ...
  MetaEditorControl.prototype._processNodeUnload = function (gmeID) {
    ...
    if (this._GMEID2ComponentID.hasOwnProperty(gmeID)) {
      ...
      while (len--) {
        ...
        this._saveConnectionToWaitingList(this._connectionListByID[connectionID].GMESrcId, this._connectionListByID[
          connectionID].GMEDstId, this._connectionListByID[connectionID].type, this._connectionListByID[
          connectionID].connTexts);
        this._removeConnection(this._connectionListByID[connectionID].GMESrcId, this._connectionListByID[connectionID]
          .GMEDstId, this._connectionListByID[connectionID].type);
      }
      ...
      while (len--) {
        ...
        if (this._connectionListByID[connectionID]) {
          ...
          this._saveConnectionToWaitingList(this._connectionListByID[connectionID].GMESrcId, this._connectionListByID[
            connectionID].GMEDstId, this._connectionListByID[connectionID].type, this._connectionListByID[
            connectionID].connTexts);
          this._removeConnection(this._connectionListByID[connectionID].GMESrcId, this._connectionListByID[
            connectionID].GMEDstId, this._connectionListByID[connectionID].type);
        }
      }
  ...
  MetaEditorControl.prototype._saveConnectionToWaitingList = function (gmeSrcId, gmeDstId, connType, connTexts) {
    if (this._GMENodes.indexOf(gmeSrcId) !== -1 && this._GMENodes.indexOf(gmeDstId) === -1) {
      ...
      this._connectionWaitingListByDstGMEID[gmeDstId][gmeSrcId] = this._connectionWaitingListByDstGMEID[gmeDstId][
        gmeSrcId
      ] || [];
      ...
      this._connectionWaitingListBySrcGMEID[gmeSrcId][gmeDstId] = this._connectionWaitingListBySrcGMEID[gmeSrcId][
        gmeDstId
      ] || [];
  ...
  MetaEditorControl.prototype._saveConnection = function (gmeSrcId, gmeDstId, connType, connComponentId, connTexts) {
    ...
    this._connectionListBySrcGMEID[gmeSrcId][gmeDstId][connType] = this._connectionListBySrcGMEID[gmeSrcId][gmeDstId]
    [connType] || [];
    ...
    this._connectionListByDstGMEID[gmeDstId][gmeSrcId][connType] = this._connectionListByDstGMEID[gmeDstId][gmeSrcId]
    [connType] || [];
    ...
    while (len--) {
      if (containmentOwnTypes.indexOf(containmentMetaDescriptor[len].id) !== -1) {
        ...
        newMetaContainment[containmentMetaDescriptor[len].id] = {
          'multiplicity': '' + (containmentMetaDescriptor[len].min || 0) + '..' + (containmentMetaDescriptor[len].max ||
            '*')
        };
      }
    }
      ...
      if (pointerMetaDescriptor) {
        ...
        while (lenTargets--) {
          if (pointerOwnMetaTypes.indexOf(pointerMetaDescriptor[lenTargets].id) !== -1) {
            ...
            if (isSet) {
              newMetaPointers[combinedName].multiplicity = '' + (pointerMetaDescriptor[lenTargets].min || 0) + '..' +
  ...
  MetaEditorControl.prototype._createPointerRelationship = function (sourceID, targetID, isSet) {
    ...
    if (sourceNode && targetNode) {
      ...
      this.diagramDesigner.selectNewPointerName(existingPointerNames, notAllowedPointerNames, isSet, function (
        ...
        if (!pointerMetaDescriptor) {
          ...
          if (isSet !== true) {
            //single pointer
            self._client.updateValidTargetItem(sourceID, userSelectedPointerName, {
              id: targetID,
              max: 1
            });
          } else {
  ...
  MetaEditorControl.prototype._deleteInheritanceRelationship = function (parentID, objectID) {
    ...
    if (objectNode) {
      ...
      if (objectBaseId) {
        ...
        this.logger.warning('Deleting InheritanceRelationship from "' + objectNode.getAttribute(nodePropertyNames.Attributes
          .name) + '" (' + objectID + ') to parent "' + objectBaseId + '" is not allowed...');
      }
    }
  };
  /****************************************************************************/
  ...
  MetaEditorControl.prototype._filterConnType = function (connType) {
    var len = this._connectionListByType && this._connectionListByType.hasOwnProperty(connType) ? this._connectionListByType[
        connType].length : 0,
      connComponentId,
      gmeSrcId,
      gmeDstId,
  ...
  MetaEditorControl.prototype._unfilterConnType = function (connType) {
    var len = this._filteredOutConnectionDescriptors && this._filteredOutConnectionDescriptors.hasOwnProperty(
        connType) ? this._filteredOutConnectionDescriptors[connType].length : 0,
      gmeSrcId,
      gmeDstId,
      connTexts;
  ...
  MetaEditorControl.prototype._onConnectionDstTextChanged = function (connectionID, oldValue, newValue) {
    ...
    if (connDesc.type === MetaRelations.META_RELATIONS.CONTAINMENT) {
      ...
      this._pointerRelationshipMultiplicityUpdate(connDesc.GMESrcId, connDesc.GMEDstId, connDesc.name, oldValue,
        newValue);
  ...
  MetaEditorControl.prototype._processMetaAspectSheetsRegistry = function () {
    ...
    for (i = 0; i < len; i += 1) {
      ...
      while (j--) {
        ...
        this._metaAspectMembersCoordinatesPerSheet[setName][gmeID] = aspectNode.getMemberRegistry(setName, gmeID,
