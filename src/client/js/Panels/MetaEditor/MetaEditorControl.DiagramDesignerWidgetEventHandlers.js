...
define(['logManager',
  ...
  MetaEditorControlDiagramDesignerWidgetEventHandlers.prototype.attachDiagramDesignerWidgetEventHandlers =
    function () {
      var self = this;
      ...
      this.diagramDesigner.onBackgroundDroppableAccept = function (event,
        dragInfo) {
        return self._onBackgroundDroppableAccept(event, dragInfo);
      };

      this.diagramDesigner.onBackgroundDrop = function (event, dragInfo,
        position) {
      ...
      this.diagramDesigner.onConnectionDstTextChanged = function (connId,
        oldValue, newValue) {
      ...
      this.diagramDesigner.onFilterNewConnectionDroppableEnds = function (
        params) {
      ...
      this.diagramDesigner.onTabTitleChanged = function (tabID, oldValue,
        newValue) {
      ...
      this.diagramDesigner.onSelectionFillColorChanged = function (
        selectedElements, color) {
        self._onSelectionFillColorChanged(selectedElements, color);
      };

      this.diagramDesigner.onSelectionBorderColorChanged = function (
        selectedElements, color) {
        self._onSelectionBorderColorChanged(selectedElements, color);
      };

      this.diagramDesigner.onSelectionTextColorChanged = function (
        selectedElements, color) {
  ...
  /**********************************************************/
  MetaEditorControlDiagramDesignerWidgetEventHandlers.prototype._onBackgroundDroppableAccept =
      ...
      if (this._selectedMetaAspectSet) {
        ...
        if (params && params.hasOwnProperty(DRAG_PARAMS_META_CONTAINER_ID)) {
          ...
          if (dragEffects.length === 1 &&
            ...
            if (gmeIDList.length > 0 && gmeIDList.indexOf(CONSTANTS.PROJECT_ROOT_ID) ===
              -1) {
  ...
  /**********************************************************/
  MetaEditorControlDiagramDesignerWidgetEventHandlers.prototype._onBackgroundDrop =
      ...
      if (params &&
        ...
        for (i in params.positions) {
          if (params.positions.hasOwnProperty(i)) {
            ...
            _client.setMemberRegistry(aspectNodeID, i, this._selectedMetaAspectSet,
            ...
            this.diagramDesigner.updateDesignerItem(componentID, {
              "position": {
                'x': posX,
                'y': posY
              }
        ...
        if (dragEffects.length === 1 &&
          ...
          if (newGmeID) {
            ...
            if (origNode) {
              var newName = origNode.getAttribute(nodePropertyNames.Attributes
                .name) + "_instance";
              _client.setAttributes(newGmeID, nodePropertyNames.Attributes.name,
                newName);
            }

            _client.addMember(aspectNodeID, newGmeID, this._selectedMetaAspectSet);
            _client.setMemberRegistry(aspectNodeID, newGmeID, this._selectedMetaAspectSet,
              REGISTRY_KEYS.POSITION, {
                'x': position.x,
                'y': position.y
              });
            ...
            _client.setMemberRegistry(aspectNodeID, newGmeID,
              MetaEditorConstants.META_ASPECT_SET_NAME, REGISTRY_KEYS.POSITION, {
                'x': position.x,
                'y': position.y
              });
          }
            ...
            for (i = 0; i < gmeIDList.length; i += 1) {
              ...
              if (this._metaAspectMembersPerSheet[this._selectedMetaAspectSet]
                ...
                if (params &&
                  ...
                  if (params && params.positions && params.positions[
                    componentID]) {
                ...
                _client.setMemberRegistry(aspectNodeID, componentID, this._selectedMetaAspectSet,
                  REGISTRY_KEYS.POSITION, {
                    'x': posX,
                    'y': posY
                  });
                ...
                if (this._metaAspectMembersAll.indexOf(componentID) === -1) {
                  _client.addMember(aspectNodeID, componentID,
                    MetaEditorConstants.META_ASPECT_SET_NAME);
                  _client.setMemberRegistry(aspectNodeID, componentID,
                    MetaEditorConstants.META_ASPECT_SET_NAME, REGISTRY_KEYS.POSITION, {
                      'x': posX,
                      'y': posY
                    });
  ...
  /*************************************************************/
  MetaEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSelectionDelete =
    function (idList) {
      var _client = this._client,
        aspectNodeID = this.metaAspectContainerNodeID,
        len,
      ...
      deleteConnection = function (connectionID) {
        ...
        if (connDesc.type === MetaRelations.META_RELATIONS.CONTAINMENT) {
          ...
          self._deletePointerRelationship(connDesc.GMESrcId, connDesc.GMEDstId,
            connDesc.name, false);
      ...
      doDelete = function (itemsToDelete) {
        ...
        while (len--) {
          ...
          } else if (self._connectionListByID.hasOwnProperty(itemsToDelete[
            len])) {
      ...
      if (metaInfoToBeLost.length > 0) {
        ...
        var confirmMsg =
          "The following items you are about to delete are not present on any other sheet and will be permanently removed from the META aspect:<br><br>";
        var itemNames = [];
        var nodeObj;
  ...
  MetaEditorControlDiagramDesignerWidgetEventHandlers.prototype._getDragParams =
    function (selectedElements, event) {
      var oParams = this._oGetDragParams.call(this.diagramDesigner,
  ...
  MetaEditorControlDiagramDesignerWidgetEventHandlers.prototype._getDragItems =
  ...
  MetaEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSelectionChanged =
    function (selectedIds) {
      ...
      while (len--) {
        ...
        if (id &&
          ...
          onlyConnectionTypeSelected = onlyConnectionTypeSelected &&
            GMEConcepts.isConnectionType(id);
        } else {
          onlyConnectionTypeSelected = false;
        }
      }

      this.diagramDesigner.toolbarItems.ddbtnConnectionArrowStart.enabled(
        onlyConnectionTypeSelected);
      this.diagramDesigner.toolbarItems.ddbtnConnectionPattern.enabled(
        onlyConnectionTypeSelected);
      this.diagramDesigner.toolbarItems.ddbtnConnectionArrowEnd.enabled(
        onlyConnectionTypeSelected);
      this.diagramDesigner.toolbarItems.ddbtnConnectionLineType.enabled(
        onlyConnectionTypeSelected);
      this.diagramDesigner.toolbarItems.ddbtnConnectionLineWidth.enabled(
        onlyConnectionTypeSelected);
  ...
  MetaEditorControlDiagramDesignerWidgetEventHandlers.prototype._onTabAddClicked =
    function () {
      var aspectNodeID = this.metaAspectContainerNodeID,
        aspectNode = this._client.getNode(aspectNodeID),
        metaAspectSheetsRegistry = aspectNode.getEditableRegistry(
      ...
      newSetID = MetaEditorConstants.META_ASPECT_SHEET_NAME_PREFIX +
        generateGuid();
      ...
      if (metaAspectSheetsRegistry.length === 1) {
        ...
        while (len--) {
          ...
          if (this._metaAspectMembersCoordinatesGlobal[componentID]) {
            this._client.setMemberRegistry(aspectNodeID, componentID,
              newSetID, REGISTRY_KEYS.POSITION, {
      ...
      this._client.setRegistry(aspectNodeID, REGISTRY_KEYS.META_SHEETS,
        metaAspectSheetsRegistry);
  ...
  MetaEditorControlDiagramDesignerWidgetEventHandlers.prototype._onTabTitleChanged =
  ...
  MetaEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSelectedTabChanged =
    function (tabID) {
      if (this._sheets[tabID] && this._selectedMetaAspectSet !== this._sheets[
        tabID]) {
  ...
  MetaEditorControlDiagramDesignerWidgetEventHandlers.prototype._onTabDeleteClicked =
    function (tabID) {
      ...
      doDeleteTab = function () {
        ...
        _client.setRegistry(aspectNodeID, REGISTRY_KEYS.META_SHEETS,
          metaAspectSheetsRegistry);
      ...
      if (metaAspectMemberToBeLost.length > 0) {
        //need user confirmation because there is some meta info to be lost
        confirmMsg =
          "You are about to delete a sheet that contains the following items that are not present on any other sheet and will be permanently removed from the META aspect:<br><br>";
        var itemNames = [];
        var nodeObj;
        len = metaAspectMemberToBeLost.length;
        while (len--) {
  ...
  MetaEditorControlDiagramDesignerWidgetEventHandlers.prototype._onTabsSorted =
  ...
  MetaEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSelectionFillColorChanged =
    function (selectedElements, color) {
      this._onSelectionSetColor(selectedElements, color, REGISTRY_KEYS.COLOR);
  };

  MetaEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSelectionBorderColorChanged =
    function (selectedElements, color) {
      this._onSelectionSetColor(selectedElements, color, REGISTRY_KEYS.BORDER_COLOR);
  };

  MetaEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSelectionTextColorChanged =
    function (selectedElements, color) {
      this._onSelectionSetColor(selectedElements, color, REGISTRY_KEYS.TEXT_COLOR);
  };

  MetaEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSelectionSetColor =
      ...
      while (i--) {
        ...
        if (color) {
          this._client.setMemberRegistry(this.metaAspectContainerNodeID,
            gmeID, MetaEditorConstants.META_ASPECT_SET_NAME, regKey, color);
        } else {
          this._client.delMemberRegistry(this.metaAspectContainerNodeID,
            gmeID, MetaEditorConstants.META_ASPECT_SET_NAME, regKey);
        }
      }
      this._client.completeTransaction();
  };
