...
define(['logManager',
    ...
    if (params &&
      ...
      for (i in params.positions) {
        if (params.positions.hasOwnProperty(i)) {
          ...
          _client.setMemberRegistry(aspectNodeID, i, this._selectedMetaAspectSet, REGISTRY_KEYS.POSITION, {
            'x': posX,
            'y': posY
          });
      ...
      if (dragEffects.length === 1 &&
        ...
        if (newGmeID) {
          ...
          _client.setMemberRegistry(aspectNodeID, newGmeID, this._selectedMetaAspectSet, REGISTRY_KEYS.POSITION, {
            'x': position.x,
            'y': position.y
          });
          ...
          _client.setMemberRegistry(aspectNodeID, newGmeID, MetaEditorConstants.META_ASPECT_SET_NAME, REGISTRY_KEYS.POSITION, {
            'x': position.x,
            'y': position.y
          });
        }
        ...
        if (gmeIDList.length > 0) {
          for (i = 0; i < gmeIDList.length; i += 1) {
            ...
            if (this._metaAspectMembersPerSheet[this._selectedMetaAspectSet].indexOf(componentID) === -1) {
              ...
              _client.setMemberRegistry(aspectNodeID, componentID, this._selectedMetaAspectSet, REGISTRY_KEYS.POSITION, {
                'x': posX,
                'y': posY
              });
              ...
              if (this._metaAspectMembersAll.indexOf(componentID) === -1) {
                ...
                _client.setMemberRegistry(aspectNodeID, componentID, MetaEditorConstants.META_ASPECT_SET_NAME,
    ...
    if (metaAspectSheetsRegistry.length === 1) {
      ...
      while (len--) {
        ...
        if (this._metaAspectMembersCoordinatesGlobal[componentID]) {
          this._client.setMemberRegistry(aspectNodeID, componentID, newSetID, REGISTRY_KEYS.POSITION, {
  ...
  MetaEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSelectionFillColorChanged = function (
    selectedElements, color) {
    this._onSelectionSetColor(selectedElements, color, REGISTRY_KEYS.COLOR);
  };

  MetaEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSelectionBorderColorChanged = function (
    selectedElements, color) {
    this._onSelectionSetColor(selectedElements, color, REGISTRY_KEYS.BORDER_COLOR);
  };

  MetaEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSelectionTextColorChanged = function (
    selectedElements, color) {
  ...
  MetaEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSelectionSetColor = function (selectedIds, color,
    ...
    while (i--) {
      ...
      if (color) {
        this._client.setMemberRegistry(this.metaAspectContainerNodeID, gmeID, MetaEditorConstants.META_ASPECT_SET_NAME,
          regKey, color);
      } else {
        this._client.delMemberRegistry(this.metaAspectContainerNodeID, gmeID, MetaEditorConstants.META_ASPECT_SET_NAME,
          regKey);
      }
    }
    this._client.completeTransaction();
