...
define(['logManager',
  ...
  VisualizerPanel.prototype._initialize = function () {
    ...
    this._client.addEventListener(this._client.events.PROJECT_CLOSED,
      function (__project, nodeId) {
        self._p2Editor(false);
      });

    this._client.addEventListener(this._client.events.PROJECT_OPENED,
      function (__project, nodeId) {
        self._p2Editor(false);
      });

    this._client.addEventListener(this._client.events.BRANCH_CHANGED,
      function (__project, nodeId) {
        self._p2Editor(false);
      });

    WebGMEGlobal.State.on('change:' + CONSTANTS.STATE_ACTIVE_VISUALIZER,
      function (model, activeVisualizer) {
    ...
    this._layoutManager.addPanel('visualizerSplitPanel', this._splitPanel,
      'center');
  };
  ...
  VisualizerPanel.prototype._setActiveVisualizer = function (visualizer, ul) {
    ...
    if (this._activeVisualizer[panel] !== visualizer && this._visualizers.hasOwnProperty(
      visualizer)) {
      ...
      if (this._visualizers[visualizer]) {
        ...
        if (PanelClass) {
          this._activePanel[panel] = new PanelClass(this._layoutManager, {
            'client': this._client
          });
  ...
  VisualizerPanel.prototype.setActiveVisualizer = function (visualizer) {
    var panel = WebGMEGlobal.PanelManager.getActivePanel() === this._activePanel
      .p1 ? 'p1' : 'p2',
  ...
  VisualizerPanel.prototype.add = function (menuDesc, callback) {
    var li = $(
        '<li class="center pointer"><a class="btn-env" id=""></a></li>'),
      a = li.find('> a'),
      self = this,
      loaderDiv,
      doCallBack;
    ...
    if (this._visualizers[menuDesc.id]) {
      this.logger.warning("A visualizer with the ID '" + menuDesc.id +
        "' already exists...");
      doCallBack();
    } else {
      ...
      if (menuDesc.panel) {
        ...
        li.loader = new LoaderProgressBar({
          "containerElement": loaderDiv
        });
        li.loader.start();
        a.append(loaderDiv);
        ...
        require([menuDesc.panel],
          ...
          function (err) {
            ...
            a.append(' <i class="glyphicon glyphicon-warning-sign" title="' +
              msg + '"></i>');
            self._removeLoader(li, loaderDiv);
            doCallBack();
          });
        ...
        this.logger.warning("The visualizer with the ID '" + menuDesc.id +
