...
define(['logManager',
  ...
  var PropertyEditorController,
    ...
    NON_RESETABLE_POINTRS = [CONSTANTS.POINTER_BASE, CONSTANTS.POINTER_SOURCE,
  ...
  PropertyEditorController.prototype._initEventHandlers = function () {
    ...
    WebGMEGlobal.State.on('change:' + CONSTANTS.STATE_ACTIVE_SELECTION,
      function (model, activeSelection) {
        if (activeSelection) {
  ...
  PropertyEditorController.prototype._selectedObjectsChanged = function (
    idList) {
    var patterns = {},
      i,
      self = this;
  ...
  PropertyEditorController.prototype._getCommonPropertiesForSelection =
    function (selectedObjIDs) {
      var propList = {},
            ...
            if (resultList.hasOwnProperty(it)) {
              if (otherList.hasOwnProperty(it)) {
                if (resultList[it].isCommon) {
                  resultList[it].isCommon = resultList[it].value ===
                    otherList[it];
                }
              } else {
          ...
          if (commonAttrMeta.hasOwnProperty(attrName)) {
            ...
            if (attrMetaDescriptor) {
              if (commonAttrMeta[attrName].type === attrMetaDescriptor.type) {
                isEnumCommon = commonAttrMeta[attrName].enum &&
                  commonAttrMeta[attrName].enum.length > 0;
                isEnumAttrMeta = attrMetaDescriptor.enum &&
                  attrMetaDescriptor.enum.length > 0;
                if (isEnumCommon && isEnumAttrMeta) {
                  ...
                  if (commonEnumValues.length !== commonAttrMeta[attrName].enum
                    .length) {
                    if (commonEnumValues.length === 0) {
                      ...
                      commonAttrMeta[attrName].enum = commonEnumValues.slice(
                        0);
                    }
                  }
                } else {
      ...
      if (selectionLength > 0) {
        ...
        while (--i >= 0) {
          ...
          if (cNode) {
            ...
            _filterCommon(commonAttrs, flattenedAttrs, i === selectionLength -
              1);

            flattenedPreferences = _getNodeRegistryValues(cNode,
              PREFERENCES_REGISTRY_KEYS);
            _filterCommon(commonPreferences, flattenedPreferences, i ===
              selectionLength - 1);

            flattenedMeta = _getNodeRegistryValues(cNode, META_REGISTRY_KEYS);
            _filterCommon(commonMeta, flattenedMeta, i === selectionLength -
              1);

            flattenedPointers = _getPointerInfo(cNode);
            _filterCommon(commonPointers, flattenedPointers, i ===
              selectionLength - 1);
          }
        }
          ...
          while (i--) {
            ...
            if (cNode) {
              ...
              if (ownAttrNames.indexOf(attrName) !== -1) {
                ...
                if (baseNode) {
                  resetable = baseNode.getAttributeNames().indexOf(attrName) !==
                    -1;
                } else {
                  resetable = false;
                }
          ...
          while (i--) {
            ...
            if (cNode) {
              ...
              if (ownRegistryNames.indexOf(regName) !== -1) {
                ...
                if (baseNode) {
                  resetable = baseNode.getRegistryNames().indexOf(regName) !==
                    -1;
                } else {
                  resetable = false;
                }
          ...
          while (i--) {
            ...
            if (cNode) {
              ...
              if (ownPointerNames.indexOf(pointerName) !== -1) {
                ...
                //#2: the registry is inherited and overridden on this level (but defined somewhere up in the hierarchy)
                if (baseNode) {
                  resetable = baseNode.getPointerNames().indexOf(pointerName) !==
                    -1;
                } else {
                  resetable = false;
                }
          ...
          for (i in srcList) {
            if (srcList.hasOwnProperty(i)) {
              ...
              if (doDisplay) {
                ...
                if (srcList[i].readOnly === false || srcList[i].readOnly ===
                  true) {
                ...
                if (srcList[i].isCommon === false) {
                  ...
                  dstList[extKey].options = {
                    "textColor": noCommonValueColor
                  };
                }
                ...
                if (isAttribute && commonAttrMeta[i].enum && commonAttrMeta[i]
                  .enum.length > 0) {
        ...
        propList[PROPERTY_GROUP_ATTRIBUTES] = {
          "name": PROPERTY_GROUP_ATTRIBUTES,
          "text": PROPERTY_GROUP_ATTRIBUTES,
          "value": undefined
        };

        propList[PROPERTY_GROUP_PREFERENCES] = {
          "name": PROPERTY_GROUP_PREFERENCES,
          "text": PROPERTY_GROUP_PREFERENCES,
          "value": undefined
        };

        propList[PROPERTY_GROUP_META] = {
          "name": PROPERTY_GROUP_META,
          "text": PROPERTY_GROUP_META,
          "value": undefined
        };

        propList[PROPERTY_GROUP_POINTERS] = {
          "name": PROPERTY_GROUP_POINTERS,
          "text": PROPERTY_GROUP_POINTERS,
          "value": undefined
        };

        _addItemsToResultList(commonAttrs, PROPERTY_GROUP_ATTRIBUTES,
          propList, true, false, false);

        _addItemsToResultList(commonPreferences, PROPERTY_GROUP_PREFERENCES,
          propList, false, true, false);

        _addItemsToResultList(commonMeta, PROPERTY_GROUP_META, propList,
          false, true, false);

        _addItemsToResultList(commonPointers, PROPERTY_GROUP_POINTERS,
          propList, false, false, true);
      }
  ...
  PropertyEditorController.prototype._onPropertyChanged = function (args) {
    ...
    while (--i >= 0) {
      ...
      } else if (keyArr[0] === PROPERTY_GROUP_PREFERENCES || keyArr[0] ===
        PROPERTY_GROUP_META) {
