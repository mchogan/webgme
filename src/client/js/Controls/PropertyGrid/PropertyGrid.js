...
define(['logManager',
  ...
  PropertyGrid = function () {
    ...
    this._gui = new PropertyGridPart({
      "el": this.$el
    });
  ...
  PropertyGrid.prototype._render = function () {
    ...
    if (this._ordered === true) {
      ...
      for (i = 0; i < orderedPropNames.length; i += 1) {
        this._addPropertyItem(orderedPropNames[i].split("."), "", this._propertyList[
          orderedPropNames[i]], this._gui);
      }
    } else {
      for (i in this._propertyList) {
        if (this._propertyList.hasOwnProperty(i)) {
          this._propertyList[i].id = i;
          this._addPropertyItem(i.split("."), "", this._propertyList[i], this
            ._gui);
        }
      }
    }
  ...
  PropertyGrid.prototype._addPropertyItem = function (attrID, prefix,
    propDesc, guiObj) {
    var parentFolderKey,
      parentFolderName;
    ...
    if (attrID.length > 1) {
      ...
      parentFolderKey = prefix !== "" ? prefix + parentFolderName :
        parentFolderName;
      this._folders[parentFolderKey] = this._folders[parentFolderKey] ||
        guiObj.addFolder(parentFolderName);
      attrID.splice(0, 1);
      this._addPropertyItem(attrID, parentFolderKey + ".", propDesc, this._folders[
        parentFolderKey]);
    } else {
      if (propDesc.value === undefined || propDesc.value === null) {
        this._folders[propDesc.name] = guiObj.addFolder(propDesc.name,
          propDesc.text);
      } else {
