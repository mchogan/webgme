...
define(['js/Controls/PropertyGrid/PropertyGridWidgetManager',
  ...
  var PropertyGridPart,
    ...
    RESET_BUTTON_BASE = $(
      '<i class="glyphicon glyphicon-remove-circle btn-reset" title="Reset value"/>'
    );
  ...
  PropertyGridPart.prototype._addNestedGUI = function (params) {
    var title_row = this._addRow(this, $(document.createTextNode(params.text ||
        params.name))),
      on_click_title,
      self = this;
  ...
  PropertyGridPart.prototype._getAccumulatedName = function () {
    var parentName = this._parent ? this._parent._getAccumulatedName() :
      undefined;
  ...
  PropertyGridPart.prototype.add = function (propertyDesc) {
    ...
    if (this.__widgets[propertyDesc.name] !== undefined) {
      throw new Error('You already have a widget with the name "' +
        propertyDesc.name + '"');
    }
