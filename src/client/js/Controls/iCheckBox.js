...
define(['jquery',
  ...
  var iCheckBox,
    ...
    EL_BASE = $(
      '<div class="iCheckBox checked"><div class="sw"></div><div class="txt"></div></div>'
    );

  iCheckBox = function (options) {
    var self = this;

    this._checkedText = (options && options.checkedText) ||
      DEFAULT_CHECKED_TEXT;
    this._checkedText = this._checkedText.toUpperCase();
    this._uncheckedText = (options && options.uncheckedText) ||
      DEFAULT_UNCHECKED_TEXT;
