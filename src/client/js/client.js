...
define([
  ...
  function (
    ...
    function Client(_configuration) {
      ...
      _configuration.autoreconnect = _configuration.autoreconnect === null ||
        _configuration.autoreconnect === undefined ? true : _configuration.autoreconnect;
      _configuration.reconndelay = _configuration.reconndelay || 1000;
      _configuration.reconnamount = _configuration.reconnamount || 1000;
      _configuration.autostart = _configuration.autostart === null ||
        _configuration.autostart === undefined ? false : _configuration.autostart;
          ...
          nextServerEvent = function (err, guid, parameters) {
            ...
            if (!err && parameters) {
              ...
              case "PROJECT_CREATED":
                _self.dispatchEvent(_self.events.SERVER_PROJECT_CREATED,
                  parameters.project);
                break;
              case "PROJECT_DELETED":
                _self.dispatchEvent(_self.events.SERVER_PROJECT_DELETED,
                  parameters.project);
                break;
              case "BRANCH_CREATED":
        ...
        var branchHashUpdated = function (err, newhash, forked) {
          if (branch === _branch && !_offline) {
            if (!err && typeof newhash === 'string') {
              if (newhash === '') {
                logger.warning('The current branch ' + branch +
                  ' have been deleted!');
                ...
                return _project.getBranchHash(branch, _recentCommits[0],
                  branchHashUpdated);
              }
            } else {
              if (callback) {
      ...
      function networkWatcher() {
        ...
        var reconnecting = function () {
          ...
          var timerId = setInterval(function () {
            if (counter < reConnAmount && _networkStatus === _self.networkStates
        ...
        var dbStatusUpdated = function (err, newstatus) {
          if (running) {
            if (!err && newstatus && _networkStatus !== newstatus) {
              ...
              _self.dispatchEvent(_self.events.NETWORKSTATUS_CHANGED,
                _networkStatus);
            }
            return _database.getDatabaseStatus(_networkStatus,
              dbStatusUpdated);
          }
          return;
        };
      ...
      function commitCache() {
        ...
        function addCommit(commitObject) {
          if (_cache[commitObject._id]) {
            ...
            var index = 0;
            while (index < _timeOrder.length && _cache[_timeOrder[index]].time >
              commitObject.time) {
              index++;
            }
            ...
            if (_cache[commitHash]) {
              ...
              if (_timeOrder.length > index + number) {
                ...
                fillCache(_cache[_timeOrder[_timeOrder.length - 1]].time,
                  number - (_timeOrder.length - (index + 1)), cacheFilled);
              }
            } else {
        ...
        _database.openProject(name, function (err, p) {
          if (!err && p) {
            _database.getAuthorizationInfo(name, function (err, authInfo) {
              _readOnlyProject = authInfo ? (authInfo.write === true ?
                false : true) : true;
              _project = p;
              _projectName = name;
              _inTransaction = false;
                  ...
                  if (firstName) {
                    branchWatcher(firstName, function (err) {
                      if (!err) {
                        ...
                        logger.error('The branch ' + firstName +
                          ' of project ' + name +
                          ' cannot be selected! [' + JSON.stringify(err) +
                          ']');
                        callback(err);
                      }
                    });
            ...
            logger.error('The project ' + name + ' cannot be opened! [' +
              JSON.stringify(err) + ']');
            callback(err);
          }
        });
      ...
      function createEmptyProject(project, callback) {
        ...
        var commitHash = project.makeCommit([], rootHash,
          'project creation commit', function (err) {});
      ...
      function storeNode(node, basic) {
        ...
        if (_nodes[path]) {
          ...
          _nodes[path] = {
            node: node,
            hash: "" /*,incomplete:true,basic:basic*/
          };
        }
        return path;
      }
          ...
          if (core.getChildrenRelids(nodesSoFar[path].node).length > 0) {
            core.loadChildren(nodesSoFar[path].node, function (err, children) {
              if (!err && children) {
                ...
                var error = null;
                for (var i = 0; i < children.length; i++) {
                  loadChildrenPattern(core, nodesSoFar, children[i], level -
                    1, function (err) {
                      error = error || err;
                      if (--missing === 0) {
                        callback(error);
          ...
          if (missing > 0) {
            for (i = 0; i < childrenPaths.length; i++) {
              if (nodesSoFar[childrenPaths[i]]) {
                loadChildrenPattern(core, nodesSoFar, nodesSoFar[childrenPaths[
                  i]].node, level - 1, function (err) {
                  error = error || err;
                  if (--missing === 0) {
                    callback(error);
                  }
                ...
                core.loadChild(node, childrenRelids[i], function (err, child) {
                  if (err || child === null) {
                    ...
                    loadChildrenPattern(core, nodesSoFar, child, level - 1,
                      function (err) {
                        error = error || err;
                        if (--missing === 0) {
                          callback(error);
        ...
        var patterns = {},
          ...
          loadNextPattern = function (index) {
            if (index < orderedPatternIds.length) {
              loadPattern(_core, orderedPatternIds[index], patterns[
                orderedPatternIds[index]], _loadNodes, function (err) {
                error = error || err;
        ...
        for (i = 0; i < keysi.length; i++) {
          ...
          for (j = 0; j < keysj.length; j++) {
            if (patterns[keysj[j]]) {
              ...
              if (patterns[keysj[j]].children < _users[keysi[i]].PATTERNS[keysj[
                j]].children) {
                patterns[keysj[j]].children = _users[keysi[i]].PATTERNS[keysj[j]]
                  .children;
              }
            } else {
      ...
      function saveRoot(msg, callback) {
        ...
        if (!_viewer && !_readOnlyProject) {
          ...
          if (!_inTransaction) {
            ...
            var newCommitHash = _project.makeCommit([_recentCommits[0]],
              newRootHash, _msg, function (err) {
                //TODO now what??? - could we end up here?
              });
            _msg = "";
            addCommit(newCommitHash);
            _project.setBranchHash(_branch, _recentCommits[1], _recentCommits[0],
              function (err) {
            ...
            _project.getBranchNames(function (err, names) {
              if (!err && names) {
                ...
                var getBranchValues = function (name) {
                  _project.getBranchHash(name, '#hack', function (err,
                    newhash, forked) {
                    if (!err && newhash) {
                      var element = {
                        name: name,
      ...
      function viewerCommit(hash, callback) {
        ...
        _project.loadObject(hash, function (err, commitObj) {
          if (!err && commitObj) {
            ...
            logger.error('Cannot view given ' + hash +
              ' commit as it\'s root cannot be loaded! [' + JSON.stringify(
                err) + ']');
            callback(err);
          }
        });
      ...
      function deleteBranchAsync(branchName, callback) {
        if (_database) {
          if (_project) {
            _project.getBranchHash(branchName, '', function (err, newhash,
              forkedhash) {
              if (!err && newhash) {
                if (forkedhash) {
                  _project.setBranchHash(branchName, newhash, forkedhash,
                    function (err) {
                      if (!err) {
                        changeBranchState(_self.branchStates.SYNC);
                      }
      ...
      function connectToDatabaseAsync(options, callback) {
        ...
        options = options || {};
        callback = callback || function () {};
        options.open = (options.open !== undefined || options.open !== null) ?
          options.open : false;
              ...
              if (options.project) {
                ...
                _database.getProjectNames(function (err, names) {
                  if (!err && names && names.length > 0) {
                    ...
                    logger.error(
                      'Cannot get project names / There is no project on the server'
                    );
                    callback(err);
                  }
                });
      ...
      function copyMoreNodes(parameters, msg) {
        ...
        if (typeof parameters.parentId === 'string' && _nodes[parameters.parentId] &&
          ...
          msg = msg || 'copyMoreNodes(' + pathestocopy + ',' + parameters.parentId +
            ')';
          if (pathestocopy.length < 1) {} else if (pathestocopy.length === 1) {
            var newNode = _core.copyNode(_nodes[pathestocopy[0]].node, _nodes[
              parameters.parentId].node);
            storeNode(newNode);
            if (parameters[pathestocopy[0]]) {
              for (var j in parameters[pathestocopy[0]].attributes) {
                _core.setAttribute(newNode, j, parameters[pathestocopy[0]].attributes[
                  j]);
              }
              for (j in parameters[pathestocopy[0]].registry) {
                _core.setRegistry(newNode, j, parameters[pathestocopy[0]].registry[
                  j]);
              }
            }
            saveRoot(msg);
          } else {
            copyMoreNodesAsync(pathestocopy, parameters.parentId, function (err,
              copyarr) {
              if (err) {
                ...
                for (var i in copyarr) {
                  if (parameters[i]) {
                    for (var j in parameters[i].attributes) {
                      _core.setAttribute(copyarr[i], j, parameters[i].attributes[
                        j]);
                    }
                    for (j in parameters[i].registry) {
                      _core.setRegistry(copyarr[i], j, parameters[i].registry[
                        j]);
                    }
                  }
                }
      ...
      function copyMoreNodesAsync(nodePaths, parentPath, callback) {
        var checkPaths = function () {
          ...
          for (var i = 0; i < nodePaths.length; i++) {
            result = result && (_nodes[nodePaths[i]] && typeof _nodes[
              nodePaths[i]].node === 'object');
          }
          return result;
        };
        ...
        if (_nodes[parentPath] && typeof _nodes[parentPath].node === 'object' &&
          ...
          for (var i = 0; i < nodePaths.length; i++) {
            ...
            helpArray[nodePaths[i]].origparent = _core.getParent(_nodes[
              nodePaths[i]].node);
            helpArray[nodePaths[i]].tempnode = _core.moveNode(_nodes[nodePaths[
              i]].node, tempFrom);
            subPathArray[_core.getRelid(helpArray[nodePaths[i]].tempnode)] =
              nodePaths[i];
          ...
          for (var i = 0; i < nodePaths.length; i++) {
            helpArray[nodePaths[i]].node = _core.moveNode(helpArray[nodePaths[i]]
              .tempnode, helpArray[nodePaths[i]].origparent);
            ...
            if (!err && children && children.length > 0) {
              for (i = 0; i < children.length; i++) {
                if (subPathArray[_core.getRelid(children[i])]) {
                  ...
                  storeNode(newNode);
                  returnArray[subPathArray[_core.getRelid(children[i])]] =
                    newNode;
                } else {
      ...
      function _copyMoreNodes(parameters) {
        ...
        for (i = 0; i < paths.length; i++) {
          ...
          for (j = 0; j < keys.length; j++) {
            _core.setAttribute(copiedNodes[i], keys[j], parameters[paths[i]].attributes[
              keys[j]]);
          }

          keys = Object.keys(parameters[paths[i]].registry || {});
          for (j = 0; j < keys.length; j++) {
            _core.setRegistry(copiedNodes[i], keys[j], parameters[paths[i]].registry[
              keys[j]]);
          }
        }
      ...
      function moveMoreNodes(parameters) {
        ...
        if (pathsToMove.length > 0 && typeof parameters.parentId === 'string' &&
          _nodes[parameters.parentId] && typeof _nodes[parameters.parentId].node ===
          'object') {
          for (var i = 0; i < pathsToMove.length; i++) {
            if (_nodes[pathsToMove[i]] && typeof _nodes[pathsToMove[i]].node ===
              'object') {
              var newNode = _core.moveNode(_nodes[pathsToMove[i]].node, _nodes[
                parameters.parentId].node);
              ...
              if (parameters[pathsToMove[i]].attributes) {
                for (var j in parameters[pathsToMove[i]].attributes) {
                  _core.setAttribute(newNode, j, parameters[pathsToMove[i]].attributes[
                    j]);
                }
              }
              if (parameters[pathsToMove[i]].registry) {
                for (var j in parameters[pathsToMove[i]].registry) {
                  _core.setRegistry(newNode, j, parameters[pathsToMove[i]].registry[
                    j]);
                }
              }
      ...
      function createChildren(parameters, msg) {
        ...
        for (i = 0; i < nodes.length; i++) {
          newChildren.push(_core.createNode({
            parent: parent,
            base: nodes[i]
          }));
        }
        ...
        for (i = 0; i < paths.length; i++) {
          ...
          for (j = 0; j < names.length; j++) {
            _core.setAttribute(newChildren[i], names[j], parameters[paths[i]].attributes[
              names[j]]);
          }
          //registry
          names = Object.keys(parameters[paths[i]].registry || {});
          for (j = 0; j < names.length; j++) {
            _core.setRegistry(newChildren[i], names[j], parameters[paths[i]].registry[
              names[j]]);
          }
          ...
          for (j = 0; j < names.length; j++) {
            _core.setPointer(newChildren[i], names[j], newChildren[relations[i]
              [names[j]]]);
          }
        ...
        if (_core) {
          if (typeof parameters.parentId === 'string' && _nodes[parameters.parentId] &&
            ...
            if (parameters.position) {
              _core.setRegistry(child, "position", {
            ...
            msg = msg || 'createChild(' + parameters.parentId + ',' +
              parameters.baseId + ',' + newID + ')';
            saveRoot(msg);
          }
        }
      ...
      function setMemberAttribute(path, memberpath, setid, name, value, msg) {
        if (_nodes[path] && typeof _nodes[path].node === 'object') {
          _core.setMemberAttribute(_nodes[path].node, setid, memberpath, name,
            value);
          msg = msg || 'setMemberAttribute(' + path + "," + memberpath + "," +
            setid + "," + name + "," + value + ")";
          saveRoot(msg);
        }
      }
      ...
      function delMemberAttribute(path, memberpath, setid, name, msg) {
        if (_nodes[path] && typeof _nodes[path].node === 'object') {
          ...
          msg = msg || 'delMemberAttribute(' + path + "," + memberpath + "," +
            setid + "," + name + ")";
          saveRoot(msg);
        }
      }
      ...
      function setMemberRegistry(path, memberpath, setid, name, value, msg) {
        if (_nodes[path] && typeof _nodes[path].node === 'object') {
          _core.setMemberRegistry(_nodes[path].node, setid, memberpath, name,
            value);
          msg = msg || 'setMemberRegistry(' + path + "," + memberpath + "," +
            setid + "," + name + "," + value + ")";
          saveRoot(msg);
        }
      }
      ...
      function delMemberRegistry(path, memberpath, setid, name, msg) {
        if (_nodes[path] && typeof _nodes[path].node === 'object') {
          ...
          msg = msg || 'delMemberRegistry(' + path + "," + memberpath + "," +
            setid + "," + name + ")";
          saveRoot(msg);
        }
      }
      ...
      function setAttributeDescriptor(path, attributename, descriptor) {
        if (_core && _nodes[path] && typeof _nodes[path].node === 'object') {
          _core.setAttributeDescriptor(_nodes[path].node, attributename,
            descriptor);
      ...
      function setBase(path, basepath) {
        ...
        if (_core && _nodes[path] && typeof _nodes[path].node === 'object' &&
          _nodes[basepath] && typeof _nodes[basepath].node === 'object') {
      ...
      function getNode(_id) {
        ...
        var getPointer = function (name) {
          ...
          if (name === 'base') {
            ...
            return {
              to: _core.getPath(_core.getBase(_nodes[_id].node)),
              from: []
            };
          }
          return {
            to: _core.getPointerPath(_nodes[_id].node, name),
            from: []
          };
        };
        var getOwnPointer = function (name) {
          return {
            to: _core.getOwnPointerPath(_nodes[_id].node, name),
            from: []
          };
        };
        ...
        var getMemberAttributeNames = function (setid, memberid) {
          return _core.getMemberAttributeNames(_nodes[_id].node, setid,
            memberid);
        };
        var getMemberAttribute = function (setid, memberid, name) {
          return _core.getMemberAttribute(_nodes[_id].node, setid, memberid,
            name);
        };
        var getEditableMemberAttribute = function (setid, memberid, name) {
          var attr = _core.getMemberAttribute(_nodes[_id].node, setid,
            memberid, name);
        ...
        var getMemberRegistryNames = function (setid, memberid) {
          return _core.getMemberRegistryNames(_nodes[_id].node, setid,
            memberid);
        };
        var getMemberRegistry = function (setid, memberid, name) {
          return _core.getMemberRegistry(_nodes[_id].node, setid, memberid,
            name);
        };
        var getEditableMemberRegistry = function (setid, memberid, name) {
          var attr = _core.getMemberRegistry(_nodes[_id].node, setid,
            memberid, name);
          if (attr !== null && attr !== undefined) {
            return JSON.parse(JSON.stringify(attr));
          }
        ...
        var getEditableAttributeDescriptor = function (attributename) {
          var descriptor = _core.getAttributeDescriptor(_nodes[_id].node,
            attributename);
        ...
        var getEditablePointerDescriptor = function (pointername) {
          var descriptor = _core.getPointerDescriptor(_nodes[_id].node,
            pointername);
          if (typeof descriptor === 'object') {
            descriptor = JSON.parse(JSON.stringify(descriptor));
          }
      ...
      function getExportItemsUrlAsync(paths, filename, callback) {
        _database.simpleRequest({
          ...
          if (err) {
            ...
            callback(null, window.location.protocol + '//' + window.location.host +
              '/worker/simpleResult/' + resId + '/' + filename);
          }
        });
      }

      function getExternalInterpreterConfigUrlAsync(selectedItemsPaths,
        filename, callback) {
        var config = {};
        config.host = window.location.protocol + "//" + window.location.host;
        config.project = _projectName;
        config.token = _TOKEN.getToken();
        config.selected = plainUrl({
          command: 'node',
          path: selectedItemsPaths[0] || ""
        });
        config.commit = URL.addSpecialChars(_recentCommits[0] || "");
        config.root = plainUrl({
          command: 'node'
        });
        config.branch = _branch
        _database.simpleRequest({
          command: 'generateJsonURL',
          object: config
        }, function (err, resId) {
          if (err) {
            callback(err);
        ...
        if (command.name && command.hash) {
          _database.simpleRequest(command, function (err, resId) {
            if (err) {
              ...
              callback(null, window.location.protocol + '//' + window.location
                .host + '/worker/simpleResult/' + resId + '/' + filename);
            }
          });
        } else {
          callback(new Error('there is no open project!'));
      ...
      function updateLibraryAsync(libraryRootPath, newLibrary, callback) {
        Serialization.import(_core, _nodes[libraryRootPath].node, newLibrary,
          function (err, log) {
            if (err) {
              return callback(err);
            }
      ...
      function addLibraryAsync(libraryParentPath, newLibrary, callback) {
        ...
        var libraryRoot = createChild({
          parentId: libraryParentPath,
          baseId: null
        }, "library placeholder");
        Serialization.import(_core, _nodes[libraryRoot].node, newLibrary,
          function (err, log) {
            if (err) {
              return callback(err);
            }
      ...
      function createProjectFromFileAsync(projectname, jProject, callback) {
        ...
        createProjectAsync(projectname, function (err) {
          selectProjectAsync(projectname, function (err) {
            Serialization.import(_core, _nodes[ROOT_PATH].node, jProject,
              function (err) {
                if (err) {
                  return callback(err);
                }
      ...
      function getAvailableInterpreterNames() {
        var names = [];
        var valids = _nodes[ROOT_PATH] ? _core.getRegistry(_nodes[ROOT_PATH].node,
          'validPlugins') || "" : "";
        valids = valids.split(" ");
          ...
          if (!err) {
            ...
            _database.getProjectNames(function (err, names) {
              if (!err && names && names.length > 0) {
                ...
                if (_configuration.project && names.indexOf(_configuration.project) !==
                  -1) {
                ...
                openProject(projectName, function (err) {
                  if (err) {
                    logger.error('Problem during project opening:' + JSON
                      .stringify(err));
                  }
                });
              } else {
