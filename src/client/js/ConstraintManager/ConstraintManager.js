  ...
  ConstraintManager.prototype.validate = function (id) {
    ...
    doValidate = function () {
      if (constraints.length === 0) {
        addValidationResult('SUCCESS: No constraints defined for <strong>' +
          node_name + '</strong>', 'alert-success');
      } else {
        constraints.forEach(function (constraint) {
          var msg = ': <strong>' + constraint.name + '</strong>' +
          ...
          try {
            ...
            if (result === true) {
              ...
              addValidationResult('FAIL' + msg +
                '<br/><span class="muted">' + constraint.message +
                '</span>', 'alert-error');
            }
          } catch (exp) {
            addValidationResult('ERROR' + msg +
              '<br/><span class="muted">Constraint threw an exception:<br/>' +
              exp.message + '</span>', 'alert-error');
          }
        });
      }
    };
