...
define(['js/PanelBase/PanelBase',
  ...
  PanelBaseWithHeader = function (options, layoutManager) {
    ...
    options[PanelBaseWithHeader.OPTIONS.LOGGER_INSTANCE_NAME] = options[
      PanelBaseWithHeader.OPTIONS.LOGGER_INSTANCE_NAME] ||
      "PanelBaseWithHeader";

    //add PanelBaseWithHeader specific options when not present
    options[PanelBaseWithHeader.OPTIONS.HEADER_TITLE] = options[
      PanelBaseWithHeader.OPTIONS.HEADER_TITLE] === true;
    options[PanelBaseWithHeader.OPTIONS.FLOATING_TITLE] = options[
      PanelBaseWithHeader.OPTIONS.FLOATING_TITLE] === true;
  ...
  PanelBaseWithHeader.prototype.initUI = function (options) {
    ...
    this.$panelReadOnlyIndicator.append($(
  ...
  PanelBaseWithHeader.prototype._setSize = function (w, h) {
    var panelHeaderHeight = this.$panelHeader.outerHeight(true),
      ...
      panelBorder = parseInt(this.$_el.css('border-left-width')) + parseInt(
        this.$_el.css('border-right-width')),
      panelMargin = parseInt(this.$_el.css('margin-top')) + parseInt(this.$_el
        .css('margin-bottom'));
    ...
    this.$panelBody.width(w - panelBodyPadding).height(h - panelHeaderHeight -
      panelBodyPaddingV);
