  ...
  BezierHelper = {
    ...
    getBezierControlPoints: function (boundingBox1, boundingBox2) {
      ...
      if ((bb1.x === bb2.x) && (bb1.y === bb2.y)) {
        ...
        for (i = 0; i < 4; i += 1) {
          for (j = 4; j < 8; j += 1) {
            ...
            if ((i === j - 4) || (((i !== 3 && j !== 6) || p[i].x < p[j].x) &&
              ((i !== 2 && j !== 7) || p[i].x > p[j].x) && ((i !== 0 && j !==
                5) || p[i].y > p[j].y) && ((i !== 1 && j !== 4) || p[i].y <
                p[j].y))) {
              dis.push(dx + dy);
        ...
        x[3] = [0, 0, 0, 0, x[4], x[4], x[4] - dx, x[4] + dx][res[1]].toFixed(
          3);
        y[3] = [0, 0, 0, 0, y[1] + dy, y[1] - dy, y[4], y[4]][res[1]].toFixed(
          3);
      }

      result.push({
        "x": parseFloat(x[1].toFixed(3)),
        "y": parseFloat(y[1].toFixed(3))
      });
      result.push({
        "x": parseFloat(x[2]),
        "y": parseFloat(y[2])
      ...
      result.push({
        "x": parseFloat(x[4].toFixed(3)),
        "y": parseFloat(y[4].toFixed(3))
      });
    ...
    getBezierControlPoints2: function (pointA, pointB) {
      ...
      if ((pointA.x === pointB.x) && (pointA.y === pointB.y)) {
        ...
        x[2] = [x[1], x[1], x[1] - dx, x[1] + dx][sideDescriptor[0]].toFixed(
          3);
        y[2] = [y[1] - dy, y[1] + dy, y[1], y[1]][sideDescriptor[0]].toFixed(
          3);
        x[3] = [x[4], x[4], x[4] - dx, x[4] + dx][sideDescriptor[1]].toFixed(
          3);
        y[3] = [y[1] + dy, y[1] - dy, y[4], y[4]][sideDescriptor[1]].toFixed(
          3);
      }
