...
define(['jquery',
  ...
  var _exIntConf = function (objIDs) {
    ...
    if (_.isArray(objIDs) &&
      objIDs.length > 0) {
      _client.getExternalInterpreterConfigUrlAsync(objIDs, fileName,
        function (err, url) {
          if (!err) {
            window.location = url;
          }
  ...
  var _expLib = function (objID) {
    if (objID) {
      var object = _client.getNode(objID),
        fileName = _client.getActiveProjectName() + "_" + _client.getActualBranch() +
        "_" + object.getAttribute('name') + "_lib";
