...
define(['jquery',
  ...
  var _getValidConnectionTypes = function (sourceID, targetID, parentID) {
    var validTypes = [],
      validChildrenTypes = _getValidConnectionTypesFromSource(sourceID,
        parentID),
      len = validChildrenTypes.length;

    while (len--) {
      if (_client.isValidTarget(validChildrenTypes[len], CONSTANTS.POINTER_TARGET,
        targetID)) {
  ...
  var _isValidConnection = function (sourceID, targetID, connectionID) {
    ...
    if (sourceID !== undefined && targetID !== undefined && connectionID !==
      undefined) {
      if (_client.isValidTarget(connectionID, CONSTANTS.POINTER_SOURCE,
          sourceID) &&
        _client.isValidTarget(connectionID, CONSTANTS.POINTER_TARGET,
          targetID)) {
        valid = true;
      }
    }
  ...
  var _createBasicProjectSeed = function () {
    ...
    var FCO_ID = _client.createChild({
      'parentId': CONSTANTS.PROJECT_ROOT_ID,
    ...
    _client.setAttributes(CONSTANTS.PROJECT_ROOT_ID, nodePropertyNames.Attributes
      .name, CONSTANTS.PROJECT_ROOT_NAME);
    ...
    _client.setRegistry(CONSTANTS.PROJECT_ROOT_ID, REGISTRY_KEYS.PROJECT_REGISTRY,
      projectRegistry);

    //set META ASPECT to show FCO
    _client.addMember(CONSTANTS.PROJECT_ROOT_ID, FCO_ID,
      MetaEditorConstants.META_ASPECT_SET_NAME);
    _client.setMemberRegistry(CONSTANTS.PROJECT_ROOT_ID, FCO_ID,
    ...
    var defaultMetaSheetID = MetaEditorConstants.META_ASPECT_SHEET_NAME_PREFIX +
      generateGuid();
    _client.createSet(CONSTANTS.PROJECT_ROOT_ID, defaultMetaSheetID);

    var defaultMetaSheetDesc = {
      'SetID': defaultMetaSheetID,
      'order': 0,
      'title': 'META'
    };

    _client.setRegistry(CONSTANTS.PROJECT_ROOT_ID, REGISTRY_KEYS.META_SHEETS, [
      defaultMetaSheetDesc
    ]);

    //add the FCO to the default META sheet
    _client.addMember(CONSTANTS.PROJECT_ROOT_ID, FCO_ID, defaultMetaSheetID);
    _client.setMemberRegistry(CONSTANTS.PROJECT_ROOT_ID, FCO_ID,
      defaultMetaSheetID, REGISTRY_KEYS.POSITION, {
        'x': 100,
        'y': 100
      });
        ...
        while (len--) {
          if (_client.isTypeOf(baseId, validChildrenItems[len].id)) {
            ...
            if (validChildrenItems[len].max !== undefined &&
              ...
              counter + validChildrenTypeMap[validChildrenItems[len].id] >
              validChildrenItems[len].max) {
              result = false;
              break;
            }
          }
  ...
  var _getValidPointerTypes = function (parentId, targetId) {
    var validChildrenTypes = _getMETAAspectMergedValidChildrenTypes(
        parentId),
      i,
      childObj,
      ptrNames,
    ...
    while (i--) {
      if (_canCreateChild(parentId, validChildrenTypes[i])) {
        ...
        if (childObj) {
          ptrNames = _.difference(childObj.getPointerNames().slice(0),
            EXCLUDED_POINTERS);
          j = ptrNames.length;
          while (j--) {
            if (_client.isValidTarget(validChildrenTypes[i], ptrNames[j],
              targetId)) {
   ...
   */
  var _getValidConnectionTypesFromSourceInAspect = function (sourceID,
    parentID, aspectName) {
    var validTypes = [],
      i,
      j,
    ...
    if (aspectName) {
      if (aspectName !== CONSTANTS.ASPECT_ALL) {
        ...
        if (metaAspectDesc) {
          ...
          if (aspectTypes.length > 0) {
            validTypes = _getValidConnectionTypesFromSource(sourceID,
              parentID);
   ...
   */
  var _getValidConnectionTypesInAspect = function (sourceID, targetID,
    parentID, aspectName) {
    var validTypes = [],
      canCreateInAspect,
      i,
    ...
    if (aspectName) {
      if (aspectName !== CONSTANTS.ASPECT_ALL) {
        ...
        if (metaAspectDesc) {
          ...
          if (aspectTypes.length > 0) {
            validTypes = _getValidConnectionTypes(sourceID, targetID,
              parentID);
            //each item in validTypes has to be a descendant of any item in aspectTypes
            i = validTypes.length;
            while (i--) {
  ...
  var _isValidTypeInAspect = function (objID, aspectContainerID, aspectName) {
    var result = false,
      aspectTerritoryPattern = _client.getAspectTerritoryPattern(
        aspectContainerID, aspectName),
  ...
  var _getValidPointerTargetTypesFromSource = function (objID, isSet) {
    var result = [],
      ...
      pointerNames = isSet ? _.difference(nodeObj.getSetNames(),
        EXCLUDED_SETS) : _.difference(nodeObj.getPointerNames(),
        EXCLUDED_POINTERS),
    ...
    while (len--) {
      pointerMetaDescriptor = _client.getValidTargetItems(objID,
        pointerNames[len]);
  ...
  var _getValidPointerTypesFromSourceToTarget = function (sourceId, targetId) {
    var result = [],
      ...
      pointerNames = _.difference(nodeObj.getPointerNames(),
        EXCLUDED_POINTERS),
      len = pointerNames.length;
  ...
  var _getFCOId = function () {
    ...
    if (projectRootNode) {
      if (projectRootNode.getRegistryNames().indexOf(REGISTRY_KEYS.PROJECT_REGISTRY) !==
        -1) {
        FCO_ID = projectRootNode.getRegistry(REGISTRY_KEYS.PROJECT_REGISTRY)[
          CONSTANTS.PROJECT_FCO_ID];
      }
    }
