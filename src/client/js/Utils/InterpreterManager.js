  ...
  ClientInterpreterManager.prototype.run = function (name, callback) {
    ...
    getPlugin(name, function (err, plugin) {
      if (!err && plugin) {
        var plugins = {};
        plugins[name] = plugin;
        var pluginManager = new PluginManagerBase(self._client.getProjectObject(),
          Core, plugins);
        pluginManager.initialize(null, function (pluginConfigs,
          configSaveCallback) {
              ...
              if (self._savedConfigs.hasOwnProperty(i)) {
                ...
                while (len--) {
                  if (iConfig.hasOwnProperty(hackedConfig[i][len].name)) {
                    hackedConfig[i][len].value = iConfig[hackedConfig[i][
                      len
                    ].name];
                  }
                }
          ...
          d.show(hackedConfig, function (updatedConfig) {
            ...
            if (configSaveCallback) {
              ...
              if (globalconfig.runOnServer === true) {
                ...
                self._client.runServerPlugin(name, context, function (
                  err, result) {
                  if (err) {
                    console.error(err);
                ...
                pluginManager.executePlugin(name, config, function (err,
                  result) {
                  if (err) {
                    console.error(err);
                  }
