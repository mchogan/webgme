...
define(['jquery',
  ...
  var META_RULES_CONTAINER_NODE_ID = MetaEditorConstants.META_ASPECT_CONTAINER_ID,
    ...
    _events = {
      'META_ASPECT_CHANGED': 'META_ASPECT_CHANGED'
    },
    _metaTypes;
  ...
  var _reset = function () {
    ...
    if (_client.getActiveProjectName()) {
      ...
      _patterns = {};
      _metaMembers = [];
      _patterns[META_RULES_CONTAINER_NODE_ID] = {
        "children": 0
      };
  ...
  var _processMetaContainer = function () {
    ...
    if (metaContainer) {
      ...
      var metaAspectSetMembers = metaContainer.getMemberIds(
      ...
      if (metaAspectSetMembers && metaAspectSetMembers.length > 0) {
        ...
        while (len--) {
          ...
          if (metaMemberNode) {
            nodeName = _metaFriendlyName(metaMemberNode.getAttribute(
              nodePropertyNames.Attributes.name));

            if (nodeName === undefined || nodeName === null || nodeName ===
              "") {
              _logger.error('META item "' + nodeID +
                '" has an invalid name of: ' + nodeName);
            } else {
  ...
  var _generateMETAAspectJavaScript = function () {
    ...
    if (!_.isEmpty(_metaTypes)) {
      ...
      var typeCheckMethodTemplate =
        'var _is__METATYPE__ = function (objID) { return METAAspectHelper.isMETAType(objID, _metaTypes.__METATYPE__); };';
      ...
      for (var t in sortedMetaTypes) {
        if (sortedMetaTypes.hasOwnProperty(t)) {
          typeCheckMethods += typeCheckMethodTemplate.replace(
            /__METATYPE__/g, t) + '\n\t';
          typeCheckMethodsMap.push(typeCheckMethodNamePrefix + t + ": _" +
            typeCheckMethodNamePrefix + t);
          metaAspectTypesMap.push('\'' + t + '\'' + ': ' + '\'' +
            sortedMetaTypes[t] + '\'');
        }
      }
      ...
      content = content.replace(/__META_ASPECT_TYPE_CHECKING__/g,
        typeCheckMethods);
