...
(function ($) { // Hide scope, no $ conflict
  ...
  $.extend(CSSZoom.prototype, {
    ...
    setDefaults: function (options) {
      ...
      this._defaults.zoomLevels.sort(function (a, b) {
        return a - b
      });
      return this;
    },
    ...
    _optionPlugin: function (target, options, value) {
      ...
      if (options.zoomLevels) { //add new zoom-levels
        ...
        inst.zoomLevels.sort(function (a, b) {
          return a - b
        });
      } else {
    ...
    _enablePlugin: function (target) {
      ...
      target.prop('disabled', false).removeClass(this.propertyName +
        '-disabled');
      var inst = target.data(this.propertyName);
    ...
    _disablePlugin: function (target) {
      ...
      target.prop('disabled', true).addClass(this.propertyName +
        '-disabled');
      var inst = target.data(this.propertyName);
  ...
  $.fn.csszoom = function (options) {
    ...
    if (isNotChained(options, otherArgs)) {
      return plugin['_' + options + 'Plugin'].apply(plugin, [this[0]].concat(
        otherArgs));
    }
    return this.each(function () {
      if (typeof options == 'string') {
        ...
        plugin['_' + options + 'Plugin'].apply(plugin, [this].concat(
          otherArgs));
      } else {
