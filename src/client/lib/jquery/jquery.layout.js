  ...
  $.layout = {
    ...
    plugins: {
      ...
      effects: {
        ...
        ,
        slide: $.effects && ($.effects.slide || ($.effects.effect && $.effects
    ...
    cssWidth: function ($E, outerWidth) {
      ...
      var bs = !$.layout.browser.boxModel ? "border-box" : $.support.boxSizing ?
...
,	trackMouse: function (evt) {
  ...
  var u = navigator.userAgent.toLowerCase(),
    m = /(chrome)[ \/]([\w.]+)/.exec(u) || /(webkit)[ \/]([\w.]+)/.exec(u) ||
    /(opera)(?:.*version|)[ \/]([\w.]+)/.exec(u) || /(msie) ([\w.]+)/.exec(u) ||
  ...
  $.fn.layout = function (opts) {
    ...
    , cssW = $.layout.cssWidth, cssH = $.layout.cssHeight, elDims = $.layout.getElementDimensions,
      styles = $.layout.getElementStyles, evtObj = $.layout.getEventObject,
      evtPane = $.layout.parsePaneName

    /**
     * options - populated by initOptions()
     */
    , options = $.extend(true, {}, $.layout.defaults), effects = options.effects =
      $.extend(true, {}, $.layout.effects)
    ...
    , _runCallbacks = function (evtName, pane, skipBoundEvents) {
      ...
      if (fn) {
        try {
          ...
          _log(options.errors.callbackError.replace(/EVENT/, $.trim((pane ||
            "") + " " + lng)), false);
	...
	*/
    , _parseSize = function (pane, size, dir) {
      ...
      if (dir == "horz") // north or south or center.minHeight
        ...
        avail = sC.innerWidth - ($Ps.west ? o.west.spacing_open : 0) - ($Ps.east ?
    ...
    , setSizeLimits = function (pane, slide) {
      ...
      var
        ...
        altPaneSpacing = ((!$altP || altS.isHidden ? 0 : options[altPane][
        ...
        limitSize = (containerSize - paneSpacing - (isSliding ? 0 : (
          ...
          altPaneSpacing))),
        minSize = s.minSize = max(_parseSize(pane, o.minSize), cssMinDims(
          pane).minSize),
        maxSize = s.maxSize = min((o.maxSize ? _parseSize(pane, o.maxSize) :
          100000), limitSize),
    ...
    , _initContainer = function () {
      ...
      for (name in layoutMethods) {
        $N.bind("layout" + name.toLowerCase() + "." + sID, Instance[
          layoutMethods[name] || name]);
      }
    ...
    , addPane = function (pane, force) {
      ...
      for (name in paneMethods) {
        $P.bind("layoutpane" + name.toLowerCase() + "." + sID, Instance[
          paneMethods[name] || name]);
      }
    ...
    , initHandles = function (panes) {
      ...
      $.each(panes, function (i, pane) {
        ...
        var o = options[pane],
          s = state[pane],
          c = _c[pane],
          paneId = o.paneSelector.substr(0, 1) === "#" ? o.paneSelector.substr(
            1) : "",
          ...
          // INIT TOGGLER BUTTON
          ,
          $T = I.toggler = (o.closable ? $Ts[pane] = $("<div></div>") :
            false);
    ...
    , initContent = function (pane, resize) {
      ...
      var
        ...
        $P = $Ps[pane],
        $C;
      if (sel) $C = I.content = $Cs[pane] = (o.findNestedContent) ? $P.find(
          sel).eq(0) // match 1-element only
        : $P.children(sel).eq(0);
      if ($C && $C.length) {
      ...
      $.each(panes, function (idx, pane) {
        ...
        $R.draggable({
          ...
          start: function (e, ui) {
            ...
            if (false === _runCallbacks("ondrag_start", pane)) return
              false;
          ...
          drag: function (e, ui) {
            ...
            if (limit) {
              ui.helper.addClass(helperLimitClass + " " +
                helperPaneLimitClass); // at dragging-limit
              window.defaultStatus = (limit > 0 && pane.match(
                /(north|west)/)) || (limit < 0 && pane.match(
                /(south|east)/)) ? o.tips.maxSizeWarning : o.tips.minSizeWarning;
            } else {
              ui.helper.removeClass(helperLimitClass + " " +
                helperPaneLimitClass); // not at dragging-limit
    ...
    , setAsOpen = function (pane, skipCallback) {
      ...
      if (!skipCallback && (state.initialized || o.triggerEventsOnLoad) && $P
        .is(":visible")) {
        // onopen callback
    ...
    , slideClose = function (evt_or_pane) {
      ...
      // 1 sec delay if 'opening', else .3 sec
        timer.set(pane + "_closeSlider", close_NOW, max(o.slideDelay_close,
        delay));
    ...
    , lockPaneForFX = function (pane, doLock) {
      ...
      if (doLock) {
        ...
        if (browser.msie && o.fxOpacityFix && o.fxName_open != "slide" && $P.css(
          "filter") && $P.css("opacity") == 1)
    ...
    , sizePane = function (evt_or_pane, size, skipCallback, noAnimation,
      ...
      function sizePane_2() {
        ...
        if (!skipCallback) {
          ...
          if (!s.isSliding) sizeMidPanes(_c[pane].dir == "horz" ? "" :
            "center", skipResizeWhileDragging, force);
          sizeHandles();
        }
    ...
    , sizeContent = function (evt_or_panes, remeasure) {
      ...
      $.each(panes, function (idx, pane) {
        ...
        if ((!s.isMoving && !s.isResizing) || o.liveContentResizing ||
          remeasure || m.top == undefined) {
          _measure();
        ...
        var newH = s.innerHeight - (m.spaceAbove - s.css.paddingTop) - (m.spaceBelow -
          s.css.paddingBottom);
        ...
        function _measure() {
          var
            ...
            $Fs = $C.nextAll().not(".ui-layout-mask").not(ignore ||
              ":lt(0)") // not :lt(0) = ALL
            ,
    ...
    , swapPanes = function (evt_or_pane1, pane2) {
      ...
      if (false === _runCallbacks("onswap_start", pane1) || false ===
        _runCallbacks("onswap_start", pane2)
      ) {
      ...
      function move(oPane, pane) {
        ...
        if (c.dir != _c[oldPane].dir) {
          ...
          $Rs[pane].css(c.side, sC.inset[c.side] + (state[pane].isVisible ?
          getPaneSize(pane) : 0));
 ...
 */
  ...
  $.layout.state = {
    ...
    saveCookie: function (inst, keys, cookieOpts) {
      ...
      ;
      $.ui.cookie.write(oC.name || o.name || "Layout", $.layout.state.encodeJSON(
        data), oC);
    ...
    readCookie: function (inst) {
      var o = inst.options;
      var c = $.ui.cookie.read(o.stateManagement.cookie.name || o.name ||
        "Layout");
  ...
  $.layout.browserZoom = {
    ...
    _setTimer: function (inst) {
      ...
      var o = inst.options,
        ...
        ,
        ms = inst.hasParentLayout ? 5000 : Math.max(o.browserZoomCheckInterval,
          100);
      // set the timer
      setTimeout(function () {
