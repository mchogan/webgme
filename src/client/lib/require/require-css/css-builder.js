define(['require', './normalize'], function (req, normalize) {
  ...
  function compress(css) {
    if (typeof process !== "undefined" && process.versions && !!process.versions
      .node && require.nodeRequire) {
      try {
      ...
      console.log('Compressed CSS output to ' + Math.round(css.length /
        csslen * 100) + '%.');
      return css;
    }
  ...
  function loadFile(path) {
    if (typeof process !== "undefined" && process.versions && !!process.versions
      ...
      var file = new java.io.File(path),
        ...
        input = new java.io.BufferedReader(new java.io.InputStreamReader(new java
          .io.FileInputStream(file), 'utf-8')),
        stringBuffer, line;
      try {
  ...
  cssAPI.load = function (name, req, load, _config) {
    ...
    //add to the buffer
    cssBuffer[name] = normalize(loadFile(fileUrl), isWindows ? fileUrl.replace(
      /\\/g, '/') : fileUrl, siteRoot);
  ...
  cssAPI.onLayerEnd = function (write, data) {
    ...
    if (config.separateCSS) {
      ...
      var outPath = config.dir ? path.resolve(config.dir, config.baseUrl,
        data.name + '.css') : config.out.replace(/(\.js)?$/, '.css');

      var css = layerBuffer.join('');

      if (fs.existsSync(outPath))
        console.log('RequireCSS: Warning, separateCSS module path "' +
          outPath +
          '" already exists and is being replaced by the layer CSS.');
