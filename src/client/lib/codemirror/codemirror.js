...
window.CodeMirror = (function () {
  ...
  function buildLineElement(cm, line, lineNo, dims, reuse) {
    ...
    if (cm.options.lineNumbers || markers) {
      var gutterWrap = wrap.insertBefore(elt("div", null, null,
          "position: absolute; left: " +
          (cm.options.fixedGutter ? dims.fixedPos : -dims.gutterTotalWidth) +
          "px"),
        wrap.firstChild);
      if (cm.options.fixedGutter)(wrap.alignable || (wrap.alignable = [])).push(
        gutterWrap);
  ...
  function findStartLine(cm, n, precise) {
    ...
    for (var search = n; search > lim; --search) {
      ...
      if (line.stateAfter && (!precise || search <= doc.frontier)) return
        search;
  ...
  function paddingLeft(display) {
    var e = removeChildrenAndAdd(display.measure, elt("pre", null, null,
      "text-align: left")).appendChild(elt("span", "x"));
    return e.offsetLeft;
  }
  ...
  function coordsChar(cm, x, y) {
    ...
    if (lineNo > last)
      return PosWithInfo(doc.first + doc.size - 1, getLine(doc, last).text.length,
        true, 1);
    if (x < 0) x = 0;
  ...
  function registerEventHandlers(cm) {
    ...
    function onResize() {
      if (resizeTimer == null) resizeTimer = setTimeout(function () {
        ...
        d.cachedCharWidth = d.cachedTextHeight = knownScrollbarWidth =
          null;
        clearCaches(cm);
  ...
  function onMouseDown(e) {
    ...
    if (cm.options.dragDrop && dragAndDrop && !isReadOnly(cm) && !posEq(sel.from,
        sel.to) &&
      !posLess(start, sel.from) && !posLess(sel.to, start) && type ==
      "single") {
  ...
  function clipPostChange(doc, change, pos) {
    ...
    if (pos.line > change.to.line + diff) {
      ...
      if (preLine > lastLine) return Pos(lastLine, getLine(doc, lastLine).text
        .length);
  ...
  CodeMirror.prototype = {
    ...
    addLineClass: operation(null, function (handle, where, cls) {
      return changeLine(this, handle, function (line) {
        var prop = where == "text" ? "textClass" : where ==
          "background" ? "bgClass" : "wrapClass";
        if (!line[prop]) line[prop] = cls;
        else if (new RegExp("(?:^|\\s)" + cls + "(?:$|\\s)").test(line[
          prop])) return false;
        else line[prop] += " " + cls;
        return true;
      });
  ...
  function stretchSpansOverChange(doc, change) {
    ...
    if (sameLine && first) {
      ...
      for (var i = 0; i < first.length; ++i)
        if (first[i].from != null && first[i].from == first[i].to && first[i]
  ...
  function buildTokenSplitSpaces(inner) {
    ...
    return function (builder, text, style, startStyle, endStyle, title) {
      return inner(builder, text.replace(/ {3,}/g, split), style,
    ...
    return function (str) {
      ...
      for (var i = 0; i < len; ++i) {
        ...
        if (type == ",") types[i] = "N";
          ...
          var replace = (i && types[i - 1] == "!") || (end < len - 1 &&
            types[end] == "1") ? "1" : "N";
