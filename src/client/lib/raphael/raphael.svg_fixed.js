...
window.Raphael && window.Raphael.svg && function (R) {
  ...
  var xlink = "http://www.w3.org/1999/xlink",
    markers = {
      block: [{
        'path': "M5,0 0,2.5 5,5z"
      }],
      classic: [{
        'path': "M5,0 0,2.5 5,5 3.5,3 3.5,2z"
      }],
      diamond: [{
        'path': "M2.5,0 5,2.5 2.5,5 0,2.5z"
      }],
      open: [{
        'path': "M4.5,0.5 0.5,2.5 3.5,2.5 0.5,2.5 4.5,4.5"
      }],
      oval: [{
        'path': "M2.5,0A2.5,2.5,0,0,1,2.5,5 2.5,2.5,0,0,1,2.5,0z"
      }],
      diamond2: [{
        'path': "M2.5,1 5,2.5 2.5,4 0,2.5z"
      }],
      inheritance: [{
        'path': "M5,0 0,2.5 5,5z"
      }, {
  ...
  var $ = function (el, attr) {
    ...
    addGradientFill = function (element, gradient) {
      ...
      if (!el) {
        gradient = Str(gradient).replace(R._radial_gradient, function (all, _fx,
          _fy) {
          type = "radial";
          if (_fx && _fy) {
            fx = toFloat(_fx);
    ...
    updatePosition = function (o) {
      ...
      $(o.pattern, {
        patternTransform: o.matrix.invert() + " translate(" + bbox.x + "," +
          bbox.y + ")"
      });
    },
      ...
      if (o.type == "path") {
        ...
        if (type != "none") {
          ...
          var svgId = p.canvas.getAttribute('id'),
            ...
            markerId = "raphael-marker-" + se + type + "-" + svgId + "-" + w +
            h + colorId,
          ...
          if (found === false) {
            ...
            for (marker_type_parts_i = 0; marker_type_parts_i <
              marker_type_parts_len; marker_type_parts_i += 1) {
          ...
          if (!marker) {
            ...
            for (marker_type_parts_i = 0; marker_type_parts_i <
              ...
              use = $($("use"), {
                ...
                transform: (isEnd ? "rotate(180 " + w / 2 + " " + h / 2 +
                  ") " : E) + "scale(" + w / t + "," + h / t + ")",
          ...
          for (marker_type_parts_i = 0; marker_type_parts_i <
            ...
            if (markers[type][marker_type_parts_i].attr) {
              $(use, extendObj(extendObj({}, attr), markers[type][
                marker_type_parts_i
              ].attr));
            } else {
              $(use, attr);
        ...
        if (params[has](att)) {
          ...
          case "clip-rect":
            ...
            if (!value) {
              ...
              if (path) {
                var clip = R._g.doc.getElementById(path.replace(
                  /(^url\(#|\)$)/g, E));
          ...
          case "path":
            if (o.type == "path") {
              ...
              if (o._.arrows) {
                ...
                "endString" in o._.arrows && addArrow(o, o._.arrows.endString,
                  1);
              }
            }
            break;
          ...
          case "fill-opacity":
            if (attrs.gradient) {
              gradient = R._g.doc.getElementById(node.getAttribute("fill").replace(
                /^url\(#|\)$/g, E));
              if (gradient) {
    ...
    tuneText = function (el, params) {
      ...
      var a = el.attrs,
        node = el.node,
        fontSize = node.firstChild ? toInt(R._g.doc.defaultView.getComputedStyle(
          node.firstChild, E).getPropertyValue("font-size"), 10) : 10;
  ...
  elproto.toBack = function () {
    ...
    if (parent.tagName.toLowerCase() == "a") {
      parent.parentNode.insertBefore(this.node.parentNode, this.node.parentNode
  ...
  R._engine.create = function () {
    ...
    if (container == 1) {
      cnvs.style.cssText = css + "position:absolute;left:" + x + "px;top:" + y +
        "px";
