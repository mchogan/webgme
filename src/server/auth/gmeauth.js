...
define(["storage/mongo", "storage/commit", "core/core", "util/guid"], function (
  Mongo, Commit, Core, GUID) {
  function GMEAuth(_options) {
              ...
              _core.loadChildren(root, function (err, children) {
                ...
                if (!err && children && children.length > 0) {
                  for (var i = 0; i < children.length; i++) {
                    ...
                    if ('guest' === _core.getAttribute(children[i],
                      'name')) {
                      guest = children[i];
                    }
                  }
                ...
                if (!err && children && children.length > 0) {
                  for (var i = 0; i < children.length; i++) {
                    ...
                    if (token !== undefined && token !== null) {
                      if (tokenId === token.id && isTokenValid(token.created) ===
                        true) {
    ...
    function getUser(id, callback) {
      if (_cachedUserData[id]) {
        ...
        getUserNode(id, function (err, node) {
          if (!err) {
            _cachedUserData[id] = {
              create: _core.getRegistry(node, 'create')
            };
    ...
    function getUserProject(id, projectName, callback) {
      ...
      if (_cachedUserData[pId]) {
        ...
        getUserNode(id, function (err, node) {
          if (!err) {
            _cachedUserData[pId] = _core.getRegistry(node, 'projects')[
              projectName];
      ...
      getUserNode(userId, function (err, userNode) {
        if (!err && userNode) {
          ...
          getLatestCommit(function (err, oldCommit) {
            if (!err && oldCommit) {
              var newCommitHash = _project.makeCommit([oldCommit.root],
                newHash, 'user ' + userId + 'have created ' +
                projectName + ' project', function (err) {});
              _project.setBranchHash('master', oldCommit['_id'],
                newCommitHash, function (err) {
                  if (!err) {
                    _cachedUserData[getProjectId(userId, projectName)] = {
                      read: true,
                      write: true,
                      delete: true
                    };
          ...
          getLatestCommit(function (err, oldCommit) {
            if (!err && oldCommit) {
              ...
              _project.setBranchHash('master', oldCommit['_id'],
                newCommitHash, function (err) {
                  if (!err) {
                    delete _cachedUserData[getProjectId(userId,
                      projectName)];
                    callback(null);
                  } else {
                    callback(err);
              ...
              getUserProject(userID, projectName, function (err, userData) {
                if (!err && userData) {
                  if (userData['delete'] === true) {
                    removeProjectFromUser(userID, projectName, function (
                      err) {
                      if (err) {
                        callback(err, false);
                      } else {
        ...
        if (!err && userID) {
          getUserNode(userID, function (err, userNode) {
            if (!err && userNode) {
              ...
              _core.setAttribute(userNode, 'token', {
                id: token,
              ...
              getLatestCommit(function (err, oldCommit) {
                if (!err && oldCommit) {
                  var newCommitHash = _project.makeCommit([oldCommit.root],
                    newHash, 'token for user ' + userID +
                    ' have been created', function (err) {});
                  _project.setBranchHash('master', oldCommit['_id'],
                    newCommitHash, function (err) {
                      if (!err) {
                        callback(null, token);
