define([
  ...
  'blob/BlobFSBackend',
  'blob/BlobS3Backend'
], function (ASSERT, PluginManager, BlobRunPluginClient, PluginResult, Core,
  Storage, FS, PATH, logManager, BlobFSBackend, BlobS3Backend) {
  ...
  function RunPlugin() {
    ...
    var main = function (CONFIG, pluginConfig, callback) {
      ...
      Plugin = requirejs('plugin/' + pluginName + '/' + pluginName + '/' +
        pluginName);
        ...
        if (!err) {
          storage.openProject(config.project, function (err, project) {
            if (!err) {
              ...
              var pluginManager = new PluginManager(project, Core,
                plugins);
              ...
              pluginManager.executePlugin(pluginName, config, function (
                err, result) {
