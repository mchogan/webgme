...
requirejs(['worker/constants',
  ...
  function (CONSTANT, Core, Storage, GUID, DUMP, logManager, FS, PATH,
    BlobServerClient, PluginManagerBase, PluginResult, ConnectedStorage,
    Serialization, GMEAUTH) {
    var storage = null,
      core = null,
      result = null,
      resultReady = false,
      ...
      if (initialized !== true) {
        ...
        storage.openDatabase(function (err) {
          if (err) {
            ...
            process.send({
              pid: process.pid,
              type: CONSTANT.msgTypes.initialized
            });
          }
        });
      }
    ...
    var exportLibrary = function (name, hash, libraryRootPath, callback) {
      ...
      storage.openProject(name, function (err, project) {
        ...
        core.loadRoot(hash, function (err, root) {
          ...
          core.loadByPath(root, libraryRootPath, function (err,
            libraryRoot) {
            if (err) {
              return callback(err);
            }
                ...
                if (err) {
                  ...
                  if (needed > 0) {
                    for (var i = 0; i < nodePaths.length; i++) {
                      core.loadByPath(root, nodePaths[i], function (err,
                        node) {
    ...
    var getProject = function (projectName, sessionId, callback) {
      var pluginStorage = new ConnectedStorage({
        type: 'node',
        host: '127.0.0.1',
        port: serverPort,
        ...
        for (var i = 0; i < pluginBasePaths.length; i++) {
          ...
          for (var j = 0; j < additional.length; j++) {
            if (additional[j] === pluginName) {
              if (isGoodExtraAsset(additional[j], PATH.join(pluginBasePaths[i],
                additional[j]))) {
    ...
    var runInterpreter = function (userId, name, sessionId, context, callback) {
      ...
      if (interpreter) {
        getProject(context.managerConfig.project, sessionId, function (err,
          project) {
          if (!err) {
            ...
            manager.initialize(null, function (pluginConfigs,
              configSaveCallback) {
              if (configSaveCallback) {
                configSaveCallback(context.pluginConfigs);
              }

              manager.executePlugin(name, context.managerConfig, function (
                err, result) {
                if (!err && result) {
            ...
            var newErrorPluginResult = new PluginResult();
            callback(new Error('unable to get project'),
              newErrorPluginResult.serialize());
          }
        });
      } else {
        var newErrorPluginResult = new PluginResult();
      ...
      var projectNames,
        ...
        addUserAuthInfo = function (projectName) {
          if (userAuthInfo === null) {
            completeInfo[projectName].rights = {
              read: true,
              write: true,
              delete: true
            };
    ...
    var setBranch = function (sessionId, projectName, branchName, oldHash,
      newHash, callback) {
      if (storage) {
        if (initialized) {
          storage.getProjectNames(function (err, projectlist) {
            ...
            getProject(projectName, sessionId, function (err, project) {
              ...
              project.setBranchHash(branchName, oldHash, newHash,
                callback);
            });
          });
        } else {
    ...
    process.on('message', function (parameters) {
      ...
      case CONSTANT.workerCommands.dumpMoreNodes:
        if (typeof parameters.name === 'string' && typeof parameters.hash ===
          'string' && parameters.nodes && parameters.nodes.length) {
          resultId = GUID();
          process.send({
            pid: process.pid,
          ...
          dumpMoreNodes(parameters.name, parameters.hash, parameters.nodes,
            function (err, r) {
      ...
      case CONSTANT.workerCommands.executePlugin:
        if (typeof parameters.name === 'string' && typeof parameters.context ===
          'object') {
          runInterpreter(parameters.user, parameters.name, parameters.webGMESessionId,
            parameters.context, function (err, result) {
              process.send({
                pid: process.pid,
                type: CONSTANT.msgTypes.result,
                error: err,
      ...
      case CONSTANT.workerCommands.exportLibrary:
        if (typeof parameters.name === 'string' && typeof parameters.hash ===
          'string' && typeof parameters.path === 'string') {
          resultId = GUID();
          process.send({
            pid: process.pid,
            type: CONSTANT.msgTypes.request,
          ...
          exportLibrary(parameters.name, parameters.hash, parameters.path,
            function (err, r) {
              if (resultRequested === true) {
                initResult();
                process.send({
      ...
      case CONSTANT.workerCommands.setBranch:
        ...
        setBranch(parameters.webGMESessionId, parameters.project, parameters.branch,
          parameters.old, parameters.new, function (err, r) {
            if (resultRequested === true) {
              initResult();
              process.send({
                pid: process.pid,
    ...
    process.send({
      pid: process.pid,
      type: CONSTANT.msgTypes.initialize
    });
  });