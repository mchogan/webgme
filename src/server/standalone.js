define(['logManager',
  ...
  function StandAloneServer(CONFIG) {
    ...
    function start() {
      ...
      __storageOptions.log = LogManager.create(
        'StandAloneWebGMEServer-storage');
    ...
    function storeQueryString(req, res, next) {
      if (req && req.session && req.session.originalQuery === undefined) {
        ...
        req.session.originalQuery = index === -1 ? "" : req.url.substring(
          index);
      }
      if (req && req.session && req.session.originalUrl === undefined) {
        req.session.originalUrl = req.url;
      }
      if (typeof CONFIG.defaultUser === 'string' && req.session.authenticated !==
        true) {
    ...
    function checkGoogleAuthentication(req, res, next) {
      if (__googleAuthenticationSet === true) {
        ...
        var protocolPrefix = CONFIG.httpsecure === true ? 'https://' :
          'http://';
        Passport.use(new __googleStrategy({
            returnURL: protocolPrefix + req.headers.host +
              '/login/google/return',
    ...
    function checkREST(req, res, next) {
      var baseUrl = CONFIG.httpsecure === true ? 'https://' : 'http://' + req
        .headers.host + '/rest';
      if (__REST === null) {
      ...
      if (true === CONFIG.authentication) {
        if (req.isAuthenticated() || (req.session && true === req.session.authenticated)) {
          ...
          if (req.headers.webgmeclientsession) {
            __sessionStore.get(req.headers.webgmeclientsession, function (err,
              clientSession) {
              if (!err) {
          ...
          //request which use token may be authenticated directly
            ...
            __gmeAuth.checkToken(req.headers.webGMEToken, function (isOk,
              userId) {
              if (isOk) {
                req.session.authenticated = true;
        ...
        for (var i = 0; i < CONFIG.pluginBasePaths.length; i++) {
          ...
          for (var j = 0; j < additional.length; j++) {
            if (additional[j] === pluginName) {
              if (isGoodExtraAsset(additional[j], Path.join(CONFIG.pluginBasePaths[
                i], additional[j]))) {
    ...
    function getVisualizersDescriptor() {
      ...
      for (i = 0; i < CONFIG.visualizerDescriptors.length; i++) {
        var descriptor = getVisualizerDescriptor(CONFIG.visualizerDescriptors[
          i]);
    ...
    function setupExternalRestModules() {
      ...
      for (i = 0; i < keys.length; i++) {
        ...
        if (modul) {
          __logger.info('adding RExtraST [' + CONFIG.rextrast[keys[i]] +
            '] to - /rest/external/' + keys[i]);
    ...
    __app.configure(function () {
      ...
      if (CONFIG.debug === true) {
        setInterval(function () {
          if (__reportedRequestCounter !== __requestCounter) {
            ...
            console.log("...handled " + __reportedRequestCounter +
              " requests so far...");
          }
      ...
      __app.use(function (req, res, next) {
        var infoguid = GUID(),
          infotxt = "request[" + infoguid + "]:" + req.headers.host +
      ...
      __app.use(Express.multipart({
        defer: true
      })); // required to upload files. (body parser should not be used!)
    ...
    __app.get('/', storeQueryString, checkVF, ensureAuthenticated, function (
      req, res) {
    ...
    __app.post('/login' /*,storeQueryString*/ , __gmeAuth.authenticate,
      function (req, res) {
        res.cookie('webgme', req.session.udmId);
        //res.redirect('/'+req.session.originalQuery || "");
        res.redirect(req.session.originalUrl);
      });
    __app.post('/login/client', prepClientLogin, __gmeAuth.authenticate,
      function (req, res) {
        res.cookie('webgme', req.session.udmId);
        res.send(200);
      });
    ...
    __app.get('/login/google' /*,storeQueryString*/ ,
      checkGoogleAuthentication, Passport.authenticate('google'));
    __app.get('/login/google/return' /*,storeQueryString*/ , __gmeAuth.authenticate,
      function (req, res) {
        res.cookie('webgme', req.session.udmId);
        res.redirect('/' + req.session.originalQuery || "");
      });
    __app.get('/login/forge' /*,storeQueryString*/ , __forgeAuth.authenticate,
      function (req, res) {
        res.cookie('webgme', req.session.udmId);
        res.redirect('/');
      });
    ...
    __app.get(/^\/decorators\/.*/, ensureAuthenticated, function (req, res) {
      var tryNext = function (index) {
        if (index < CONFIG.decoratorpaths.length) {
          res.sendfile(Path.join(CONFIG.decoratorpaths[index], req.url.substring(
            12)), function (err) {
            if (err && err.code !== 'ECONNRESET') {
              tryNext(index + 1);
            }
      ...
      res.sendfile(Path.join(__baseDir, req.path), function (err) {
        if (err && err.code !== 'ECONNRESET') {
          ...
          if (typeof basePath === 'string' && typeof relPath === 'string') {
            expressFileSending(res, Path.resolve(Path.join(basePath,
              relPath)));
          } else {
            res.send(404);
          }
    ...
    __app.get(/^\/(common|config|bin|middleware)\/.*\.js$/,
    ...
    __app.get('/rest/blob/metadata/:metadataHash', ensureAuthenticated,
      function (req, res) {
        blobBackend.getMetadata(req.params.metadataHash, function (err, hash,
          metadata) {
          if (err) {
            // FIXME: make sure we set the status code correctly like 404 etc.
            res.status(500);
    ...
    __app.post('/rest/blob/createFile/:filename', ensureAuthenticated,
      function (req, res) {
        __logger.info('file creation request: user[' + req.session.udmId +
        ...
        blobBackend.putFile(filename, req, function (err, hash) {
          __logger.info('file creation request finished: user[' + req.session
            .udmId + '], filename[' + req.params.filename + '], error[' +
            err + '], hash:[' + hash + ']');
          if (err) {
            // FIXME: make sure we set the status code correctly like 404 etc.
            res.status(500);
            ...
            blobBackend.getMetadata(hash, function (err, metadataHash,
              metadata) {
              if (err) {
                // FIXME: make sure we set the status code correctly like 404 etc.
                res.status(500);
    ...
    __app.post('/rest/blob/createMetadata', ensureAuthenticated, function (
      req, res) {
    ...
    var sendBlobContent = function (req, res, metadataHash, subpartPath,
      download) {
      ...
      blobBackend.getMetadata(metadataHash, function (err, hash, metadata) {
        if (err) {
          ...
          if (subpartPath) {
            filename = subpartPath.substring(subpartPath.lastIndexOf('/') +
              1);
          }

          var mimeType = mime.lookup(filename);

          if (download || mimeType === 'application/octet-stream' ||
          ...
          blobBackend.getFile(metadataHash, subpartPath, res, function (
            err, hash) {
            if (err) {
    ...
    __app.get(/^\/rest\/blob\/download\/([0-9a-f]{40,40})(\/(.*))?$/,
      ensureAuthenticated, function (req, res) {
    ...
    __app.get(/^\/rest\/blob\/view\/([0-9a-f]{40,40})(\/(.*))?$/,
      ensureAuthenticated, function (req, res) {
        var metadataHash = req.params[0];
        var subpartPath = req.params[2];
    ...
    __app.get(/^\/.*\.(js|html|gif|png|bmp|svg|json|map)$/,
      ensureAuthenticated, function (req, res) {
        //package.json
    ...
    __app.get('/gettoken', storeQueryString, ensureAuthenticated, function (
      req, res) {
    ...
    __app.get('/rest/:command', storeQueryString, ensureAuthenticated,
      checkREST, function (req, res) {
        __REST.initialize(function (err) {
          if (err) {
            res.send(500);
          } else {
            __REST.doRESTCommand(__REST.request.GET, req.params.command,
              req.headers.webGMEToken, req.query, function (httpStatus,
                object) {
        ...
        for (var i = 0; i < CONFIG.decoratorpaths.length; i++) {
          ...
          for (var j = 0; j < additional.length; j++) {
            if (names.indexOf(additional[j]) === -1) {
              if (isGoodExtraAsset(additional[j], Path.join(CONFIG.decoratorpaths[
                i], additional[j]))) {
    ...
    __app.get('/listAllVisualizerDescriptors', ensureAuthenticated, function (
      req, res) {
      var allVisualizerDescriptors = getVisualizersDescriptor();
      res.status(200);
      res.setHeader('Content-type', 'application/json');
      res.end("define([],function(){ return " + JSON.stringify(
        allVisualizerDescriptors) + ";});");
    });
    ...
    for (var dev in networkIfs) {
      networkIfs[dev].forEach(function (netIf) {
        if (netIf.family === 'IPv4') {
          var address = CONFIG.httpsecure ? 'https' : 'http' + '://' +
            netIf.address + ':' + CONFIG.port;
