...
if (typeof define !== "function") {
  ...
  requirejs(["util/common", "util/assert", "core/tasync", 'util/guid'],
    function (COMMON, ASSERT, TASYNC, GUID) {
      "use strict";
      ...
      function main() {
        ...
        if (COMMON.getParameters("help") !== null) {
          ...
          console.log("");
          console.log(
            "Parses a GME xme file and stores it int a WEBGME database. Possible options:"
          );
          console.log("");
          console.log(
            "  -mongo [database [host [port]]]\t\t\topens a mongo database");
          ...
          console.log(
            "  -user <username>\t\t\t\t\tthe user which we would like to manage"
          );
          console.log(
            "  -adduser <write = true/false> [password email]\tthe user to add"
          );
          console.log(
            "  -addproject <projectname> <mode = r|rw|rwd>\t\tadds a project to the user data"
          );
          console.log(
            "  -removeproject <projectname>\t\t\t\tremoves a project from the user data"
          );
          console.log("  -token \t\t\t\t\tgenerates a token for the user");
          console.log("  -removeuser \t\t\t\t\t\tremoves a user data");
          console.log(
            "  -info\t\t\t\t\t\t\tprints out the data of the user, or if no user is given then the data of all users"
          );
        ...
        if (COMMON.getParameters("create")) {
          ...
          if (COMMON.getParameters("info")) {
            ...
            done = TASYNC.call(addProject, core, _startHash, _user, projpars[0],
              projpars[1] || "");
          } else if (COMMON.getParameters("removeproject")) {
            projpars = COMMON.getParameters("removeproject");
            done = TASYNC.call(removeProject, core, _startHash, _user, projpars[
              0]);
          } else if (COMMON.getParameters("adduser")) {
            projpars = COMMON.getParameters("adduser");
            done = TASYNC.call(addUser, core, _startHash, _user, projpars[0] ||
              "false", projpars[1] || null, projpars[2] || null);
      ...
      function createEmptyDb(core, branch) {
        ...
        var project = COMMON.getProject();
        var newcommit = TASYNC.call(project.makeCommit, [], newroothash,
          "creating empty user database");
        var oldcommit = project.getBranchHash(branch, null);
        var done = TASYNC.call(project.setBranchHash, branch, oldcommit,
          newcommit);
        return TASYNC.call(function () {
          console.log("Commit " + newcommit + " written to branch " +
            branch);
        }, done);
      }
      ...
      function infoPrint(core, roothash, userName) {
        function printUser(userObject) {
          var outstring = "";
          outstring += "userName: " + padString(core.getAttribute(userObject,
            'name'), 20) + " | ";
          outstring += "canCreate: " + (core.getRegistry(userObject, 'create') ===
            true ? " true" : "false") + " | ";
          outstring += "projects: ";
      ...
      function generateToken(core, roothash, userName) {
        function iterateChildren(parentObject) {
          ...
          return TASYNC.call(function (objectArray) {
            ...
            if (child) {
              core.setAttribute(child, 'token', {
                id: GUID() + 'token',
                created: (new Date()).getDate()
              });
              var newroothash = persist(core, parentObject);
              return TASYNC.call(saveModifications, newroothash,
                "token have been generated for user: " + userName);
            } else {
              return null;
            }
          }, children);
        ...
        function iterateChildren(parentObject) {
          ...
          return TASYNC.call(function (objectArray) {
            ...
            if (child) {
              ...
              var newroothash = persist(core, parentObject);
              return TASYNC.call(saveModifications, newroothash, "project " +
                projectname + " has been added to user " + username);
            } else {
              return null;
            }
          }, children);
          ...
          return TASYNC.call(function (objectArray) {
            ...
            if (child) {
              ...
              if (projects[projectname]) {
                ...
                return TASYNC.call(saveModifications, newroothash,
                  "project " + projectname +
                  " has been removed from user " + username);
              } else {
                return null;
              }
            } else {
        ...
        function iterateChildren(parentObject) {
          ...
          return TASYNC.call(function (objectArray) {
            ...
            if (child === null) {
              ...
              var newroothash = persist(core, parentObject);
              return TASYNC.call(saveModifications, newroothash,
                "the basic data of user " + username +
                " has been changed");
            }
            return null;
          }, children);
        ...
        function iterateChildren(parentObject) {
          ...
          return TASYNC.call(function (objectArray) {
            ...
            if (child) {
              ...
              var newroothash = persist(core, parentObject);
              return TASYNC.call(saveModifications, newroothash, "user " +
                username + " has been removed from database");
            }
            return null;
          }, children);
        }
