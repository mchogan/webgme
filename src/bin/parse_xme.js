...
if (typeof define !== "function") {
  ...
  requirejs(["util/common", "util/assert", "core/tasync", "bin/parse_xme"],
    function (COMMON, ASSERT, TASYNC, parser) {
      "use strict";
      ...
      function main() {
        ...
        if (args.length < 1 || COMMON.getParameters("help") !== null) {
          ...
          console.log(
            "  -branch <branch>\t\t\tselects the branch to be overwritten");
  ...
  define(["util/assert", "core/tasync", "util/common"], function (ASSERT,
    TASYNC, COMMON) {
    function parser(xmlfile, core) {
      ...
      function opentag(tag) {
        ...
        } else if (name === "folder" || name === "model" || name === "atom" ||
          name === "connection" || name === "reference" || name === "set") {
      ...
      function closetag(name) {
        ...
        if (name === "project" || name === "folder" || name === "model" ||
          name === "atom" || name === "connection" || name === "reference" ||
          name === "set") {
          ...
          ASSERT(name === "attribute" || name === "regnode" || name ===
            "connpoint");
        }
      }
    ...
    function parseObject(core, tag, global) {
      ...
      if (typeof tag.attributes.id !== "undefined") {
        if (typeof global.ids[tag.attributes.id] === "undefined") {
          ...
          console.log("Warning: multiple object with the same id: " + tag.attributes
            .id);
        }
      }
    ...
    function parseValue(core, tag) {
      ...
      if (tag.parent.name === "attribute") {
        ...
        core.setAttribute(tag.parent.parent.node, tag.parent.attributes[
          'kind'], tag.text);
      } else {
    ...
    function resolveAll(core, root, global) {
      ...
      for (i = 0; i < global.cps.length; ++i) {
        done = TASYNC.join(done, resolveConnPoint(core, global.ids, global.cps[
          i]));
      }
    ...
    function resolveConnPoint(core, ids, cp) {
      ...
      if (cp.refs) {
        ...
        return TASYNC.call(resolveRefPort, core, cp.node, role, ref, target,
          cp.target, children);
      } else {
    ...
    function resolveRefPort(core, source, role, reference, target, targetid,
      children) {
      ...
      if (!refport) {
        refport = core.createNode({
          parent: reference,
          relid: "" + core.getRegistry(reference, "refPortCount")
        });
        core.setRegistry(reference, 'refPortCount', core.getRegistry(
          reference, "refPortCount") + 1);
        core.setAttribute(refport, "name", core.getAttribute(target, "name"));
        core.setRegistry(refport, "position", core.getRegistry(target,
          "position"));
