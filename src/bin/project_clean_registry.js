...
requirejs(["util/common",
  ...
  "js/RegistryKeys",
  "js/Constants",
  "client/js/Utils/GMEConcepts.FCO"
], function (COMMON,
  ASSERT,
  TASYNC,
  GUID,
  ...
  function main() {
    ...
    if (COMMON.getParameters("help") !== null) {
      ...
      console.log("");
      console.log(
        "Updates the given branch to the latest webgme format. Possible options:"
      );
      console.log("");
  ...
  function makeCommit(rootHash, parentCommitHash) {
    ...
    return project.makeCommit([parentCommitHash], rootHash,
      "Updating project to be visible with latest webGME.");
  }
  ...
  function updateNode(core, node) { //TODO this method should be always up-to-date...
    if (!__FCOID) {
      __FCOID = core.getRegistry(core.getRoot(node), REGISTRY_KEYS.PROJECT_REGISTRY)[
      ...
      for (rk in GMEConceptsFCO.FCO_REGISTRY) {
        if (GMEConceptsFCO.FCO_REGISTRY.hasOwnProperty(rk)) {
          ...
          if (registryKeys.indexOf(rk) !== -1) {
            ...
            msg += 'was missing, created with value: "' + GMEConceptsFCO.FCO_REGISTRY[
              rk] + '"';
          }
          console.log(msg);
        }
    ...
    while (len--) {
      ...
      while (setMemberIdx--) {
        ...
        if (removedRegKeys !== "") {
          console.log('\tRemoved set member registry entries for SET "' + set +
            '" and MEMBER "' + sm + '": ' + removedRegKeys);
        }
      }
    }
