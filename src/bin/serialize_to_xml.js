...
if (typeof define !== "function") {
  ...
  requirejs(["util/common", "util/assert", "core/tasync",
    "bin/serialize_to_xml"
  ], function (COMMON, ASSERT, TASYNC, serializer) {
    "use strict";
    ...
    function makeCommit(xmlfile, rootHash, commitHash) {
      ...
      return project.makeCommit([commitHash], rootHash,
        "normalizing during serialization to " + xmlfile);
    }
...
define(["util/assert", "core/tasync", "util/common", 'fs', 'storage/commit',
  'storage/cache', 'storage/mongo'
], function (ASSERT, TASYNC, COMMON, fs, Commit, Cache, Mongo) {
  ...
  var webOnly = ['position', 'isPort', 'isConnection', 'metameta', 'author',
    'comment', 'refPortCount', 'decorator', 'lineStyle'
  ];
      ...
      if (attrNames[i] !== 'name') {
        initJ._children.push({
          ...
          _children: [{
            _type: "value",
            _empty: false,
            _string: stringToXMLString(theCore.getAttribute(gmeNode,
              attrNames[i]))
          }]
        });
      }
  ...
  function registryToChildrenArray(registryObj) {
    ...
    for (var i in registryObj) {
      if (i !== '_value') {
        ...
        var valueString = "";
        if (!(registryObj[i]._value === null || registryObj[i]._value ===
          undefined)) {
  ...
  function createRegistryObject(gmeNode) {
    ...
    for (i = 0; i < regNames.length; i++) {
      if ((registry[regNames[i]] === undefined || registry[regNames[i]] ===
        null) && webOnly.indexOf(regNames[i]) === -1) {
  ...
  function relIdScanningSync(objectArray) {
    ...
    for (var i = 0; i < generate.length; i++) {
      theCore.setRegistry(objectArray[generate[i]], 'relid', '0x' + (++max).toString(
        16));
    }
  }
  ...
  function idReallocationSync(object) {
    ...
    if (needId) {
      if (!theIds[idType].paths[theCore.getPath(object)]) {
        ...
        var newId = 'id-' + idType + '-' + padding(8 - maxIdEnd.length) +
          maxIdEnd;
        theCore.setRegistry(object, 'id', newId);
        //these nodes needs new GUIDs as well, so let's generate them
        var newGuid = 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g,
          function (c) {
  ...
  function getChildren(object, indent) {
    ...
    case 'reference':
      ...
      var done = TASYNC.call(function (objectArray, referred) {
        if (referred !== null) {
          jsonObject._attr['referred'] = theCore.getRegistry(referred,
            'id');
        }
        ...
        for (var i = 0; i < objectArray.length; i++) {
          ...
          mydone = TASYNC.call(getChildren, objectArray[i], indent + "  ",
            mydone);
        }
        return mydone;
