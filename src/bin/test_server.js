...
requirejs(['logManager',
  ...
  var storage = new Server(new Log(new Cache(new Mongo({
    ...
    log: logManager.create('test-storage')
  }), {
    port: 9889,
    logger: logManager.create('test-storage-socket.io')
  });
  storage.open();
  ...
  var _database = new Log(new Client({
      ...
      log: logManager.create('test-transfer-out')
    }),
  ...
  var testPageResponse = function (res) {
    ...
    for (var i in _clientData) {
      data.push({
        id: i,
        delay: _clientData[i].delay,
        blocked: _clientData[i].blocked,
        address: _clientData[i].address,
        port: _clientData[i].port
      });
    }
  ...
  io.sockets.on('connection', function (socket) {
    _clientData[socket.id] = {
      blocked: false,
      delay: 0,
      address: socket.manager.handshaken[socket.id].address.address,
      port: socket.manager.handshaken[socket.id].address.port
    };
    ...
    socket.on('getBranchHash', function (projectName, branch, oldhash,
      ...
      checkProject(socket.id, projectName, function (err) {
        if (err) {
          ...
          _projects[projectName].getBranchHash(branch, oldhash,
            callback);
        }
      });
    });
    socket.on('setBranchHash', function (projectName, branch, oldhash,
      ...
      checkProject(socket.id, projectName, function (err) {
        if (err) {
          ...
          _projects[projectName].setBranchHash(branch, oldhash, newhash,
            callback);
        }
      });
    });
