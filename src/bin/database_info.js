...
requirejs(["util/assert", "core/tasync", "util/common"], function (ASSERT,
  TASYNC, COMMON) {
  "use strict";
  ...
  function main() {
    if (COMMON.getParameters("help") !== null) {
      ...
      console.log("");
      console.log(
        "Prints out the projects and branches in the given database. The possible"
      );
      console.log("options are the following:");
      console.log("");
      console.log("  -mongo [database [host [port]]]\topens a mongo database");
      console.log(
        "  -socketio [host [port]]\t\topens a conneciton to a socket.io server"
      );
  ...
  function printDatabaseInfo2(database, names) {
    ...
    if (names.length === 0) {
      ...
      for (var i = 0; i < names.length; ++i) {
        var name = names[i],
          project = database.openProject(name);
  ...
  function printProjectInfo2(project, name, branches) {
    ...
    for (var branch in branches) {
      done = TASYNC.call(printCommitInfo, project, branch, branches[branch],
        done);
    }
